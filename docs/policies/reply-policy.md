...
- 댓글과 답글은 공개 독후감, 현재 프로필 사진과 현재 배경사진을 대상으로 등록할 수 있습니다.
- 서버는 요청할 때마다 공개 범위, 현재 사진 참조와 작성자 계정 상태를 확인합니다.
- 정상 이용 중인 사용자의 현재 프로필 사진과 배경사진 댓글은 팔로우 여부와 관계없이 로그인한 정상 이용 사용자가 조회·등록할 수 있습니다.
- 사진이 교체되면 이전 사진의 댓글·답글과 댓글 좋아요를 삭제하며 새 사진으로 이전하지 않습니다.
...
- 독후감 댓글과 답글 댓글 테이블과 댓글·답글 목록 조회 및 등록 API가 연결되어 있습니다.
...
- 독후감 번호와 댓글 번호를 복합 기본키로 사용합니다.
- 댓글 번호는 자동 증가 키와 업무 중복 방지 고유 제약으로 전체 댓글에서 유일하게 발급합니다.
- 독후감, 부모 댓글 및 회원 외래키가 있으므로 영구 삭제 프로시저는 참조 순서에 맞춰 댓글과 관련 좋아요를 명시적으로 정리합니다.

## 대댓글

- 상위 댓글 번호에 부모 댓글 번호를 저장합니다.
- 상위 댓글 번호가 없으면 최상위 댓글입니다.
- 상위 댓글 번호가 있으면 대댓글입니다.
- 허용 깊이는 최상위 댓글과 그 댓글의 대댓글까지입니다.
- 대댓글에 다시 대댓글을 작성하는 3단계 구조는 허용하지 않습니다.
- 별도 레벨 컬럼 없이 부모 댓글 존재 여부와 부모의 상위 댓글 번호를 조회해 깊이를 검증합니다.

## 삭제

- 사용자가 직접 삭제한 댓글은 삭제 여부를 `Y`로 변경하고 원문 행과 대댓글 연결 구조를 유지합니다.
...
- 댓글 수정·삭제 메뉴는 `myReplyYn`이 `Y`이고 삭제 여부가 `N`인 댓글에만 표시합니다.
- 서버는 독후감 번호, 댓글 번호, 인증 사용자 번호, 미삭제 상태를 모두 변경 조건으로 사용합니다.
- 서버는 사용자 계정 상태가 `활성`인 작성자만 수정·삭제하도록 제한합니다.
- `비활성화` 및 영구 탈퇴 대기 계정은 메뉴를 제공하지 않으며 직접 API를 호출해도 변경 건수 없이 실패 처리합니다.
- 계정 비활성화로 삭제 상태가 된 댓글은 재로그인으로 계정이 `활성`로 복귀해도 자동 복원하지 않습니다.
...
- 댓글 좋아요는 공용 좋아요 테이블 사용자 좋아요 관계에 `대상 유형 = 댓글`, `대상 번호 = 댓글 번호`로 저장합니다.
- 댓글 번호는 전체 댓글에서 유일하므로 범용 좋아요 대상 번호로 사용하고, API에서는 독후감 번호와 함께 전달해 댓글 소속을 검증합니다.
- `활성` 사용자만 미삭제 댓글에 좋아요를 등록하거나 취소할 수 있습니다.
- 본인이 작성한 댓글에도 좋아요를 등록할 수 있습니다.
- `비활성화` 및 영구 탈퇴 대기 사용자는 댓글 화면과 API 모두에서 좋아요 접근을 제한합니다.
...
- 댓글 또는 대댓글에 좋아요가 신규 등록되면 해당 댓글 번호 행의 작성자에게만 원본 콘텐츠 유형별 `LIKE` 상황의 댓글 좋아요 템플릿 알림과 푸시를 생성합니다.
...
- 독후감 댓글 좋아요 알림은 해당 독후감 상세의 댓글 목록을 엽니다. 프로필·배경사진 댓글 좋아요 알림은 사진 소유자 본인이 수신하면 마이페이지를, 타인이 수신하면 사진 소유자의 공개 프로필을 열고 현재 사진과 해당 댓글을 표시합니다.
- 답글 등록 알림은 콘텐츠 소유자에게 기존 설정을 적용해 발송하며, 부모 댓글 작성자가 콘텐츠 소유자 및 답글 작성자와 다르면 같은 대상 링크의 알림을 추가로 발송합니다. 동일 수신자와 자기 알림은 생성하지 않습니다.
- 댓글 작성자가 `비활성화` 또는 영구 탈퇴 대기 상태이면 새 알림과 푸시를 생성하지 않습니다.
...
- 사용자 좋아요 관계
- 알림 템플릿의 `LIKE` 상황 독후감·프로필 사진·배경사진 댓글 좋아요 템플릿

## 개인 차단에 따른 반응 정리

- 차단 상대가 남긴 개인 댓글·답글의 삭제 범위, 제3자 답글 연결 유지 및 차단 해제·계정 복귀·영구 탈퇴 취소 시 복원 금지 기준은 [소셜 정책](social-policy.md)의 사용자 차단 기준을 따릅니다.
