...
- 독서 타이머 출처에서 도서를 선택하면 검색 화면 위에 별도 설명과 입력 항목명 없이 오늘부터 목표일까지 걸리는 일수를 왼쪽 `-`, 오른쪽 `+`로 설정하는 타이머 전용 팝업을 표시합니다. 하단에는 독서 목표 설정 팝업과 같은 `닫기`와 `확인` 명령만 표시하고 단계 진행 표시는 두지 않으며, 확인 시 기존 독후감 등록 API로 `READ` 상태 독후감을 생성합니다.
...
- `WITHDRAWN`, `DELETE_PENDING` 및 `SUSPENDED` 상태에서는 진입 출처와 관계없이 도서 검색과 후속 등록을 모두 허용하지 않습니다.
