...
사용자 안내 팝업 콘텐츠는 팝업 사용 화면 구분과 팝업 코드를 복합 기본키로 사용합니다.

| 팝업 사용 화면 코드 | 팝업 식별 코드 | 적용 화면 |
| --- | --- | --- |
| 계정 문의 | `WITHDRAWAL_POLICY` | 회원 탈퇴 화면의 계정 처리 정책 안내 |
| `PROFILE` | 목표 하향 조정 안내 | 마이페이지의 목표 내리기 도움말 |

팝업 사용 화면 코드는 공통코드 팝업 사용 화면 코드의 사용 가능한 분류값만 유효한 값으로 취급합니다.

## 콘텐츠 저장

| 데이터 항목 | 역할 | Null 정책 |
| --- | --- | --- |
| 관리 화면 제목 | 관리자 화면에서 팝업을 식별하는 제목 | 필수 |
| 첫 번째 콘텐츠 영역 | 첫 번째 화면 영역의 문자열 JSON 배열 | 필수 |
| 두 번째 콘텐츠 영역 | 두 번째 화면 영역의 문자열 JSON 배열 | 선택 |
| 세 번째 콘텐츠 영역 | 세 번째 화면 영역의 문자열 JSON 배열 | 선택 |
| 네 번째 콘텐츠 영역 | 네 번째 화면 영역의 문자열 JSON 배열 | 선택 |

- 안내문 각 영역에는 HTML 태그가 아닌 JSON 문자열 배열을 저장합니다.
...
- 첫 번째 콘텐츠 영역: 비활성화 영역의 전체 목록 문구
- 두 번째 콘텐츠 영역: 영구 탈퇴 영역의 전체 목록 문구
- 팝업 제목, 영역 제목, 배지 및 선택 카드의 한 줄 요약은 현재 화면 문구를 유지합니다.

### 목표 내리기 도움말

- 첫 번째 콘텐츠 영역: 주간·월간·연간 및 같은 값 저장 기준을 포함한 전체 목록 문구
- 팝업 제목과 목록 위의 요약 문구는 현재 메시지 프로퍼티를 유지합니다.

## 조회 및 실패 처리

- 인증된 사용자는 `GET /api/popup-content`에 `popuSitu`와 `popuCode`를 전달해 콘텐츠 한 건을 조회합니다.
- API는 사용자 안내 팝업 콘텐츠의 사용자 노출 컬럼만 반환하며 관리 화면 제목, 등록자와 수정자 등의 관리 정보는 반환하지 않습니다.
...
- 비활성화 계정은 같은 계정으로 재로그인하여 `활성` 상태로 복구된 뒤 최신 콘텐츠를 조회합니다.
