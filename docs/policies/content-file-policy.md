...
- 차단어는 금칙어 공통코드의 활성 분류값에서 조회합니다.
- 정상 허용어는 금칙어 예외 표현 공통코드의 활성 분류값에서 조회합니다.
- 금칙어에는 신체·성적 표현·성정체성·일반 단어처럼 불편할 수 있으나 욕설이 아닌 단어를 등록하지 않습니다.
- 짧은 차단어만으로 탐지되는 조합어와 반복 문자 우회 표현은 별도 분류값로 중복 등록하지 않습니다.
- 발음이나 철자가 달라 정규화 로직으로 복원할 수 없는 욕설 변형만 별도 분류값로 등록합니다.
- 분류값은 금칙어 항목 예시부터 연속해서 부여하며 정렬 순서도 같은 순번을 사용합니다.
...
- 닉네임 주어 코드의 확장 속성 1 코드부터 확장 속성 4 코드에는 함께 사용할 닉네임 서술어 코드를 등록합니다.
- 주어마다 등록된 네 개의 활성 서술어 중 하나를 무작위로 선택해 어색한 표현을 제외하면서 조합 다양성을 확보합니다.
- 자동 닉네임은 공백 없이 연결한 `조합문구_가입연월순번` 형식이며 동일한 주어·서술어·동물 분류값 조합과 연월 안에서 순번을 네 자리로 증가시킵니다.
- 닉네임 순번 테이블에는 조합 문구를 중복 저장하지 않고 주어·서술어·동물 분류값 세 개를 저장합니다.
...
- `이용정지` 계정은 고객문의 정책에 따라 허용된 이용정지 이의제기의 제목과 본문에 동일한 비속어 검사를 적용합니다.
- 비속어 검사 결과와 정규화 중간값은 별도 데이터로 저장하지 않습니다.
- `비활성화` 및 영구 탈퇴 대기 계정은 기존 접근 제한으로 콘텐츠 입력을 수행할 수 없으며 비속어 검사에서 별도 예외를 두지 않습니다.
- 계정 비활성화, 영구 탈퇴, 취소 또는 복귀 시 비속어 사전과 검사 결과를 기준으로 콘텐츠를 추가 삭제하거나 복원하지 않습니다.
- 비속어 검사 변경은 알림, 푸시 구독 및 소셜 관계 처리 범위를 변경하지 않습니다.
- `비활성화` 계정의 프로필과 배경 이미지 파일 및 업로드 파일 정보 메타정보는 유지하고 다른 사용자의 프로필 접근만 제한합니다.
- `비활성화` 계정이 재로그인으로 `활성` 상태가 되면 유지한 프로필과 배경 이미지를 다시 노출합니다.
- 영구 탈퇴 대기 유예기간에는 영구 탈퇴 취소 가능성을 위해 프로필과 배경 이미지 파일을 유지합니다.
- 관리자 `현 사용자 관리` 상세와 `신고 관리` 상세에서는 `활성`, `비활성화`, 영구 탈퇴 대기 회원의 보존된 프로필 및 배경 이미지에 보기 버튼을 제공하고 관리자 인증을 거친 모달에서만 표시합니다.
- 관리자 목록에는 이미지 썸네일이나 별도 이미지 열을 추가하지 않으며, 이미지 경로가 없거나 저장소 조회에 실패하면 미등록 또는 조회 실패 상태를 표시합니다.
- 유예기간이 끝나 회원 원본이 물리 삭제되면 해당 회원이 등록한 프로필과 배경 이미지의 물리 파일 및 업로드 파일 정보 메타정보를 모두 삭제하며 복원하지 않습니다.
...
- 프로필 또는 배경 이미지가 교체되면 사용자 참조와 기존 업로드 파일 정보 메타정보 삭제를 같은 트랜잭션에서 처리하고, 커밋이 완료된 뒤 기존 물리 파일을 삭제합니다.
- 외부 URL만 메타정보로 저장된 이미지는 로컬 물리 파일 삭제 대상에서 제외하고 업로드 파일 정보 메타정보만 삭제합니다.
...
- 임시 파일은 공개 `/uploads` 경로와 업로드 파일 정보에 등록하지 않으며 로그인 사용자와 이미지 유형으로 격리합니다. 화면에는 실제 임시 경로 대신 인증 응답의 축소 Data URL과 UUID 식별값만 전달합니다.
...
- 최종 프로필 저장은 임시 원본을 영구 저장소의 `profile/yyMMdd` 또는 `background/yyMMdd` 객체 키로 승격하고 업로드 파일 정보 메타정보와 사용자 참조가 모두 커밋된 뒤 임시 선택본을 삭제합니다. 트랜잭션이 롤백되면 영구 객체를 삭제하고 만료 전 재시도를 위해 임시 선택본을 유지합니다.
