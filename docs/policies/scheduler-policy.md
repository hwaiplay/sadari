...
- `측정 중(RUNNING)` 상태이고 `ALRM_DATE`가 현재 시각 이하이며 `SEND_DATE`가 없는 정상 이용 회원의 세션을 목표 종료시각에 `완료(COMPLETED)` 처리합니다.
- 자동 완료 시 `ALRM_DATE`까지의 측정 구간만 세션과 날짜별 누적 독서시간에 반영하며 스케줄러 실행 지연 시간은 제외합니다.
...
- 자동 완료됐지만 알림 저장에 실패한 `완료(COMPLETED)` 세션은 `ALRM_DATE`를 유지하며 다음 실행에서 알림만 재시도합니다.
- 알림 저장과 `SEND_DATE` 갱신은 같은 트랜잭션으로 처리하며 성공하면 `ALRM_DATE`를 제거합니다.
- `일시정지(PAUSED)`, 수동 `완료(COMPLETED)`, `계정 비활성화(WITHDRAWN)` 또는 `영구 탈퇴 대기(DELETE_PENDING)` 전환으로 `ALRM_DATE`가 제거된 세션은 완료 또는 알림 대상으로 처리하지 않습니다.
