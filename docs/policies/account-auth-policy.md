...
- 로그인 제공자 코드는 축약형 대신 `KAKAO`, `NAVER`, `GOOGLE` 풀네임을 사용하며 `USER_PROV` 공통코드로 관리합니다.
- 회원 원본의 제공자 코드는 `TM_USERXM.USER_PROV`, 로그인 이력의 제공자 코드는 `TB_LOGHIS.PROV_CODE`에 같은 값으로 저장합니다.
...
- 회원 상태 캐시는 Refresh Token과 같은 TTL을 유지하며 `ACTIVE`만 짧은 TTL로 반복 조회하지 않습니다.
...
| `SUSPENDED` | 관리자 이용 정지 상태, 정지 안내·영구 탈퇴·로그아웃 및 최소 인증 API만 허용 |
| `DELETE_PENDING` | 영구 삭제 대기 화면, 탈퇴 취소, 로그아웃 및 최소 인증 API만 허용 |

## 관리자 이용 정지 계정 로그인

- 이용 정지 중에는 `TM_USERXM` 원본 행과 암호화된 `USER_IDXX`를 유지합니다.
- 같은 Kakao 계정으로 로그인하면 기존 회원 행을 조회하므로 신규 가입으로 우회하지 못하고 `SUSPENDED` 상태를 계속 적용합니다.
- 관리자는 사용자 Redis에 직접 연결하지 않고 회원 상태 변경과 `TB_EVTBOX` 이벤트를 같은 DB 트랜잭션으로 저장합니다.
- 사용자 서버는 `USER_STATUS_SYNC` 스케줄러를 5분마다 실행해 이벤트 대상의 현재 DB 상태를 `auth:user:status:{userNumb}`에 반영합니다.
- 따라서 이미 로그인한 세션의 관리자 정지·해제 반영은 정상 운영 시 최대 5분이 걸릴 수 있습니다.
- Redis 반영에 실패한 `TB_EVTBOX` 이벤트는 삭제하지 않고 다음 5분 주기에 재시도합니다.
- Redis 반영에 성공하면 `TH_USSPND.SYNC_STAT`을 `COMPLETED`로 변경한 뒤 임시 전달 이벤트를 삭제합니다.
...
- 관리자 `현 사용자 관리`는 `TM_USERXM`에 현재 원본 행이 존재하는 회원만 조회합니다.
- 목록 기본 조건은 `ACTIVE`이며 검색 시 `WITHDRAWN`, `SUSPENDED`, `DELETE_PENDING`을 포함한 전체 상태를 선택할 수 있습니다.
- 로그인 제공자는 `USER_PROV` 공통코드 선택값으로 검색하며 목록·상세·로그인 이력의 표시명은 `FN_GET_CODE_NAME`으로 조회합니다.
...
- 암호화된 외부 사용자 식별값인 `TM_USERXM.USER_IDXX`는 관리자 API 응답과 화면에 제공하지 않습니다.
...
- 영구 삭제가 완료되어 `TM_USERXM` 원본 행이 제거된 회원은 현재 사용자 목록과 상세에서 조회하지 않습니다.
