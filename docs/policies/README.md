...
| [전역 연결 오류 화면 정책](connection-error-screen-policy.md) | JDBC, 브라우저 오프라인, 백엔드 단절의 표시·해제·제외 기준 |
...
1. 이 문서는 현재 소스에 구현된 동작을 기준으로 작성하며, 구현 전 확정 정책은 각 문서에서 적용 시점과 미구현 상태를 명시합니다.
...
5. 정책에 사용되는 공통코드는 `사용 여부 = 'Y'`인 분류값만 유효한 값으로 취급합니다.
...
| 회원 정지 유형 | 회원 정지 유형 |
| 이용정지 사유 | 회원 정지 사유 |
| 이용정지 처리 상태 | 이용정지 처리 상태 |
| 팔로우 관계 상태 | 팔로우 버튼 상태 |
| 알림 상황 코드 | 알림 상황 |
| 팝업 사용 화면 코드 | 팝업 사용 화면 구분 |
| 스케줄러 코드 | 스케줄러 활성 상태 |
| 금칙어 | 차단 비속어 |
| 금칙어 예외 표현 | 비속어 예외 허용어 |
| 신고 대상 유형 | 신고 대상 유형 |
| 신고 사유 분류값 | 신고 사유 |
| 신고 처리 상태 분류값 | 신고 처리 상태 |
| 신고 조치 결과 | 신고 누적 자동 조치 유형 |
| 신고 처리 결과 | 신고 누적 자동 조치 결과 |
