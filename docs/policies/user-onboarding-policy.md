...
4. 네 번째 슬라이드에는 가입 시 발급한 랜덤 닉네임을 표시하며, 사용자는 기본 닉네임을 유지하거나 수정한 뒤 다음 단계로 이동합니다.
5. 마지막 슬라이드에는 `CATE_CODE`의 활성 1뎁스 대분류별로 그 하위 관심분야를 나누어 항상 표시합니다.
6. 각 대분류의 첫 선택 항목은 해당 대분류 전체 선택 및 해제를 처리하고, 모든 소분류를 개별 선택해도 전체 선택 상태로 표시합니다.
7. 마지막 슬라이드는 닉네임 단계에서 확정한 닉네임과 함께 관심분야를 2개 이상 선택하도록 안내합니다.
8. 사용자는 관심분야를 2개 이상 선택한 뒤 화면 하단의 시작하기를 누르며, 선택 개수가 부족하면 경고를 표시하고 완료 처리를 중단합니다.
9. 선택한 관심분야는 별도 요청으로 `TB_USINTR`를 전체 교체합니다.
10. 서버는 닉네임과 온보딩 완료 여부를 한 트랜잭션으로 저장합니다.
11. 인증 상태를 다시 조회한 뒤 홈 화면으로 이동합니다.
...
- 관심분야 저장에 실패하면 닉네임과 `ONBD_YSNO = 'Y'` 저장을 진행하지 않고 현재 화면에서 재시도할 수 있습니다.
...
| `TB_USINTR` | 회원·세부코드 조합 | 최초 로그인에서 선택한 `CATE_CODE` 하위 독서 관심분야. 웰컴 완료 시 2개 이상 저장 |
