...
- 모임원 프로필 목록은 같은 모임의 `ACTIVE` 회원에게만 제공하며, 계정 상태가 `ACTIVE`이고 `TB_CLMEMX.PROF_YSNO = 'Y'`인 모임원만 노출합니다.
- `WITHDRAWN` 또는 `DELETE_PENDING` 전환 시 모든 모임 관계의 `PROF_YSNO`를 즉시 `N`으로 변경합니다. 비활성화 복귀 또는 영구 탈퇴 취소 후에도 자동으로 `Y`로 복원하지 않습니다.
...
- 비활성화 복귀 또는 영구 탈퇴 취소 시에는 계정 제한 동안 모임 편집·삭제가 실행되지 않았으므로 기존 모임 데이터를 별도로 복원하지 않고 유지된 모임원 자격과 운영 상태를 기준으로 접근을 재개합니다. 단, 모임원 프로필 노출 상태는 자동 복원하지 않습니다.
