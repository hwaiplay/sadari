...
- 현재 구현 범위는 공통코드, `TH_CMPLNT` 테이블, 화면 전용 사용자 신고 페이지 및 관리자 신고 조회·처리 화면입니다.
- 사용자 신고 대상은 관리자 신고 상세에서 기존 회원 이용정지 기능을 동일하게 사용할 수 있습니다.
- 신고 등록 API와 사용자 화면의 실제 접수 및 콘텐츠 대상 자동 제재는 미적용 상태입니다.
...
- 현재 사용자 신고 화면은 독후감과 댓글 정보를 표시하지만 서버에 신고 데이터를 전송하지 않습니다.
...
- `CMPL_OTHER`를 선택하면 사용자 화면에서 상세 사유를 필수로 입력합니다.
- 상세 사유는 현재 화면에서 최대 500자로 제한하며 신고 API 구현 시 저장 길이와 검증 기준을 별도로 확정합니다.
