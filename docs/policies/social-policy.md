...
| --- | --- |
| 내가 상대를 팔로우함 | 팔로잉 |
| 나는 팔로우하지 않고 상대만 나를 팔로우함 | 맞팔로우 |
| 양쪽 모두 팔로우하지 않음 | 팔로우 |

- 내가 상대를 팔로우 중이면 상대의 반대 방향 관계와 관계없이 `팔로잉`을 우선 표시합니다.
...
- 공개 독후감 목록에서는 로그인 사용자가 팔로우하는 `ACTIVE` 작성자의 공개 독후감을 먼저 배치합니다.
- 유지 중인 팔로우 관계라도 작성자가 `WITHDRAWN` 또는 `DELETE_PENDING`이면 비공개 전환된 독후감을 목록과 우선 정렬에 사용하지 않습니다.
