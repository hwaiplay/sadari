...
관리자 서비스는 `ALIM_SITU`와 `TEMP_CODE` 중복을 검사하고 `TB_ALTEMP`에 등록·수정 관리자와 일시를 함께 저장한다. 쓰기 권한이 있는 관리자에게만 저장 버튼을 노출하고, 백엔드 `MenuPermissionInterceptor`가 API 쓰기 권한을 다시 확인한다.
...
서비스는 관리자가 `TB_ALTEMP`에 등록한 사용 중 템플릿을 조회하고 제목·내용·링크를 조합해 `TB_ALIMXX`에 저장한다. 좋아요, 팔로우, 독서 목표기간 초과 등 서로 다른 도메인이 같은 발송 흐름을 재사용한다.

| 발생 기능 | 알림 상황 | 템플릿 코드 | 관리자 템플릿과 사용자 실행 연결 |
| --- | --- | --- | --- |
| 독후감 좋아요 | `LIKE` | `LIKE_REPORT` | 관리자 문구의 사용자명 치환 후 독후감 링크 생성 |
| 댓글·대댓글 좋아요 | `LIKE` | `REPLY_LIKE` | 관리자 문구의 사용자명 치환 후 해당 댓글이 속한 독후감 링크 생성 |
| 사용자 팔로우 | `FOLLOW` | `FOLLOW_USER` | 관리자 문구의 사용자명 치환 후 프로필 링크 생성 |
| 독서 모임 초대 | `FOLLOW` | `INVITE_CLUB` | 관리자 문구의 모임장 닉네임과 모임명 치환 후 모임 링크 생성 |
| 목표기간 초과 | `REPORT` | `REPORT_DATE_OVER` | 관리자 문구의 `#{bookTitl}` 치환 후 독후감 링크 생성 |
...
- 일반 알림은 `USEE_YSNO = 'Y'`인 템플릿만 조회한다.
- 목표기간 초과 대상 SQL은 활성 `REPORT_DATE_OVER` 템플릿을 `INNER JOIN`한다.
...
알림센터 아이콘도 관리자 공통코드와 연결된다. `ALIM_SITU`의 `LIKE`는 독후감과 댓글 좋아요가 공유하고 `FOLLOW`는 팔로우 요청과 독서 모임이 공유한다. 사용자 알림 조회는 `TB_ALIMXX.ALIM_SITU`로 `TM_ALICON`을 조인해 저장된 아이콘을 반환하며, 등록된 아이콘이 없으면 `DEFAULT` 아이콘을 사용한다.
...
`SCHD_CODE`도 `sadari-admin`의 공통코드 관리 화면에서 변경한다. 관리자가 특정 상세코드의 사용 여부를 `N`으로 바꾸면 사용자 백엔드는 다음 Cron 실행에서 업무 서비스와 실행 로그 생성을 모두 건너뛴다. 서버 재기동이나 사용자 서비스 재배포는 필요하지 않다.
...
사용자 백엔드가 `TL_SCLOGX`, `TL_SCFAIL`에 기록한 결과를 관리자 서비스가 읽는다.
