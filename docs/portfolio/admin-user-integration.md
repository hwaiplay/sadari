...
| --- | --- | --- | --- |
| 공통코드 관리 | 공통코드 그룹과 세부 항목 | 상태 검증, 화면 선택지, 비속어 사전, 색상, 닉네임, 스케줄러 | 사용자 API와 백엔드가 활성 분류값을 조회 |
| 알림 템플릿 관리 | 알림 템플릿 | 좋아요, 팔로우, 목표기간 초과 알림 | 상황·템플릿 코드로 활성 템플릿 조회 후 치환 |
| 사용자 메뉴 관리 | 사용자 화면 메뉴 | Header 제목, 햄버거 메뉴 | 현재 URL과 노출·사용 여부를 조회 |
| 스케줄러 실행 제어 | 공통코드 세부 항목의 스케줄러 코드 | 목표기간 초과, 알림 삭제, 회원 영구 삭제 | 실행 직전 활성 코드 존재 여부 확인 |
| 스케줄러 로그 조회 | 스케줄러 실행 로그, 스케줄러 실행 실패 상세 | 사용자 백엔드가 기록한 실행 결과 | 관리자 목록·상세 화면에서 조회 |
| 관리자 메뉴·권한 | 관리자 화면 메뉴, 관리자 권한그룹, 권한그룹별 관리자 메뉴 권한, 관리자 계정 | 운영 데이터 변경 권한 통제 | 메뉴별 조회·쓰기·삭제 권한 검사 |

## 공통코드 운영

### 관리자 제어

관리자는 공통코드 마스터와 분류값을 등록·수정하고 분류값의 사용 여부, 정렬 순서, 옵션값을 관리한다. 관리 API는 중복 코드와 필수값을 검증하고 등록·수정 관리자 정보를 남긴다.
...
| 코드 그룹 | 사용자 기능 |
| --- | --- |
| 독서 상태 | 독후감 독서 상태 선택과 백엔드 유효성 검증 |
| 도서 표지 대표 색상 | 독서 달력 색상과 표지 대표색의 CIELAB 매칭 후보 |
| 팔로우 관계 상태 | 팔로우 관계별 버튼명 |
| 알림 상황 코드 | 알림 상황명과 아이콘 옵션 |
| 스케줄러 코드 | 스케줄러별 실행 중지·재개 |
| 금칙어, 금칙어 예외 표현 | 비속어 차단 사전과 정상 표현 예외 사전 |
| 닉네임 주어, 닉네임 서술어, 닉네임 동물 명사 | 신규 회원 자동 닉네임 조합 |
| 사용자 계정 상태 | 활성, 계정 비활성화, 영구 삭제 대기 상태 |
| 계정 처리 유형, 계정 처리 사유 코드 | 탈퇴 방식과 탈퇴 사유 검증 |
...
알림 상황 코드의 `확장 속성 1 코드 = '1'`과 확장 속성 1 이름은 알림센터 아이콘 표현에도 사용된다. 사용자 알림 조회 SQL이 옵션값을 함께 반환하고 `AlimPage`가 `HEART`, `FOLLOW` 같은 이름을 실제 아이콘과 색상 스타일로 매핑한다. 따라서 관리자는 코드명뿐 아니라 사용자 화면의 알림 시각 분류에 필요한 메타데이터도 관리한다.
...
관리자 알림 템플릿 화면은 다음 값을 알림 템플릿에 등록·수정한다.

- 알림 상황
- 템플릿 식별값
- 관리용 제목 템플릿 관리 제목
- 사용자에게 전달할 알림 제목
- `#{key}` 형식의 치환 문구가 포함된 내용 템플릿 내용
- 대상 번호와 결합할 이동 URL 연결 화면 URL
- 사용 여부
- 등록·수정 관리자와 일시

알림 상황 코드와 템플릿 코드를 복합 식별값으로 사용하며, 템플릿 코드는 영문 대문자와 밑줄만 허용한다.
...
2. 알림 서비스는 알림 템플릿에서 `사용 여부 = 'Y'`인 템플릿을 조회한다.
3. 템플릿 내용의 `#{key}`를 전달받은 Map 값으로 치환한다.
4. 연결 화면 URL와 대상 번호를 결합한다.
5. 동일한 최종 알림이 최근 1시간 안에 없을 때 사용자 알림 발송 내역에 저장한다.
...
| --- | --- | --- | --- |
| 독후감 좋아요 | `LIKE` | 독후감 좋아요 | 발신 사용자명 |
| 댓글·대댓글 좋아요 | `LIKE` | 댓글 좋아요 | 발신 사용자명 |
| 사용자 팔로우 | `FOLLOW` | 사용자 팔로우 | 발신 사용자명 |
| 독서 모임 초대 | `FOLLOW` | 독서모임 초대 | 모임장 닉네임, 모임명 |
| 목표 독서기간 초과 | `독후감` | `REPORT_처리 기준일 초과 상태` | 도서 제목 `bookTitl` |
...
- 목표기간 초과 대상 조회는 활성 `REPORT_처리 기준일 초과 상태` 템플릿을 `INNER JOIN`하므로 템플릿이 없거나 미사용이면 대상 자체를 선택하지 않는다.
...
관리자는 사용자 화면 메뉴에서 단일 상위 메뉴 번호, 자기참조 상위 메뉴 번호, 메뉴 단계를 기준으로 최대 3단계 메뉴의 이름, URL, 계층별 정렬 순서, 노출 여부와 사용 여부를 관리한다. 관리자 서비스는 순환 참조, 4단계 이동과 하위 메뉴가 있는 메뉴 삭제를 차단하고, 사용자 API는 유효한 부모에서 이어지는 메뉴만 `childList` 트리로 제공한다.
...
3. 현재 메뉴가 있으면 Header에 메뉴 이름을 표시한다.
4. 일치하는 메뉴가 없으면 기존 로고를 표시한다.
5. 햄버거 메뉴에는 `화면 노출 여부 = 'Y'`이면서 `사용 여부 = 'Y'`인 메뉴만 정렬 순서대로 표시한다.
...
사용자 백엔드의 `Scheduler`는 Cron 실행 직전에 스케줄러 코드의 활성 분류값을 확인한다.

| 코드 | 업무 |
| --- | --- |
| `REPORT_처리 기준일 초과 상태` | 목표 독서기간 초과 알림 |
| 알림 만료 정리 | 삭제 상태 알림 물리 삭제 |
| `장기 미접속 사용자 기준_DELETE` | 영구 삭제 대기 회원 물리 삭제 |
...
- 스케줄러 실행 로그: 실행 번호, 스케줄러 코드, 메서드명, 실행 상태, 시작·종료 시각, 대상·성공·실패 건수, 실행 시간
- 스케줄러 실행 실패 상세: 실행 번호별 실패 순번, 실패 유형, 업무 결과 코드·메시지, 예외 유형·내용, 실패 시각
...
- 관리자 권한그룹: 관리자 권한 그룹
- 관리자 화면 메뉴: 관리자 화면 메뉴
- 권한그룹별 관리자 메뉴 권한: 권한 그룹별 메뉴 조회·쓰기·삭제 권한
- 관리자 계정의 권한그룹 코드: 관리자에게 부여한 권한 그룹

관리자 로그인은 관리자 계정의 권한그룹 코드와 공통코드 그룹 권한그룹 코드를 연결해 권한 레벨을 조회한다. 관리자 계정의 권한 코드, 권한 그룹 마스터와 권한 레벨 공통코드가 함께 일치해야 정상 인증과 메뉴 권한 판단이 가능하다.
...
같은 권한 그룹의 메뉴 권한 변경은 인터셉터가 요청마다 DB를 조회하므로 다음 요청부터 반영된다. 특정 관리자의 권한그룹 코드를 다른 그룹으로 변경하면 기존 Redis 세션에는 이전 코드가 남으므로 재로그인 또는 세션 만료 전까지 이전 그룹 기준으로 동작할 수 있다.
