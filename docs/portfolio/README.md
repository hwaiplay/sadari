...
| 인증·세션 | Kakao OAuth, `sid` JWT, Redis 기기별 세션, 현재·전체 디바이스 로그아웃과 다중 탭 Token 회전 | 구현 완료 |
...
마이페이지 개선의 `89.5%`는 소스에서 계산한 SQL 왕복 횟수 감소율이다. 개발 DB의 API 응답 시간 중앙값은 219.012ms에서 103.999ms로 52.51% 감소했으며, 운영 환경의 응답 시간과 처리량은 별도 부하 측정이 필요하다.
...
- 배포 설정은 EC2, 외부 MySQL 8.4 DB, 컨테이너 Redis를 대상으로 준비되어 있으나 저장소만으로 실제 운영 배포 완료 여부를 판단할 수 없다.
- 파일 저장은 Docker 볼륨의 로컬 업로드 경로를 사용하며 AWS S3 저장소 연동은 현재 구현 근거가 없다.
- 개발 DB의 단일 클라이언트 API 응답 시간은 측정했지만 운영 환경의 평균, P95, P99와 동시 처리량은 측정하지 않았다.
