...
Kakao 로그인 성공 시 Provider 사용자 식별값을 내부 회원 조회 키로 사용한다. 이 값은 평문으로 저장하지 않고 `UserIdEncryptionService.encryptForStorage`를 거친 결정적 암호문으로 사용자 계정과 프로필의 OAuth 사용자 식별값 암호문에 저장한다.
...
- 사용자 번호, 권한, Token ID `jti`와 기기별 세션 식별자 `sid`를 담는다.
- 유효시간이 짧다.
- API 인증에 사용한다.
...
- Access Token과 같은 기기별 `sid`를 담는다.
...
`TokenRedisService`는 Lua 스크립트를 사용해 세션 Hash, 회원별 세션 Set, 닉네임과 회원 상태를 한 번에 생성한다. 세션 Hash와 Set은 Refresh Token 유효시간을 적용하고 재발급 시 TTL을 갱신한다. 닉네임은 로그인 또는 수정 시 활성 세션 TTL을 적용하며, 회원 상태는 로그아웃으로 계정 제한이 사라지지 않도록 TTL 없는 별도 키로 유지한다. 회원 상태 캐시가 없으면 `활성`로 가정하지 않고 DB 원본 상태를 조회해 다시 저장한다.

구형 `auth:refresh:{userNumb}`는 사용자당 Refresh Token 하나만 저장해 다른 기기 로그인이 기존 토큰을 덮어쓸 수 있었다. 현재 코드는 이 키를 읽거나 생성하지 않으며, 기기별 `sid` 세션 전환 이후 남은 구형 키는 모든 구버전 서버가 종료된 뒤 TTL 만료 또는 운영 정리 대상으로 처리한다.
...
4. JWT의 `sid`가 Redis에서 같은 회원의 활성 세션인지 확인한다.
5. Redis 회원 상태를 확인하고 캐시가 없으면 DB 원본으로 보정한다.
6. 회원 상태별 허용 경로를 적용한 뒤 Spring Security 인증 객체를 구성한다.
...
`src/main/frontend/src/app/api/axios.ts`는 한 탭에서 진행 중인 재발급 Promise를 `refreshRequest` 하나로 공유한다. 최초 요청만 Refresh API를 호출하고 나머지는 같은 Promise를 기다린 뒤 원래 요청을 재시도한다. Refresh 또는 Logout 요청 자체는 재발급 대상에서 제외해 순환 호출을 차단한다.

브라우저의 다른 탭과 서비스워커는 메모리의 Promise를 공유할 수 없다. 서버는 Redis Lua 스크립트로 현재 Refresh Token을 직전 Token으로 이동하고 새 Token을 저장하는 과정을 원자적으로 처리한다. 기본 10초 유예시간 안에 직전 Token을 제출한 동시 요청에는 이미 저장된 최신 Token을 반환해 정상 탭이 세션 충돌로 로그아웃되지 않게 한다.

## 로그아웃 범위와 탭 동기화

- 로그아웃 Alert는 현재 디바이스, 전체 디바이스와 취소를 구분한다.
- 현재 디바이스 로그아웃은 현재 `sid` 세션과 브라우저에서 확인할 수 있는 현재 FCM Token만 비활성화한다.
- 전체 디바이스 로그아웃은 회원의 모든 `sid` 세션과 모든 FCM 구독을 비활성화한다.
- 같은 브라우저 프로필의 탭은 Cookie를 공유하므로 `BroadcastChannel`과 `storage` 이벤트로 인증 Store와 Query 캐시를 함께 정리한다.
- 다른 디바이스는 다음 인증 요청에서 Redis 세션 부재를 확인해 로그아웃된다.
...
1. 관리자 계정에서 관리자 계정과 권한 코드를 조회한다.
...
관리자 권한은 관리자 권한그룹, 관리자 화면 메뉴, 권한그룹별 관리자 메뉴 권한으로 관리하고 관리자 계정의 권한그룹 코드로 관리자에게 그룹을 부여한다. 같은 그룹의 메뉴 권한 변경은 요청마다 DB에서 검사하므로 다음 요청부터 반영된다. 반면 특정 관리자의 권한 그룹 변경은 기존 Redis 세션에 이전 권한 코드가 남아 재로그인이나 세션 무효화가 필요하다.
...
- 영구 탈퇴: 영구 탈퇴 대기로 전환하고 기본 30일 유예 후 스케줄러가 물리 삭제
