...
`TokenRedisService`는 Lua 스크립트를 사용해 세션 Hash, 회원별 세션 Set, 닉네임과 회원 상태를 한 번에 생성한다. 세션 Hash와 Set은 Refresh Token 유효시간을 적용하고 재발급 시 TTL을 갱신한다. 닉네임은 로그인 또는 수정 시 활성 세션 TTL을 적용하며, 회원 상태는 로그아웃으로 계정 제한이 사라지지 않도록 TTL 없는 별도 키로 유지한다. 회원 상태 캐시가 없으면 `ACTIVE`로 가정하지 않고 DB 원본 상태를 조회해 다시 저장한다.
