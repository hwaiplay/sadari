...
소스 구조 기준 SQL 왕복 횟수는 약 89.5% 감소했다. 같은 개발 DB에서 개선 전 구조와 현재 구조를 각각 10회 준비한 뒤 `GET /api/user/monthly-reading-summary`를 100회씩 교차 호출했다. 요청 시작부터 응답 본문 수신 완료까지의 평균은 228.044ms에서 107.761ms로 52.75%, P95는 279.559ms에서 129.038ms로 53.84% 감소했다. 두 서버의 응답 본문도 동일함을 확인했다.

API 처리에는 인증·세션 확인, 공통 프로필 통계 조회와 JSON 직렬화가 포함된다. 단일 클라이언트의 개발 환경 측정이므로 운영 데이터와 동시 요청 기준 검증은 추가로 필요하다.

규모 증가 검증은 격리 MySQL 8.4.10에 최근 5년치 독후감과 주·월·연 목표 329건을 생성해 수행했다. 개선 전후 결과 동등성을 확인한 뒤 준비 5회, 본 측정 30회를 실행했으며 독후감 100건, 1,000건, 10,000건에서 모두 JDBC 처리 시간 감소를 확인했다. 로컬 단일 연결의 합성 데이터 결과이며 API 계층과 동시 요청은 포함하지 않았다.
