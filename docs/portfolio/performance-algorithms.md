...
2. `getReadingSummaryList`: 현재 읽는 책과 현재 연도 완료 책을 한 번 조회
...
소스 구조 기준 SQL 왕복 횟수는 약 89.5% 감소했다. 같은 개발 DB에서 개선 전 구조와 현재 구조를 각각 10회 준비한 뒤 `GET /api/user/monthly-reading-summary`를 100회씩 교차 호출했다. 요청 시작부터 응답 본문 수신 완료까지의 중앙값은 219.012ms에서 103.999ms로 52.51%, P95는 279.559ms에서 129.038ms로 53.84% 감소했다. 두 서버의 응답 본문도 동일함을 확인했다.

API 처리에는 인증·세션 확인, 공통 프로필 통계 조회와 JSON 직렬화가 포함된다. 단일 클라이언트의 개발 환경 측정이므로 운영 데이터와 동시 요청 기준 검증은 추가로 필요하다.

규모 증가 검증은 격리 MySQL 8.4.10에 최근 5년치 독후감과 주·월·연 목표 329건을 생성해 수행했다. 개선 전후 결과 동등성을 확인한 뒤 준비 5회, 본 측정 30회를 실행한 결과 JDBC 전체 처리 중앙값은 독후감 100건에서 14.953ms→5.664ms(62.12%), 1,000건에서 34.558ms→16.296ms(52.85%), 10,000건에서 314.315ms→175.091ms(44.29%)로 감소했다. 로컬 단일 연결의 합성 데이터 결과이며 API 계층과 동시 요청은 포함하지 않았다.
...
- 공백이 없는 단어 안의 반복 문자 우회 표현 탐지
- 공백으로 분리된 정상 문장의 단어 경계 보존
...
1. 금칙어와 금칙어 예외 표현 공통코드를 한 번 조회한다.
...
7. 숫자·특수문자 우회는 공백을 보존한 별도 정규화 문자열로 검사한다.
8. 공백이 없는 단어 안에서 같은 문자가 두 번 이상 반복되면 반복 구간의 축약본과 제거본을 추가 검사한다.
9. 일반 한글·영문 한 글자 삽입과 공백으로 분리된 문자열은 하나의 비속어로 합치지 않는다.
...
`BookCoverColorService`는 카카오 도서 표지를 분석해 도서 표지 대표 색상 공통코드 중 가장 가까운 색상을 선택한다.

1. 신뢰한 카카오 이미지 URL과 기존 데이터 호환용 NAVER 이미지 URL만 다운로드한다.
...
도서 표지 대표 색상 팔레트는 기존 파스텔 중심 20색에 아래 6색을 추가해 26색으로 확장했다. 난색, 저명도 색상, 중성색 영역을 보완해 표지 대표색과 가까운 후보를 선택할 수 있다.
...
스케줄러 코드는 스케줄러 실행 직전에 백엔드가 직접 조회하므로 다음 Cron 실행부터 변경을 반영한다. 사용자 메뉴와 알림 템플릿도 별도 장기 캐시 없이 다음 조회·발송에서 DB 값을 사용한다.
...
| 통합 SQL | 소스 구현 완료 | MySQL `EXPLAIN ANALYZE`와 논리 읽기 |
