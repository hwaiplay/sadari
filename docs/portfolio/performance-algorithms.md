...
소스 구조 기준 SQL 왕복 횟수는 약 89.5% 감소했다. 비로컬 MySQL 8.4.10 개발 DB에서 준비 10회 후 100회 측정한 JDBC 전체 처리 시간은 중앙값 215.241ms에서 26.436ms로 87.72%, P95 578.272ms에서 80.201ms로 86.13% 감소했다. 대표 사용자의 독후감은 6건, 목표는 4건이었으며 Spring Boot API 계층을 포함하지 않은 작은 개발 데이터셋 결과이다. 운영 데이터와 동시 요청 기준 측정은 추가로 필요하다.
