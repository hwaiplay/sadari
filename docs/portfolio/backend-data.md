...
NAVER 도서 API 검색 결과는 외부 데이터이므로 내부 DB에 항상 존재하지 않는다. 독후감을 먼저 저장하거나 도서 저장과 독후감 저장을 별개 요청으로 처리하면 도서 없는 독후감이나 중복 도서가 생길 수 있다.
...
팔로우 버튼은 양방향 관계에 따라 `팔로우`, `맞팔로우`, `팔로잉`으로 달라진다. 이 판단을 화면마다 중복하지 않고 Oracle 함수 `FN_GET_FOLW_STAT`에 로그인 사용자와 대상 사용자 번호를 전달해 결정한다.
...
2. `OPT1_CODE`로 자연스러운 주어·서술어 조합을 제한한다.
3. 조합을 무작위로 선택한다.
4. `TB_NKSEQX`에서 닉네임 본문과 연월별 순번을 원자적으로 증가시킨다.
