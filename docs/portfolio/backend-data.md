...
- `읽는 중`: 목표 독서 기간을 사용하고 기록은 선택값으로 처리
- `완독`: 종료일과 평점을 저장할 수 있음
- `중단`: 중단일을 종료일로 처리
...
공통코드는 단순한 선택 목록을 넘어 서비스의 상태와 운영 기준을 일관되게 관리하는 기준정보로 사용된다.

| 코드 그룹 | 활용 |
| --- | --- |
| 독서 상태 | 독서 상태와 서비스 검증 |
| 도서 표지 대표 색상 | 달력 색상과 표지 대표색 매칭 |
| 팔로우 관계 상태 | 팔로우 버튼 상태 |
| 알림 상황 코드 | 알림 상황과 화면 아이콘 |
| 스케줄러 코드 | 스케줄러 실행 여부 |
| 금칙어 | 차단 비속어 사전 |
| 금칙어 예외 표현 | 정상 표현 예외 사전 |
| 닉네임 주어, 닉네임 서술어, 닉네임 동물 명사 | 신규 회원 닉네임 조합 |
...
`sadari-admin`의 공통코드 관리 기능은 분류 그룹과 세부 항목을 직접 관리한다. 사용자 서비스는 활성화된 기준정보를 상태 검증, 선택지, 비속어 사전, 색상 후보, 자동 닉네임과 스케줄러 실행 조건으로 사용한다.
...
| 테이블 | 관리자 서비스 | 사용자 서비스 |
| --- | --- | --- |
| 공통코드 그룹과 세부 항목 | 코드 등록·수정·삭제 | 상태·정책·표시값 조회 |
| 알림 템플릿 | 알림 템플릿 등록·수정 | 템플릿 조회·치환·알림 생성 |
| 사용자 화면 메뉴 | 단일 키·자기참조 부모 키 기반 최대 3단계 사용자 메뉴 계층·노출 관리 | Header 제목과 재귀 햄버거 메뉴 트리 조회 |
| 스케줄러 실행 로그, 스케줄러 실행 실패 상세 | 실행·실패 로그 조회 | 스케줄러 실행 결과 기록 |
...
사용자 좋아요 관계는 독후감 전용 번호 대신 다음 키를 사용한다.

- 사용자 번호
- 대상 유형
- 대상 번호

현재 서비스는 공개 독후감과 삭제되지 않은 댓글에만 좋아요를 허용한다. 독후감 좋아요는 social 도메인에서, 댓글 좋아요는 reply 도메인에서 대상의 공개·삭제 상태와 로그인 회원의 활성 상태를 각각 검증한다. 범용 관계 구조를 사용하더라도 임의의 대상 유형 문자열은 저장하지 않는다.
...
팔로우 버튼은 양방향 관계에 따라 `팔로우`, `맞팔로우`, `팔로잉`으로 달라진다. 이 판단을 화면마다 중복하지 않고 소셜 Mapper가 양방향 관계를 조회한 뒤 공통코드 표시명을 반환하도록 구성한다.
...
2. 주어의 확장 속성 1 코드부터 확장 속성 4 코드에 등록된 네 개의 서술어 코드로 자연스러운 조합을 제한한다.
3. 조합을 무작위로 선택한다.
4. 닉네임 조합별 연월 순번에서 주어·서술어·동물 분류값 조합과 연월별 순번을 원자적으로 증가시킨다.
5. 최초 동시 등록 충돌은 `DuplicateKeyException`을 처리한 뒤 기존 행 증가로 재시도한다.
6. 최대 100개 조합을 시도하고 25자 정책과 최종 중복 여부를 검사한다.

닉네임 조합별 연월 순번의 데이터 항목은 복합키인 주어 코드, 서술어 코드, 동물 명사 코드, 닉네임 발급 연월을 먼저 배치하고 마지막 발급 번호, 등록 일시, 수정 일시를 이어서 배치한다.
