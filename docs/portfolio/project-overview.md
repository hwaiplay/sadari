...
4. ISBN 기준 도서 마스터가 없으면 도서 마스터에 먼저 등록한다.
5. 확보한 도서 번호로 독후감에 독후감을 등록한다.
...
2. 관리자가 `sadari-admin`에서 등록한 활성 알림 템플릿을 식별할 상황·템플릿 코드를 사용한다.
3. 소셜 트랜잭션이 성공하면 공통 알림 서비스에 수신자, 상황, 템플릿, 이동 대상 정보와 치환값을 전달한다.
4. 알림 서비스가 관리 템플릿을 조회해 문구를 치환하고 이동 대상 정보와 함께 저장한다.
...
3. 활성 상태면 제한된 대상만 처리하고 스케줄러 실행 로그, 스케줄러 실행 실패 상세에 결과를 기록한다.
