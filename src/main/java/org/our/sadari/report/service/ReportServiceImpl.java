 ...
 * date           : 2026-07-17
 * description    : 독후감과 독서 목표 업무 로직을 구현
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class ReportServiceImpl implements ReportService {
    ...
    private static final int MONTH_GOAL_MAX_UPDATE_COUNT = 3; // 한 달 단위 월간 목표 내리기를 최대 3회까지 허용
    // 연간 목표 최대 UPDATE 건수 설정값
    private static final int YEAR_GOAL_MAX_UPDATE_COUNT = 5; // 장기 연간 목표 내리기를 최대 5회까지 허용
    ...
    /**
     * 로그인 사용자의 독후감 목록을 검색어와 정렬 조건에 맞춰 조회
     * 검색어는 HTML entity를 일반 텍스트로 보정한 뒤 Mapper에 전달
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData getReportByIsbnDtl(Long userNumb, String bookIsbn) {
        ...
        // 동일 ISBN의 기존 독후감 조회 조건을 담을 객체를 생성
        ReportDto reportDto = new ReportDto();
        // 로그인 사용자 본인의 독후감만 조회하도록 사용자 번호를 설정
        reportDto.setUserNumb(userNumb);
        // 외부 도서 검색 결과의 ISBN을 일반 텍스트로 보정해 조회 조건에 설정
        reportDto.setBookIsbn(StringUtil.normalizePlainText(bookIsbn));
        // 가장 최근에 작성한 동일 ISBN 독후감 조회 결과를 성공 응답으로 반환
        return ResultData.success(reportMapper.getReportByIsbnDtl(reportDto));
    }

    /**
     * 본인 또는 다른 사용자 화면에 표시할 주간, 월간, 연간 독서량 요약과 목표 달성 정보를 조회
     * 현재 기간과 직전 기간을 같은 기준으로 비교해 증감값과 펼침 목록을 함께 구성
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 독서 요약 통합 조회에 사용할 기간과 공통코드 조건을 생성
     *
     * @author SeungHyeon.Kang
     * @param userNumb 조회할 사용자 번호
     * @param pubcYsno 다른 사용자 조회에 적용할 독후감 공개 여부
    ...
    private ReadingSummaryQueryDto getReadingSummaryQueryReq(Long userNumb, String pubcYsno, LocalDate today) {
        ...
        // 기간과 목표 기준값이 설정된 통합 조회 조건을 반환
        return req;
    }

    /**
     * 통합 집계 결과를 마이페이지 독서 요약 응답으로 변환
     *
     * @author SeungHyeon.Kang
    ...
    private MonthlyReadingSummaryDto getReadingSummaryResponse(ReadingSummaryQueryDto result, LocalDate today) {
        ...
        // 화면 응답 형식으로 변환한 독서 요약을 반환
        return summary;
    }

    /**
     * 한 번 조회한 독후감 목록을 현재 읽는 책과 주간 및 월간 및 연간 목록으로 분류
     *
     * @author SeungHyeon.Kang
    ...
    private void applyReadingSummary(MonthlyReadingSummaryDto summary, List<ReportDto> reportList
        ...
        // 조회 결과가 있는 경우에만 각 화면 기간으로 목록을 분류
        if (!StringUtil.isEmpty(reportList)) {
            // 현재 읽는 책과 완료한 책을 상태 및 종료일 기준으로 순차 분류
            for (ReportDto report : reportList) {
                // 읽는 중인 독후감은 완료 기간과 관계없이 현재 읽는 책 목록에 포함
                ...
                // 완료 상태가 아니거나 종료일이 없는 데이터는 기간별 완료 목록에서 제외
    ...
    /**
     * 독서 종료일이 시작일 이상이고 종료 경계일 미만인지 확인
     *
     * @author SeungHyeon.Kang
    ...
    private boolean isReportInPeriod(String reptEndt, LocalDate periodStart, LocalDate periodEndExclusive) {
        ...
        // YYYY-MM-DD 형식의 문자열 순서가 날짜 순서와 같으므로 불필요한 날짜 변환 없이 범위를 비교
        return reptEndt.compareTo(periodStart.toString()) >= 0
                && reptEndt.compareTo(periodEndExclusive.toString()) < 0;
    }

    /**
     * 목표 기준일과 목표 유형을 DB 조회용 GOAL_DATE 값으로 변환해 현재 목표를 조회
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
    ...
    private ReadingGoalDto getReadingGoalDtl(Long userNumb, LocalDate targetDate, String goalType) {
        // 독서 목표 조회 또는 저장 조건을 담을 객체를 생성
        ReadingGoalDto req = new ReadingGoalDto();
        // UserNumb 업무 값을 req DTO에 설정
        req.setUserNumb(userNumb);
        // GoalDate 업무 값을 req DTO에 설정
        req.setGoalDate(getGoalDate(targetDate, goalType));
        // GoalType 업무 값을 req DTO에 설정
        req.setGoalType(goalType);
        // 목표 기준일과 목표 유형을 DB 조회용 GOAL_DATE 값으로 변환해 현재 목표를 조회 결과를 반환
        return reportMapper.getReadingGoalDtl(req);
    }

    /**
     * 완료 독후감 수와 목표 권수를 비교해 화면 표시용 달성률을 계산
     * 100%를 넘는 경우에도 진행 막대는 최대값으로 표시해야 하므로 100으로 제한
     *
     * @author SeungHyeon.Kang
    ...
    private int getGoalRate(int doneCount, Integer goalCount) {
        // 목표 권수가 없거나 0 이하이면 달성률 계산이 불가능하므로 0%로 처리
        if (StringUtil.isEmpty(goalCount) || goalCount <= 0) {
            // 완료 독후감 수와 목표 권수를 비교해 화면 표시용 달성률을 계산 결과를 반환
            return 0;
        }
     ...
     * 주간은 ISO week 기준 YYYYWW, 월간은 YYYYMM, 연간은 YYYY00 형식을 사용
     *
     * @author SeungHyeon.Kang
     * @param targetDate 목표 기준일
    ...
    private String getGoalDate(LocalDate targetDate, String goalType) {
        // 주간 목표는 ISO 주차 기준값을 사용해야 하므로 별도 변환 로직으로 분기
        if (Constant.GOAL_TYPE_WEEK.equals(goalType)) {
            // 목표 유형에 따라 TM_GOALXM.GOAL_DATE에 저장할 기준값을 만든다 결과를 반환
            return getGoalWeekDate(targetDate);
        }

        // 연간 목표는 월 정보가 필요 없으므로 YYYY00 형식으로 저장
    ...
    /**
     * ISO 주차 기준으로 주간 목표의 GOAL_DATE 값을 생성
     * 연말과 연초가 겹치는 주차를 올바르게 처리하기 위해 week-based-year를 사용
     *
     * @author SeungHyeon.Kang
     * @param targetDate 목표 기준일
    ...
    private String getGoalWeekDate(LocalDate targetDate) {
        ...
        // ISO 주차 기준으로 주간 목표의 GOAL_DATE 값을 생성 결과를 반환
        return String.format("%04d%02d", weekYear, weekNumber);
    }

    /**
     * 주간, 월간, 연간 독서 목표를 한 번에 저장
     * 목표를 올리는 것은 항상 허용하고, 목표를 낮추는 경우에만 기간과 횟수 제한을 적용
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData setReadingGoal(Long userNumb, ReadingGoalDto readingGoalDto) {
        // 주간, 월간, 연간 목표 중 하나라도 유효하지 않으면 저장 요청 전체를 거절
        if (!isValidReadingGoal(readingGoalDto)) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 목표 기간 계산의 기준이 되는 오늘 날짜를 조회
        LocalDate today = LocalDate.now();
        // getWeekGoalCnt 조회로 후속 처리에 필요한 데이터를 가져옴
    ...
    public ResultData copyPreviousReadingGoal(Long userNumb) {
        ...
        // 3. 복사된 목표가 단 1건도 없는 경우(이미 목표가 존재하거나 이전 목표 데이터가 없는 경우) 요청 실패로 응답
        if (copiedCount == 0) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 직전 기간(지난주, 지난달, 작년)에 설정되어 있던 독서 목표 데이터를 조회하여 현재 기간의 목표로 일괄 복사한 결과를 반환
        return getMonthlyReadingSummary(userNumb, null);
    }

    /**
     * 목표 타입별로 직전 기간의 목표 데이터를 검증하고 현재 기간의 목표로 단건 복사
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인한 회원 번호
     * @param today 현재 날짜
    ...
    private int copyPrevReadingGoal(Long userNumb, LocalDate today, LocalDate currentDate
        ...
        // 1. 이미 현재 기간에 설정된 목표가 존재하는 경우 덮어쓰지 않고 즉시 스킵
        ReadingGoalDto currentGoal = getReadingGoalDtl(userNumb, currentDate, goalType);
        // currentGoal 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (!StringUtil.isEmpty(currentGoal)) {
            // 목표 타입별로 직전 기간의 목표 데이터를 검증하고 현재 기간의 목표로 단건 복사한 결과를 반환
            return 0;
        }

        // 2. 직전 기간의 목표 데이터를 조회하여 값이 없거나 0 이하의 유효하지 않은 권수인 경우 스킵
        ReadingGoalDto previousGoal = getReadingGoalDtl(userNumb, previousDate, goalType);
        // previousGoal 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        ...
        // 3. 직전 기간의 목표 권수를 기반으로 현재 기간의 목표를 새로 등록
    ...
    /**
     * 목표 유형 하나에 대해 현재 목표와 신규 목표를 비교한 뒤 저장
     * 같은 값이면 DB 갱신을 생략하고, 낮추는 값이면 별도 제한 검증을 수행
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
     * @param today 현재 날짜
    ...
    private ResultEnum setReadingGoalByType(Long userNumb, LocalDate today, String goalType
        ...
        // GoalType 업무 값을 req DTO에 설정
        req.setGoalType(goalType);
        // GoalCnt 업무 값을 req DTO에 설정
        req.setGoalCnt(goalCnt);
        // ReadingGoal 업무 값을 reportMapper DTO에 설정
    ...
    /**
     * 목표 권수를 낮추는 요청인지 판단하고 낮추기 제한을 검증
     * 신규 설정 또는 목표 올리기는 제한하지 않고, 낮추기만 횟수와 마감 기간을 적용
     *
     * @author SeungHyeon.Kang
    ...
    private ResultEnum validateReadingGoalDown(ReadingGoalDto currentGoal, LocalDate today, String goalType
        ...
        if (getGoalUpdateLimit(goalType) <= getGoalUpdateCount(currentGoal)) {
            // 목표 권수를 낮추는 요청인지 판단하고 낮추기 제한을 검증 결과를 반환
    ...
    /**
     * 목표 유형별 목표 내리기 가능 횟수를 반환
     * 주간 1회, 월간 3회, 연간 5회 제한을 적용
     *
     * @author SeungHyeon.Kang
     * @param goalType 목표 유형
     * @return 목표 내리기 허용 횟수
    ...
    private int getGoalUpdateLimit(String goalType) {
        ...
        // 월간 목표는 주간보다 넓은 기간을 다루므로 3회까지 목표 내리기를 허용
    ...
    /**
     * 목표 권수와 사용한 수정 횟수를 기준으로 목표 내리기 잔여 횟수를 계산
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 목표 내리기가 가능한 잔여 일수를 계산
     * 기간이 이미 마감되었으면 음수가 내려가지 않도록 0으로 보정
     *
     * @author SeungHyeon.Kang
     * @param today 현재 날짜
     * @param goalType 목표 유형
    ...
    /**
     * 로그인 사용자가 작성한 독후감 상세 정보와 연결된 도서 정보를 조회
     * 사용자 번호를 조건에 포함해 다른 사용자의 비공개 독후감을 조회하지 못하도록 처리
     *
     * @author SeungHyeon.Kang
     * @return 독후감 상세 조회 결과
     */
    private String normalizeListSortType(String sortType) {
        // 허용된 정렬값만 Mapper에 전달해 동적 정렬 조건이 임의로 확장되지 않도록 처리
        if (Constant.SORT_START_DATE_DESC.equals(sortType) || Constant.SORT_GRADE_DESC.equals(sortType)) {
            // 로그인 사용자가 작성한 독후감 상세 정보와 연결된 도서 정보를 조회 결과를 반환
            return sortType;
        }
    ...
    public ResultData getDetail(Long userNumb, Long reptNumb) {
        // 대상 독후감 번호가 없으면 상세, 수정, 삭제 대상을 특정할 수 없으므로 실패 처리
        ...
        // UserNumb 업무 값을 reportDto DTO에 설정
        reportDto.setUserNumb(userNumb);
        // ReptNumb 업무 값을 reportDto DTO에 설정
        reportDto.setReptNumb(reptNumb);
        // Locale 업무 값을 reportDto DTO에 설정
        reportDto.setLocale(LocaleUtil.getLocale());
        // ReptStat 업무 값을 reportDto DTO에 설정
        reportDto.setReptStat(Constant.REPORT_STAT_DONE);

        // ReportDtl 데이터를 DB에서 조회
        ReportDto detail = reportMapper.getReportDtl(reportDto);

        // 조회 결과가 없으면 존재하지 않거나 접근할 수 없는 독후감으로 판단
        if (StringUtil.isEmpty(detail)) {
            // "조회 결과가 없어요."
            return ResultData.fail(ResultEnum.COMMON_NO_DATA);
        }

        // 독후감 번호로 독후감과 도서 상세 정보를 조회 결과를 성공 응답으로 반환
        return ResultData.success(detail);
    }

    /**
     * ISBN 기준 활성 사용자의 공개 독후감을 요청한 정렬 기준으로 조회
     * 로그인 사용자의 좋아요와 작성자 팔로우 여부를 함께 표시하기 위해 사용자 번호를 Mapper에 전달
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
     * @param bookIsbn 조회할 도서 ISBN
    ...
    public ResultData getPublicReportsByIsbn(Long userNumb, String bookIsbn, String sortType
        ...
        // UserNumb 업무 값을 reportDto DTO에 설정
        reportDto.setUserNumb(userNumb);
        // BookIsbn 업무 값을 reportDto DTO에 설정
        reportDto.setBookIsbn(StringUtil.normalizePlainText(bookIsbn));
        // 외부 정렬 코드를 허용 목록과 비교할 수 있는 문자열로 정규화
        String normalizedSortType = StringUtil.normalizePlainText(sortType);
        // 허용된 정렬 코드가 아니면 친구와 팔로잉 우선 기본순으로 보정
        ...
        // 다음 페이지 존재 여부를 판정할 한 건을 추가해 조회
        reportDto.setPageLimit(PAGE_SIZE + 1);
        // ISBN과 상태 및 정렬 조건으로 범위를 제한한 공개 독후감을 조회
        ...
        // 화면에는 현재 페이지 크기만 전달
        List<ReportDto> visibleList = hasNext ? safeList.subList(0, PAGE_SIZE) : safeList;
        // 현재 표시 언어와 월간 잔여량 및 캐시 상태로 카드별 번역 버튼 노출 여부를 설정
        reportTranslationService.applyReportAvailability(visibleList);
        // ISBN 기준 공개 독후감 페이지와 다음 페이지 여부를 반환
        return ResultData.success(new PageDto<>(visibleList, normalizedPage, hasNext));
    }

    /** 알림이 지정한 공개 독후감 한 건과 도서 정보를 조회 */
    @Override
    public ResultData getPublicReportTarget(Long userNumb, Long reptNumb) {
        ...
        // 공개 독후감 한 건을 성공 응답으로 반환
        return ResultData.success(target);
    }

    /**
     * ISBN 기준 도서 평균 별점을 조회
     * 평균 별점은 공개 여부와 무관하게 읽는 중 상태를 제외한 독후감을 기준으로 계산
     *
     * @author SeungHyeon.Kang
     * @param bookIsbn 조회할 도서 ISBN
     * @return 평균 별점 조회 결과
    ...
    public ResultData setReport(Long userNumb, ReportDto reportDto) {
        // 등록 요청의 도서 필수값이 누락되면 도서와 독후감 저장을 모두 중단
        ...
        // UserNumb 업무 값을 reportDto DTO에 설정
        reportDto.setUserNumb(userNumb);
        // setDefaultReportColor 호출로 업무 처리에 필요한 값을 설정
        setDefaultReportColor(reportDto);
        // 신규 독후감에 사용자 공개 및 알림 기본값을 적용
        applyNewReportDefaults(reportDto);
        // 독후감 입력값에서 허용하지 않는 스크립트 내용을 제거
        sanitizeReport(reportDto, true);
        // 읽는 중 독후감은 공개 목록과 평점 집계에 들어가지 않도록 저장값을 제한
        applyReadingStatusPolicy(reportDto);

        // validateReport 검증으로 잘못된 요청이 업무 로직에 진입하지 않도록 차단
        ReportValidationResult validationResult = validateReport(reportDto, true);
        // 업무 검증 실패가 있으면 DB 변경 전에 사용자에게 전달할 실패 결과를 반환
    ...
    public ResultData uptReport(Long userNumb, Long reptNumb, ReportDto reportDto) {
        ...
        if (reportMapper.uptReport(reportDto) == 0) {
            // 다른 탭이나 기기에서 먼저 변경한 원본을 덮어쓰지 않도록 충돌 결과를 반환
            return ResultData.fail(ResultEnum.COMMON_EDIT_CONFLICT);
        }

        // 기존 독후감 정보를 수정 결과를 성공 응답으로 반환
        return ResultData.success(reportDto.getReptNumb());
    }

    /**
     * 로그인 사용자가 작성한 독후감의 좋아요 또는 댓글 알림 사용 여부를 변경
     * 독후감 공개 여부와 관계없이 작성자 본인은 설정할 수 있으며, 유형별 전용 컬럼만 수정
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
     * @param reptNumb 수정할 독후감 번호
    ...
    public ResultData uptReportAlim(Long userNumb, Long reptNumb, String alimType
        ...
        // 인증 사용자, 변경 대상, 설정 유형과 사용 여부가 없으면 수정 대상을 확정할 수 없으므로 요청을 거부
        ...
        // 인증 정보에서 확인한 사용자 번호를 소유자 변경 조건으로 설정
        reportAlimDto.setUserNumb(userNumb);
        // URL에서 확정한 독후감 번호를 변경 조건으로 설정
        reportAlimDto.setReptNumb(reptNumb);
        // 정규화한 알림 유형을 응답과 업무 분기에 사용할 값으로 설정
        reportAlimDto.setAlimType(normalizedAlimType);
        // 정규화한 알림 사용 여부를 DB 변경값으로 설정
        reportAlimDto.setUseYsno(normalizedUseYsno);

        // 요청 유형에 해당하는 알림 설정 컬럼 하나의 변경 건수를 저장
        int updateCnt;

        // 좋아요 유형이면 독후감 좋아요 알림 컬럼만 변경
        if (Constant.REPORT_ALIM_LIKE.equals(normalizedAlimType)) {
            // 독후감 좋아요 알림 사용 여부를 소유자 조건으로 변경
            updateCnt = reportMapper.uptLikeAlim(reportAlimDto);
        }

        // 댓글 유형이면 독후감 댓글 알림 컬럼만 변경
        else {
            // 독후감 댓글 알림 사용 여부를 소유자 조건으로 변경
            updateCnt = reportMapper.uptReplyAlim(reportAlimDto);
        }

        // 작성자 소유 독후감이 없어 반영된 행이 없으면 다른 사용자 데이터 접근 요청으로 거부
        if (updateCnt == 0) {
            // "접근할 수 없는 요청이에요."
            return ResultData.fail(ResultEnum.COMMON_ACCESS_REJECTED);
        }

        // 화면에서 즉시 현재 메뉴 문구를 갱신할 수 있도록 변경된 설정 정보를 반환
        return ResultData.success(reportAlimDto);
    }

    /**
     * 마이페이지의 현재 읽고 있는 책 목록에서 독서 상태와 별점 및 공개 여부를 빠르게 수정
     * 전체 독후감 수정 화면으로 이동하지 않아도 완료 여부와 공개 범위를 즉시 반영할 수 있도록 별도 수정 범위를 사용
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
     * @param reptNumb 수정할 독후감 번호
    ...
    public ResultData uptReptStatusGrade(Long userNumb, Long reptNumb, ReportDto reportDto) {
        // 대상 독후감 번호가 없으면 수정 대상을 특정할 수 없으므로 실패 처리
        if (StringUtil.isEmpty(reptNumb)) {
            // "조회 결과가 없어요."
            return ResultData.fail(ResultEnum.COMMON_NO_DATA);
        }
        ...
        // ReptNumb 업무 값을 reportDto DTO에 설정
        reportDto.setReptNumb(reptNumb);
        // ReptGrde 업무 값을 reportDto DTO에 설정
        ...
        // PubcYsno 업무 값을 reportDto DTO에 설정
        reportDto.setPubcYsno(StringUtil.normalizePlainText(reportDto.getPubcYsno()));
        // 빠른 수정 요청에 공개 여부가 없으면 기존 클라이언트도 안전하게 비공개로 처리
        setDefaultPublicFlag(reportDto);
        // 읽는 중 상태를 빠른 수정 API로 전달해도 공개와 평점 정책을 우회하지 못하도록 처리
        applyReadingStatusPolicy(reportDto);
        // ReptEndt 업무 값을 reportDto DTO에 설정
        reportDto.setReptEndt(LocalDate.now().toString()); // 빠른 완료·중단 저장 시점을 실제 독서 종료일로 기록
        ...
        // 마이페이지의 현재 읽고 있는 책 목록에서 독서 상태와 별점만 빠르게 수정 결과를 성공 응답으로 반환
        return ResultData.success(reportDto.getReptNumb());
    }

    /**
     * 로그인 사용자의 독후감을 삭제
     * 사용자 번호와 독후감 번호를 함께 조건으로 사용해 본인 독후감만 삭제되도록 처리
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData delReport(Long userNumb, Long reptNumb) {
        ...
        // 진행 중인 모임 회차의 연결 독후감이면 참여 기록을 중도하차로 확정
        reportMapper.uptClubReadingDropout(reportDto);
        // 댓글을 제거하기 전에 댓글과 답글을 대상으로 등록된 공용 좋아요를 정리
        reportMapper.delReportReplyLikes(reportDto);
        // 자기 참조 외래키에 막히지 않도록 최상위 댓글보다 대댓글을 먼저 정리
        reportMapper.delReportChildReplies(reportDto);
        // 대댓글이 제거된 뒤 독후감을 참조하는 나머지 댓글을 정리
        reportMapper.delReportReplies(reportDto);
        // 독후감 원본을 제거하기 전에 독후감 대상 공용 좋아요를 정리
        reportMapper.delReportLikes(reportDto);

        // 삭제 반영 건수가 없으면 본인 독후감이 아니거나 이미 삭제된 데이터로 판단
        ...
        // 로그인 사용자의 독후감을 삭제 결과를 성공 응답으로 반환
        return ResultData.success();
    }
    /**
     * 독후감 등록에 필요한 도서 필수값이 모두 존재하는지 확인
     * 도서 검색 API 응답을 조작해 들어오는 경우에도 백엔드에서 한 번 더 검증
     *
     * @author SeungHyeon.Kang
    ...
    private boolean hasInvalidBookFields(ReportDto reportDto) {
        // 독후감 등록에 필요한 도서 필수값이 모두 존재하는지 확인 결과를 반환
    ...
    /**
     * 독후감 등록과 수정에 공통으로 적용되는 업무 검증을 수행
     * 필수값, 공통코드, 날짜 범위, 본문 byte 길이, 공개 여부 코드를 순서대로 확인
     *
     * @author SeungHyeon.Kang
    ...
    private ReportValidationResult validateReport(ReportDto reportDto, boolean isFullScan) {
        ...
        // 독서 상태는 필수값이며 READ_STAT 공통코드에 등록된 값만 저장
        ...
        if(isFullScan) {
            ...
            // 읽고 있어요 상태는 사용자가 아직 기록을 남기지 않을 수 있으므로 본문 필수 검증에서 제외
            // 완료/중단 상태는 실제 독후감 기록 저장 단계이므로 기존처럼 본문을 필수값으로 유지
            if (!isReadingStatus && !hasReportContent) {
                // 처리한 값을 결과 컬렉션에 추가
                missingFields.add(MessageUtils.getMessage(REPORT_FIELD_CONTENT_KEY));
            }

            // 전체 입력에서 누락된 항목이 있으면 날짜와 본문 상세 검증보다 필수값 안내를 우선
            if (!missingFields.isEmpty()) {
                // 새로 생성한 ReportValidationResult 객체를 반환
            ...
            // 데이터베이스 저장 한도를 넘는 본문은 DB 오류가 나기 전에 업무 검증으로 차단
            ...
            // 비속어 필터링
            if (hasReportContent) {
                // findBadWord 업무 로직을 badWordDetectionService에 위임
        ...
        // 전체 편집과 빠른 수정에 필요한 필수값이 하나라도 없으면 DB 변경 전에 안내
        if (!missingFields.isEmpty()) {
            // 새로 생성한 ReportValidationResult 객체를 반환
            return new ReportValidationResult(ResultEnum.COMMON_REPORT_REQUIRED_MISSING, formatMissingFields(missingFields));
        }

        // 공개 여부는 Y 또는 N만 허용해 공개 독후감 조회 조건을 안정적으로 유지
    ...
    /**
     * 누락된 필수 항목 목록을 사용자에게 보여줄 수 있는 줄바꿈 문장으로 변환
     *
     * @author SeungHyeon.Kang
    ...
    private String formatMissingFields(List<String> missingFields) {
        // 누락된 필수 항목 목록을 사용자에게 보여줄 수 있는 줄바꿈 문장으로 변환 결과를 반환
        return "- " + String.join("\n- ", missingFields);
    }

    /**
     * 별점 값이 숫자이며 0점부터 5점 범위 안의 0.5점 단위인지 확인
     * 0점은 읽고있어요 상태에서 별점을 선택하지 않은 값을 저장하기 위한 내부 보정값으로 허용
     *
     * @author SeungHyeon.Kang
    ...
    private boolean isValidReportGrade(String reptGrde) {
        // 별점이 비어 있으면 호출한 검증 흐름에서 상태별 필수 여부를 먼저 판단하도록 false를 반환
        if (StringUtil.isEmpty(reptGrde)) {
            // 비어 있는 별점의 검증 실패 여부를 반환
            return false;
        }

        // 사용자 입력 문자열을 정밀한 소수로 변환해 부동소수점 오차 없이 0.5점 간격을 확인
        try {
            // 독후감 별점 문자열을 소수점 비교가 가능한 값으로 변환
    ...
    /**
     * 사용자 설정의 표시 언어를 독후감 원문 언어 코드로 변환
     *
     * @author SeungHyeon.Kang
    ...
    private void applyNewReportDefaults(ReportDto reportDto) {
        ...
        // 등록 시점의 사용자 표시 언어를 독후감 원문 언어로 설정
    ...
    /**
     * 읽고 있는 독후감의 평점과 공개 여부를 집계 및 공개 대상이 아닌 값으로 강제
     * 화면에서 숨긴 입력값을 조작하더라도 서버 저장 정책이 동일하게 유지되도록 처리
     *
     * @author SeungHyeon.Kang
    ...
    private void applyReadingStatusPolicy(ReportDto reportDto) {
        ...
        // 읽는 중이 아닌 완료와 중단 독후감은 사용자가 선택한 평점과 공개 여부를 유지
        if (!Constant.REPORT_STAT_READ.equals(reportDto.getReptStat())) {
            return;
        }

        // 선택 불가한 평점은 집계에 사용되지 않는 내부값 0으로 저장
        reportDto.setReptGrde("0");
        // 읽는 중 독후감은 다른 사용자에게 노출되지 않도록 비공개로 저장
        reportDto.setPubcYsno(Constant.COMM_NO);
    }

    /**
     * 독후감 입력값의 HTML entity와 불필요한 텍스트 표현을 일반 문자열로 정규화
     * 등록 시에는 도서 정보도 함께 정규화하고, 수정 시에는 독후감 필드만 정규화
     *
     * @author SeungHyeon.Kang
    ...
    private void sanitizeReport(ReportDto reportDto, boolean includeBookFields) {
        // ReptStat 업무 값을 reportDto DTO에 설정
        reportDto.setReptStat(StringUtil.normalizePlainText(reportDto.getReptStat()));
        // ReptStdt 업무 값을 reportDto DTO에 설정
