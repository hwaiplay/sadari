 ...
 * date           : 2026-07-27
 * description    : 사용자 메뉴 조회와 트리 구성을 처리
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class UserMenuServiceImpl implements UserMenuService {
    ...
    /**
     * 기준 화면 아래의 노출 가능한 사용자 메뉴 트리를 조회
     *
     * @author SeungHyeon.Kang
    ...
    private List<UserMenuDto.UserMenuItemDto> getMenuTree(
        ...
        // 조회 결과가 없으면 변경할 수 없는 빈 메뉴 목록을 반환
        if (StringUtil.isEmpty(menuList)) {
            // 빈 사용자 메뉴 트리를 반환
            return List.of();
        }

        // 메뉴 번호로 부모 객체를 찾기 위한 순서 보존 맵을 생성
        ...
        // 모든 메뉴의 하위 목록을 초기화하고 번호별 조회 맵에 저장
        for (UserMenuDto.UserMenuItemDto menu : menuList) {
            // 이전 매핑 결과가 응답에 섞이지 않도록 빈 하위 목록을 설정
            menu.setChildList(new ArrayList<>());
            // 메뉴 번호로 부모를 찾을 수 있도록 현재 메뉴를 저장
            menuMap.put(menu.getMenuNumb(), menu);
        }

        // 각 메뉴를 최상위 목록 또는 부모 메뉴 하위 목록에 연결
        for (UserMenuDto.UserMenuItemDto menu : menuList) {
            // 상위 메뉴 번호가 없으면 최상위 메뉴 목록에 추가
            if (menu.getParnNumb() == null) {
                // 최상위 사용자 메뉴를 트리 루트 목록에 추가
                rootMenuList.add(menu);
                continue;
            }

            // 현재 메뉴가 참조하는 상위 메뉴를 조회
            UserMenuDto.UserMenuItemDto parentMenu = menuMap.get(menu.getParnNumb());
            // 노출 가능한 상위 메뉴가 있을 때만 현재 메뉴를 트리에 연결
            if (parentMenu != null) {
                // 현재 메뉴를 상위 메뉴의 하위 목록에 추가
                parentMenu.getChildList().add(menu);
            }
        }

        // 최상위 메뉴와 모든 하위 메뉴를 정렬
