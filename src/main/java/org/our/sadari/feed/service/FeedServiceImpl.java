 ...
 * date           : 2026-08-25
 * description    : 본인과 팔로잉 피드 조회 업무 로직을 구현한다
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class FeedServiceImpl implements FeedService {
    ...
    public ResultData getFeedList(Long userNumb, int page) {
        ...
        // 공개 범위와 페이지 조건이 적용된 본인 및 팔로잉 피드를 최신 활동순으로 조회한다
     ...
     * 일반 목록에서 팔로우하지 않는 작성자도 허용하되 탈퇴·비공개·교체된 대상은 우회 조회하지 않는다.
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData getFeedDtl(Long userNumb, String tagtType, Long tagtNumb) {
        ...
        // 지원하지 않는 대상 유형이나 유효하지 않은 번호로 임의 조회하는 요청을 거부한다
        ...
        // 인증 사용자와 정확한 피드 대상 조건을 Mapper에 전달할 요청 객체를 생성한다
        FeedDto request = new FeedDto();
        // 로그인 사용자를 기준으로 활성 계정과 현재 공개 콘텐츠 범위를 적용한다
        request.setLoginUserNumb(userNumb);
        // 알림 링크에 포함된 대상 유형과 번호를 정확한 단건 조건으로 설정한다
        request.setTagtType(tagtType);
        request.setTagtNumb(tagtNumb);
        // 단건 조회는 첫 번째 일치 항목만 반환하도록 조회 범위를 제한한다
        request.setPageOffset(0);
        request.setPageLimit(1);

        // 현재 공개 콘텐츠 여부와 정확한 대상 조건이 적용된 피드 한 건을 조회한다
        List<FeedDto> result = feedMapper.getFeedList(request);

        // 현재 공개 상태인 대상이 없으면 만료되거나 접근할 수 없는 알림으로 처리한다
        if (result.isEmpty()) {
            // "조회된 데이터가 없어요."
            return ResultData.fail(ResultEnum.COMMON_NO_DATA);
        }

        // 알림 링크가 지정한 첫 번째 피드 항목을 반환한다
