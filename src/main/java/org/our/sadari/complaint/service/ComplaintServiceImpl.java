 ...
 * date           : 2026-08-22
 * description    : 신고 접수와 대상별 누적 임계치 자동 조치를 처리
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class ComplaintServiceImpl implements ComplaintService {
    ...
    /**
     * 활성 사용자의 동일 대상 재신고를 차단하고 대상 원문 스냅샷과 신고 사유를 접수
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData setComplaint(Long userNumb, ComplaintCreateDto complaintCreateDto) {
        ...
        // 텍스트 또는 실제 이미지 원본으로 변경 불가능한 대상 버전 해시를 계산
        ...
        // 같은 번호의 수정 전후 콘텐츠가 섞이지 않도록 현재 버전 해시를 설정
    ...
    /**
     * 활성 사용자가 아직 확인하지 않은 신고 조치 결과 건수와 마지막 번호를 조회
     *
     * @author HanWon.Jang
    ...
    /**
     * 활성 사용자가 팝업에서 확인한 시점까지의 신고 조치 결과를 확인 처리
     *
     * @author HanWon.Jang
     * @param userNumb 인증 사용자 번호
    ...
    public ResultData uptResultConfirm(Long userNumb, Long resultNumb) {
        ...
        // 조회 이후 새로 생성된 결과는 남겨두고 요청 번호까지의 결과만 확인 처리
        complaintMapper.uptResultConfirm(userNumb, resultNumb);
        // 이미 확인한 요청도 같은 결과를 반환할 수 있도록 성공 처리
        return ResultData.success();
    }

    /**
     * 신고 대상과 사유 및 상세 내용의 저장 가능 형식을 검증
     *
     * @author SeungHyeon.Kang
    ...
    private String normalizeContent(String content) {
        ...
        if (StringUtil.isEmpty(content) || content.trim().isEmpty()) {
            // 입력하지 않은 신고 상세 내용은 null로 반환
            return null;
        }

        // 실제 관리자가 확인할 신고 상세 내용만 반환
        return content.trim();
    }

    /**
     * 신고 대상 유형에 고정된 원본 테이블에서 서버가 실제 내용과 소유자를 조회
     *
     * @author SeungHyeon.Kang
    ...
    private byte[] getEvidenceBytes(String tagtType, ComplaintDto target) {
        ...
        // DB 파일 경로를 상위 경로 이동이 불가능한 내부 저장소 객체 키로 변환
    ...
    private String getStoredObjectKey(String tagtType, ComplaintDto target) {
        ...
        // 운영체제 경로 구분자를 저장소 공통 객체 키 구분자로 변환해 반환
        return storedPath.toString().replace('\\', '/');
    }

    /**
     * 신고 대상이 원본 바이트 증거를 보존하는 이미지 유형인지 확인
     *
     * @author SeungHyeon.Kang
     * @param tagtType 신고 대상 유형
    ...
    private String getTargetHash(String tagtType, String tagtCntn, byte[] evidenceBytes) {
        ...
        try {
            // 모든 대상 유형에서 동일한 SHA-256 알고리즘을 사용
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            // 유형이 다른 대상의 동일 바이트가 같은 버전으로 표현되지 않도록 구분자를 포함
            digest.update((tagtType + "\u0000").getBytes(StandardCharsets.UTF_8));
            // 프로필 사진은 파일명이 아니라 실제 이미지 바이트를 해시 입력으로 사용
            if (evidenceBytes != null) {
                // 신고 시점 원본 이미지 바이트를 해시에 반영
                digest.update(evidenceBytes);
            } else {
                // 텍스트 대상은 DB에서 조회한 실제 스냅샷 전체를 해시에 반영
                digest.update(Optional.ofNullable(tagtCntn).orElse("").getBytes(StandardCharsets.UTF_8));
            }
            // DB CHAR(64) 컬럼에 저장할 소문자 16진수 해시를 반환
            return HexFormat.of().formatHex(digest.digest());
        }

        // 모든 지원 JDK에 필수인 SHA-256을 사용할 수 없으면 데이터 정합성을 보장할 수 없음
        catch (NoSuchAlgorithmException e) {
            // 신고 접수 전체를 롤백하도록 실행 불가능 상태로 변환
            throw new IllegalStateException("SHA-256 is not available.", e);
        }
    }

    /**
     * 동일 이미지 버전의 관리자 전용 증거를 한 번만 저장하고 연결 번호를 반환
     *
     * @author SeungHyeon.Kang
    ...
    private Long setEvidence(ComplaintDto complaint, ComplaintDto target, byte[] evidenceBytes) {
        ...
        // 신고 시점 이미지 원본과 메타정보를 담을 증거 객체를 생성
    ...
    private void setAutoAction(ComplaintDto complaint) {
        ...
        // 대상 유형별 삭제 또는 초기화를 실행하고 결과 설명을 조회
        String resultContent = applyAutoAction(complaint, threshold);
        // 자동 조치 결과를 변경 불가능한 이력으로 저장할 객체를 생성
    ...
    /** 피신고자에게 공개할 누적 신고와 대상별 조치 내용을 조회 */
    private String getTargetResult(ComplaintResultEventDto event, String tagtType) {
        ...
        // 건수 없이 누적 사실과 안전하게 요약한 신고 유형을 안내
        return reasonContent + "가 누적되어 " + getReporterResult(tagtType);
    }

    /** 피신고자에게 공개할 누적 신고와 대상별 영문 조치 내용을 조회 */
    ...
    /**
     * 신고 대상 유형에 맞춰 독후감·댓글 삭제 또는 프로필 정보를 초기화
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 독후감 원본과 연결 데이터를 보존하면서 공개 여부만 비공개로 변경
     *
     * @author SeungHyeon.Kang
    ...
    private String delAutoReply(ComplaintDto complaint, int threshold) {
        // 서버에서 잠금 조회한 소유자와 일치하는 댓글만 삭제 상태로 변경
        int updateCount = complaintMapper.delAutoReply(
                complaint.getTagtNumb(), complaint.getTagtUser()
        );
        // 잠금 조회한 댓글이 변경되지 않으면 신고와 조치 결과를 함께 롤백
        validateActionCount(updateCount);
        // 자동 조치 이력과 신고 처리 내용에 저장할 결과를 반환
    ...
    /**
     * 프로필 사진 참조를 제거하고 더 이상 사용하지 않는 파일을 정리
     *
     * @author SeungHyeon.Kang
    ...
    private String uptAutoProfile(ComplaintDto complaint, int threshold) {
        // 활성·비활성화·삭제 대기 회원의 현재 프로필 사진 참조를 제거
        int updateCount = complaintMapper.uptAutoProfile(complaint.getTagtUser());
        // 잠금 조회한 프로필 사진이 변경되지 않으면 신고와 조치 결과를 함께 롤백
        validateActionCount(updateCount);
        // 프로필과 배경에서 더 이상 참조하지 않는 파일은 커밋 뒤 물리 저장소까지 정리
    ...
    /**
     * 배경사진 참조를 제거하고 더 이상 사용하지 않는 파일을 정리
     *
     * @author SeungHyeon.Kang
    ...
    private String uptAutoIntro(ComplaintDto complaint, int threshold) {
        // 활성·비활성화·삭제 대기 회원의 현재 한줄소개를 Null로 변경
        int updateCount = complaintMapper.uptAutoIntro(complaint.getTagtUser());
        // 잠금 조회한 한줄소개가 변경되지 않으면 신고와 조치 결과를 함께 롤백
        validateActionCount(updateCount);
        // 자동 조치 이력과 신고 처리 내용에 저장할 결과를 반환
    ...
    /**
     * 잠금 조회한 신고 대상 원본에 자동 조치가 한 건 반영됐는지 검증
     *
     * @author SeungHyeon.Kang
     * @param updateCount 자동 조치 반영 건수
     */
    private void validateActionCount(int updateCount) {
        // 대상 원본 한 건이 변경되지 않으면 부분 조치를 방지하기 위해 전체 트랜잭션을 롤백
        if (updateCount != 1) {
