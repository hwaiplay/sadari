 ...
 * author         : HanWon.Jang
 * date           : 2026-08-28
 * description    : 모임장 승계 선거의 조회, 투표와 자동 마감 계약을 정의한다
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public interface OwnerElectionService {
    ...
    /** 진행 중인 모임장 선거를 조회한다. @param userNumb 사용자 번호 @param clubNumb 모임 번호 @return 선거 정보 */
    ResultData getElection(Long userNumb, Long clubNumb);

    /** 진행 중인 모임장 선거에 투표한다. @param userNumb 사용자 번호 @param clubNumb 모임 번호 @param request 후보 선택값 @return 투표 결과 */
    ResultData uptElectionVote(Long userNumb, Long clubNumb, OwnerElectionDto.VoteReqDto request);

    /** 상태 전환 후 생성되지 않은 모임장 선거를 생성한다. */
    void startPendingElection();

    /** 마감 시각이 지난 모임장 선거를 확정한다. */
