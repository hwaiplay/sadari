 ...
 * date           : 2026-07-29
 * description    : Kakao 재인증을 거친 계정 비활성화와 영구 삭제 대기 업무를 처리함
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class UserWithdrawalServiceImpl implements UserWithdrawalService {
    ...
    /**
     * 영구 삭제 유예기간 설정값이 유효한지 애플리케이션 시작 시 검증함
     *
     * @author SeungHyeon.Kang
     */
    @PostConstruct
    private void validateConfiguration() {
        ...
        // 음수 유예기간은 요청 시점보다 과거를 삭제 예정일로 만들기 때문에 실행을 차단함
        if (hardDeleteWaitDays < 0) {
            // 잘못된 영구 삭제 유예기간이면 애플리케이션 시작을 중단함
    ...
    public ResultData setWithdrawalRequest(Long userNumb, UserWithdrawalDto request) {
        ...
        // 공통코드에 등록되지 않은 탈퇴 유형이나 사유는 임의 요청으로 판단함
        ...
        // 정지 회원은 복구 가능한 비활성화만 제한하고 제재를 보존하는 영구 탈퇴는 허용함
        ...
        // 탈퇴 사유 상세 내용이 DB 저장 기준을 넘으면 API를 직접 호출한 요청도 거절함
        ...
        // Redis에 저장할 탈퇴 요청에 로그인 회원 번호를 설정함
        request.setUserNumb(userNumb);
        // OAuth 요청과 콜백을 일회성으로 연결할 예측 불가능한 상태값을 생성함
        String state = UUID.randomUUID().toString();

        // 재인증 요청을 직렬화하고 제한된 시간 동안만 Redis에 보관함
        try {
            // 탈퇴 요청 본문을 Redis 저장 문자열로 변환함
            String requestJson = objectMapper.writeValueAsString(request);
            // 콜백 검증에 사용할 일회성 요청을 10분 동안 저장함
    ...
    public ResultData setWithdrawalCallback(String code, String state) {
        ...
        // 재인증한 Kakao 식별값을 기존 DB 조회 형식으로 암호화함
        ...
        // 계정 비활성화와 영구 삭제 대기는 서로 다른 회원 상태와 이력 상태를 적용함
        if (Constant.WITHDRAWAL_TYPE_SOFT.equals(request.getWthdType())) {
            // 계정 비활성화 이력을 완료 상태로 설정함
        ...
        // 영구 탈퇴는 환경별 유예기간 동안 취소할 수 있는 삭제 대기 상태로 전환함
        else {
            // 영구 삭제 대기 이력 상태를 설정함
        ...
        // 탈퇴 처리 후 기존 Refresh Token과 닉네임 캐시를 제거함
    ...
    public ResultData uptWithdrawalCancel(Long userNumb) {
        ...
        // 현재 로그인 세션도 즉시 정상 이용 상태로 변경함
        tokenRedisService.uptUserStatus(userNumb, restoredUserStat);

        // 프론트엔드가 정지 안내 또는 정상 화면을 선택할 수 있도록 복구 상태를 반환함
        return ResultData.success(restoredUserStat);
    }

    /**
     * 탈퇴 유형에 맞는 회원 상태와 연관 데이터 제한을 한 트랜잭션으로 적용함
     *
     * @author SeungHyeon.Kang
    ...
    private void applyWithdrawalStatus(UserWithdrawalDto request, String userStat
        ...
        // 계정이 비활성 상태가 되기 전에 실행 중인 독서 시간을 확정하고 세션을 완료함
        ...
        // 탈퇴·삭제대기 결과 비보존 정책에 따라 기존 신고 결과 수신 이력을 삭제함
        userWithdrawalMapper.delComplaintResults(request.getUserNumb());
    }

    /**
     * 탈퇴 재인증 요청을 저장할 Redis 키를 생성함
     *
     * @author SeungHyeon.Kang
    ...
    private String getWithdrawalStateKey(String state) {
        // 탈퇴 요청 전용 접두사와 일회성 상태값을 조합해 반환함
