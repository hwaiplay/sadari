 ...
 * date           : 2026-09-03
 * description    : 로컬 프로필에서 활성 회원의 개발용 로그인 세션을 생성
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class LocalAuthService {
    ...
    /**
     * 활성 회원 번호로 로컬 개발용 Access Token과 Refresh Token을 발급
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData setLocalLogin(Long userNumb) {
        // 회원 번호가 없으면 임의 계정 조회와 불완전한 세션 생성을 차단
        if (StringUtil.isEmpty(userNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // 요청한 회원 번호의 현재 계정 상태와 권한을 DB 원본에서 조회
        UserDto savedUser = userMapper.getUserByNumb(userNumb);

        // 활성 회원과 서비스가 허용한 권한만 개발용 로그인 대상으로 인정
        ...
        // 브라우저 인증 쿠키에 사용할 두 토큰을 응답 데이터로 구성
        TokenDto token = TokenDto.of(accessToken, refreshToken, false);
        // 계정 상태를 변경하지 않은 로컬 개발용 로그인 결과를 반환
