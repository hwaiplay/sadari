 ...
 * date           : 2026-03-15
 * description    : Kakao 계정 기반 회원 등록과 JWT 로그인 업무를 구현함
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class AuthServiceImpl implements AuthService {
    ...
    /**
     * Kakao 계정으로 신규 회원 등록과 JWT 로그인을 처리함
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData kakaoLogin(String code, String lognIpxx, String userAgnt) {
        // code 값이 비어 있으면 후속 참조를 차단하기 위해 분기함
        ...
        // 만료되거나 재사용된 인가 코드 등 Kakao가 거절한 인증 요청은 공통 인증 실패로 변환함
        ...
        // Kakao 서버 오류와 연결 실패도 원시 통신 예외가 컨트롤러 밖으로 전파되지 않도록 변환함
        ...
        // encryptForStorage 업무 로직을 userIdEncryptionService에 위임함
        ...
        // 카카오 로그인 사용자 정보를 담을 객체를 생성함
        UserDto userDto = new UserDto();
        // 일반 로그인과 비활성화 계정 복귀 로그인을 구분할 상태를 초기화함
        boolean accountReactivated = false;

        // 외부 연동이나 데이터 변환 실패를 예외 흐름으로 분리하기 위한 블록임
        try {
            // UserByIdxx 데이터를 DB에서 조회함
            UserDto savedUser = userMapper.getUserByIdxx(encryptedProviderId);

            // 기간 정지 종료 뒤 같은 카카오 계정으로 로그인하면 기존 계정을 복구한 뒤 상태를 다시 조회함
            if (!StringUtil.isEmpty(savedUser)
                ...
                // 만료 처리로 변경된 최신 회원 상태를 다시 조회함
                savedUser = userMapper.getUserByIdxx(encryptedProviderId);
            }

            // UserProv 업무 값을 userDto DTO에 설정함
            userDto.setUserProv(AuthConstant.PROV_KAKAO);
            // USER_IDXX는 외부 OAuth 제공자의 고유 식별값이라 DB에는 평문을 남기지 않고 결정적 암호문으로 저장함
            // 로그인 조회도 같은 암호문으로 수행하므로 별도 복호화 없이 기존 사용자 식별이 가능함
            userDto.setUserIdxx(encryptedProviderId);
            // UserRole 업무 값을 userDto DTO에 설정함
            userDto.setUserRole(AuthConstant.ROLE_USER);
            // savedUser 값이 비어 있으면 후속 참조를 차단하기 위해 분기함
            if (StringUtil.isEmpty(savedUser)) {
            ...
            // 앞선 조건에 해당하지 않는 대체 업무 흐름으로 전환함
                ...
                if (Constant.USER_STAT_WITHDRAWN.equals(savedUser.getUserStat())) {
                    ...
                    // 로그인 완료 전에 회원 상태를 정상으로 복구함
                    userMapper.uptUserStatus(savedUser);
                    // OAuth 완료 화면이 일반 로그인과 구분해 복귀 정책을 안내하도록 상태를 기록함
                ...
                // 기존 사용자가 프로필에서 수정한 닉네임이 있으면 Kakao 기본 닉네임으로 다시 덮지 않고 DB 값을 사용함
                userDto.setUserNick(savedUser.getUserNick());
                // 로그인 세션에 반영할 현재 회원 상태를 설정함
                userDto.setUserStat(savedUser.getUserStat());

                // savedUser.getProfNumb( 값이 비어 있으면 후속 참조를 차단하기 위해 분기함
        ...
        // 로그인 이력 저장 데이터를 담을 객체를 생성함
        LoginHistoryDto loginHistoryDto = new LoginHistoryDto();
        // UserNumb 업무 값을 loginHistoryDto DTO에 설정함
        loginHistoryDto.setUserNumb(userDto.getUserNumb());
        // LognDate 업무 값을 loginHistoryDto DTO에 설정함
        loginHistoryDto.setLognDate(LocalDateTime.now());
        // LognIpxx 업무 값을 loginHistoryDto DTO에 설정함
        loginHistoryDto.setLognIpxx(lognIpxx);
        // UserAgnt 업무 값을 loginHistoryDto DTO에 설정함
        loginHistoryDto.setUserAgnt(StringUtil.cutString(userAgnt, USER_AGENT_MAX_LENGTH));
        // ProvCode 업무 값을 loginHistoryDto DTO에 설정함
        ...
        // Kakao 계정 확인과 JWT 로그인 처리 결과를 성공 응답으로 반환함
