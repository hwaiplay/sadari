 ...
 * date           : 2026-03-15
 * description    : Kakao 계정 기반 회원 등록과 JWT 로그인 업무를 구현
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class AuthServiceImpl implements AuthService {
    ...
    /**
     * Kakao 계정으로 신규 회원 등록과 JWT 로그인을 처리
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData kakaoLogin(String code, String lognIpxx, String userAgnt) {
        // code 값이 비어 있으면 후속 참조를 차단하기 위해 분기
        ...
        try {
            // getKakaoToken 조회로 후속 처리에 필요한 데이터를 가져옴
            kakaoTokenDto = kakaoAuthProvider.getKakaoToken(code);
            // getKakaoAccount 조회로 후속 처리에 필요한 데이터를 가져옴
            kakaoAccountDto = kakaoAuthProvider.getKakaoAccount(kakaoTokenDto);
        }

        // 만료되거나 재사용된 인가 코드 등 Kakao가 거절한 인증 요청은 공통 인증 실패로 변환
        catch (HttpClientErrorException e) {
            // Kakao 오류 응답 본문과 인가 코드를 제외하고 상태 코드만 경고 로그에 남김
            log.warn("Kakao OAuth request rejected. status={}", e.getStatusCode().value());
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // Kakao 서버 오류와 연결 실패도 원시 통신 예외가 컨트롤러 밖으로 전파되지 않도록 변환
        catch (RestClientException e) {
            // 외부 응답 본문이나 인증값을 기록하지 않고 장애 종류만 오류 로그에 남김
            log.error("Kakao OAuth request failed. type={}", e.getClass().getSimpleName());
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // Kakao 성공 응답을 DTO로 변환할 수 없으면 공통 인증 실패로 변환
        ...
        // encryptForStorage 업무 로직을 userIdEncryptionService에 위임
        String encryptedProviderId = userIdEncryptionService.encryptForStorage(providerId);
        // Kakao 기본 실루엣은 서비스의 실제 사용자 프로필 사진으로 저장하지 않음
        ...
        // 카카오 로그인 사용자 정보를 담을 객체를 생성
        UserDto userDto = new UserDto();
        // 일반 로그인과 비활성화 계정 복귀 로그인을 구분할 상태를 초기화
        boolean accountReactivated = false;

        // 외부 연동이나 데이터 변환 실패를 예외 흐름으로 분리하기 위한 블록
        try {
            // UserByIdxx 데이터를 DB에서 조회
            UserDto savedUser = userMapper.getUserByIdxx(encryptedProviderId);

            // 기간 정지 종료 뒤 같은 카카오 계정으로 로그인하면 기존 계정을 복구한 뒤 상태를 다시 조회
            if (!StringUtil.isEmpty(savedUser)
                ...
                // 만료 처리로 변경된 최신 회원 상태를 다시 조회
                savedUser = userMapper.getUserByIdxx(encryptedProviderId);
            }

            // UserProv 업무 값을 userDto DTO에 설정
            userDto.setUserProv(AuthConstant.PROV_KAKAO);
            // USER_IDXX는 외부 OAuth 제공자의 고유 식별값이라 DB에는 평문을 남기지 않고 결정적 암호문으로 저장
            // 로그인 조회도 같은 암호문으로 수행하므로 별도 복호화 없이 기존 사용자 식별이 가능
            userDto.setUserIdxx(encryptedProviderId);
            // UserRole 업무 값을 userDto DTO에 설정
            userDto.setUserRole(AuthConstant.ROLE_USER);
            // savedUser 값이 비어 있으면 후속 참조를 차단하기 위해 분기
            if (StringUtil.isEmpty(savedUser)) {
                // 과거 탈퇴 계정의 회원 번호 중 하나라도 유효한 정지가 남아 있으면 새 회원 번호를 발급하지 않음
            ...
            // 앞선 조건에 해당하지 않는 대체 업무 흐름으로 전환
                ...
                /*
                 * 비활성화 회원은 Kakao 재로그인을 계정 재활성화 의사로 판단
                 * 기존 프로필과 관계 데이터는 유지하고 회원 상태와 탈퇴 일시만 정상 상태로 되돌림
                 */
                if (Constant.USER_STAT_WITHDRAWN.equals(savedUser.getUserStat())) {
                    // 복구할 회원 번호를 상태 변경 요청에 설정
                    ...
                    // 로그인 완료 전에 회원 상태를 정상으로 복구
                    userMapper.uptUserStatus(savedUser);
                    // OAuth 완료 화면이 일반 로그인과 구분해 복귀 정책을 안내하도록 상태를 기록
                ...
                // 기존 사용자가 프로필에서 수정한 닉네임이 있으면 Kakao 기본 닉네임으로 다시 덮지 않고 DB 값을 사용
                userDto.setUserNick(savedUser.getUserNick());
                // 로그인 세션에 반영할 현재 회원 상태를 설정
                userDto.setUserStat(savedUser.getUserStat());

                // savedUser.getProfNumb( 값이 비어 있으면 후속 참조를 차단하기 위해 분기
        ...
        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
            ...
            // 사용자 등록 일부만 커밋되지 않도록 로그인 쓰기 트랜잭션 전체를 롤백
        ...
        // 이번 브라우저 로그인을 다른 기기 세션과 구분할 식별자를 생성
        String sessionId = UUID.randomUUID().toString();
        // 동일 기기 세션 식별자를 포함한 Access Token을 생성
        String accessToken = jwtProvider.createAccessToken(userDto.getUserNumb(), userDto.getUserRole(), sessionId);
        // 동일 기기 세션 식별자를 포함한 Refresh Token을 생성
        String refreshToken = jwtProvider.createRefreshToken(userDto.getUserNumb(), sessionId);

        /*
         * 알림 발송 시 발신자 닉네임을 다시 DB에서 조회하지 않도록 로그인 시점의 최신 닉네임을 Refresh Token과 함께 저장
         * 두 값은 TokenRedisService의 Lua 스크립트에서 같은 TTL로 원자 반영됨
         */
        ...
        // Kakao 계정 확인과 JWT 로그인 처리 결과를 성공 응답으로 반환
