 ...
 * date           : 2026-07-25
 * description    : 푸시 알림 외부 연동 기능을 제공
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class FirebaseMessagingProvider {
    ...
    /*
     * Firebase SDK 타입을 필드/메서드 시그니처에 직접 노출하지 않음
     * IntelliJ 실행 classpath가 Gradle 의존성을 늦게 반영하는 경우, Spring bean introspection 단계에서
     * NoClassDefFoundError가 발생할 수 있으므로 실제 SDK 접근은 초기화/발송 시점에 reflection으로만 수행
     */
    ...
    /**
     * 서버 시작 시 Firebase Admin SDK를 초기화
     * credentials 경로 누락, 파일 없음, SDK classpath 누락은 모두 push 비활성화로만 처리
     *
     * @author SeungHyeon.Kang
     */
    @PostConstruct
    public void init() {
        // credentialsPath 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (StringUtil.isEmpty(credentialsPath)) {
            // Firebase service account 경로가 없으면 서버 발송 자체가 불가능하므로 push만 비활성화
            log.warn("Firebase credentials path is empty. Push sending is disabled.");
            // 서버 시작 시 Firebase Admin SDK를 초기화한 결과를 반환
            return;
        }

        // 외부 연동이나 데이터 변환 실패를 예외 흐름으로 분리하기 위한 블록
        try {
            // Firebase 인증정보로 메시징 클라이언트를 초기화
        ...
        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
             ...
             * 알림함 저장 기능까지 같이 죽으면 안 됨. 푸시는 부가 기능으로 보고 서버 기동은 유지
             */
    ...
    /**
     * FCM registration token으로 푸시 메시지를 발송
     * Firebase가 초기화되지 않았거나 token이 없으면 실패가 아니라 발송 생략으로 처리
     *
     * @author SeungHyeon.Kang
    ...
    public boolean send(String token, String title, String body
        ...
        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
            ...
            /*
             * 토큰 만료, Firebase 장애, 런타임 의존성 미반영 같은 문제는 개별 푸시 발송 실패로만 처리
             * 알림 insert 트랜잭션의 성공 여부와 푸시 전송 성공 여부를 강하게 묶지 않기 위한 분기
             */
    ...
    /**
     * Firebase Admin SDK class를 로딩하고 service account json으로 FirebaseMessaging 객체를 준비
     * devtools/IDE 실행 classpath의 늦은 반영으로 인한 서버 기동 차단을 피하기 위한 SDK 타입 직접 import 배제
     *
     * @author SeungHyeon.Kang
     */
    private void initFirebaseMessaging() throws Exception {
        // Firebase 인증 파일 경로의 classpath 접두사를 정규화
    ...
    /**
     * Firebase Admin SDK에서 필요한 class들을 문자열 이름으로 로딩
     * 이 단계가 실패하면 Gradle 의존성이 실행 classpath에 없다는 뜻이므로 push 기능만 비활성화됨
     *
     * @author SeungHyeon.Kang
     * @return Firebase Admin SDK class 묶음
     */
    private FirebaseSdkClasses loadFirebaseSdkClasses() throws ClassNotFoundException {
        // 런타임에 조회한 Firebase Admin SDK 클래스를 묶은 객체를 반환
    ...
    /**
     * service account json 스트림을 FirebaseOptions 객체로 변환
     * GoogleCredentials.fromStream -> FirebaseOptions.builder().setCredentials(...).build() 순서를 reflection으로 수행
     *
     * @author SeungHyeon.Kang
    ...
    private Object getFirebaseApp(FirebaseSdkClasses sdkClasses, Object options) throws Exception {
        ...
        // 요청값이 업무에서 허용한 범위와 상태를 만족하는지 구분
        if (appList.isEmpty()) {
            // JVM 안에 이미 FirebaseApp이 있으면 재사용하고, 없으면 새로 초기화한 결과를 반환
    ...
    /**
     * Firebase Messaging이 발송할 Message 객체를 생성
     * 브라우저 foreground/background 처리 양쪽에서 사용할 수 있도록 notification과 data payload를 함께 넣음
     *
     * @author SeungHyeon.Kang
     * @param token FCM registration token
     * @param title 알림 제목
    ...
    private Object createMessage(String token, String title, String body
        ...
        // 알림 번호가 있어야 서비스워커가 사용자가 클릭한 정확한 알림 한 건을 읽음 처리할 수 있음
        ...
        // Firebase Messaging이 발송할 Message 객체를 생성한 결과를 반환
        return invoke(messageDataBuilder, "build");
    }

    /**
     * 브라우저 시스템 알림에 표시할 Notification 객체를 생성
     * title/body가 비어 있어도 Firebase 메시지 생성이 실패하지 않도록 기본값으로 보정
     *
     * @author SeungHyeon.Kang
    ...
    private Object createNotification(Class<?> notificationClass, String title, String body) throws Exception {
        ...
        // 브라우저 시스템 알림에 표시할 Notification 객체를 생성한 결과를 반환
        return invoke(bodyBuilder, "build");
    }

    /**
     * Firebase service account json 경로를 Spring ResourceLoader가 해석할 수 있는 값으로 보정
     * yml에는 classpath: 접두사를 한 번만 쓰는 것이 정상
     * 다만 실행 환경변수에 classpath:가 이미 들어간 값을 다시 조합하면 classpath:classpath:... 형태가 될 수 있어
     * 서버 기동 시 파일을 못 찾지 않도록 여기서 한 번만 남겨 정리
     *
     * @author SeungHyeon.Kang
    ...
    private String normalizeCredentialsPath(String path) {
        ...
        while (normalizedPath.startsWith(CLASSPATH_DUPLICATED_PREFIX)) {
            // 정규식과 처음 일치하는 문자열을 치환
            normalizedPath = normalizedPath.replaceFirst(CLASSPATH_DUPLICATED_PREFIX, CLASSPATH_PREFIX);
        }

        // Firebase service account json 경로를 Spring ResourceLoader가 해석할 수 있는 값으로 보정한 결과를 반환
        return normalizedPath;
    }

    /**
     * Firebase 초기화가 완료되어 실제 발송을 시도할 수 있는지 확인
     * 하나라도 비어 있으면 초기화 실패 상태로 보고 발송을 생략
     *
     * @author SeungHyeon.Kang
     * @return Firebase 발송 가능 여부
     */
    private boolean isFirebaseMessagingReady() {
        // Firebase 초기화가 완료되어 실제 발송을 시도할 수 있는지 확인한 결과를 반환
        return !StringUtil.hasEmpty(firebaseMessaging, firebaseMessagingClass, messageClass);
    }

    /**
     * 초기화 실패 시 중간까지 채워진 Firebase 객체/class 참조를 비움
     * 일부 필드만 남아 있으면 다음 발송 시 NPE나 잘못된 상태로 이어질 수 있어 명시적으로 정리
     *
     * @author SeungHyeon.Kang
     */
    ...
    private String getPushBody(String body) {
        // 비어 있는 푸시 본문을 빈 문자열로 보정한 결과를 반환
        return StringUtil.isEmpty(body) ? "" : body;
    }

    /**
     * 비어 있는 클릭 링크를 알림 목록 경로로 보정
     *
     * @author SeungHyeon.Kang
    ...
    private String getPushLink(String linkUrlx) {
        // 비어 있는 클릭 링크를 알림 목록 경로로 보정한 결과를 반환
        return StringUtil.isEmpty(linkUrlx) ? DEFAULT_PUSH_LINK : linkUrlx;
    }

    /**
     * 파라미터 없는 static 메서드를 reflection으로 호출
     *
     * @author SeungHyeon.Kang
    ...
    private Object invokeStatic(Class<?> targetClass, String methodName) throws Exception {
        // 파라미터 없는 static 메서드를 reflection으로 호출한 결과를 반환
        return invokeStatic(targetClass, methodName, new Class<?>[]{});
    }

    /**
     * static 메서드를 reflection으로 호출
     * Firebase SDK 타입을 직접 참조하지 않기 위한 공통 호출 지점
     *
     * @author SeungHyeon.Kang
     * @param targetClass 호출 대상 class
     * @param methodName 호출할 메서드명
    ...
    private Object invokeStatic(Class<?> targetClass, String methodName, Class<?>[] parameterTypes
        ...
        // static 메서드를 reflection으로 호출한 결과를 반환
        return method.invoke(null, args);
    }

    /**
     * 파라미터 없는 인스턴스 메서드를 reflection으로 호출
     *
     * @author SeungHyeon.Kang
    ...
    private Object invoke(Object target, String methodName) throws Exception {
        // 파라미터 없는 인스턴스 메서드를 reflection으로 호출한 결과를 반환
        return invoke(target, methodName, new Class<?>[]{});
    }

    /**
     * 인스턴스 메서드를 reflection으로 호출
     * Firebase builder 체인 호출을 한 곳으로 모아 예외 처리와 호출 방식을 일관되게 유지
     *
     * @author SeungHyeon.Kang
     * @param target 호출 대상 객체
     * @param methodName 호출할 메서드명
    ...
    private Object invoke(Object target, String methodName, Class<?>[] parameterTypes
                        , Object... args) throws Exception {
        // getClass 조회로 후속 처리에 필요한 데이터를 가져옴
        Method method = target.getClass().getMethod(methodName, parameterTypes);
        // 인스턴스 메서드를 reflection으로 호출한 결과를 반환
        return method.invoke(target, args);
    }

    /**
     * reflection으로 로딩한 Firebase Admin SDK class 묶음
     * class 참조를 한 객체로 묶어 초기화 단계의 파라미터가 길어지는 것을 막음
     *
     * @author SeungHyeon.Kang
