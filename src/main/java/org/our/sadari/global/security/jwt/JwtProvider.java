 ...
 * date           : 2026-03-22
 * description    : 인증과 보안 외부 연동 기능을 제공
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class JwtProvider {
    ...
    /**
     * JwtProvider 생성자로, application.yml 설정 파일에서 시크릿 키와 토큰 유효시간(초 단위)을 주입받아 밀리초 단위로 변환
     *
    ...
    public JwtProvider(@Value("${jwt.secret_key}") String secret, @Value("${jwt.access-token-validity-in-seconds}") long accessTokenValiditySeconds
        ...
        this.secret = secret;
        // yml 설정값은 초 단위이고, JWT exp를 만드는 Date 계산은 millisecond 기준이라 1000을 곱해 변환
    ...
    /**
     * 빈(Bean) 생성 및 의존성 주입 완료 후 실행되는 초기화 메서드로, Base64 시크릿 키를 데코딩하여 SecretKey 객체를 생성
     */
    @PostConstruct
    public void initKey() {
        // 인코딩된 값을 원문 형식으로 복원
        byte[] keyBytes = Decoders.BASE64.decode(secret);
        // 설정된 비밀키로 JWT HMAC 서명 키를 생성
        this.secretKey = Keys.hmacShaKeyFor(keyBytes);
    }

    /**
     * 회원 번호와 권한 정보를 바탕으로 Access Token을 발급
     *
     * @author SeungHyeon.Kang
    ...
    public String createAccessToken(Long userNumb, String role, String sessionId) {
        // JWT 만료 시각을 담을 객체를 생성
        Date now = new Date();
        // 회원 번호와 권한 정보를 바탕으로 Access Token을 발급 결과를 반환
        return Jwts.builder()
                ...
                // 0.13 API의 알고리즘 레지스트리를 명시해 서명 알고리즘을 HS256으로 고정
                .signWith(secretKey, Jwts.SIG.HS256)
                .compact();
    }

    /**
     * 회원 번호를 바탕으로 Access Token 재발급용 Refresh Token을 발급
     *
     * @author SeungHyeon.Kang
     * @param userNumb 회원 번호 (PK)
     * @param sessionId 기기별 로그인 세션 식별자
    ...
    public String createRefreshToken(Long userNumb, String sessionId) {
        // JWT 만료 시각을 담을 객체를 생성
        Date now = new Date();
        // 회원 번호를 바탕으로 Access Token 재발급용 Refresh Token을 발급 결과를 반환
        return Jwts.builder()
                .subject(String.valueOf(userNumb))
                .id(UUID.randomUUID().toString())
                .claim("sid", sessionId)
    ...
    /**
     * 전달받은 JWT 토큰의 서명 위변조 및 만료 여부를 검증
     *
     * @author SeungHyeon.Kang
    ...
    public boolean validateAccessToken(String token) {
        ...
        try {
            // 서명과 만료가 유효하고 Access 용도 및 권한을 가진 토큰만 API 인증에 허용
            Claims claims = getClaims(token);
            String role = claims.get("role", String.class);
            // Refresh Token이 Access Token 위치에서 인증 수단으로 사용되지 않도록 용도를 고정
        ...
        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
            ...
            // Access Token의 서명, 만료 또는 용도 검증 실패를 반환
            return false;
        }
    }

    /**
     * 전달받은 JWT가 Access Token 재발급에 사용할 Refresh Token인지 검증
     * 이전 버전 Refresh Token은 role 클레임이 없는 경우에만 한 차례 회전을 허용
     *
     * @author SeungHyeon.Kang
    ...
    public Long getUserNumb(String token) {
        // JWT 토큰에서 회원 번호(sub)를 추출 결과를 반환
        return Long.parseLong(getClaims(token).getSubject());
    }

    /**
     * JWT 토큰에서 사용자 권한(role)을 추출
     *
     * @author SeungHyeon.Kang
     * @param token JWT 토큰
     * @return 사용자 권한 문자열
    ...
    public String getRole(String token) {
        // JWT 토큰에서 사용자 권한(role)을 추출 결과를 반환
        return getClaims(token).get("role", String.class);
    }

    /**
     * JWT 토큰에서 고유 식별자(jti)를 추출
     *
     * @author SeungHyeon.Kang
     * @param token JWT 토큰
     * @return 토큰 고유 ID (jti)
    ...
    public String getTokenId(String token) {
        // JWT 토큰에서 고유 식별자(jti)를 추출 결과를 반환
        return getClaims(token).getId();
    }

    /**
     * JWT 토큰에서 기기별 로그인 세션 식별자(sid)를 추출
     *
     * @author SeungHyeon.Kang
     * @param token JWT 토큰
     * @return 로그인 세션 식별자
    ...
    public String getSessionId(String token) {
        // 동일 브라우저의 Access Token과 Refresh Token을 연결하는 세션 식별자를 반환
    ...
    public long getRemainingSeconds(String token) {
        ...
        // JWT 토큰의 만료 시간까지 남아있는 시간을 초(second) 단위로 계산하여 반환함. (블랙리스트 TTL 설정 시 사용) 결과를 반환
        return Math.max(TimeUnit.MILLISECONDS.toSeconds(remainingMillis), 0);
    }

    /**
     * Refresh Token의 설정된 전체 유효기간(초)을 반환
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * JWT 토큰의 서명을 검증하고 클레임(Claims) 정보를 추출
     *
     * @param token JWT 토큰
     * @return 추출된 Claims 객체
     */
    private Claims getClaims(String token) {
        // JWT 토큰의 서명을 검증하고 클레임(Claims) 정보를 추출 결과를 반환
        return Jwts.parser()
    ...
    /**
     * Access Token의 클레임 정보를 바탕으로 Spring Security의 Authentication(인증) 객체를 생성
     *
     * @author SeungHyeon.Kang
     * @param token JWT 토큰
    ...
    public Authentication getAuthentication(String token) {
        ...
        // Spring Security 표준 권한 형식("ROLE_")에 맞춰 GrantedAuthority 리스트를 구성
        List<GrantedAuthority> authorities = List.of(new SimpleGrantedAuthority("ROLE_" + role));

        // 새로 생성한 UsernamePasswordAuthenticationToken 객체를 반환
