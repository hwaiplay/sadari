 ...
 * date           : 2026-03-22
 * description    : 인증과 보안 요청의 인증 상태를 검사
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    /**
     * HTTP 요청 헤더/쿠키에서 Access Token을 추출하여 유효성 및 블랙리스트 등록 여부를 검증한 후 SecurityContext에 인증 객체를 등록
     *
     * @author SeungHyeon.Kang
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // Access Token이 존재하고, 서명/만료시간이 유효하며, Redis 블랙리스트(로그아웃된 토큰)에 등록되지 않은 경우 인증 객체를 생성
        if (!StringUtil.isEmpty(token) && jwtProvider.validateAccessToken(token) && !tokenRedisService.hasAccessTokenBlacklist(jwtProvider.getTokenId(token))) {
            ...
            // 한 요청에서 동일 Redis 회원 상태를 한 번만 조회
            String userStat = tokenRedisService.getUserStatus(userNumb);

            // 상태 캐시가 없으면 ACTIVE로 추정하지 않고 DB 계정 원본으로 보정
            if (StringUtil.isEmpty(userStat)) {
                ...
                if (StringUtil.isEmpty(savedUser) || StringUtil.isEmpty(savedUser.getUserStat())) {
                    ...
                    // 인증 없이 남은 필터 체인을 수행
                    filterChain.doFilter(request, response);
                    // 존재하지 않는 회원의 요청 처리를 종료
                    return;
                }

                // DB의 현재 계정 상태를 이번 요청 판단값으로 사용
            ...
            if (Constant.USER_STAT_WITHDRAWN.equals(userStat)
                ...
                // 비활성화 회원의 일반 서비스 API 요청을 권한 없음으로 응답
                response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                // 사용자 식별값 없이 계정 제한 원인 기록
                request.setAttribute(RequestLogFilter.SECURITY_REASON, "account_restricted");
                // 비활성화 회원 요청의 필터 처리를 종료
                return;
            }
            ...
            if (Constant.USER_STAT_SUSPENDED.equals(userStat)
                ...
                // 정지 회원의 일반 서비스 API 요청을 권한 없음으로 응답
                response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                // 사용자 식별값 없이 계정 제한 원인 기록
                request.setAttribute(RequestLogFilter.SECURITY_REASON, "account_restricted");
                // 정지 회원 요청의 필터 처리를 종료
            ...
            // SecurityContext에 Authentication 객체를 세팅하여 이 후 컨트롤러에서 @AuthenticationPrincipal 등으로 유저 정보를 참조할 수 있도록 처리
            SecurityContextHolder.getContext().setAuthentication(authentication);
        }

        // JWT 검증 뒤 남은 보안 필터 체인을 계속 실행
        filterChain.doFilter(request, response);
    }

    /**
     * 특정 요청 URI에 대해 해당 JWT 필터 수행을 건너뛸지 여부를 결정
     * Refresh Token 재발급과 로컬 개발용 계정 전환은 기존 Access Token 상태와 무관해야 하므로 검증 대상에서 제외
     *
     * @author SeungHyeon.Kang
     * @param request 서블릿 요청 객체
    ...
    private String extractAccessToken(HttpServletRequest request) {
        ...
        for (Cookie cookie : request.getCookies()) {
            // Access Token 쿠키명과 일치하는 쿠키가 존재하면 해당 토큰 값을 반환
            if (ACCESS_TOKEN_COOKIE_NAME.equals(cookie.getName())) {
                // HTTP 요청의 쿠키 목록에서 Access Token 쿠키 값을 추출 결과를 반환
    ...
    /**
     * 영구 삭제 대기 회원에게 허용된 최소 API 경로인지 확인
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 비활성화 회원에게 허용된 최소 인증 API 경로인지 확인
     *
     * @author SeungHyeon.Kang
     * @param requestUri 현재 요청 URI
     * @return 허용 경로 여부
    ...
    private boolean isWithdrawnAllowedPath(String requestUri) {
        ...
        // 재로그인 전에는 인증 상태 확인과 로그아웃 경로만 허용
    ...
    /**
     * 정지 회원에게 정지 안내와 로그아웃 관련 API만 허용
     *
     * @author SeungHyeon.Kang
    ...
    private boolean isSuspendedAllowedPath(String requestUri) {
        ...
        // 정지 상태 확인과 본인 고객문의 및 인증 종료 경로만 허용
