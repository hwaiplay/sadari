 ...
 * date           : 2026-07-13
 * description    : 기기별 로그인 세션과 사용자 인증 캐시를 Redis에서 관리
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class TokenRedisService {
    ...
    /**
     * 기기별 Refresh Token 세션과 회원 공용 표시 정보를 Redis에 생성
     *
     * @author SeungHyeon.Kang
    ...
    public void setLoginUserInfo(Long userNumb, String sessionId, String refreshToken
        ...
        // 필수 인증 값이나 만료 시간이 올바르지 않으면 불완전한 세션 생성을 차단
        ...
        // 세션 Hash와 사용자 세션 색인 및 공용 캐시를 한 번의 Redis 명령으로 반영
    ...
    /**
     * 제출된 Refresh Token을 원자적으로 회전하고 동시 요청에는 같은 최신 토큰을 반환
     *
     * @author SeungHyeon.Kang
     * @param userNumb 회원 번호
     * @param sessionId 기기별 로그인 세션 식별자
    ...
    public String rotateRefreshToken(Long userNumb, String sessionId, String presentedToken
        ...
        // Redis 서버 시각 기준의 유예 구간 안에서는 여러 탭과 서비스 워커에 같은 최신 토큰을 반환
    ...
    /**
     * Access Token의 회원과 세션 식별자가 현재 활성 세션을 가리키는지 확인
     *
     * @author SeungHyeon.Kang
    ...
    public String getUserStatus(Long userNumb) {
        ...
        // 로그인 세션 삭제와 분리된 회원 상태 캐시를 반환
        return redisTemplate.opsForValue().get(getUserStatusKey(userNumb));
    }

    /**
     * 로그인 세션 유무와 관계없이 회원 상태 캐시를 최신 값으로 변경
     *
     * @author SeungHyeon.Kang
    ...
    public void uptUserStatus(Long userNumb, String userStat) {
        ...
        // 로그아웃이 계정 제한 상태를 삭제하지 못하도록 만료 없는 별도 키로 저장
        redisTemplate.opsForValue().set(getUserStatusKey(userNumb), userStat);
    }

    /**
     * 로그인 사용자 번호로 Redis에 저장된 닉네임을 조회
     *
     * @author SeungHyeon.Kang
    ...
    public String getUserNick(Long userNumb) {
        ...
        // 로그인 사용자 번호에 대응하는 닉네임 캐시를 반환
        return redisTemplate.opsForValue().get(getUserNickKey(userNumb));
    }

    /**
     * 활성 로그인 세션의 남은 최대 TTL 동안 닉네임 캐시를 갱신
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 회원의 모든 기기 로그인 세션과 닉네임 캐시를 제거
     * 계정 제한 상태는 별도 키에 남겨 로그아웃으로 우회할 수 없도록 처리
     *
     * @author SeungHyeon.Kang
    ...
    public void delAllUserInfo(Long userNumb) {
        ...
        // 회원 원본이 사라진 뒤 남을 수 없는 상태 캐시도 물리 제거
        if (!StringUtil.isEmpty(userNumb)) {
    ...
    /**
     * 로그아웃 처리된 Access Token의 식별자를 남은 유효시간 동안 차단
     *
     * @author SeungHyeon.Kang
    ...
    private String getSessionKey(String sessionId) {
        // 세션 식별자를 포함한 Redis 키를 반환
        return SESSION_PREFIX + sessionId;
    }

    /**
     * 회원별 로그인 세션 색인 Redis 키를 생성
     *
     * @author SeungHyeon.Kang
     * @param userNumb 회원 번호
    ...
    private String getUserStatusKey(Long userNumb) {
        // 회원 번호를 포함한 상태 Redis 키를 반환
        return USER_STATUS_PREFIX + userNumb;
    }

    /**
     * Access Token 블랙리스트 Redis 키를 생성
     *
     * @author SeungHyeon.Kang
     * @param tokenId Access Token 식별자
    ...
    private String getTokenBlacklistKey(String tokenId) {
        // Access Token 식별자를 포함한 블랙리스트 Redis 키를 반환
