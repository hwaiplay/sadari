 ...
 * author         : HanWon.Jang
 * date           : 2026-09-16
 * description    : React 진입 문서가 이전 배포 Cache로 재사용되지 않도록 응답 정책을 적용함
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class SpaDocumentCacheFilter extends OncePerRequestFilter {
    ...
    /**
     * React 화면 문서 응답에 재사용 금지 Header를 설정한 뒤 요청 처리를 계속함
     *
     * @author HanWon.Jang
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response
        ...
        // 배포마다 달라지는 HTML 문서만 브라우저 저장 대상에서 제외함
        if (isSpaDocumentRequest(request)) {
            // 브라우저와 Cloudflare가 이전 HTML을 다시 사용하지 않도록 표준 Cache Header를 설정함
            response.setHeader(HttpHeaders.CACHE_CONTROL, SPA_CACHE_CONTROL);
            // HTTP/1.0 호환 Cache에도 같은 재사용 금지 정책을 전달함
            response.setHeader(HttpHeaders.PRAGMA, "no-cache");
            // 이미 저장된 화면 문서도 즉시 만료된 것으로 처리함
            response.setDateHeader(HttpHeaders.EXPIRES, 0);
        }

        // Header 설정 뒤 기존 보안과 Controller 및 정적 자원 처리를 그대로 수행함
        filterChain.doFilter(request, response);
    }

    /**
     * 요청 경로가 API나 정적 파일이 아닌 React 화면 문서인지 판정함
     *
     * @author HanWon.Jang
     * @param request 현재 HTTP 요청
    ...
    private boolean isSpaDocumentRequest(HttpServletRequest request) {
        ...
        // 애플리케이션 Context Path를 제외한 실제 요청 경로를 조회함
        String requestPath = request.getRequestURI().substring(request.getContextPath().length());

        // 정적 진입 문서는 확장자가 있어도 React 실행 문서이므로 명시적으로 포함함
        if ("/".equals(requestPath) || "/index.html".equals(requestPath)) {
            // 브라우저가 저장하면 안 되는 React 진입 문서임을 반환함
            return true;
        }

        // 확장자가 있는 파일은 해시 또는 파일별 Cache 정책을 유지함
        if (requestPath.contains(".")) {
            // React 화면 문서가 아님을 반환함
            return false;
        }

        // API와 업로드 및 정적 자원 경로를 화면 문서 정책에서 제외함
        for (String excludedPathPrefix : EXCLUDED_PATH_PREFIX_LIST) {
            // 경로 자체 또는 하위 경로가 제외 접두사와 일치하는지 확인함
            if (requestPath.equals(excludedPathPrefix) || requestPath.startsWith(excludedPathPrefix + "/")) {
                // 개별 Endpoint 또는 파일 Cache 정책을 유지하도록 제외함
                return false;
            }
        }

        // 확장자 없는 나머지 경로는 React Router가 처리할 화면 문서로 판정함
        return true;
    }
}
