 ...
 * date           : 2026-07-27
 * description    : 삭제 상태 알림을 물리 삭제하고 스케줄러 실행 결과를 기록
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class AlimDeleteServiceImpl implements AlimDeleteService {
    ...
    /**
     * 삭제 상태 알림을 물리 삭제하고 실제 삭제 건수가 있는 실행 결과만 기록
     * 삭제 SQL이 실패하면 실행 로그와 실패 상세를 기록한 후 원래 예외를 다시 전달
     *
     * @author SeungHyeon.Kang
     */
    @Override
    ...
    public void delAlim() {
        ...
        // 알림 삭제 SQL 실패를 스케줄러 실패 로그로 전환하기 위해 예외 흐름을 분리
        try {
            /*
             * 삭제 대상 조회와 삭제를 분리하면 두 SQL 사이에 데이터가 추가될 수 있어 로그 건수와 실제 삭제 건수가 달라짐
             * DELETE 반환 건수를 대상 건수와 성공 건수로 함께 사용하여 실제 반영 결과를 정확하게 기록
             */
        ...
        // 삭제 SQL 예외를 실행 실패와 실패 상세 로그에 함께 기록
            ...
            // 알림 삭제 실패 원인과 예외 스택을 운영 로그에 기록
            log.error("삭제 상태 알림 정리 중 오류가 발생했습니다. failure={}", LogSafe.getFailure(e));
            throw e;
        }

        // 정상 완료와 예외 발생 모두에서 최종 실행 건수와 소요 시간을 확정
        finally {
            // 기존에 생성된 스케줄러 실행 번호를 최종 실행 정보에 설정
            schedulerRunDto.setRunxNumb(runxNumb);
            // 삭제 결과로 결정된 최종 실행 상태를 설정
            schedulerRunDto.setExecStat(executionStatus);
            // 실제 DELETE 반영 건수를 처리 대상 건수로 설정
            schedulerRunDto.setTrgtCntt(targetCnt);
            // 정상적으로 삭제된 알림 건수를 성공 건수로 설정
            schedulerRunDto.setSuccCntt(successCnt);
            // 삭제 SQL 실행 중 발생한 실패 건수를 설정
            schedulerRunDto.setFailCntt(failureCnt);
            // 알림 삭제 스케줄러의 전체 실행 시간을 밀리초 단위로 설정
            schedulerRunDto.setExecMsec(
                    // 나노초 단위 실행 시간을 스케줄러 로그 저장 단위인 밀리초로 변환
                    TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startNanoTime)
            );
            // 삭제 대상이나 실패가 있는 실행만 관리자 확인용 스케줄러 로그로 관리
            if (targetCnt > 0 || successCnt > 0 || failureCnt > 0) {
                // 처리 건수가 있지만 마스터 로그가 없으면 최종 건수로 실행 로그를 생성
                if (StringUtil.isEmpty(runxNumb)) {
                    // 최종 처리 건수가 반영된 스케줄러 마스터 로그를 등록
                    runxNumb = schedulerLogSupport.setSchedulerLogSafely(schedulerRunDto);
                    // 등록된 실행 번호를 종료 로그 수정에 사용하도록 설정
                    schedulerRunDto.setRunxNumb(runxNumb);
                }

                // 등록된 스케줄러 마스터 로그에 최종 실행 상태와 처리 건수를 반영
