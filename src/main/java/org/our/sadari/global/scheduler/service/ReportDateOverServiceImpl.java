...
public class ReportDateOverServiceImpl implements ReportDateOverService {
    ...
    public void sendReportDateOverAlim() {
        ...
        /*
         * 공통 유틸리티에서 stack index를 계산하면 호출 깊이에 따라 다른 메서드명이 저장될 수 있음
         * 실제 스케줄러 업무 메서드 안에서 현재 Thread의 두 번째 stack frame을 읽어 sendReportDateOverAlim을 기록
         */
        ...
        // StrtDate 업무 값을 schedulerRunDto DTO에 설정
        ...
        try {
            // ReportDateOverTargetList 데이터를 DB에서 조회
            List<ReportDto> targetList = reportDateOverMapper.getOverdueReportList(maxSize);

            // MyBatis는 일반적으로 빈 List를 반환하지만 비정상 null 반환도 대상 없음으로 보정해 NPE를 차단
            if (StringUtil.isEmpty(targetList) || targetList.isEmpty()) {
                ...
                // yml에 설정한 최대 건수만큼 대상을 조회하고 각 대상에게 목표기간 초과 알림을 발송 결과를 반환
                return;
            }
            ...
            for (ReportDto target : targetList) {
                ...
                try {
                    /*
                     * Mapper가 ReportDto에 독후감 번호, 사용자 번호, 책 제목을 함께 담아 반환
                     * 조회한 제목을 그대로 치환 Map에 사용하므로 알림 발송 과정에서 도서 정보를 다시 조회하지 않음
                     */
                    ...
                    // ResultData가 존재하고 성공 코드인 경우에만 성공 건수로 집계
                    ...
                    // TrgtCntt 업무 값을 schedulerRunDto DTO에 설정
                    schedulerRunDto.setTrgtCntt(targetCnt);
                    // 실패 상세를 연결할 실행 번호가 없으면 최초 실패 시점에 마스터 로그를 생성
                ...
                // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
                    ...
                    /*
                     * 한 대상의 예외가 남은 배치를 중단시키지 않도록 실패 건만 기록하고 다음 대상으로 진행
                     * 저장되지 않은 대상은 조회 SQL의 NOT EXISTS 조건을 계속 만족하므로 다음 실행에서 다시 시도됨
                     */
                    failureCnt++;
                    // FailCntt 업무 값을 schedulerRunDto DTO에 설정
                    schedulerRunDto.setFailCntt(failureCnt);
                    // TrgtCntt 업무 값을 schedulerRunDto DTO에 설정
                    schedulerRunDto.setTrgtCntt(targetCnt);
                    // 실패 상세를 연결할 실행 번호가 없으면 최초 예외 시점에 마스터 로그를 생성
                    if (StringUtil.isEmpty(runxNumb)) {
            ...
            // getSchedulerExecStatus 조회로 후속 처리에 필요한 데이터를 가져옴
        ...
        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
            ...
            /*
             * 대상 목록 조회처럼 개별 대상 처리 이전에 발생한 예외도 실행 실패로 남겨 관리자 화면에서 확인할 수 있도록 처리
             * 최종 마스터 로그를 갱신한 뒤 Spring 스케줄러에도 실패가 전달되도록 원래 예외를 다시 던짐
             */
            failureCnt++;
            executionStatus = Constant.SCHEDULER_EXEC_FAILURE;
            // FailCntt 업무 값을 schedulerRunDto DTO에 설정
            schedulerRunDto.setFailCntt(failureCnt);
            // 실행 전반의 예외를 연결할 실행 번호가 없으면 마스터 로그를 생성
            if (StringUtil.isEmpty(runxNumb)) {
        ...
        // 성공 여부와 관계없이 반드시 자원을 정리하기 위한 블록
            ...
            schedulerRunDto.setExecMsec(
                    // 측정한 실행 시간을 밀리초 단위로 변환
                    TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startNanoTime)
            );
            // 처리 결과가 존재하지만 실패가 없어 마스터 로그를 만들지 않았다면 최종 건수로 로그를 생성
            if (StringUtil.isEmpty(runxNumb)) {
                // SchedulerLogSafely 업무 값을 schedulerLogSupport DTO에 설정
                runxNumb = schedulerLogSupport.setSchedulerLogSafely(schedulerRunDto);
