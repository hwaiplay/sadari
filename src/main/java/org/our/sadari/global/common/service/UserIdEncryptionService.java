...
public class UserIdEncryptionService {
    ...
    /**
     * 암호화 키는 별도 환경변수 app.crypto.user-id-key가 있으면 그것을 우선 사용하고, 없으면 JWT secret을 재사용
    ...
    public UserIdEncryptionService(@Value("${app.crypto.user-id-key:${jwt.secret_key}}") String userIdEncryptionKey) {
        // 사용자 식별자 암호화에 사용할 비밀키를 담을 객체를 생성
        this.secretKeySpec = new SecretKeySpec(createAesKey(userIdEncryptionKey), KEY_ALGORITHM);
    }

    /**
     * 로그인 조회와 신규 회원 저장에 사용할 USER_IDXX 암호문을 생성
     * 이미 ENC: 접두어가 붙은 값은 마이그레이션이나 재호출 과정에서 중복 암호화되지 않도록 그대로 반환
     *
     * @author SeungHyeon.Kang
    ...
    public String encryptForStorage(String plainUserId) {
        // plainUserId 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (StringUtil.isEmpty(plainUserId) || plainUserId.startsWith(ENCRYPTED_PREFIX)) {
            // 로그인 조회와 신규 회원 저장에 사용할 USER_IDXX 암호문을 생성 결과를 반환
            return plainUserId;
        }

        // 외부 연동이나 데이터 변환 실패를 예외 흐름으로 분리하기 위한 블록
        try {
            // 초기화된 Firebase 서비스 인스턴스를 조회
            Cipher cipher = Cipher.getInstance(CIPHER_TRANSFORMATION);
            // 암호화 또는 복호화 모드와 키로 Cipher를 초기화
    ...
    /**
     * 탈퇴 이력 비교에 사용할 OAuth 사용자 식별값의 SHA-256 해시를 생성
     *
     * @author SeungHyeon.Kang
     * @param plainUserId 외부 OAuth 제공자의 원본 사용자 식별값
    ...
    public String hashForAudit(String plainUserId) {
        ...
        // JVM 기본 제공 SHA-256으로 복구할 수 없는 비교값을 생성
        try {
            // 원본 식별값을 고정 길이 해시 바이트로 변환
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            // DB의 USER_IDHS 형식과 같은 소문자 16진수 문자열을 반환
            return HexFormat.of().formatHex(digest.digest(plainUserId.getBytes(StandardCharsets.UTF_8)));
        }

        // 필수 해시 알고리즘을 사용할 수 없으면 식별 이력 저장과 가입 판별을 중단
        catch (Exception e) {
    ...
    /**
     * 임의 길이의 설정 키를 AES-128 키 길이에 맞게 축약
     * 설정 문자열을 그대로 잘라 쓰지 않고 SHA-256 해시 후 앞 16바이트를 사용해 키 길이 오류를 방지
     *
     * @author SeungHyeon.Kang
    ...
    private byte[] createAesKey(String sourceKey) {
        ...
        try {
            // 초기화된 Firebase 서비스 인스턴스를 조회
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            // 임의 길이의 설정 키를 AES-128 키 길이에 맞게 축약 결과를 반환
