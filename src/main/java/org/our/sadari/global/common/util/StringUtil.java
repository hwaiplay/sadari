...
public final class StringUtil {
    ...
    public static String cutString(String source, String output, int maxLength) {
        // 비어 있는 입력은 기존 호출부의 선택 처리 계약을 유지하도록 null로 반환한다
        if (isEmpty(source)) {
            // 변환할 원본 문자열이 없음을 반환한다
            return null;
        }

        // 허용 길이를 초과한 문자열에만 접미사를 적용한다
        if (source.length() > maxLength) {
            // 최대 길이로 자른 문자열과 접미사를 반환한다
            return source.substring(0, maxLength) + output;
        }

        // 길이 제한 안에 있는 원본 문자열을 그대로 반환한다
        return source;
    }
    ...
    public static String cutString(String source, int maxLength) {
        // 접미사 없이 동일한 길이 제한 계약을 적용한 결과를 반환한다
    ...
    public static String normalizePlainText(String value) {
        // 비어 있는 입력은 저장 조건에서 누락값으로 판단할 수 있도록 null로 통일한다
        if (isEmpty(value)) {
            // 정규화할 사용자 입력이 없음을 반환한다
            return null;
        }

        // 앞뒤 공백을 제거한 사용자 입력을 반환한다
        return value.trim();
    }
    ...
    public static String normalizePlainText(String value, int maxLength) {
        // 길이 제한 전에 공백 정책을 한 번만 적용한다
        String normalizedValue = normalizePlainText(value);

        // 공백 제거 뒤 값이 없으면 길이 계산을 진행하지 않는다
        if (isEmpty(normalizedValue)) {
            // 정규화 뒤 남은 사용자 입력이 없음을 반환한다
            return null;
        }

        // 공백이 제거된 입력에 최대 문자 길이를 적용한 결과를 반환한다
    ...
    public static boolean isEmpty(Object value) {
        // 공통 빈 값 판정 메서드 내부에서는 재귀 호출을 피하기 위해 null 자체를 확인한다
        if (value == null) {
            // null 값을 빈 상태로 반환한다
            return true;
        }

        // 문자열은 앞뒤 공백을 제외한 내용이 있어야 유효한 값으로 판정한다
        if (value instanceof String stringValue) {
            // 공백이 제거된 문자열의 빈 상태를 반환한다
            return stringValue.trim().isEmpty();
        }

        // 목록은 포함된 항목 수를 기준으로 빈 상태를 판정한다
        if (value instanceof List<?> listValue) {
            // 목록의 빈 상태를 반환한다
            return listValue.isEmpty();
        }

        // 맵은 포함된 항목 수를 기준으로 빈 상태를 판정한다
        if (value instanceof Map<?, ?> mapValue) {
            // 맵의 빈 상태를 반환한다
            return mapValue.isEmpty();
        }

        // 객체 배열은 배열 길이를 기준으로 빈 상태를 판정한다
        if (value instanceof Object[] arrayValue) {
            // 객체 배열의 빈 상태를 반환한다
            return arrayValue.length == 0;
        }

        // 지원 타입이 아닌 객체는 존재하는 값으로 반환한다
        return false;
    }
    ...
    public static boolean hasEmpty(Object... values) {
        // 가변 인자 배열 자체가 없으면 필수 값이 누락된 요청으로 판정한다
        if (values == null) {
            // 검사할 값 목록이 없으므로 빈 상태를 반환한다
            return true;
        }

        // 모든 전달값에 동일한 공통 빈 값 정책을 적용한다
        for (Object value : values) {
            // 하나라도 비어 있으면 나머지 값을 검사하지 않는다
            if (isEmpty(value)) {
                // 빈 값이 포함된 상태를 반환한다
                return true;
            }
