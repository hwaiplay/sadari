 ...
 * date           : 2026-07-21
 * description    : 공통코드 사전으로 사용자 입력의 비속어와 우회 표현을 탐지
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class BadWordDetectionService {
    ...
    // 공통코드 데이터를 데이터베이스에서 조회해 오는 유틸리티 클래스
    private final CodeUtil codeUtil;

    // 비속어 목록과 만료 시간을 담고 있는 캐시 객체
    ...
    /**
     * 입력된 문자열에서 탐지된 비속어 단어를 찾아 Optional 형태로 반환
     * 비속어가 발견되더라도 같은 문자열 안에서 EXCP_WORD 허용어가 더 넓은 범위로 감싸고 있으면 정상 표현으로 보고 통과시킴
     * 공백은 단어 경계로 보존하고 숫자·특수문자 제거본과 숫자 보존 정규화본을 순차적으로 검사
     * 공백이 없는 단어 안에서 같은 문자를 두 번 이상 반복한 구간은 한 글자로 축약한 값과 완전히 제거한 값을 추가 검사
    ...
    public Optional<String> findBadWord(String value) {
        ...
        // 메모리 캐시 조회나 정규화 연산을 수행할 필요가 없으므로 즉시 빈 Optional을 반환
        if (StringUtil.isEmpty(value)) {
            // 입력 문자열에서 처음 탐지된 비속어를 Optional로 반환
            return Optional.empty();
        }

        // 메모리 캐시에서 아호-코라식 자동자를 가져옴. 만료되었다면 내부적으로 DB에서 비속어 사전을 다시 읽고 자동자를 재생성
        BadWordCache cache = getBadWordCache();

        // 1단계 변환: 공백 경계와 한글 및 영문만 남기고 특수문자와 숫자를 제거한 문자열
        String normalizedWithoutDigits = normalizeBadWord(value, false);

        // 2단계 변환: 숫자 포함 비속어 검사를 위해 공백 경계와 숫자를 남겨두고 특수문자만 제거한 문자열
        String normalizedWithDigits = normalizeBadWord(value, true);

        // 공백 경계를 유지한 정규화본과 반복 문자 변환본에서 일반 및 숫자 포함 비속어를 순차 탐지
        // 입력 문자열에서 처음 탐지된 비속어를 Optional로 반환
    ...
    /** 한 글자 토큰 사이 공백 제거와 문장 단어 경계 보존 */
    private String normalizeSpacedCharacters(String value) {
        return SPACED_CHARACTER_PATTERN.matcher(value).replaceAll("");
    }

    /**
     * 메모리에 캐싱된 아호-코라식 자동자 묶음을 반환
     * 캐시 만료 시 Double Checked Locking 패턴을 사용하여 단 하나의 스레드만 데이터베이스를 조회하도록 제어
     *
     * @author SeungHyeon.Kang
     * @return 메인 메모리에 적재된 비속어 탐색 캐시
     */
    private BadWordCache getBadWordCache() {
        // 만료 시각 계산에 사용할 현재 시각을 조회
        long now = System.currentTimeMillis();
        BadWordCache currentCache = badWordCache;

        // 1차 검사: 동기화 블록 밖에서 빠르게 캐시 만료 여부를 확인
        // 99퍼센트의 정상 요청은 synchronized 락을 획득하는 오버헤드 없이 바로 메인 메모리의 캐시 데이터를 반환
        if (!currentCache.isExpired(now)) {
            // 메모리에 캐싱된 아호-코라식 자동자 묶음을 반환
            return currentCache;
        }

        // 캐시가 만료된 경우 여러 스레드가 동시에 DB 조회를 시도하는 것을 막기 위해 동기화 블록에 진입
        synchronized (this) {

            currentCache = badWordCache;

            // 2차 검사: 락 내부에서 만료 여부를 한 번 더 확인
            // 락 획득을 위해 대기하던 다른 스레드들이 1등 스레드가 이미 캐시를 갱신해 둔 것을 확인하고
            // 중복해서 데이터베이스를 조회하지 않도록 차단
            if (!currentCache.isExpired(now)) {
                // 메모리에 캐싱된 아호-코라식 자동자 묶음을 반환
                return currentCache;
            }
            ...
            // getExceptionWordList 호출로 처리에 사용할 기준 데이터를 적재
            List<String> reloadedExceptionWords = getExceptionWordList();
            // 컬렉션 데이터를 순차 처리할 스트림을 생성
            ...
            // 이 작업을 캐시 갱신 시점에 한 번만 수행하면 실제 저장/수정 검증 요청에서는 입력 문자열 순회 비용만 발생
            AhoCorasickMatcher badWordMatcher = AhoCorasickMatcher.from(reloadedBadWords);
            // 비속어 원문에서 탐색용 문자 단위를 생성
            ...
            // 생성된 자동자와 만료 시각을 담은 새 BadWordCache 인스턴스를 생성하여 참조를 교체
    ...
    /**
     * 데이터베이스의 공통코드 테이블에서 BADX_WORD 리스트를 조회하여 문자열 목록으로 변환
     *
     * @author SeungHyeon.Kang
     * @return 데이터베이스에서 조회한 비속어 문자열 리스트
     */
    private List<String> loadBadWordsFromCodeList() {
        // 데이터베이스의 공통코드 테이블에서 BADX_WORD 리스트를 조회하여 문자열 목록으로 변환 결과를 반환
    ...
    /**
     * 데이터베이스의 공통코드 테이블에서 EXCP_WORD 리스트를 조회하여 문자열 목록으로 변환
     * EXCP_WORD는 "시발점"처럼 비속어 문자열을 포함하지만 실제 서비스에서는 허용해야 하는 정상 단어 사전
     *
     * @author SeungHyeon.Kang
     * @return 데이터베이스에서 조회한 비속어 예외 허용어 문자열 리스트
     */
    private List<String> getExceptionWordList() {
        // 데이터베이스의 공통코드 테이블에서 EXCP_WORD 리스트를 조회하여 문자열 목록으로 변환 결과를 반환
    ...
    /**
     * 아호-코라식 자동자를 사용해 입력 문자열에 포함된 비속어가 있는지 탐지
     * 여러 비속어가 동시에 걸릴 경우 사용자 알림의 정확도를 위해 가장 긴 단어를 우선 선택
     *
     * @author SeungHyeon.Kang
    ...
    private Optional<String> findBadWord(AhoCorasickMatcher matcher, AhoCorasickMatcher exceptionMatcher, String value) {
        // 검사할 대상 문자열이 없으면 contains 비교 자체가 불필요하므로 즉시 반환
        if (StringUtil.isEmpty(value)) {
            // 아호-코라식 자동자를 사용해 입력 문자열에 포함된 비속어가 있는지 탐지 결과를 반환
            return Optional.empty();
        }
    ...
    /**
     * 정규화 문자열과 반복 문자 축약본 및 제거본에서 비속어를 순차적으로 탐지
     * 반복 문자를 한 글자로 줄이는 경우와 반복 구간 자체를 끼워 넣은 경우를 모두 차단
     *
     * @author SeungHyeon.Kang
     * @param matcher 비속어 사전으로 구성한 아호-코라식 자동자
    ...
    private Optional<String> getRepeatedBadWordDtl(AhoCorasickMatcher matcher, AhoCorasickMatcher exceptionMatcher, String value) {
        // 반복 문자가 없는 일반 입력은 기존 정규화 문자열을 한 번만 탐색
        Optional<String> matchedWord = findBadWord(matcher, exceptionMatcher, value);

        // 원본 정규화 문자열에서 비속어가 발견되면 반복 문자 변환 없이 결과를 확정
        if (matchedWord.isPresent()) {
            // 원본 정규화 문자열에서 탐지된 비속어를 반환
            return matchedWord;
        }

        // 같은 문자가 두 번 이상 이어지지 않으면 추가 정규화와 자동자 재탐색을 생략
        if (!REPEATED_CHARACTER_PATTERN.matcher(value).find()) {
            // 반복 문자 우회가 없는 정상 입력의 빈 탐지 결과를 반환
            return Optional.empty();
        }

        // 같은 문자 반복을 한 글자로 축약하여 원래 비속어 글자를 늘인 우회 표현을 복원
        String collapsedRepeatedValue = REPEATED_CHARACTER_PATTERN.matcher(value).replaceAll("$1");
        // 같은 문자 반복 구간을 제거하여 비속어 사이에 별도 문자를 반복 삽입한 우회 표현을 복원
        String removedRepeatedValue = REPEATED_CHARACTER_PATTERN.matcher(value).replaceAll("");

        // 정규화 문자열과 반복 문자 축약본 및 제거본에서 비속어를 순차적으로 탐지 결과를 반환
    ...
    /**
     * 비속어 매칭 결과 중 EXCP_WORD 허용어 범위 밖에 있는 가장 긴 비속어를 선택
    ...
    private Optional<String> getLongestBadWordMatch(List<MatchedWord> badWordMatches, List<MatchedWord> exceptionMatches) {
        ...
        for (MatchedWord badWordMatch : badWordMatches) {
            ...
            // longestMatchedWord 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        ...
        // 비속어 매칭 결과 중 EXCP_WORD 허용어 범위 밖에 있는 가장 긴 비속어를 선택 결과를 반환
        return Optional.ofNullable(longestMatchedWord);
    }

    /**
     * 비속어 매칭 구간이 허용어 매칭 구간 안에 완전히 포함되는지 판단
    ...
    private String normalizeBadWord(String value, boolean keepDigits) {
        // keepDigits 옵션이 true이면 한글, 영문, 숫자 및 공백을 제외한 모든 문자를 제거
        // keepDigits 옵션이 false이면 한글, 영문 및 공백만 남기고 숫자까지 포함한 모든 특수문자를 제거
        // 유니코드 프로퍼티 표현식을 사용하여 완성형 및 조합형 한글과 영문 대소문자를 정확하게 판별
        String allowedPattern = keepDigits
                ? "[^\\p{IsHangul}\\p{IsAlphabetic}\\p{IsDigit}\\p{javaWhitespace}\\p{Z}]"
                : "[^\\p{IsHangul}\\p{IsAlphabetic}\\p{javaWhitespace}\\p{Z}]";
        // 비속어 사이에 끼워 넣은 특수문자나 기호를 제거하기 위해 정규식을 사용하여 정규화 결과를 반환
        return value.replaceAll(allowedPattern, "");
    }

    /**
     * 비속어 단어 내부에 숫자가 포함되어 있는지 검사
     *
     * @author SeungHyeon.Kang
    ...
    private boolean hasDigit(String value) {
        // 비속어 단어 내부에 숫자가 포함되어 있는지 검사 결과를 반환
        return value.chars().anyMatch(Character::isDigit);
    }

    /**
     * 아호-코라식 탐색 결과 한 건을 표현하는 불변 데이터 객체
     * 예외 허용어가 실제 비속어 구간을 감싸는지 비교해야 하므로 단어 문자열뿐 아니라 시작/끝 위치를 함께 보관
     *
     * @author SeungHyeon.Kang
     * @param word 탐지된 단어
    ...
    /**
     * 비속어 탐색 자동자와 해당 데이터의 만료 시각을 하나로 묶어 보관하는 불변 데이터 객체
     * 자바 record 스펙을 사용하여 생성 후 내부 값을 변경할 수 없도록 보장
     *
     * @author SeungHyeon.Kang
    ...
    private record BadWordCache(AhoCorasickMatcher badWordMatcher, AhoCorasickMatcher exceptionWordMatcher, AhoCorasickMatcher digitBadWordMatcher
        ...
        /**
         * 애플리케이션 최초 구동 시 사용할 빈 캐시 인스턴스
         * 만료 시각을 0으로 설정하여 첫 요청 시 무조건 데이터베이스에서 조회가 일어나도록 유도
         *
         * @author SeungHyeon.Kang
         * @return 만료 시각이 0인 빈 BadWordCache 객체
         */
        private static BadWordCache empty() {
            // 비속어 사전이 로딩되기 전 사용할 빈 캐시 객체를 반환
        ...
        /**
         * 현재 시각과 비교하여 캐시가 만료되었는지 여부를 판단
         * 데이터베이스에 비속어 목록이 존재하지 않더라도 10분의 TTL 시간 동안은 재조회를 방지하기 위해
         * 리스트 크기가 아닌 만료 시각만을 기준으로 판별
         *
         * @author SeungHyeon.Kang
        ...
        private boolean isExpired(long nowMillis) {
            // 현재 시각과 비교하여 캐시가 만료되었는지 여부를 판단 결과를 반환
            return nowMillis >= expiresAtMillis;
        }
    }

    /**
     * 비속어 목록을 아호-코라식 알고리즘으로 탐색하기 위한 전용 매처
     * 여러 단어를 하나의 트라이에 넣고 실패 링크를 구성하여 입력 문자열을 한 번만 순회해도 모든 단어 포함 여부를 판별
     *
     * @author SeungHyeon.Kang
     */
    private static final class AhoCorasickMatcher {
        ...
        /**
         * 외부에서 직접 생성하지 않고 from 또는 empty 팩토리 메서드로만 만들도록 제한
         * 트라이 생성 후에는 검사 요청에서 구조가 바뀌지 않아야 하므로 생성 시점에 완성된 root만 보관
         *
         * @author SeungHyeon.Kang
        ...
        /**
         * 비속어 목록을 아호-코라식 트라이와 실패 링크로 변환
         * 이 비용은 캐시 갱신 시점에만 발생하고, 이후 여러 저장/수정 요청은 이미 만들어진 자동자를 공유
         *
         * @author SeungHyeon.Kang
        ...
        private static AhoCorasickMatcher from(List<String> words) {
            ...
            // 새로 생성한 AhoCorasickMatcher 객체를 반환
            return new AhoCorasickMatcher(root);
        }

        /**
         * 비속어가 하나도 없을 때 사용할 빈 매처를 생성
         ...
         * 사용자에게 감지 단어를 알려줄 때 짧은 초성어보다 더 구체적인 긴 단어가 유용하므로 전체 순회 중 최장 결과를 보관
         *
         * 동작 시뮬레이션 예시:
         * 비속어 사전 = ["카카오톡", "카오스"]
         * 사용자 입력 = "카카오스"
        ...
        private List<MatchedWord> findMatches(String value) {
            ...
            for (int index = 0; index < value.length(); ) {
                // 현재 위치의 유니코드 코드 포인트를 확인
                int codePoint = value.codePointAt(index);
                // 유니코드 문자가 차지하는 길이를 계산
                ...
                // 여러 단어가 동시에 끝나는 위치라면 모두 기록
                // 호출부에서 EXCP_WORD 범위와 비교한 뒤 최종적으로 가장 긴 차단 단어를 선택해야 하기 때문
                for (String matchedWord : node.outputs) {
                    // 탐지된 비속어와 원문 위치를 담을 객체를 생성
                    matchedWords.add(new MatchedWord(matchedWord, index - matchedWord.length(), index));
                }
            }

            // 입력 문자열에서 가장 긴 비속어 매칭 결과를 찾는다 결과를 반환
            return matchedWords;
        }

        /**
         * 단어 하나를 트라이에 삽입
         * codePoint 단위로 순회해 한글, 영문, 숫자 외에 유니코드 조합 문자가 들어오더라도 문자 경계를 안전하게 처리
         *
         * 동작 방식 예시:
        ...
        private static void addWord(TrieNode root, String word) {
            ...
            for (int index = 0; index < word.length(); ) {
                ...
                // computeIfAbsent 구문으로 이미 존재하는 첫 글자 노드는 새로 만들지 않고 기존 노드를 공유 및 재사용
                node = node.children.computeIfAbsent(codePoint, ignored -> new TrieNode());
            }

            // 단어의 마지막 글자 노드에 미리 완공된 비속어 명칭을 적어둠
            // 사용자가 탐색할 때 노드에 도착하자마자 이 주머니를 꺼내어 어떤 비속어가 완성되었는지 즉시 판단
            node.outputs.add(word);
        }

        /**
         * 트라이 노드마다 실패 링크를 구성
         * 실패 링크는 현재까지 일치한 접두사가 끊겼을 때 재검사를 시작할 가장 긴 접미사 상태를 가리키는 포인터
         *
         * BFS 및 큐 사용 이유:
         * 3단계 깊이 노드의 실패 링크를 계산하려면 2단계 부모 노드의 실패 링크가 이미 완성이 되어 있어야 하는 조건
         * 따라서 깊이 1단계(루트 직속 자식)부터 깊이 2단계, 3단계 순서로 위층에서 아래층으로 내려가며
         * 차례대로 실패 링크를 공사하기 위해 큐(Queue) 대기열을 사용
         *
         * outputs 병합 이유:
        ...
        private static void buildFailureLinks(TrieNode root) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (Map.Entry<Integer, TrieNode> entry : current.children.entrySet()) {
                    ...
                    // 실패 노드에서 끝나는 단어도 현재 노드에서 함께 끝난 것으로 봐야 하는 조건
                    // 예를 들어 긴 단어와 짧은 단어가 접미사를 공유할 때 누락 없이 탐지하기 위한 병합
