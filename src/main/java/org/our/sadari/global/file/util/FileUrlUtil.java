 ...
 * date           : 2026-08-26
 * description    : 저장된 원본 이미지 경로를 화면 용도별 공개 URL로 변환
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public final class FileUrlUtil {
    ...
    /** 공통 URL 변환 유틸리티의 인스턴스 생성을 차단 */
    private FileUrlUtil() {

    }

    /**
     * 원본 배경사진 경로를 일반 화면용 파생본 경로로 변환
     *
     * @author SeungHyeon.Kang
    ...
    public static String getBgDisplayPath(String filePath) {
        // 외부 URL이나 지원하지 않는 이전 경로는 기존 표시 계약을 유지
        if (StringUtil.isEmpty(filePath) || !filePath.startsWith(BACKGROUND_ACCESS_PREFIX)) {
            // 변환할 수 없는 경로를 변경 없이 반환
            return filePath;
        }

        // 서버의 화면용 파생 이미지 조회 계약을 원본 경로에 추가
