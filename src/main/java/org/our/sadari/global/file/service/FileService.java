...
public class FileService {
    ...
    /**
     * 사용자가 업로드한 이미지 파일을 프로젝트 내부 저장소에 저장하고 파일 번호를 반환
     *
     * @author SeungHyeon.Kang
    ...
    private Long setValidatedImage(ValidatedImage validatedImage, String originalName, String imageType
        ...
        try {
            ...
            /*
             * 뒤에서 사용자 프로필 UPDATE가 실패하면 DB 메타정보는 트랜잭션으로 롤백되지만 실제 파일은 자동 복구되지 않음
             * 현재 트랜잭션의 최종 상태를 확인해 롤백 시 물리 파일까지 함께 제거하도록 정리 작업을 예약
             */
            registerRollbackCleanup(getRelatedKeys(objectKey));
            fileMetadataSaved = true;
            // 사용자가 업로드한 이미지 파일을 프로젝트 내부 저장소에 저장하고 파일 번호를 반환한 결과를 반환
            return fileDto.getFileNumb();
        }

        // 성공 여부와 관계없이 반드시 자원을 정리하기 위한 블록
        finally {
            // DB 메타정보 저장 전에 실패한 파일을 남기면 접근되지 않는 파일이 누적되므로 즉시 정리
            if (!fileMetadataSaved) {
                // 메타정보 등록에 실패한 원본과 화면용 파생 객체를 함께 삭제
    ...
    /**
     * Kakao에서 전달받은 프로필 이미지를 내부 저장소에 복사하고 파일 번호를 반환
     *
     * @author SeungHyeon.Kang
    ...
    public Long setKakaoProfileImage(String profileImageUrl, String userIdxx, Long regiUser) {
        // Kakao 계정에 프로필 이미지가 없을 수 있으므로 회원 생성 흐름은 계속 진행
        ...
        try {
            ...
            try {
                ...
                // DB 저장이 롤백되면 업로드 파일도 제거되도록 정리 작업을 등록
            ...
            // 성공 여부와 관계없이 반드시 자원을 정리하기 위한 블록
                ...
                // 메타정보 저장 전 실패한 외부 프로필 파일도 서버에 남기지 않음
                if (!fileMetadataSaved) {
                    // 검증 중 생성된 임시 파일이 있으면 삭제
                    fileStorage.delFile(objectKey);
                }
            }
        }

        // 예외 발생 시 기본값 보정 또는 공통 실패 흐름으로 전환
        catch (Exception e) {
            // 외부 이미지 다운로드 실패가 로그인 실패로 이어지지 않도록 원본 URL을 대체 경로로 저장
    ...
    /**
     * 선택한 프로필 또는 배경 이미지를 비공개 임시 저장소에 보관하고 축소 미리보기를 반환
     *
     * @author SeungHyeon.Kang
    ...
    public ProfileImageDraftDto setProfileImageDraft(MultipartFile imageFile, String imageType
        ...
        // 인증 정보와 파일이 없으면 사용자별 임시 저장을 시작하지 않음
        ...
        // 허용된 프로필과 배경 유형만 사용자 임시 경로로 변환
        validateImageType(imageType);
        // 서버에서 시그니처와 크기 및 EXIF 방향을 검증하고 픽셀을 정규화
        ValidatedImage validatedImage = validateAndNormalizeImage(imageFile.getBytes());
        // 화면 표시용 미리보기는 대상별 제한 크기로 별도 축소
    ...
    /**
     * 로그인 사용자의 만료되지 않은 프로필과 배경 임시 이미지를 조회
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
    ...
    public List<ProfileImageDraftDto> getProfileImageDraftList(Long userNumb) {
        // 인증 사용자 번호가 없으면 다른 사용자의 임시 경로를 조회하지 않음
        if (StringUtil.isEmpty(userNumb)) {
            return List.of();
        }

        // 프로필과 배경 임시 선택본을 각각 조회해 만료되지 않은 값만 반환
    ...
    /**
     * 임시 이미지 식별값을 로그인 사용자와 유형에 맞춰 영구 파일로 승격
     *
     * @author SeungHyeon.Kang
    ...
    public Long setUploadedImageDraft(String draftToken, String imageType, Long userNumb) throws IOException {
        ...
        // 비공개 임시 원본의 실제 시그니처로 최종 저장 형식을 다시 확인
    ...
    public void delProfileImageDraft(Long userNumb, String imageType) {
        // 안전한 사용자 및 이미지 유형 경로만 삭제 대상으로 허용
        if (StringUtil.isEmpty(userNumb)) {
            return;
        }
    ...
    /**
     * 로그아웃과 계정 상태 변경 시 로그인 사용자의 모든 임시 이미지를 삭제
     *
     * @author SeungHyeon.Kang
     * @param userNumb 삭제할 사용자 번호
     */
    public void delAllProfileImageDrafts(Long userNumb) {
        // 사용자 번호가 없으면 임시 저장 루트에 접근하지 않음
        if (StringUtil.isEmpty(userNumb)) {
            return;
        }

        // 프로필과 배경 임시 파일을 각각 안전한 하위 경로에서 제거
    ...
    /**
     * 계정 상태 변경 트랜잭션이 커밋된 뒤 사용자의 모든 임시 이미지를 삭제
     *
     * @author SeungHyeon.Kang
     * @param userNumb 삭제할 사용자 번호
     */
    public void delProfileDraftsOnCommit(Long userNumb) {
        // 트랜잭션 밖에서는 이미 확정된 상태에 맞춰 즉시 임시 이미지를 삭제
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            /**
             * 계정 상태 변경이 커밋된 경우에만 사용자 임시 이미지를 삭제
             *
             * @author SeungHyeon.Kang
            ...
            public void afterCompletion(int status) {
                // 롤백된 계정 상태에서는 사용자가 편집을 계속할 수 있도록 임시 선택본을 유지
                if (status != TransactionSynchronization.STATUS_COMMITTED) {
                    return;
                }

                // 확정된 비활성화 또는 영구 탈퇴 상태의 사용자 임시 이미지를 모두 삭제
                delAllProfileImageDrafts(userNumb);
            }
        });
    }
    ...
    /**
     * 파일 등록 사용자 번호로 영구 탈퇴 시 삭제할 파일 메타정보를 조회
     *
     * @author SeungHyeon.Kang
    ...
    public List<FileDto> getFileListByRegiUser(Long regiUser) {
        // 사용자 번호가 없으면 다른 사용자의 파일을 잘못 조회하지 않도록 빈 목록을 반환
        if (StringUtil.isEmpty(regiUser)) {
            // 영구 삭제할 파일이 없는 상태를 반환
            return List.of();
        }

        // 영구 탈퇴 트랜잭션이 삭제하기 전에 사용자가 등록한 파일 정보를 조회
        List<FileDto> fileList = fileMapper.getFileListByRegiUser(regiUser);

        // Mapper가 Null을 반환해도 영구 탈퇴 반복 처리가 중단되지 않도록 빈 목록으로 보정
        if (StringUtil.isEmpty(fileList)) {
            // 영구 삭제할 파일이 없는 상태를 반환
            return List.of();
        }

        // 영구 탈퇴 대상 사용자가 등록한 파일 메타정보 목록을 반환
        return fileList;
    }

    /**
     * 사용자 프로필과 배경에서 더 이상 참조하지 않는 파일 메타정보를 삭제하고 커밋 후 물리 파일을 정리
     *
     * @author SeungHyeon.Kang
     * @param fileNumb 교체 전 파일 번호
     */
    public void delFile(Long fileNumb) {
        // 교체 전 파일 번호가 없으면 기존 파일 정리 대상이 아니므로 종료
        if (StringUtil.isEmpty(fileNumb)) {
            // 기존 파일 정리를 종료
            return;
        }

        // 물리 파일 삭제 경로를 커밋 이후에도 사용할 수 있도록 메타정보를 먼저 조회
        FileDto fileDto = fileMapper.getFileByNumb(fileNumb);

        // 이미 정리된 파일 번호이면 중복 삭제를 성공 상태로 처리
        if (StringUtil.isEmpty(fileDto)) {
            // 중복 파일 정리를 종료
            return;
        }

        // 다른 프로필이나 배경에서 참조하지 않는 파일 메타정보만 삭제
        int deleteCnt = fileMapper.delFileIfUnreferenced(fileNumb);

        // 다른 이미지 컬럼이 같은 파일을 참조하면 해당 참조가 교체될 때까지 물리 파일을 유지
        if (deleteCnt != 1) {
            // 아직 참조 중인 파일 정리를 종료
            return;
        }

        // DB 참조와 메타정보 삭제가 커밋된 뒤에만 교체 전 물리 파일을 삭제
        registerCommitCleanup(List.of(fileDto));
    }

    /**
     * 영구 탈퇴 트랜잭션이 커밋된 뒤 사용자가 등록한 물리 파일을 모두 삭제
     *
     * @author SeungHyeon.Kang
     * @param fileList 영구 탈퇴 전에 조회한 파일 메타정보 목록
     */
    public void delFilesAfterCommit(List<FileDto> fileList) {
        // 영구 탈퇴 대상 파일이 없으면 커밋 후 정리 작업을 등록하지 않음
        if (StringUtil.isEmpty(fileList)) {
            // 영구 탈퇴 물리 파일 정리를 종료
            return;
        }

        // 회원과 파일 메타정보 삭제가 커밋된 뒤 물리 파일을 제거하도록 정리 작업을 등록
        registerCommitCleanup(List.copyOf(fileList));
    }

    /**
     * 배경사진의 일반 화면용 파생본을 조회하고, 기존 이미지이면 최초 요청에서 생성
     *
     * @author SeungHyeon.Kang
    ...
    public Optional<StoredFile> getBgDisplayFile(String objectKey) throws IOException {
        // 배경사진 원본 경로에서만 화면용 파생 객체 키를 생성
        String displayObjectKey = getBgDisplayKey(objectKey);

        if (StringUtil.isEmpty(displayObjectKey)) {
            // 지원하지 않는 저장소 경로는 조회하지 않음
            return Optional.empty();
        }

        // 이미 생성된 파생본이 있으면 이미지 디코딩과 원본 저장소 조회 없이 즉시 반환
        Optional<StoredFile> displayFile = fileStorage.getFile(displayObjectKey);

        if (displayFile.isPresent()) {
            // 재사용 가능한 화면용 파생 이미지를 반환
            return displayFile;
        }

        // 배포 이전에 저장된 배경사진의 원본 객체를 조회
        Optional<StoredFile> originalFile = fileStorage.getFile(objectKey);

        if (originalFile.isEmpty()) {
            // 원본이 없는 경로에는 파생본을 생성하지 않음
            return Optional.empty();
        }

        // 저장 파일 확장자로 서버가 허용한 이미지 형식을 복원
    ...
    public String getBgDisplayTag(String storedName) {
        // 파생본 규격이 바뀌면 기존 브라우저 캐시와 구분되도록 최대 길이를 포함
        return storedName + "-display-" + BACKGROUND_DISPLAY_MAX_EDGE;
    }

    /**
     * 외부 이미지 URL 자체를 파일 경로로 저장
     *
     * @author SeungHyeon.Kang
    ...
    private Long setExternalImage(String imageUrl, String ownerKey, String imageType
        ...
        // FilePath 업무 값을 fileDto DTO에 설정
        ...
        // 외부 이미지 URL 자체를 파일 경로로 저장한 결과를 반환
        return fileDto.getFileNumb();
    }

    /**
     * 이미지 타입과 업로드 날짜 및 저장 파일명으로 저장소 객체 키를 생성
     *
     * @author SeungHyeon.Kang
     * @param imageType 이미지 파일 타입
    ...
    private String getObjectKey(String imageType, String uploadDate, String storedName) {
        // 이미지 유형 아래에 업로드 날짜와 파일명을 포함한 객체 키를 반환
        return getUploadDirectoryName(imageType) + "/" + uploadDate + "/" + storedName;
    }

    /**
     * 원본 배경사진 객체 키를 일반 화면용 파생본 객체 키로 변환
     *
     * @author SeungHyeon.Kang
    ...
    private String getBgDisplayKey(String objectKey) {
        // 비어 있는 객체 키는 저장 경로로 변환하지 않음
        if (StringUtil.isEmpty(objectKey)) {
            // 배경사진이 아닌 객체에는 파생 경로가 없음을 반환
            return null;
        }

        // 운영체제 경로 규칙으로 정규화하여 상위 디렉터리 이동 문자를 제거
        ...
        if (storedPath.isAbsolute() || storedPath.getNameCount() != 3
            ...
            // 서버가 생성한 background/yyMMdd/file 형식이 아닌 객체 키를 차단
            return null;
        }

        // 검증된 상대 객체 키의 구분자를 S3 형식으로 통일
        String normalizedObjectKey = storedPath.toString().replace('\\', '/');
        // 마지막 파일명 앞에 화면용 파생본 전용 디렉터리를 삽입
        int fileNameIndex = normalizedObjectKey.lastIndexOf('/');
        // 원본과 같은 날짜 경로 아래의 display 디렉터리에 파생 객체를 보관
    ...
    /**
     * 원본 이미지와 함께 생성하거나 삭제해야 하는 저장소 객체 키를 반환
     *
     * @author SeungHyeon.Kang
    ...
    private List<String> getRelatedKeys(String objectKey) {
        ...
        // 배경사진은 원본과 화면용 파생 객체를 같은 수명주기로 관리
        return List.of(objectKey, displayObjectKey);
    }

    /**
     * 화면용 배경사진을 재생성 가능한 저장 캐시로 기록
     *
     * @author SeungHyeon.Kang
    ...
    private String getUploadDirectoryName(String imageType) {
        ...
        if (Constant.FILE_TYPE_BACKGROUND.equals(imageType)) {
            // 이미지 타입을 실제 저장 디렉터리명으로 변환한 결과를 반환
            return "background";
        }
    ...
    /**
     * 서버가 검증해 결정한 확장자로 충돌 없는 저장 파일명을 생성
     *
     * @author SeungHyeon.Kang
    ...
    private String createStoredFileName(String extension) {
        // 서버가 검증해 결정한 확장자로 충돌 없는 저장 파일명을 생성한 결과를 반환
        return UUID.randomUUID() + extension;
    }

    /**
     * 사용자와 이미지 유형에 제한된 비공개 임시 저장 경로를 반환
     *
     * @author SeungHyeon.Kang
     * @param userNumb 로그인 사용자 번호
     * @param imageType 프로필 또는 배경 이미지 구분값
    ...
    private Path getProfileDraftDir(Long userNumb, String imageType) {
        ...
        // 사용자별 이미지 유형 임시 디렉터리를 반환
        return draftDirectory;
    }

    /**
     * 임시 이미지 식별값과 용도 및 확장자를 안전한 파일명으로 결합
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 특정 임시 이미지 식별값에 대응하는 원본 또는 미리보기 파일을 조회
     *
     * @author SeungHyeon.Kang
    ...
    private ProfileImageDraftDto getProfileImageDraft(Long userNumb, String imageType) {
        ...
        try (Stream<Path> pathStream = Files.list(draftDirectory)) {
            ...
            // 원본이 없거나 만료되었으면 해당 유형의 남은 임시 파일을 정리
            if (StringUtil.isEmpty(originalPath) || isExpiredDraft(originalPath)) {
            ...
            // 원본과 한 쌍인 미리보기가 없으면 불완전한 임시 선택본을 제거
            if (StringUtil.isEmpty(previewPath)) {
                delFilesInDirectory(draftDirectory);
                return null;
            }

            // 미리보기 시그니처로 Data URL MIME 형식을 결정
            ...
            // 같은 로그인 사용자에게만 서버 미리보기와 만료 시각을 반환
    ...
    private ValidatedImage createPreviewImage(ValidatedImage sourceImage, int maxEdge) {
        try {
            ...
            try {
                // 고해상도 원본을 축소할 때 계단 현상을 줄이는 보간 설정을 적용
    ...
    /**
     * 새 임시 선택본을 제외한 같은 유형의 이전 파일을 삭제
     *
     * @author SeungHyeon.Kang
     * @param draftDirectory 사용자별 임시 디렉터리
    ...
    /**
     * 파일 수정 시각을 정렬과 만료 계산에 사용할 밀리초 값으로 반환
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 파일 타입 파라미터가 프로필 또는 배경 이미지인지 검증
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 원본 파일명에서 경로와 제어문자를 제거해 메타정보로 저장할 안전한 이름을 만듦
     *
     * @author SeungHyeon.Kang
    ...
    private String normalizeOriginalName(String originalName) {
        // originalName 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (StringUtil.isEmpty(originalName)) {
            // 원본 파일명에서 경로와 제어문자를 제거해 메타정보로 저장할 안전한 이름을 만듭니다 결과를 반환
            return "image";
        }
        ...
        // 정규식과 일치하는 문자열을 일괄 치환
        normalizedName = normalizedName.replaceAll("[\\p{Cntrl}]", "").trim();

        // normalizedName 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
    ...
    /**
     * 실제 파일 시그니처, 이미지 헤더, 해상도와 디코딩 결과를 검증한 뒤 픽셀만 새 이미지로 재인코딩
     *
     * @author SeungHyeon.Kang
    ...
    private ValidatedImage validateAndNormalizeImage(byte[] originalBytes) {
        // originalBytes 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        ...
        try (
            ...
            try {
                ...
                // getWidth 조회로 후속 처리에 필요한 데이터를 가져옴
                int width = reader.getWidth(0);
                // getHeight 조회로 후속 처리에 필요한 데이터를 가져옴
                int height = reader.getHeight(0);
                // 이미지 픽셀 수를 오버플로 없이 계산
                long pixelCount = Math.multiplyExact((long) width, (long) height);

                // 압축 해제 폭탄을 막기 위해 전체 픽셀 수를 확인한 뒤에만 실제 픽셀 디코딩을 수행
                if (width <= 0 || height <= 0
    ...
    private ImageFormat detectImageFormat(byte[] bytes) {
        ...
        if (startsWith(bytes, JPEG_SIGNATURE)) {
            // 이미지 선두 바이트를 기준으로 허용 형식을 판별한 결과를 반환
    ...
    private boolean startsWith(byte[] bytes, byte[] signature) {
        ...
        if (bytes.length < signature.length) {
            // 파일 바이트가 지정된 시그니처로 시작하는지 확인한다 판정값을 반환
            return false;
        }

        // 목록 또는 문자열 항목을 누락 없이 순차 처리하기 위한 반복 블록
    ...
    /**
     * JPEG APP1 메타정보에서 촬영 당시 이미지 방향값을 조회
     *
     * @author SeungHyeon.Kang
    ...
    private int getExifOrientation(byte[] bytes, ImageFormat imageFormat) {
        // PNG에는 JPEG APP1 메타정보가 없으므로 방향 보정을 적용하지 않음
        if (imageFormat != ImageFormat.JPEG) {
            // 원본 픽셀 방향을 유지하는 기본 방향값을 반환
            return 1;
        }

        int markerOffset = JPEG_SIGNATURE.length - 1;

        // 압축 이미지 데이터가 시작되기 전의 JPEG 메타정보 구간만 순차 검사
        while (markerOffset + 4 <= bytes.length) {
            // JPEG 마커 시작값이 아니면 손상된 메타정보로 보고 기본 방향을 사용
            if ((bytes[markerOffset] & 0xFF) != 0xFF) {
                // 손상된 메타정보 대신 원본 픽셀 방향을 유지
                return 1;
            }

            int marker = bytes[markerOffset + 1] & 0xFF;

            // 압축 이미지 또는 파일 종료 지점 이후의 바이트는 EXIF 메타정보로 해석하지 않음
            if (marker == JPEG_START_OF_SCAN_MARKER || marker == JPEG_END_OF_IMAGE_MARKER) {
                // EXIF 방향값이 없는 이미지의 기본 방향을 반환
                return 1;
            }

            // JPEG 구간 길이를 네트워크 바이트 순서로 조회
            int segmentLength = getUnsignedShort(bytes, markerOffset + 2, false);

            // 구간 길이가 헤더보다 짧거나 파일 범위를 벗어나면 손상된 메타정보로 처리
    ...
    private boolean isExifHeader(byte[] bytes, int offset, int length) {
        ...
        // APP1 데이터가 EXIF 고정 헤더와 일치함을 반환
        return true;
    }

    /**
     * EXIF TIFF 구간의 첫 번째 이미지 디렉터리에서 방향 태그를 조회
     *
     * @author SeungHyeon.Kang
     * @param bytes 검사할 원본 이미지 바이트
    ...
    private int getTiffOrientation(byte[] bytes, int tiffOffset, int tiffLength) {
        // TIFF 헤더와 첫 번째 이미지 디렉터리 위치를 읽을 수 없는 구간은 기본 방향으로 처리
        if (tiffLength < 8 || tiffOffset < 0 || tiffOffset + (long) tiffLength > bytes.length) {
            // 손상된 TIFF 메타정보 대신 원본 픽셀 방향을 유지
            return 1;
        }

        boolean littleEndian;

        // Intel 바이트 순서로 기록된 TIFF 메타정보를 구분
        if (bytes[tiffOffset] == 0x49 && bytes[tiffOffset + 1] == 0x49) {
            littleEndian = true;
        }

        // Motorola 바이트 순서로 기록된 TIFF 메타정보를 구분
        else if (bytes[tiffOffset] == 0x4D && bytes[tiffOffset + 1] == 0x4D) {
            littleEndian = false;
        }

        // 지원하지 않는 바이트 순서는 손상된 메타정보로 처리
        else {
            // 해석할 수 없는 TIFF 메타정보의 기본 방향값을 반환
            return 1;
        }

        // TIFF 고정 식별값이 아니면 이미지 디렉터리를 읽지 않음
        if (getUnsignedShort(bytes, tiffOffset + 2, littleEndian) != 42) {
            // 유효하지 않은 TIFF 메타정보의 기본 방향값을 반환
            return 1;
        }

        // TIFF 기준 상대 위치를 파일 내 절대 위치로 변환
        ...
        for (int index = 0; index < entryCount; index++) {
            ...
            // 이미지 방향 이외의 태그는 후속 항목에서 계속 탐색
            if (tag != EXIF_ORIENTATION_TAG) {
                // 다음 TIFF 태그 항목을 확인
                continue;
            }

            // Orientation은 SHORT 한 개로 저장되어야 하므로 타입과 개수를 함께 검증
            if (getUnsignedShort(bytes, (int) entryOffset + 2, littleEndian) != 3
                    || getUnsignedInt(bytes, (int) entryOffset + 4, littleEndian) != 1) {
                // 유효하지 않은 방향 태그 대신 원본 픽셀 방향을 유지
                return 1;
            }

            // SHORT 한 개는 태그 값 영역의 선두 두 바이트에 직접 저장됨
            int orientation = getUnsignedShort(bytes, (int) entryOffset + 8, littleEndian);

            // EXIF 표준이 정의한 여덟 방향값만 픽셀 변환에 사용
            if (orientation >= 1 && orientation <= 8) {
                // 검증된 EXIF 이미지 방향값을 반환
                return orientation;
            }

            // 허용 범위 밖의 방향값은 원본 픽셀 방향으로 보정
            return 1;
        }

        // 이미지 디렉터리에 Orientation 태그가 없으면 원본 픽셀 방향을 유지
        return 1;
    }

    /**
     * 바이트 배열의 두 바이트를 부호 없는 정수로 변환
     *
     * @author SeungHyeon.Kang
    ...
    private int getUnsignedShort(byte[] bytes, int offset, boolean littleEndian) {
        // TIFF 바이트 순서에 따라 하위 바이트부터 조합
        if (littleEndian) {
            // 리틀 엔디언으로 조합한 부호 없는 정수값을 반환
            return (bytes[offset] & 0xFF) | ((bytes[offset + 1] & 0xFF) << 8);
        }

        // 네트워크 또는 Motorola 바이트 순서로 조합한 부호 없는 정수값을 반환
        return ((bytes[offset] & 0xFF) << 8) | (bytes[offset + 1] & 0xFF);
    }

    /**
     * 바이트 배열의 네 바이트를 부호 없는 정수로 변환
     *
     * @author SeungHyeon.Kang
     * @param bytes 변환할 바이트 배열
     * @param offset 값의 시작 위치
    ...
    private BufferedImage uptImageOrientation(BufferedImage sourceImage, int orientation) {
        // 방향 보정이 필요하지 않으면 추가 이미지 할당 없이 원본을 사용
        if (orientation == 1) {
            // 원본 픽셀 방향의 이미지를 반환
            return sourceImage;
        }

        // 픽셀 좌표 변환과 출력 크기 계산에 사용할 원본 너비를 조회
        int sourceWidth = sourceImage.getWidth();
        // 픽셀 좌표 변환과 출력 크기 계산에 사용할 원본 높이를 조회
        int sourceHeight = sourceImage.getHeight();
        AffineTransform transform;

        // EXIF 방향값에 대응하는 미러링과 회전 좌표 변환을 선택
        switch (orientation) {
            // 좌우가 반전된 픽셀을 수평으로 복원
            case 2:
                transform = new AffineTransform(-1, 0, 0, 1, sourceWidth, 0);
                break;
            // 상하좌우가 반전된 픽셀을 180도 회전
            case 3:
        ...
        // 90도 계열 회전은 출력 이미지의 가로와 세로 크기를 서로 교환
        if (orientation >= 5 && orientation <= 8) {
            targetWidth = sourceHeight;
            targetHeight = sourceWidth;
        }

        // 사용자 정의 이미지 형식은 알파 채널 여부에 맞는 표준 픽셀 형식으로 보정
        int targetType = sourceImage.getType();

        // 사용자 정의 색상 모델은 BufferedImage 생성자에서 직접 재사용할 수 없어 표준 형식으로 변환
        if (targetType == BufferedImage.TYPE_CUSTOM) {
            // 알파 채널이 있는 사용자 정의 이미지는 투명도를 유지하는 표준 형식으로 변환
            if (sourceImage.getColorModel().hasAlpha()) {
                targetType = BufferedImage.TYPE_INT_ARGB;
            }

            // 알파 채널이 없는 사용자 정의 이미지는 불투명 표준 형식으로 변환
            else {
                targetType = BufferedImage.TYPE_INT_RGB;
            }
        }

        // 방향이 반영된 픽셀을 담을 출력 이미지를 생성
        BufferedImage orientedImage = new BufferedImage(targetWidth, targetHeight, targetType);
        // 원본 픽셀을 좌표 변환하여 출력 이미지에 그릴 그래픽 객체를 생성
        Graphics2D graphics = orientedImage.createGraphics();

        // 그래픽 자원이 예외 상황에서도 해제되도록 이미지 변환을 격리
        try {
            // EXIF 방향에 대응하는 좌표 변환으로 원본 픽셀을 출력 이미지에 기록
            graphics.drawImage(sourceImage, transform, null);
        }

        // 이미지 변환이 끝나면 네이티브 그래픽 자원을 해제
        finally {
            // 이미지 변환에 사용한 그래픽 자원을 해제
            graphics.dispose();
        }

        // EXIF 표시 방향이 실제 픽셀에 반영된 이미지를 반환
        return orientedImage;
    }

    /**
     * DB 트랜잭션이 커밋된 뒤 교체되거나 영구 탈퇴로 제거된 물리 파일을 삭제하도록 정리 작업을 등록
     *
     * @author SeungHyeon.Kang
     * @param fileList 커밋 후 삭제할 파일 메타정보 목록
     */
    private void registerCommitCleanup(List<FileDto> fileList) {
        // 트랜잭션 밖에서 호출되면 이미 완료된 DB 처리에 맞춰 물리 파일을 즉시 정리
        if (!TransactionSynchronizationManager.isSynchronizationActive()) {
            // 트랜잭션이 없는 호출의 물리 파일을 즉시 삭제
            delPhysicalFileList(fileList);
            // 즉시 물리 파일 정리를 마침
            return;
        }

        // DB 최종 상태에 따라 물리 파일 삭제 여부를 결정할 동기화 작업을 등록
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            /**
             * DB 커밋이 완료된 경우에만 기존 물리 파일을 삭제
             *
             * @author SeungHyeon.Kang
             * @param status 트랜잭션 종료 상태
             * @return 반환값이 없음
            ...
            @Override
            public void afterCompletion(int status) {
                // 롤백된 DB가 기존 파일을 계속 참조할 수 있으므로 커밋 외 상태에서는 물리 파일을 유지
                if (status != TransactionSynchronization.STATUS_COMMITTED) {
                    // 기존 물리 파일을 유지하고 커밋 후 정리를 종료
                    return;
                }

                // DB에서 참조와 메타정보가 제거된 물리 파일을 삭제
                delPhysicalFileList(fileList);
            }
        });
    }
    ...
    /**
     * 내부 업로드 경로로 확인된 저장소 객체를 목록 단위로 삭제
     *
     * @author SeungHyeon.Kang
     * @param fileList 삭제할 파일 메타정보 목록
     */
    private void delPhysicalFileList(List<FileDto> fileList) {
        // 하나의 파일 삭제 실패가 나머지 영구 탈퇴 파일 정리를 중단하지 않도록 개별 처리
        for (FileDto fileDto : fileList) {
            // 외부 URL과 허용된 업로드 경로 밖의 값은 영구 객체 삭제 대상에서 제외
            String objectKey = getStoredObjectKey(fileDto);

            // 내부 업로드 객체 키가 아니면 메타정보 삭제만 유지
            if (StringUtil.isEmpty(objectKey)) {
                // 다음 파일 메타정보를 확인
                continue;
            }

            // DB에서 참조가 제거된 원본과 재생성 가능한 화면용 파생 객체를 함께 삭제
            delStoredFiles(getRelatedKeys(objectKey), fileDto.getFileNumb());
        }
    }

    /**
     * 검증된 저장소 객체 키들을 개별 오류로 격리하여 삭제
     *
     * @author SeungHyeon.Kang
    ...
    private void delStoredFiles(List<String> objectKeyList, Long fileNumb) {
        // 하나의 저장소 삭제 실패가 같은 이미지의 나머지 객체 정리를 막지 않도록 개별 처리
        for (String objectKey : objectKeyList) {
            try {
                // 검증된 원본 또는 화면용 파생 이미지 객체를 삭제
                fileStorage.delFile(objectKey);
            }
    ...
    private String getStoredObjectKey(FileDto fileDto) {
        ...
        if (storedPath.isAbsolute() || storedPath.getNameCount() != 3
            ...
            // 안전한 업로드 파일 경로가 아님을 반환
            return null;
        }

        // 운영체제 경로 구분자를 S3 객체 키 구분자로 통일해 검증된 상대 키를 반환
        return storedPath.toString().replace('\\', '/');
    }

    /**
     * DB 트랜잭션이 롤백될 때 이미 생성한 실제 이미지 파일도 함께 제거하도록 정리 작업을 등록
     *
     * @author SeungHyeon.Kang
    ...
    private void registerRollbackCleanup(List<String> objectKeyList) {
        // 요청값이 업무에서 허용한 범위와 상태를 만족하는지 구분
        if (!TransactionSynchronizationManager.isSynchronizationActive()) {
            // DB 트랜잭션이 롤백될 때 이미 생성한 실제 이미지 파일도 함께 제거하도록 정리 작업을 등록한 결과를 반환
            return;
        }

        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            /**
             * 트랜잭션 종료 상태에 맞춰 임시 파일을 정리
             *
             * @author SeungHyeon.Kang
             * @param status 트랜잭션 종료 상태
             * @return 반환값이 없음
            ...
            public void afterCompletion(int status) {
                ...
                if (status != TransactionSynchronization.STATUS_ROLLED_BACK) {
                    // 트랜잭션 종료 상태에 맞춰 임시 파일을 정리 결과를 반환
                    return;
                }

                // 롤백된 메타정보가 참조하지 않는 원본과 화면용 파생 객체를 함께 삭제
    ...
    /**
     * 프로필 저장 트랜잭션이 커밋된 경우에만 승격을 마친 임시 이미지 쌍을 삭제
     *
     * @author SeungHyeon.Kang
     * @param draftDirectory 사용자별 임시 디렉터리
    ...
    private void setDraftCleanupOnCommit(Path draftDirectory, String draftToken) {
        // 트랜잭션 밖에서 영구 저장이 완료되었다면 임시 파일을 즉시 삭제
        if (!TransactionSynchronizationManager.isSynchronizationActive()) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            @Override
            public void afterCompletion(int status) {
                // 롤백 시에는 사용자가 다시 저장할 수 있도록 만료 전 임시 이미지를 유지
                if (status != TransactionSynchronization.STATUS_COMMITTED) {
                    return;
                }

                // 최종 프로필에 반영된 임시 원본과 미리보기를 삭제
                delDraftTokenFiles(draftDirectory, draftToken);
            }
        });
    }
    ...
    /**
     * 특정 임시 이미지 식별값에 대응하는 원본과 미리보기 파일을 삭제
     *
     * @author SeungHyeon.Kang
     * @param draftDirectory 사용자별 임시 디렉터리
     * @param draftToken 삭제할 임시 이미지 식별값
    ...
    /**
     * 검증을 마친 이미지 바이트와 서버 저장 형식을 함께 보관
     *
