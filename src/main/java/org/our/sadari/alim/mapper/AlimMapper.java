 ...
 * date           : 2026-07-24
 * description    : 알림 데이터베이스 접근 메서드를 정의
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public interface AlimMapper {
    /**
     * 알림 상황과 템플릿 코드로 사용 가능한 템플릿 1건을 조회
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 템플릿 내용을 치환한 뒤 사용자 알림함에 실제 발송 알림을 저장
     *
     * @author SeungHyeon.Kang
     * @param req 저장할 알림 정보
     * @return 반영 건수
    ...
    /**
     * 같은 수신자, 상황, 템플릿, 제목, 내용과 이동 대상 정보를 가진 알림이 최근 1시간 안에 있는지 확인
     * 알림 폭주를 막기 위한 발송 직전 중복 방지 조건
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 로그인 사용자의 삭제되지 않은 알림 목록을 최신순으로 조회
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 인증 사용자의 알림번호로 원본 콘텐츠와 현재 접근 관계를 조회
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 사용자가 클릭한 알림 한 건을 읽음 처리
     * 사용자별 알림 번호이므로 인증 사용자 번호와 ALIM_NUMB를 함께 조건으로 사용
     *
     * @author SeungHyeon.Kang
     ...
     * 화면에 아직 로드하지 않은 알림까지 처리해야 하므로 사용자 번호만으로 갱신
     *
     * @author SeungHyeon.Kang
