 ...
 * author         : HanWon.Jang
 * date           : 2026-08-26
 * description    : 좋아요 트랜잭션과 알림 후처리 사이의 이벤트 경계를 제공한다
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class LikeAlimPublisher {
    ...
    /**
     * 신규 좋아요 알림을 현재 트랜잭션의 커밋 이후 처리 대상으로 등록한다.
     * 이벤트 등록 자체가 실패해도 핵심 좋아요 저장은 계속 확정되도록 예외를 격리한다.
     *
     * @author HanWon.Jang
     * @param event 좋아요 알림 수신자와 템플릿 정보
     */
    public void setLikeAlim(LikeAlimEvent event) {
        // 잘못된 이벤트는 좋아요 저장 흐름과 알림 시스템 모두에 전달하지 않는다
        if (event == null) {
            // 알림 처리 없이 호출을 종료한다
            return;
        }

        // 알림 기반 정보가 잘못되어도 좋아요 저장 경로에 예외가 전파되지 않도록 분리한다
        try {
            // 현재 좋아요 트랜잭션에 커밋 이후 실행할 알림 이벤트를 등록한다
            eventPublisher.publishEvent(event);
        }

        // 이벤트 등록 실패는 좋아요 성공 여부와 분리하고 운영 로그로만 확인한다
