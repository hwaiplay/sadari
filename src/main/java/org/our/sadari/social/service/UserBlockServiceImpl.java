 ...
 * author         : HanWon.Jang
 * date           : 2026-09-03
 * description    : 사용자 차단 상태와 양방향 격리 업무 로직을 구현함
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class UserBlockServiceImpl implements UserBlockService {
    ...
    public boolean isBlocked(Long userNumb, Long targetUserNumb) {
        ...
        if (StringUtil.hasEmpty(userNumb, targetUserNumb) || userNumb.equals(targetUserNumb)) {
            // 두 사용자 사이에 적용할 차단 관계가 없음을 반환함
            return false;
        }

        // 어느 한쪽이 만든 차단 관계라도 존재하면 양방향 격리 상태로 반환함
        return userBlockMapper.getBlockCnt(userNumb, targetUserNumb) > 0;
    }

    /**
     * 차단 관계 등록과 양방향 팔로우 삭제를 하나의 트랜잭션으로 처리함
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData setBlock(Long userNumb, Long targetUserNumb) {
        // 인증 정보가 없으면 차단 관계의 소유자를 확정할 수 없어 요청을 거부함
        if (StringUtil.isEmpty(userNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // 대상이 없거나 본인이면 유효한 차단 관계가 아니므로 요청을 거부함
        if (StringUtil.isEmpty(targetUserNumb) || userNumb.equals(targetUserNumb)) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 대상 원본이 물리 삭제되었으면 차단 주체나 상태를 구분하지 않는 공통 응답을 사용함
        ...
        // 로그인 사용자가 소유한 방향의 차단 관계를 멱등 등록함
        ...
        // 차단 사용자 사이의 기존 팔로우 관계를 양방향 모두 삭제함
        userBlockMapper.delBlockFollows(blockDto);
        // 현재 차단 상태가 확정되었음을 성공 응답으로 반환함
        return ResultData.success(true);
    }

    /**
     * 로그인 사용자가 소유한 한 방향의 차단 관계만 멱등 해제함
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData delBlock(Long userNumb, Long targetUserNumb) {
        ...
        // 대상이 없거나 본인이면 다른 관계를 잘못 삭제하지 않도록 요청을 거부함
        if (StringUtil.isEmpty(targetUserNumb) || userNumb.equals(targetUserNumb)) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 로그인 사용자가 생성한 한 방향의 차단 관계만 삭제함
        userBlockMapper.delBlock(createBlockDto(userNumb, targetUserNumb));
        // 이미 해제된 요청도 현재 상태와 일치하므로 성공으로 반환함
        return ResultData.success(true);
    }

    /**
     * 로그인 사용자가 직접 차단한 사용자만 최신 차단순 페이지로 조회함
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData getBlockList(Long userNumb, int page) {
        ...
        // 첫 페이지보다 작은 요청은 첫 페이지로 보정함
        int normalizedPage = Math.max(page, 1);
        // 차단 목록 페이지 조건을 담을 객체를 생성함
        UserBlockDto request = new UserBlockDto();
        // 로그인 사용자가 소유한 차단 방향만 조회하도록 사용자 번호를 설정함
    ...
    public void lockUsers(Long userNumb, Long targetUserNumb) {
        // 항상 같은 잠금 순서를 사용하도록 작은 사용자 번호를 먼저 결정함
        long firstUserNumb = Math.min(userNumb, targetUserNumb);
        // 큰 사용자 번호를 두 번째 잠금 대상으로 결정함
