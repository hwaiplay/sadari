 ...
 * author         : HanWon.Jang
 * date           : 2026-09-03
 * description    : 사용자 차단 상태와 양방향 격리 업무 로직을 구현
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class UserBlockServiceImpl implements UserBlockService {
    ...
    /**
     * 두 사용자 사이에 어느 방향이든 차단 관계가 있는지 조회
     *
     * @author HanWon.Jang
    ...
    public ResultData setBlock(Long userNumb, Long targetUserNumb) {
        ...
        // 차단 당사자 사이에서 직접 발송된 수락 전 모임 초대를 삭제
        userBlockMapper.delBlockInvitations(blockDto);
        // 어느 한쪽이 모임장인 모임의 상대방 처리 중 가입 신청과 답변을 삭제
        userBlockMapper.delBlockApplications(blockDto);
        // 차단 사용자 사이의 기존 팔로우 관계를 양방향 모두 삭제
        userBlockMapper.delBlockFollows(blockDto);
        // 현재 차단 상태가 확정되었음을 성공 응답으로 반환
        return ResultData.success(true);
    }

    /**
     * 로그인 사용자가 소유한 한 방향의 차단 관계만 멱등 해제
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData delBlock(Long userNumb, Long targetUserNumb) {
        // 인증 정보가 없으면 삭제할 차단 관계의 소유자를 확정할 수 없음
        if (StringUtil.isEmpty(userNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // 대상이 없거나 본인이면 다른 관계를 잘못 삭제하지 않도록 요청을 거부
        if (StringUtil.isEmpty(targetUserNumb) || userNumb.equals(targetUserNumb)) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 로그인 사용자가 생성한 한 방향의 차단 관계만 삭제
        userBlockMapper.delBlock(createBlockDto(userNumb, targetUserNumb));
        // 이미 해제된 요청도 현재 상태와 일치하므로 성공으로 반환
        return ResultData.success(true);
    }

    /**
     * 로그인 사용자가 직접 차단한 사용자만 최신 차단순 페이지로 조회
     *
     * @author HanWon.Jang
     * @param userNumb 로그인 사용자 번호
    ...
    public ResultData getBlockList(Long userNumb, int page) {
        // 인증 정보가 없으면 개인 차단 목록을 조회하지 않음
        if (StringUtil.isEmpty(userNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // 첫 페이지보다 작은 요청은 첫 페이지로 보정
        int normalizedPage = Math.max(page, 1);
        // 차단 목록 페이지 조건을 담을 객체를 생성
        UserBlockDto request = new UserBlockDto();
        // 로그인 사용자가 소유한 차단 방향만 조회하도록 사용자 번호를 설정
    ...
    private UserBlockDto createBlockDto(Long userNumb, Long targetUserNumb) {
        // 차단 관계를 전달할 요청 객체를 생성
        UserBlockDto blockDto = new UserBlockDto();
        // 차단 관계를 소유한 로그인 사용자 번호를 설정
