 ...
 * date           : 2026-07-22
 * description    : 사용자 검색과 팔로우 및 좋아요 업무 로직을 구현
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
public class SocialServiceImpl implements SocialService {
    ...
    /**
     * 로그인 사용자와 상대 사용자 사이의 팔로우 버튼명을 조회
     * 버튼명 판단은 소셜 Mapper가 팔로우 관계와 공통코드를 함께 조회하여 화면과 서버가 같은 기준을 사용하도록 처리
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData getFollowStatus(SocialDto.FollowDto req) {
        // validateFollowUsers 검증으로 잘못된 요청이 업무 로직에 진입하지 않도록 차단
        ResultData invalidResult = validateFollowUsers(req);

        // invalidResult 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (!StringUtil.isEmpty(invalidResult)) {
            // 로그인 사용자와 상대 사용자 사이의 팔로우 버튼명을 조회 과정에서 확인된 사용자 메시지
            return invalidResult;
        }

        // 로그인 사용자와 상대 사용자 사이의 팔로우 버튼명을 조회 결과를 성공 응답으로 반환
        return ResultData.success(createFollowStatus(socialMapper.getFollowStatusName(req)));
    }

    /**
     * 로그인 사용자가 상대 사용자를 팔로우하도록 TB_FOLLOW에 저장
     * 이미 팔로우 중인 경우에도 MERGE 쿼리를 사용하므로 중복 오류 없이 최신 버튼 상태만 반환
     *
     * @author SeungHyeon.Kang
     * @param req 로그인 사용자 번호와 상대 사용자 번호
    ...
    public ResultData setFollow(SocialDto.FollowDto req) {
        ...
        // Follow 업무 값을 socialMapper DTO에 설정
        int insertCnt = socialMapper.setFollow(req);

        // 새 팔로우 관계가 실제로 저장된 경우에만 팔로우 알림을 발송
        // 이미 팔로우 중이라 MERGE가 아무 것도 저장하지 않은 경우에는 같은 알림을 다시 만들 필요가 없음
        if (insertCnt > 0) {
            // sendFollowAlim 호출로 검증된 알림 또는 응답을 전송
            sendFollowAlim(req);
        }

        // 로그인 사용자가 상대 사용자를 팔로우하도록 TB_FOLLOW에 저장 결과를 성공 응답으로 반환
        return ResultData.success(createFollowStatus(socialMapper.getFollowStatusName(req)));
    }

    /**
     * 로그인 사용자가 상대 사용자를 팔로우 중인 관계를 삭제
    ...
    public ResultData delFollow(SocialDto.FollowDto req) {
        ...
        // 로그인 사용자가 상대 사용자를 팔로우 중인 관계를 삭제 결과를 성공 응답으로 반환
        return ResultData.success(createFollowStatus(socialMapper.getFollowStatusName(req)));
    }

    /**
     * 대상 유형과 대상 번호 기준으로 좋아요를 등록하거나 취소
     * 현재 화면에서 지원하는 공개 독후감과 현재 프로필·배경사진만 허용하고,
     * 다른 TAGT_TYPE은 저장하지 않고 잘못된 요청으로 응답
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData setLike(SocialDto.LikeDto req) {
        // validateLikeTarget 검증으로 잘못된 요청이 업무 로직에 진입하지 않도록 차단
        ...
        // 앞선 조건에 해당하지 않는 대체 업무 흐름으로 전환
            ...
            // Like 업무 값을 socialMapper DTO에 설정
            if (socialMapper.setLike(req) == 0) {
                // "접근할 수 없는 요청이에요."
                return ResultData.fail(ResultEnum.COMMON_ACCESS_REJECTED);
            }
            // 좋아요 커밋 이후에만 알림을 처리하도록 후처리 이벤트를 등록
            sendLikeAlim(req);
        }

        // 대상 유형과 대상 번호 기준으로 좋아요를 등록하거나 취소 결과를 성공 응답으로 반환
        return ResultData.success(socialMapper.getLikeDtl(req));
    }

    /**
     * 마이페이지 프로필 상단 통계 값을 조회
     * 팔로우/팔로워/좋아요 집계는 social 도메인의 책임이고, 총 읽은 책도 같은 화면 통계 묶음으로 제공되어야 하므로
     * MyPageController가 여러 mapper를 직접 호출하지 않도록 social service에서 한 번에 조회
     *
     * @author SeungHyeon.Kang
    ...
    public ResultData getMyPageProfileStats(Long userNumb) {
        // 본인 화면은 공개 여부 조건 없이 전체 독후감을 포함한 통계를 반환
        return getProfileStatsResult(userNumb, userNumb, null);
    }

    /**
     * 사용자 프로필 통계 값을 조회
     * 마이페이지와 다른 사람 프로필은 같은 SQL을 사용하고 공개 여부 조건으로 독후감 집계 범위만 구분
     *
     * @author SeungHyeon.Kang
    ...
    private ResultData getProfileStatsResult(Long loginUserNumb, Long userNumb, String pubcYsno) {
        // validateTargetUser 검증으로 잘못된 요청이 업무 로직에 진입하지 않도록 차단
    ...
    public ResultData getFollowingList(Long loginUserNumb, Long userNumb, String keyword, int page) {
        // validateFollowListReq 검증으로 잘못된 요청이 업무 로직에 진입하지 않도록 차단
        ...
        // 페이지 조건으로 특정 사용자가 팔로우하는 사용자 목록을 조회
        List<SocialDto.FollowUserDto> searchedList = socialMapper.getFollowingList(req);
        // 팔로잉 목록의 현재 페이지와 다음 페이지 여부를 반환
        return getFollowPage(searchedList, Math.max(page, 1));
    }

    /**
     * 특정 사용자를 팔로우하는 사용자 목록을 조회
     * 팔로워 목록도 팔로잉 목록과 같은 응답 구조를 사용해 화면 모달을 공통으로 렌더링할 수 있도록 처리
     *
     * @author SeungHyeon.Kang
     * @param loginUserNumb 로그인 사용자 번호
     * @param userNumb 목록 주인 사용자 번호
    ...
    public ResultData getFollowerList(Long loginUserNumb, Long userNumb, String keyword, int page) {
        ...
        // 목록 범위를 유지한 닉네임 검색과 페이지 조건 구성
        SocialDto.FollowListReqDto req = createFollowListReq(loginUserNumb, userNumb, keyword, page);
        // 페이지 조건으로 특정 사용자를 팔로우하는 사용자 목록을 조회
        List<SocialDto.FollowUserDto> searchedList = socialMapper.getFollowerList(req);
        // 팔로워 목록의 현재 페이지와 다음 페이지 여부를 반환
        return getFollowPage(searchedList, Math.max(page, 1));
    }

    /**
     * 접근 가능한 대상에 좋아요를 등록한 활성 사용자 목록을 조회
     * 비활성 또는 삭제 대기 사용자의 좋아요 행은 보존하더라도 목록과 집계에서 제외
     *
     * @author SeungHyeon.Kang
     * @param loginUserNumb 로그인 사용자 번호
    ...
    public ResultData getLikeUserList(Long loginUserNumb, String tagtType, Long tagtNumb, int page) {
        // 인증 사용자 번호가 없으면 대상 정보 조회 전에 인증 실패로 응답
        if (StringUtil.isEmpty(loginUserNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }
        ...
        // 허용 목록 비교를 위해 대상 유형의 공백과 대소문자를 정규화
        String normalizedType = tagtType.trim().toUpperCase();
        // 현재 서비스가 좋아요를 제공하는 네 가지 대상 유형만 목록 조회를 허용
        if (!isAllowedLikeListType(normalizedType) || tagtNumb <= 0) {
            // "요청값이 올바르지 않아요."
            return ResultData.fail(ResultEnum.COMMON_INVALID_REQUEST);
        }

        // 좋아요 목록 접근 검증과 페이지 조건을 같은 요청 객체에 설정
    ...
    private ResultData validateFollowUsers(SocialDto.FollowDto req) {
        ...
        // req.getFlowNumb( 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        ...
        // targetUser 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (StringUtil.isEmpty(targetUser)) {
            // "조회 결과가 없어요."
            return ResultData.fail(ResultEnum.COMMON_NO_DATA);
        }

        // 어느 한쪽이라도 상대를 차단했으면 팔로우 상태와 변경을 모두 거절
    ...
    /**
     * 프로필 통계 또는 목록 조회 대상 사용자가 실제 존재하는지 검증
     * 존재하지 않는 사용자 번호로 집계를 수행하면 빈 통계가 정상 데이터처럼 보일 수 있으므로 조회 전 차단
     *
     * @author SeungHyeon.Kang
    ...
    /**
     * 팔로우/팔로워 목록 조회에 필요한 로그인 사용자와 목록 주인 사용자를 검증
     * 목록의 오른쪽 버튼명은 로그인 사용자 기준으로 계산되므로 로그인 사용자가 없으면 인증 실패로 응답
     *
     * @author SeungHyeon.Kang
     * @param loginUserNumb 로그인 사용자 번호
     * @param userNumb 목록 주인 사용자 번호
    ...
    private ResultData validateFollowListReq(Long loginUserNumb, Long userNumb) {
        // loginUserNumb 값이 비어 있을 때 후속 참조를 차단하기 위한 분기
        if (StringUtil.isEmpty(loginUserNumb)) {
            // "인증에 실패했어요.\n다시 로그인 해주세요."
            return ResultData.fail(ResultEnum.AUTH_FAIL);
        }

        // 팔로우 목록 주인 사용자가 실제 존재하는지 먼저 검증
        ResultData invalidResult = validateTargetUser(userNumb);
        // 대상 사용자가 없으면 차단 여부를 확인하지 않고 기존 실패 응답을 유지
        if (!StringUtil.isEmpty(invalidResult)) {
            // 대상 사용자 검증에서 확정된 실패 응답을 반환
            return invalidResult;
        }

        // 본인 목록이 아닌 경우 목록 주인과 조회자 사이의 차단 관계를 우선 적용
        if (!loginUserNumb.equals(userNumb) && userBlockService.isBlocked(loginUserNumb, userNumb)) {
            // "접근할 수 없는 요청이에요."
            return ResultData.fail(ResultEnum.COMMON_ACCESS_REJECTED);
        }

        // 팔로우 목록을 조회할 수 있는 상태임을 반환
        return null;
    }

    /**
     * 팔로우/팔로워 목록 조회 DTO를 생성
     * Controller와 Mapper가 같은 파라미터 구조를 공유하도록 Service에서 DTO 생성 지점을 고정
     *
     * @author SeungHyeon.Kang
     * @param loginUserNumb 로그인 사용자 번호
     * @param userNumb 목록 주인 사용자 번호
    ...
    private SocialDto.FollowListReqDto createFollowListReq(Long loginUserNumb, Long userNumb
        ...
        // 요청 페이지를 첫 페이지 이상으로 보정
        int normalizedPage = Math.max(page, 1);
        // 현재 팔로우 목록 페이지의 시작 위치를 설정
        req.setPageOffset((normalizedPage - 1) * FOLLOW_PAGE_SIZE);
        // 다음 페이지 판정용 한 건을 추가한 조회 수를 설정
        req.setPageLimit(FOLLOW_PAGE_SIZE + 1);
        // 팔로우/팔로워 목록 조회 DTO를 생성 결과를 반환
        return req;
    }

    /**
     * 활성 사용자 닉네임 검색과 관계 정렬에 사용할 페이지 조건을 생성
     *
     * @author HanWon.Jang
     * @param loginUserNumb 로그인 사용자 번호
    ...
    private SocialDto.UserSearchReqDto createUserSearchReq(Long loginUserNumb, String keyword, int page) {
        // 활성 사용자 검색 조건을 담을 객체를 생성
        SocialDto.UserSearchReqDto req = new SocialDto.UserSearchReqDto();
        // 관계 정렬 기준이 되는 로그인 사용자 번호를 설정
        req.setLoginUserNumb(loginUserNumb);
        // 닉네임 포함 검색에 사용할 정규화된 검색어를 설정
        req.setKeyword(keyword);
        // 요청 페이지를 첫 페이지 이상으로 보정
        int normalizedPage = Math.max(page, 1);
        // 현재 검색 페이지의 시작 위치를 설정
        req.setPageOffset((normalizedPage - 1) * FOLLOW_PAGE_SIZE);
        // 다음 페이지 판정용 한 건을 추가한 조회 수를 설정
        req.setPageLimit(FOLLOW_PAGE_SIZE + 1);
        // 완성된 활성 사용자 검색 조건을 반환
        return req;
    }

    /**
     * 팔로우 조회 결과를 현재 페이지 크기로 제한하고 다음 페이지 여부를 구성
     *
     * @author SeungHyeon.Kang
    ...
    private ResultData validateLikeTarget(SocialDto.LikeDto req) {
        ...
        // 작성자와 알림 설정은 클라이언트 값을 신뢰하지 않고 현재 대상에서 조회
    ...
    /**
     * 독후감 또는 현재 사진 좋아요가 새로 등록된 경우 커밋 이후 알림 이벤트를 등록
    ...
    private void sendLikeAlim(SocialDto.LikeDto req) {
        // 독후감 작성자가 좋아요 알림을 껐으면 알림 저장과 푸시 예약을 모두 생략
        if (!Constant.COMM_YES.equals(req.getLikeAlimYsno())) {
            // 독후감 좋아요 알림 처리 없이 호출부로 반환
            return;
        }

        // 본인이 작성한 독후감에 본인이 좋아요를 누른 경우에는 자기 자신에게 알림을 만들 필요가 없어 중단
        if (req.getTargetUserNumb().equals(req.getUserNumb())) {
            // 독후감 좋아요가 새로 등록된 경우 독후감 작성자에게 좋아요 알림을 발송 결과를 반환
            return;
        }

        // 좋아요 응답 경로에서 Redis와 알림 DB 및 FCM 접근을 제거할 커밋 이후 이벤트를 생성
        ...
        // 좋아요 트랜잭션이 커밋된 경우에만 비동기 알림 작업이 시작되도록 이벤트를 등록
        likeAlimPublisher.setLikeAlim(event);
    }

    /** 좋아요 대상 유형에 대응하는 알림 템플릿 코드를 반환 */
     ...
     * ResultData.data의 필드명을 고정해 화면에서 응답 구조를 안정적으로 사용할 수 있도록 처리
     *
     * @author SeungHyeon.Kang
    ...
    private SocialDto.FollowDto createFollowStatus(String followStatName) {
        ...
        // FollowStatName 업무 값을 followDto DTO에 설정
        followDto.setFollowStatName(followStatName);
        // MySQL 함수에서 받은 버튼명을 프론트엔드 응답 DTO로 감싼 결과를 반환
        return followDto;
    }

    /**
     * 팔로우를 받은 사용자에게 새 팔로워 알림을 발송
     * 팔로우 INSERT가 실제로 발생한 경우에만 호출되며, sendAlim 공통 로직에서 1시간 내 동일 알림을 한 번 더 차단
     *
     * @author SeungHyeon.Kang
     * @param req 팔로우를 수행한 사용자와 팔로우 대상 사용자 번호
     */
    private void sendFollowAlim(SocialDto.FollowDto req) {
        // 본인을 팔로우하는 요청은 검증에서 차단되지만, 알림 발송 직전에도 한 번 더 방어
        if (req.getUserNumb().equals(req.getFlowNumb())) {
            // 팔로우를 받은 사용자에게 새 팔로워 알림을 발송 결과를 반환
            return;
        }

        // getUserNick 업무 로직을 tokenRedisService에 위임
        String sendUserNick = tokenRedisService.getUserNick(req.getUserNumb());

        // 로그인 Redis 정보가 없으면 사용자 테이블을 다시 조회하지 않고 부가 기능인 알림만 생략
