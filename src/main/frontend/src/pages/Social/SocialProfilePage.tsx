...
/**
 * 다른 사용자 프로필의 현재 사진 반응에 서버 또는 낙관적 좋아요 상태를 병합한다.
 *
...
const mergeImageReaction = (
  ...
  // 사진이 교체되었거나 프로필 조회 전이면 이전 사진 반응을 현재 화면에 반영하지 않는다
  if (!current || !currentReaction || currentReaction.tagtNumb !== reaction.tagtNumb) {
    // 기존 소셜 프로필 상태를 그대로 반환한다
    return current;
  }

  // 지정한 현재 사진의 좋아요 수와 로그인 사용자 상태만 병합해 반환한다
  return {
    ...current,
    [reactionKey]: {
      ...currentReaction,
...
const SocialProfilePage = () => {
  ...
  // 알림이 지정한 현재 사진과 댓글 위치를 프로필 화면에서 해석한다
  ...
  // 알림이 요청한 사진 유형에 대응하는 현재 소셜 프로필 반응을 선택한다
  const requestedReaction = requestedTagtType === "PROFILE_IMAGE"
    ...
    : requestedTagtType === "BACKGROUND_IMAGE"
      ...
      : null;
  // 알림의 파일 번호가 현재 사진과 일치할 때만 자동 열기 대상으로 인정한다
  ...
  useEffect(() => {
    // 일반 프로필 진입 또는 사진 좋아요 알림은 전체 화면 사진만 열고 댓글 시트는 열지 않는다
    ...
    // 현재 사진 댓글 시트를 열고 알림이 지정한 댓글을 강조하도록 대상을 설정한다
  ...
  /**
   * 다른 사용자의 현재 프로필 또는 배경사진 좋아요를 낙관적으로 변경한다.
   *
   * @author SeungHyeon.Kang
  ...
  const handleImageLike = async (reaction: ImageReaction): Promise<void> => {
    // 같은 사진의 좋아요 요청이 진행 중이면 첫 요청이 끝날 때까지 중복 토글을 막는다
    if (imageLikeUpdatingType === reaction.tagtType) {
      // 진행 중인 첫 좋아요 요청 결과를 유지한다
      return;
    }
    ...
    // 좋아요 저장 실패 시 클릭 전 상태를 복원하고 공통 오류를 표시한다
      ...
      // 성공과 실패 모두에서 현재 사진 좋아요 버튼을 다시 활성화한다
      setImageLikeUpdatingType(null);
    }
  };

  /**
   * 다른 사용자 사진 댓글 바텀시트를 닫고 최신 사진 반응 집계를 다시 조회한다.
   *
   * @author SeungHyeon.Kang
  ...
  const handleImageReplyClose = async (): Promise<void> => {
    // 댓글 바텀시트를 먼저 닫아 소셜 프로필 조작을 복구한다
    setReplyTarget(null);

    // 댓글 등록과 삭제 결과가 반영된 현재 사진 집계를 다시 조회한다
    try {
      // 다른 사용자의 최신 공개 프로필과 사진 반응을 조회한다
      const nextProfile = (await getSocialProfileApi(targetUserNumb)).data as UserProfile;
      // 소셜 프로필의 사진 반응과 기본 정보를 최신 서버 응답으로 교체한다
      setProfile(nextProfile);
    }

    // 댓글 창은 닫은 상태로 유지하고 프로필 재조회 실패만 안내한다
    catch (error) {
      // "조회에 실패했습니다."
      await sweetError(
  ...
  /**
   * 소셜 프로필의 전체 화면 사진에 표시할 좋아요와 댓글 버튼을 구성한다.
   *
   * @author SeungHyeon.Kang
