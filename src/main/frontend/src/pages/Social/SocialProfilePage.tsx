...
/**
 * 다른 사용자 프로필의 현재 사진 반응에 서버 또는 낙관적 좋아요 상태를 병합
 *
...
const mergeImageReaction = (
  ...
  if (!current || !currentReaction || currentReaction.tagtNumb !== reaction.tagtNumb) {
    // 기존 소셜 프로필 상태를 그대로 반환
    return current;
  }

  // 지정한 현재 사진의 좋아요 수와 로그인 사용자 상태만 병합해 반환
  return {
    ...current,
    [reactionKey]: {
      ...currentReaction,
...
/**
 * 다른 사용자의 프로필과 독서 활동 현황을 보여주는 읽기 전용 페이지
 * 공개 독후감 목록에서 작성자 프로필을 눌렀을 때 진입하며, 마이페이지와 같은 활동 요약 데이터를 표시
 *
 * @author SeungHyeon.Kang
...
const SocialProfilePage = () => {
  ...
  // 알림이 지정한 현재 사진과 댓글 위치를 프로필 화면에서 해석
  ...
  // 조회된 다른 사용자의 닉네임을 공통 헤더 제목으로 표시
  ...
  useEffect(() => {
    ...
    getSocialProfileApi(targetUserNumb)
      .then(async (profileResponse) => {
        ...
        // 정상 회원에게만 독서 활동과 팔로우 관계를 병렬로 조회
      ...
      .catch(async (error: unknown) => {
        ...
        if (!ignore && error instanceof ResultDataError
          ...
          // 차단된 사용자의 프로필과 활동 상태를 표시하지 않고 종료
          return;
        }
  ...
  // 알림이 요청한 사진 유형에 대응하는 현재 소셜 프로필 반응을 선택
  const requestedReaction = requestedTagtType === "PROFILE_IMAGE"
    ...
    : requestedTagtType === "BACKGROUND_IMAGE"
      ...
      : null;
  // 알림의 파일 번호가 현재 사진과 일치할 때만 자동 열기 대상으로 인정
  ...
  const handleProfileImageLoad = (event: SyntheticEvent<HTMLImageElement>) => {
    ...
    // 현재 프로필 응답에서 기대한 이미지 경로를 브라우저 절대 경로로 변환
    ...
    // 다른 사용자 또는 이전 경로의 로드 결과와 섞이지 않도록 대상과 원본 경로를 함께 기록
  ...
  /**
   * 다른 사용자의 현재 프로필 또는 배경사진 좋아요를 낙관적으로 변경
   *
   * @author SeungHyeon.Kang
  ...
  const handleImageLike = async (reaction: ImageReaction): Promise<void> => {
    ...
    // 좋아요 저장 실패 시 클릭 전 상태를 복원하고 공통 오류를 표시
      ...
      // 성공과 실패 모두에서 현재 사진 좋아요 버튼을 다시 활성화
      setImageLikeUpdatingType(null);
    }
  };

  /**
   * 다른 사용자 사진 댓글 바텀시트를 닫고 최신 사진 반응 집계를 다시 조회
   *
   * @author SeungHyeon.Kang
  ...
  const handleImageReplyClose = async (): Promise<void> => {
    // 댓글 바텀시트를 먼저 닫아 소셜 프로필 조작을 복구
    setReplyTarget(null);

    // 댓글 등록과 삭제 결과가 반영된 현재 사진 집계를 다시 조회
    try {
      // 다른 사용자의 최신 공개 프로필과 사진 반응을 조회
      const nextProfile = (await getSocialProfileApi(targetUserNumb)).data as UserProfile;
      // 소셜 프로필의 사진 반응과 기본 정보를 최신 서버 응답으로 교체
      setProfile(nextProfile);
    }

    // 댓글 창은 닫은 상태로 유지하고 프로필 재조회 실패만 안내
    catch (error) {
      // "조회에 실패했습니다."
      await sweetError(
  ...
  /**
   * 소셜 프로필의 전체 화면 사진에 표시할 좋아요와 댓글 버튼을 구성
   *
   * @author SeungHyeon.Kang
  ...
  /**
   * 프로필 팔로우 버튼의 현재 관계에 맞춰 팔로우 또는 언팔로우 API를 호출
   * 팔로잉과 친구는 내가 상대를 팔로우 중인 상태이므로 삭제하고, 그 외에는 팔로우 관계를 저장
   *
   * @author HanWon.Jang
  ...
  const handleFollowButtonClick = async () => {
    ...
    // 중복 클릭 중에는 현재 팔로우 요청이 끝날 때까지 추가 조작을 차단
    if (isFollowUpdating) {
      // 진행 중인 팔로우 요청을 유지하고 추가 요청 없이 종료
      return;
    }

    // 현재 버튼명으로 로그인 사용자가 만든 팔로우 관계의 존재 여부를 판정
    ...
    // 팔로우 관계 변경이 끝날 때까지 버튼을 비활성화
    setIsFollowUpdating(true);

    // 현재 관계에 맞는 팔로우 등록 또는 삭제 요청을 실행
    try {
      const response =
        isFollowing
  ...
  const handleFollowListOpen = async (type: FollowListType) => {
    // 새 모달을 열 때 스크롤 상태를 초기화
    setIsFollowListScrolling(false);
    // 다른 사용자의 팔로우 목록 모달을 열고 첫 서버 페이지를 조회
    await openFollowList(type);
  };

  /**
   * handle Follow List Close 사용자 동작을 처리
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleFollowListClose = () => {
    // 공통 팔로우 목록과 페이지 상태를 초기화
    closeFollowList();
    setIsFollowListScrolling(false);
  };
  ...
  const handleFollowListUserClick = (user: FollowUser): void => {
    ...
    // 로그인 사용자 행을 선택하면 타인 프로필 API를 거치지 않고 마이페이지로 이동
    if (user.meYsno === "Y") {
      // 로그인 사용자의 마이페이지로 이동
  ...
  /**
   * 팔로우 목록 사용자의 현재 관계에 맞춰 팔로우 또는 언팔로우 API를 호출
   *
   * @author HanWon.Jang
  ...
  /**
   * 다른 사용자의 요약 독후감 항목을 선택했을 때 공개 여부에 따라 이동 또는 경고를 처리
   * 공개 독후감은 ISBN 기준 공개 독후감 목록으로 이동하고, 비공개 독후감은 사용자가 내용을 볼 수 없음을 안내
   *
   * @author HanWon.Jang
  ...
  /**
   * 다른 사용자가 현재 읽고 있는 책의 목표 종료일까지 남은 기간 정보를 렌더링
                  ...
                  <span className={styles.currentReadingText}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        // 다른 사용자 도서는 특정 독후감의 공개 여부와 무관하게 ISBN 기반 도서 정보로 이동
                        navigate(
  ...
  /**
   * 기간별 독서 활동 행과 펼침 목록을 공통 구조로 렌더링
                  ...
                  <span className={styles.readingSummaryBookText}>
                    <span
                      ...
                      onClick={(event) => {
                        ...
                        // 공개 여부와 관계없이 제목에서는 ISBN 기준 도서 정보 화면으로 이동
                        navigate(
                          `/report/public-reports/isbn?isbn=${encodeURIComponent(report.bookIsbn)}`,
                          {
                            state: {
  ...
  // 한줄소개 앞뒤 공백을 제거하여 실제 신고 선택지 표시 여부를 판단
  const profileIntroduction = profile.intrCntn?.trim();
  // 사용자 계정 신고는 세부 신고 대상과 겹치지 않도록 현재 닉네임만 표시
  const userProfileContent = targetUserNick;
  // 공통 기본 이미지와 실제 원본 프로필 사진의 경로를 같은 기준으로 비교
  const profileImageSource = normalizeProfileImageSource(profile.porfPath);
  // 현재 대상의 비기본 원본 이미지가 정상 로드된 경우에만 프로필 사진 신고를 허용
  ...
  // 다른 활성 사용자의 프로필에서 신고할 수 있는 현재 사용자 계정 대상을 구성
  ...
  // 현재 화면에 실제로 표시된 이미지와 한줄소개만 세부 신고 선택지에 포함
  ...
  // 실제 프로필 사진이 있는 사용자만 접수 시점 이미지 증거를 신고할 수 있도록 처리
  ...
  // 실제 배경사진이 있는 사용자만 접수 시점 이미지 증거를 신고할 수 있도록 처리
  if (profile.bgimPath) {
    // "배경사진 신고"
