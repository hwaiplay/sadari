...
/**
 * 종료 회차의 공동 독서 기간을 목록 표시 형식으로 변환
 *
 * @author HanWon.Jang
...
const formatReadingPeriod = (goalStdt: string, goalEndt: string): string => {
  ...
  // 같은 연도의 회차는 피그마 표시처럼 종료 연도를 생략해 반환
...
const ReadingHistoryCard = ({
  ...
  // 비정상 집계에서도 진행 막대에 사용할 유효한 달성률을 유지
  ...
  /**
   * 현재 카드의 완료 회차를 목표 결과 조회 대상으로 선택
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleSelect = (): void => {
    // 카드가 표현하는 회차와 결과 상세 접근 권한을 페이지 선택 상태에 전달
    onSelect(history);
  };

  // 도서 정보와 공통 달성률 색상 정책을 적용한 회차 카드를 반환
  return (
...
/**
 * 활성 모임원과 공개 중인 활성 모임 조회자에게 모든 종료 회차를 표시
 *
 * @author HanWon.Jang
 * @return 이전 독서 기록 목록 페이지
 */
const ClubReadingHistoryPage = () => {
  // 페이지의 경로 검증, 종료 회차 조회와 결과 오버레이 상태를 전용 훅에서 조회
  const {
    historyList,
    historyQuery,
    isValidRoute,
  ...
  /**
   * 이전 독서 기록 한 건을 선택 가능한 회차 카드로 표시
   *
   * @author HanWon.Jang
  ...
  // 모임 번호가 유효하지 않으면 서버 요청 없이 잘못된 접근을 안내
  if (!isValidRoute) {
    // 잘못된 이전 독서 기록 경로 안내를 반환
    return <p className={styles.invalidAccess}>{message("frontend.common.invalidAccess")}</p>;
  }

  // 첫 페이지를 조회하는 동안 로딩 화면을 표시
  if (historyQuery.isPending) {
    // 이전 독서 기록 카드 크기의 로딩 화면을 반환
    return (
  ...
  // 접근 거절 또는 목록 조회 실패 시 서버 오류 문구를 표시
  if (historyQuery.isError) {
    // 이전 독서 기록 조회 실패 안내를 반환
    return (
  ...
  // 조회 가능한 종료 회차가 없으면 빈 목록 안내를 표시
  if (historyList.length === 0) {
    // 이전 독서 기록 빈 상태 안내를 반환
    return (
      <p className={styles.stateMessage}>
  ...
  // 카드 목록과 뷰포트 기준 결과 오버레이를 포함한 이전 독서 기록 화면을 반환
  return (
