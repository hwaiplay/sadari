...
export default function ClubDetailPage() {
  ...
  return (
    <div className={styles.page}>
      ...
      {club.membRole === "OWNER" && <>
        <section className={styles.panel}><h2 className={styles.sectionTitle}>맞팔로워 초대</h2>{candidates.length ? <>{candidates.map((candidate) => <label className={styles.profileRow} key={candidate.userNumb}><img className={styles.avatar} src={candidate.porfPath || "/img/common/empty-profile.svg"} alt="" /><span><strong className={styles.profileName}>{candidate.userNick}</strong><span className={styles.profileIntro}>{candidate.intrCntn}</span></span><input type="checkbox" checked={selectedCandidates.has(candidate.userNumb)} onChange={() => setSelectedCandidates((current) => { const next = new Set(current); if (next.has(candidate.userNumb)) next.delete(candidate.userNumb); else next.add(candidate.userNumb); return next; })} /></label>)}<button className={styles.button} type="button" disabled={!selectedCandidates.size} onClick={() => void inviteCandidates().catch((error) => void sweetError("초대하지 못했어요", getApiErrorMessage(error, "정원을 확인해 주세요.")))}>선택한 사용자 초대</button></> : <p className={styles.empty}>초대할 수 있는 맞팔로워가 없어요.</p>}</section>
        {club.joinType === "APPROVAL" && <section className={styles.panel}><h2 className={styles.sectionTitle}>가입 승인 대기 ({applications.length})</h2>{applications.length ? applications.map((application) => <article className={styles.application} key={application.applNumb}><div className={styles.profileRow}><img className={styles.avatar} src={application.porfPath || "/img/common/empty-profile.svg"} alt="" /><strong className={styles.profileName}>{application.userNick}</strong><span /></div>{application.questionList.map((question, index) => <div className={styles.qa} key={question}><strong>Q. {question}</strong><span>A. {application.answerList[index]}</span></div>)}<div className={styles.actions}><button className={styles.button} type="button" onClick={() => void decideApplication(application.applNumb, "APPROVED")}>승인</button><button className={styles.buttonDanger} type="button" onClick={() => void decideApplication(application.applNumb, "REJECTED")}>거절</button></div></article>) : <p className={styles.empty}>대기 중인 가입 신청이 없어요.</p>}</section>}
      </>}
    </div>
  );
}
