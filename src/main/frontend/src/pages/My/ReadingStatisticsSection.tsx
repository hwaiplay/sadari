...
/**
 * 조회 가능한 독서 연도를 홈 화면 SelectBox가 사용하는 문자열 옵션으로 변환
 *
...
const getYearSelectOptions = (availableYears: number[]): Array<CustomSelectOption<string>> => {
  ...
  // 서버가 제공한 최근 연도 순서를 유지하며 SelectBox 옵션을 구성
  for (const readYear of availableYears) {
    // 연도 값을 화면 라벨과 선택값에 같은 문자열로 추가
    yearOptions.push({ value: String(readYear), label: String(readYear) });
  }

  // 홈 화면 SelectBox에 전달할 연도 옵션 목록을 반환
  return yearOptions;
};

/**
 * 독서 상태를 완독, 읽는 중, 중단 순서로 정렬
 *
 * @author SeungHyeon.Kang
...
const compareStatusDisplayOrder = (leftStatus: ReadingStatusCount, rightStatus: ReadingStatusCount): number => {
  // 완독이 읽는 중보다 먼저 표시되는 고정 순서 차이를 반환
...
function HorizontalScrollArea({ children, hint }: HorizontalScrollAreaProps) {
  ...
  // 실제 콘텐츠 폭과 표시 폭을 비교할 스크롤 요소 참조를 생성
  const scrollRef = useRef<HTMLDivElement | null>(null);
  // 한 번 스크롤한 뒤 안내가 다시 나타나지 않도록 감지 상태 참조를 생성
  const isDismissedRef = useRef(false);
  // 최초 렌더링에서 가장 최근 데이터가 있는 우측 끝으로 한 번만 이동할 상태 참조를 생성
  const isPositionedRef = useRef(false);
  // 초기 우측 이동과 사용자의 실제 스크롤 조작을 구분할 상태 참조를 생성
  const hasUserIntentRef = useRef(false);
  // 사용자가 가로 스크롤을 멈춘 뒤 표시를 숨길 타이머 참조를 생성
  const scrollbarTimerRef = useRef<number | null>(null);
  // 가로 오버플로가 있을 때만 안내를 노출할 화면 상태를 생성
  const [isHintVisible, setIsHintVisible] = useState(false);
  // 첫 스크롤 뒤 안내 문구의 지연 페이드아웃을 적용할 화면 상태를 생성
  const [isHintDismissed, setIsHintDismissed] = useState(false);
  // 초기에는 숨긴 스크롤바를 실제 사용자 스크롤 뒤 표시할 화면 상태를 생성
  const [isScrollbarVisible, setIsScrollbarVisible] = useState(false);

  /**
   * 현재 화면 너비에서 콘텐츠가 가로로 넘치는지 측정해 안내 표시 여부를 결정
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  ...
  const hideScrollbar = useCallback((): void => {
    // 스크롤 조작이 멈춘 뒤 스크롤바가 천천히 투명해지도록 표시 상태를 해제
  ...
  /**
   * 잔디 영역이 해제될 때 남아 있는 스크롤바 타이머를 정리
   *
   * @author SeungHyeon.Kang
   * @return 스크롤바 숨김 타이머 정리 함수
   */
  const prepareScrollCleanup = useCallback((): (() => void) => {
    // Effect 정리 단계에서 같은 타이머 해제 함수를 실행하도록 반환
    return clearScrollbarTimer;
  }, [clearScrollbarTimer]);

  // 화면 이탈 후 스크롤바 상태를 변경하지 않도록 숨김 타이머를 정리
  useEffect(prepareScrollCleanup, [prepareScrollCleanup]);

  /**
   * 포인터와 휠 및 키보드 입력을 사용자의 실제 스크롤 의도로 기록
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleScrollIntent = (): void => {
    // 초기 우측 위치 설정과 구분할 수 있도록 사용자 조작 상태를 기록
    hasUserIntentRef.current = true;
  };

  /**
   * 사용자가 가로 스크롤을 시작하면 안내를 지연 페이드아웃하고 스크롤바를 표시
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleHorizontalScroll = (): void => {
    ...
    // 최초 사용자 스크롤에서만 안내 문구의 페이드아웃을 시작
    if (!isDismissedRef.current) {
      // 현재 잔디에서 안내 애니메이션이 반복되지 않도록 감지 상태를 기록
      isDismissedRef.current = true;
      // 사용자가 안내를 읽을 수 있도록 첫 이동 뒤 지연 페이드아웃 상태를 적용
      setIsHintDismissed(true);
    }

    // 연속 스크롤 중에는 이전 숨김 예약을 취소해 표시를 유지
    clearScrollbarTimer();
    // 실제 스크롤이 진행되는 동안 현재 위치를 확인할 스크롤바를 표시
    setIsScrollbarVisible(true);
    // 마지막 스크롤 입력 이후 스크롤바가 천천히 사라지기 시작할 시점을 예약
    scrollbarTimerRef.current = window.setTimeout(hideScrollbar, SCROLLBAR_HIDE_DELAY_MS);
  };

  // 안내가 사라지는 동안에도 같은 요소를 유지해 지연 페이드아웃을 적용
  let scrollHintClassName = styles.scrollHint;

  // 최초 사용자 스크롤이 감지되면 안내에 종료 애니메이션 스타일을 추가
  if (isHintDismissed) {
    scrollHintClassName += ` ${styles.scrollHintDismissed}`;
  }

  // 사용자 조작 전에는 브라우저 기본 스크롤바 색상을 투명하게 유지
  let horizontalScrollClassName = styles.horizontalScroll;

  // 실제 사용자 스크롤이 감지된 뒤에는 현재 위치를 확인할 스크롤바 스타일을 추가
  if (isScrollbarVisible) {
    horizontalScrollClassName += ` ${styles.horizontalScrollActive}`;
  }

  // 실제 오버플로 안내와 가로 스크롤 콘텐츠를 함께 반환
  return (
...
/**
 * 초 단위 독서 시간을 그래프 도움말에 표시할 시간과 분 문자열로 변환
 *
 * @author SeungHyeon.Kang
...
/**
 * 전년도와 달라진 통계 값을 증가와 감소 또는 동일 문구로 변환
 *
 * @author SeungHyeon.Kang
...
const getDifferenceLabel = (difference: number, formattedAbsolute: string): string => {
  // 차이가 없으면 별도의 양수 또는 음수 기호 없이 동일 상태를 반환
  if (difference === 0) {
    // "변화 없음"
    return message("frontend.profile.readingStats.comparisonSame");
  }

  // 양수 차이는 현재 연도에 증가한 값으로 표시
  if (difference > 0) {
    // "+{0}"
...
/**
 * 현재 연도와 이전 연도의 독서 기록을 화면 비교표의 세 행으로 변환
 *
 * @author SeungHyeon.Kang
...
const getComparisonRows = (statistics: ReadingStatistics): ReadingComparisonRow[] => {
  ...
  // 현재 및 이전 연도 값과 증감값을 같은 단위로 표시하는 비교 행을 반환
  return [
    {
      key: "readTime",
...
/**
 * 별점 분포 세로 막대의 상대 높이를 계산할 최대 독후감 수를 조회
 *
 * @author SeungHyeon.Kang
...
const getRatingMaxCount = (ratingList: ReadingRatingCount[]): number => {
  ...
  // 여섯 별점 구간을 순회하며 현재까지의 최대 독후감 수를 갱신
  for (const rating of ratingList) {
    // 음수 비정상 값은 0권으로 취급하고 기존 최대값과 비교
    ratingMaxCount = Math.max(ratingMaxCount, Math.max(0, rating.reptCnt));
  }

  // 막대 높이 비율의 분모로 사용할 최대 독후감 수를 반환
  return ratingMaxCount;
};

/**
 * 별점 세로 막대그래프의 권수 축에 표시할 상단과 중간 및 0권 눈금을 구성
 *
 * @author SeungHyeon.Kang
 * @param ratingList 정수 별점별 독후감 수 목록
...
const getHeatmapLevel = (readSecs: number): number => {
  // 10분 미만은 기록이 없는 날을 포함해 첫 번째 잔디 색상을 사용
  if (readSecs < 600) {
    return 0;
  }

  // 30분 미만은 두 번째 잔디 색상을 사용
  if (readSecs < 1800) {
    return 1;
  }

  // 한 시간 미만은 세 번째 잔디 색상을 사용
  if (readSecs < 3600) {
    return 2;
  }

  // 네 시간 미만은 네 번째 잔디 색상을 사용
  if (readSecs < 14400) {
    return 3;
  }

  // 네 시간 이상은 가장 진한 다섯 번째 잔디 색상을 사용
  return 4;
};

/**
 * 잔디의 첫 날짜가 속한 요일 앞에 채울 빈 칸 수를 계산
 *
 * @author SeungHyeon.Kang
...
const getHeatmapOffset = (firstDate?: string): number => {
  ...
  // 날짜 문자열을 로컬 자정으로 해석해 일요일부터 토요일까지의 행 위치를 계산
  return new Date(`${firstDate}T00:00:00`).getDay();
};

/**
 * 날짜별 잔디 목록에서 각 월의 첫 표시 열과 영문 세 글자 월 라벨을 계산
 *
 * @author SeungHyeon.Kang
...
const getHeatmapMonths = (dailyList: ReadingTimeDaily[], heatmapOffset: number): HeatmapMonthMarker[] => {
  ...
  for (let index = 0; index < dailyList.length; index += 1) {
    ...
    // GitHub 잔디처럼 월 시작 위치에 영문 세 글자 라벨을 추가
    monthMarkers.push({
      monthKey,
  ...
  // 날짜 열과 대응하는 월 라벨 목록을 반환
  return monthMarkers;
};

/**
 * 마이페이지와 타이머 화면이 함께 사용하는 연도별 독서 시간 잔디를 표시
 *
 * @author SeungHyeon.Kang
...
export function ReadingHeatmapChart({ heatmap, onYearChange, titleClassName }: ReadingHeatmapChartProps) {
  ...
  // 홈 화면과 같은 SelectBox에 전달할 연도 옵션을 구성
  ...
  const renderHeatmapMonth = (marker: HeatmapMonthMarker) => {
    // GitHub 잔디처럼 월이 시작되는 열에 영문 세 글자 라벨을 반환
    return (
  ...
  // 두 화면에서 같은 모양과 조작을 제공하는 독서 잔디를 반환
  return (
    /* 선택 연도의 독서 시간 잔디 영역 */
...
/**
 * 독서 상태별 건수를 목표 진행 막대 색상의 연속 구간 배경으로 변환
 *
 * @author SeungHyeon.Kang
...
function ReadingStatisticsSection({ targetUserNumb }: ReadingStatisticsSectionProps) {
  ...
  // 본인 통계의 상위 책에서 연결 독후감 상세로 이동할 라우터 함수를 생성
  const navigate = useNavigate();
  // 통계 영역의 스크롤 진입 여부를 관찰할 요소 참조를 생성
  const sectionRef = useRef<HTMLElement | null>(null);
  // 화면 이탈과 재시도에서 이전 조회를 취소할 요청 참조를 생성
  const abortControllerRef = useRef<AbortController | null>(null);
  // 동일 화면에서 자동 지연 조회가 한 번만 실행되도록 상태 참조를 생성
  const requestedRef = useRef(false);
  // 서버가 반환한 본인 또는 공개 독서 통계 상태를 생성
  const [statistics, setStatistics] = useState<ReadingStatistics | null>(null);
  // 독서 통계 조회 진행 상태를 생성
  const [isLoading, setIsLoading] = useState(false);
  // 독서 통계 조회 실패 상태를 생성
  const [isError, setIsError] = useState(false);
  // 비공개 또는 제한 계정의 통계 비표시 상태를 생성
  const [isUnavailable, setIsUnavailable] = useState(false);
  // 본인 독서 통계 설정 모달 표시 상태를 생성
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);
  // 통계 설정 모달에서 선택한 공개 여부 상태를 생성
  const [selectedPublic, setSelectedPublic] = useState<"Y" | "N">("N");
  // 독서 통계 설정 저장 진행 상태를 생성
  ...
  /**
   * 본인 또는 공개 독서 통계를 조회하고 로딩 및 실패 상태를 화면에 반영
   *
   * @author SeungHyeon.Kang
  ...
  const loadStatistics = useCallback(async (readYear?: number): Promise<void> => {
    // 동일 화면에서 이전 재시도 요청이 남아 있으면 최신 요청만 유지
    abortControllerRef.current?.abort();
    // 화면 이탈 시 독서 통계 조회를 취소할 요청 제어 객체를 생성
    const abortController = new AbortController();
    // 현재 독서 통계 요청의 취소 제어 객체를 저장
    abortControllerRef.current = abortController;
    // 지연 조회가 시작됐음을 화면 상태에 반영
    setIsLoading(true);
    // 이전 독서 통계 실패 상태를 초기화
    setIsError(false);

    // 독서 통계 조회 성공과 실패 및 종료 상태를 각각 처리
    try {
      let response: ReadingStatistics | null;

      // 대상 번호가 있으면 프로필 주인이 공개한 통계만 조회
      if (targetUserNumb !== undefined) {
        // 다른 사용자가 공개한 독서 통계를 조회
        response = await getSocialReadingStatsApi(targetUserNumb, readYear, abortController.signal);

      } else {
        // 현재 화면의 본인 전용 독서 통계를 조회
        response = await getReadingStatsApi(readYear, abortController.signal);
      }

      // 화면이 유지되는 동안 조회한 독서 통계 또는 비공개 상태를 표시
      if (!abortController.signal.aborted) {
        // 공개 가능한 통계가 없는 프로필은 빈 카드를 남기지 않도록 상태를 설정
        setIsUnavailable(response === null);
        // 검증된 독서 통계 응답 데이터를 화면 상태에 설정
  ...
  /**
   * 통계 영역에 가까워졌을 때만 최초 조회를 시작하도록 관찰자를 연결
   *
   * @author SeungHyeon.Kang
  ...
  const observeStatistics = useCallback((): (() => void) | undefined => {
    ...
    // IntersectionObserver를 지원하지 않는 환경은 통계를 즉시 조회
    if (!("IntersectionObserver" in window)) {
      // 동일 화면에서 다시 자동 조회되지 않도록 최초 요청 상태를 기록
    ...
    /**
     * 통계 카드가 화면 아래 240px 범위에 들어오면 조회를 시작
     *
     * @author SeungHyeon.Kang
    ...
    const handleIntersection = (entries: IntersectionObserverEntry[]): void => {
      ...
      for (const entry of entries) {
        ...
        // 동일 화면에서 통계 쿼리가 한 번만 자동 실행되도록 요청 상태를 기록
        requestedRef.current = true;
        // 지연된 독서 통계 조회를 시작
        void loadStatistics();
        // 최초 지연 조회 뒤에는 스크롤 관찰을 종료
        observer.disconnect();
        break;
      }
    };

    // 화면 진입 직전 통계를 준비하도록 아래쪽 관찰 여백을 둔 관찰자를 생성
    const observer = new IntersectionObserver(handleIntersection, { rootMargin: "240px 0px" });
    // 통계 카드의 스크롤 위치 관찰을 시작
    observer.observe(sectionRef.current);

    /**
     * 컴포넌트가 해제될 때 통계 카드 스크롤 관찰을 종료
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const disconnectObserver = (): void => {
      // 더 이상 사용하지 않는 통계 카드 관찰자를 해제
      observer.disconnect();
    };

    // Effect 정리 단계에서 실행할 관찰 해제 함수를 반환
    return disconnectObserver;
  }, [loadStatistics]);

  /**
   * 통계 컴포넌트가 해제될 때 진행 중인 API 요청을 취소
   *
   * @author SeungHyeon.Kang
  ...
  const prepareRequestCleanup = useCallback((): (() => void) => {
    /**
     * 화면을 벗어난 뒤 독서 통계 응답이 상태를 변경하지 않도록 요청을 취소
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const abortStatisticsRequest = (): void => {
      // 진행 중인 독서 통계 요청을 화면 이탈과 함께 취소
      abortControllerRef.current?.abort();
    };

    // Effect 정리 단계에서 실행할 요청 취소 함수를 반환
    return abortStatisticsRequest;
  }, []);

  // 스크롤이 통계 영역에 접근할 때 최초 조회를 시작
  useEffect(observeStatistics, [observeStatistics]);
  // 화면 이탈 시 진행 중인 독서 통계 요청을 정리
  useEffect(prepareRequestCleanup, [prepareRequestCleanup]);

  /**
   * 독서 통계 조회 실패 후 사용자의 재시도 요청을 처리
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  ...
  const handleSettingsClose = (): void => {
    // 저장 요청 중에는 중복 조작과 화면 이탈을 막기 위해 모달을 유지
    if (isSaving) {
      return;
    }
  ...
  const handleOverlayMouseDown = (event: ReactMouseEvent<HTMLDivElement>): void => {
    // 모달 본문 클릭이 배경 닫기로 전파되지 않도록 동일 대상 클릭만 처리
    if (event.currentTarget === event.target) {
      // 저장 중 여부를 확인하는 공통 닫기 처리를 실행
      handleSettingsClose();
    }
  };

  /**
   * 공개 여부 선택 버튼의 Y 또는 N 값을 모달 상태에 반영
   *
   * @author SeungHyeon.Kang
  ...
  /**
   * 잔디에서 조회할 SelectBox 연도를 서버 조회에 반영
   *
   * @author SeungHyeon.Kang
  ...
  const handleSettingsSave = async (): Promise<void> => {
    ...
    try {
      ...
      const saveReadingStatisticsSetting = async (): Promise<void> => {
        ...
        // 갱신된 공개 상태를 현재 연도별 통계 화면에 반영
        if (statistics) {
          // 그래프 데이터는 유지하고 저장된 공개 여부만 변경
      ...
      // 설정 반영 후 처리 중 알림을 같은 저장 성공 알림으로 전환
      ...
      await sweetError(
        ...
        // 서버가 제공한 안전한 업무 실패 문구 또는 기본 저장 실패 문구를 표시
        getApiErrorMessage(error, message("frontend.profile.readingStats.saveFailed")),
      );

    } finally {
      // 독서 통계 설정 저장 완료 상태를 반영
      setIsSaving(false);
    }
  };

  /**
   * 독서 상태별 색상과 건수를 도넛 그래프 범례 한 줄로 렌더링
   *
   * @author SeungHyeon.Kang
  ...
  const renderStatusLegend = (status: ReadingStatusCount) => {
    // 독서 상태 색상과 이름 및 권수를 함께 표시하는 범례를 반환
    return (
      /* 독서 상태 비율 개별 범례 영역 */
  ...
  /**
   * 현재 연도에 타이머로 오래 읽은 책을 순위와 누적 시간으로 렌더링
   *
   * @author SeungHyeon.Kang
  ...
  const renderTopBook = (bookTime: ReadingBookTime, index: number) => {
    ...
    // 공개 프로필 응답처럼 독후감 번호가 없는 경우에는 링크를 만들지 않도록 안전한 번호를 계산
    const reportNumber = bookTime.reptNumb ?? 0;
    // 본인 화면에서 양수 독후감 번호가 확인된 항목만 링크 버튼으로 제공
    const isReportLink = isOwner && reportNumber > 0;

    /**
     * 올해 상위 책에 연결된 본인 독후감 상세 화면으로 이동
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const handleTopBookClick = (): void => {
      // 서버가 제공한 양수 독후감 번호만 상세 경로에 포함
      if (!isReportLink) {
        return;
      }

      // 선택한 올해 상위 책의 본인 독후감 상세 화면으로 이동
      navigate(`/report/detail/${reportNumber}`);
    };

    // 클릭 가능 여부와 관계없이 같은 순위 및 책 정보를 유지할 공통 콘텐츠를 구성
    ...
    // 본인 화면은 독후감 상세로 이동할 수 있는 버튼으로 순위 항목을 반환
    if (isReportLink) {
      // 키보드와 포인터로 선택 가능한 올해 상위 책 버튼을 반환
      return (
    ...
    // 공개 프로필에서는 다른 회원의 소유자 전용 독후감으로 이동하지 않는 통계 항목을 반환
    return (
  ...
  const renderRatingTick = (ratingTick: number) => {
    // 별도 축 제목 없이 단위를 확인할 수 있는 권수 눈금을 반환
    return (
  ...
  /**
   * 한 별점의 독후감 수를 권수 축에 비례하는 세로 막대로 렌더링
   *
   * @author SeungHyeon.Kang
  ...
  const renderRating = (rating: ReadingRatingCount) => {
    // 서버 집계값이 소수로 전달되더라도 화면에는 버림 처리한 정수 별점만 표시
    const ratingGrade = Math.floor(rating.reptGrde);
    // 음수인 비정상 집계값이 차트 높이와 화면 권수에 반영되지 않도록 보정
    const ratingCount = Math.max(0, rating.reptCnt);
    // 권수 축의 상단 기준에 대한 현재 별점의 세로 막대 높이를 계산
    const ratingHeight = (ratingCount / ratingAxisMax) * 100;

    // 별점과 권수 및 세로 막대를 한 열로 반환
    return (
  ...
  /**
   * 현재 연도와 이전 연도의 한 통계 지표 및 증감값을 렌더링
   *
   * @author SeungHyeon.Kang
  ...
  const renderComparisonRow = (comparisonRow: ReadingComparisonRow) => {
    // 증감 방향에 맞춰 초록 증가와 분홍 감소 및 회색 동일 배지 스타일을 결정
    ...
    // 지표명과 두 연도의 값 및 증감 배지를 같은 행으로 반환
    return (
  ...
  /**
   * 통계 설정 모달의 공개 또는 비공개 선택 버튼을 렌더링
   *
   * @author SeungHyeon.Kang
