...
/**
 * 로그인 사용자의 알림 목록을 보여주는 페이지
 * 삭제되지 않은 알림을 모두 보여주며, 개별 링크 클릭으로 읽음 처리하고 모두 지우기로 목록에서 제거
 *
 * @author HanWon.Jang
 * @return 알림 목록 화면
 */
function AlimPage() {
  ...
  const loadAlimList = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        // 목록 조회는 상태를 변경하지 않으므로 서버가 반환한 미삭제 알림을 읽음 상태 그대로 페이지 순서대로 병합
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      // 서버는 아직 불러오지 않은 알림까지 모두 삭제 처리하므로 추가 페이지 요청을 즉시 중단
      setHasNext(false);
      notifyUnreadAlimChange(response.data?.unreadCnt ?? 0);

      // 현재 화면에 카드가 있으면 순차적으로 오른쪽 퇴장시킨 뒤 목록을 비워 빈 상태 문구로 전환
  ...
  const handleAlimClick = async (alim: AlimItem) => {
    // 링크가 없는 알림은 단순 안내 알림으로 취급해 현재 화면을 유지
    ...
    try {
      ...
      // 읽은 알림도 알림센터에 유지하므로 제거하지 않고 상태만 바꾸어 어두운 스타일을 즉시 적용
      setAlimList((prevList) => (
        prevList.map((item) => (
      ...
      // 클릭 시점의 콘텐츠 공개 여부와 팔로우 관계를 반영한 최종 이동 주소를 조회
      const targetResponse = await getAlimTargetApi(alim.alimNumb);
      // 서버가 소유권과 현재 접근 권한을 검증한 내부 경로로 이동
            ...
            >
              <span className={styles.alimIconWrap} aria-hidden="true">
                <img
                  ...
                  alt=""
                  onError={(event) => {
                    // 조인된 아이콘 데이터가 손상되었으면 반복 처리를 막고 정적 기본 아이콘으로 대체
