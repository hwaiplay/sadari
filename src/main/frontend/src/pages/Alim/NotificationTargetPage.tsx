...
/**
 * 푸시 알림의 사용자별 알림번호를 현재 접근 가능한 화면 주소로 해석해 이동한다
 *
...
const NotificationTargetPage = () => {
  ...
  // 개발 모드의 Effect 재실행에서도 같은 알림 이동 요청이 중복되지 않도록 한 번만 처리한다
  useEffect(() => {
    // 이미 같은 화면 인스턴스에서 이동 주소를 조회했다면 중복 알림과 이동을 막는다
    if (isResolvingRef.current) {
      // 최초 요청이 이동을 완료하도록 추가 처리를 종료한다
      return;
    }

    // 현재 화면 인스턴스의 알림 이동 조회가 시작되었음을 기록한다
    isResolvingRef.current = true;

    /**
     * 경로의 알림번호를 검증하고 서버가 계산한 내부 화면으로 이동한다
     *
     * @author SeungHyeon.Kang
    ...
    const moveAlimTarget = async (): Promise<void> => {
      ...
      // 양수 정수 알림번호가 아니면 서버를 호출하지 않고 알림 목록으로 복귀한다
      if (!Number.isSafeInteger(targetAlimNumb) || targetAlimNumb <= 0) {
        // 유효하지 않은 알림 경로를 브라우저 이력에 남기지 않고 알림 목록으로 이동한다
        navigate("/alim", { replace: true });
        // 잘못된 알림번호의 이동 처리를 종료한다
        return;
      }

      // 알림 소유권과 현재 공개 및 팔로우 상태가 반영된 이동 주소를 조회한다
      const response = await getAlimTargetApi(targetAlimNumb);
      // 서버가 검증한 내부 이동 주소로 현재 해석 경로를 교체한다
      navigate(response.data.linkUrlx, { replace: true });
    };

    // 비동기 이동 실패를 공통 오류 안내와 알림 목록 복귀 흐름으로 격리한다
    void moveAlimTarget().catch(async (error: unknown) => {
      ...
      // 접근할 수 없는 알림 경로를 브라우저 이력에서 제거하고 알림 목록으로 복귀한다
      navigate("/alim", { replace: true });
    });
  }, [alimNumb, navigate]);

  // 알림번호의 최종 이동 주소를 계산하는 동안 공통 로딩 화면을 반환한다
  return <Loading />;
};
