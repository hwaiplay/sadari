...
const NoticeDetailPage = () => {
  ...
  /**
   * 주소의 공지사항 주키를 검증하고 현재 배포 버전 상세를 조회
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const loadNotice = useCallback((): void => {
    ...
    // 검증된 공지사항 상세 비동기 조회를 시작
    void requestNotice();
  }, [noticeNumb]);

  // 주소가 바뀔 때 해당 공지사항의 현재 배포 버전을 다시 조회
  useEffect(loadNotice, [loadNotice]);

  // 상세 조회에 실패하면 공통 여백 안에서 오류 문구를 안내
  if (error) {
    // 공지사항 상세 조회 실패 안내 화면을 반환
    return (
  ...
  // 상세 조회 중에는 사용자 공통 인라인 로딩 화면을 표시
  if (!notice) {
    // 공지사항 상세 조회 상태 화면을 반환
    return (
      <main className={styles.page}>
        {/* 공지사항 상세 조회 상태 영역 */}
  ...
  return (
    ...
    <main className={styles.page}>
      ...
      {/* 본문은 관리자 서버에서 허용 태그와 공지 전용 이미지 경로만 남겨 저장한 HTML */}
