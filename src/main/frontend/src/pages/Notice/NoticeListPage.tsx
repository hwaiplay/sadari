...
function NoticeListPage() {
  ...
  /**
   * 요청 페이지를 조회하여 첫 페이지를 교체하거나 다음 페이지를 기존 목록 뒤에 연결
   *
   * @author SeungHyeon.Kang
  ...
  const loadPage = useCallback(async (targetPage: number): Promise<void> => {
    ...
    // 목록 요청 동안 중복 조회를 막고 기존 오류 안내를 제거
    setIsLoading(true);
    setError("");

    try {
      // 현재 배포 중인 공지사항만 페이지 단위로 조회
      const data = await getNoticeListApi(targetPage);

      /**
       * 조회한 페이지를 현재 공지사항 목록에 반영
       *
       * @author SeungHyeon.Kang
      ...
      const mergeNoticeList = (current: Notice[]): Notice[] => {
        // 첫 페이지는 새 목록으로 교체하고 다음 페이지는 현재 목록 뒤에 추가
        return targetPage === 1 ? data.list : [...current, ...data.list];
      };

      // 첫 페이지는 새 목록으로 교체하고 다음 페이지는 현재 목록 뒤에 추가
      setNotices(mergeNoticeList);
      // 서버가 반환한 현재 페이지 번호를 다음 조회 기준으로 저장
      setPage(data.page);
      // 다음 공지사항 페이지 존재 여부를 더 보기 버튼 상태에 반영
      setHasNext(data.hasNext);
    } catch (loadError) {
      // "공지사항을 불러오지 못했습니다."
      setError(getApiErrorMessage(loadError, message("frontend.notice.list.loadFailed")));
    } finally {
      // 요청 성공 여부와 관계없이 로딩 상태를 종료
      setIsLoading(false);
    }
  }, []);

  /**
   * 화면 최초 진입 시 공지사항 첫 페이지 조회를 시작
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const loadInitialPage = useCallback((): void => {
    // 최신 공지사항부터 표시하기 위해 첫 페이지를 조회
    void loadPage(1);
  }, [loadPage]);

  // 화면 최초 진입 시 최신 공지사항부터 조회
  ...
  /**
   * 첫 페이지 공지사항 조회를 다시 시도
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleRetry = (): void => {
    // 오류 상태를 새 요청으로 교체하기 위해 첫 페이지를 다시 조회
    void loadPage(1);
  };

  /**
   * 현재 목록 다음의 공지사항 페이지를 조회
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleLoadMore = (): void => {
    // 서버가 마지막으로 반환한 페이지 다음 번호를 조회
    void loadPage(page + 1);
  };

  /**
   * 개별 공지사항을 제목과 배포일 및 상세 이동 버튼으로 구성
   *
   * @author SeungHyeon.Kang
  ...
  // 최초 목록을 조회하는 동안 사용자 공통 인라인 로딩 화면을 표시
  if (isLoading && notices.length === 0 && !error) {
    // 공지사항 최초 조회 상태 화면을 반환
    return (
  ...
  // 공지사항 목록과 조회 상태 및 다음 페이지 제어 화면을 반환
  return (
