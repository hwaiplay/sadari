...
/**
 * 신고 사유 선택 이후 감사 안내와 추가 차단 선택지를 표시
 *
 * @author HanWon.Jang
 * @return 신고 완료 페이지
 */
const UserReportCompletePage = () => {
  ...
  /**
   * 완료 화면에서 차단 여부를 확인하고 선택한 신고 대상의 차단 관계를 등록
   *
   * @author HanWon.Jang
  ...
  const handleBlock = async (): Promise<void> => {
    try {
      // 공통 차단 정책을 안내하고 사용자가 확인한 경우에만 상태 변경을 시작
      await confirmUserBlock(target.userNick, async () => {
        // 처리 중 화면과 이동 차단을 유지하며 신고 대상 사용자를 차단
        ...
        // 완료 화면에서 같은 사용자를 다시 차단하지 않도록 성공 상태를 저장
        setIsBlocked(true);
      });
    }
  ...
  /**
   * 신고 완료 화면의 차단 버튼 클릭을 비동기 차단 처리와 연결
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleBlockClick = (): void => {
    // 차단 확인과 API 오류는 비동기 처리 함수 내부에서 안내
    void handleBlock();
  };
