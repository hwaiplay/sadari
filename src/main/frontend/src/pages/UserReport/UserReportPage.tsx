...
// 신고 화면에서 내부 대상 유형을 사용자에게 표시할 다국어 메시지 키와 연결
...
/**
 * 전달된 글 또는 댓글 정보를 확인하고 신고 사유를 선택하는 화면을 렌더링
 *
 * @author HanWon.Jang
 * @return 신고 사유 선택 페이지
 */
const UserReportPage = () => {
  ...
  // 신고 대상 정보 없이 직접 접근한 경우 안전한 기본 화면으로 이동
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      // 서버가 동일 사용자와 대상의 기존 신고를 확인하면 재신고 제한을 안내
      if (saveError instanceof ResultDataError
        ...
        // 중복 신고 안내 뒤 일반 실패 알림이 이어서 표시되지 않도록 제출 처리를 종료
        return;
      }
      ...
      // 성공과 실패 모두에서 신고 접수 버튼을 다시 사용할 수 있도록 상태를 해제
      setIsSaving(false);
    }
  };
