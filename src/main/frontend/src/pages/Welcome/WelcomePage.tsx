...
/**
 * 최초 로그인 사용자에게 서비스 특징과 닉네임 및 관심분야 설정 흐름을 슬라이드로 제공함
 *
 * @author HanWon.Jang
...
const WelcomePage = () => {
  ...
  useEffect(() => {
    ...
    // 가입 시 서버가 발급한 고유 랜덤 닉네임을 닉네임 설정 슬라이드의 기본값으로 조회함
    queryClient.fetchQuery(getMyProfileOptions())
      .then((profile) => {

        // 화면을 떠난 뒤 도착한 프로필 응답은 입력 상태에 반영하지 않음
        if (!ignore) {
          // 서버가 발급한 닉네임을 사용자가 바로 확정하거나 수정할 수 있도록 입력값에 설정함
          setUserNick(profile.userNick ?? "");
        }
      })
      .catch((error) => {

        // 화면이 유지되는 동안 발생한 조회 실패만 사용자에게 안내함
        if (!ignore) {
          void sweetError(
      ...
      .finally(() => {
        ...
        // 화면이 유지되는 동안에만 프로필 로딩 상태를 종료함
        if (!ignore) {
          // 닉네임 입력 화면을 사용할 수 있도록 로딩 상태를 해제함
          setIsLoading(false);
        }
      });

    // 관심분야 조회 실패가 웰컴 완료를 막지 않도록 프로필과 별도 흐름으로 조회함
    getUserInterestCatalogApi()
      .then((catalog) => {
        // 화면을 떠난 뒤 도착한 관심분야 응답은 선택 상태에 반영하지 않음
        if (!ignore) {
          // 활성 공통코드로 구성된 관심분야를 선택 목록에 설정함
          setInterestCatalog(catalog);
        }
      })
      .catch(() => {
        // 공통코드 조회 실패 시 빈 관심분야 단계에서도 웰컴을 완료할 수 있게 기존 흐름을 유지함
        if (!ignore) {
          // 선택할 수 없는 빈 관심분야 목록을 명시적으로 설정함
          setInterestCatalog([]);
        }
      });

    // 관리자가 배포한 소개 페이지를 닉네임과 관심분야 고정 단계 앞에 조회함
    getWelcomePageListApi()
      .then((pages) => {
        // 화면을 떠난 뒤 도착한 응답은 슬라이드 상태에 반영하지 않음
        if (!ignore) {
          // 서버에서 정렬된 현재 배포 페이지를 관리자 관리 영역으로 설정함
          setManagedPages(pages);
          // 이미지가 없는 환영 인사 화면에서 첫 관리 이미지를 미리 불러옴
          setManagedImageLoadCount(pages.length > 0 ? 1 : 0);
        }
      })
      .catch((error) => {
        // 웰컴페이지 조회 실패를 사용자에게 안내하고 고정 설정 단계는 계속 제공함
        if (!ignore) {
          void sweetError(
            /* "조회에 실패했습니다." */ message("frontend.alert.loadFailedTitle"),
            getApiErrorMessage(error, /* "다시 시도해주세요." */ message("frontend.common.tryAgain")),
      ...
      .finally(() => {
        // 화면이 유지되는 동안에만 관리자 페이지 로딩 상태를 종료함
        if (!ignore) {
          // 고정 설정 단계를 포함한 전체 슬라이드를 표시할 수 있게 함
          setIsWelcomePageLoading(false);
        }
      });

    // 화면 전환 뒤 도착한 프로필 응답이 상태를 변경하지 않도록 정리함
    return () => {
  ...
  /**
   * 허용하지 않은 닉네임 문자를 제거하고 최대 입력 길이를 제한함
   *
   * @author HanWon.Jang
  ...
  const getValidatedUserNick = (): string | null => {
    ...
    if (normalizedUserNick.length > USER_NICK_MAX_LENGTH || !USER_NICK_REGEX.test(normalizedUserNick)) {
      ...
      // 형식이 맞지 않는 닉네임은 다음 단계와 저장에 사용하지 않음
      return null;
    }

    // 앞뒤 공백을 제거해 확정한 닉네임을 반환함
    return normalizedUserNick;
  };

  /**
   * 현재 슬라이드에서 다음 소개 또는 닉네임 설정 화면으로 이동함
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleNext = (): void => {
    ...
    // 닉네임 다음에는 검증을 통과한 사용자만 관심분야 단계로 이동함
    if (activeSlide === NICKNAME_SLIDE_INDEX) {
      const normalizedUserNick = getValidatedUserNick();
      // 닉네임 검증에 실패하면 현재 입력 단계에 머묾
      if (!normalizedUserNick) {
        // 사용자가 입력값을 수정할 수 있도록 다음 단계 이동을 중단함
        return;
      }

      // 최종 저장에서도 같은 닉네임을 사용하도록 정규화한 값을 반영함
      setUserNick(normalizedUserNick);
    }

    // 마지막 슬라이드 이전에서만 오른쪽 슬라이드로 이동함
    if (activeSlide < LAST_SLIDE_INDEX) {
      // 현재 위치보다 한 단계 뒤의 소개 화면을 선택함
      setActiveSlide((currentSlide) => currentSlide + 1);
    }
  };

  /**
   * 닉네임 폼 제출 시 검증을 거쳐 관심분야 선택 단계로 이동함
   *
   * @author SeungHyeon.Kang
  ...
  const handleNicknameNext = (event: FormEvent<HTMLFormElement>): void => {
    ...
    // 공통 다음 이동 로직에서 닉네임을 검증하고 관심분야 단계를 엶
    handleNext();
  };

  /**
   * 현재 슬라이드에서 이전 소개 화면으로 이동함
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handlePrevious = (): void => {
    ...
    // 첫 슬라이드 이후에서만 왼쪽 슬라이드로 이동함
    if (activeSlide > 0) {
      // 현재 위치보다 한 단계 앞의 소개 화면을 선택함
      setActiveSlide((currentSlide) => currentSlide - 1);
    }
  };

  /**
   * 진행 표시 버튼의 데이터 값으로 사용자가 선택한 슬라이드로 이동함
   *
   * @author HanWon.Jang
  ...
  const handleTouchStart = (event: TouchEvent<HTMLDivElement>): void => {
    ...
    // 첫 번째 터치 지점을 좌우 넘김 시작 좌표로 저장함
    touchStartXRef.current = event.touches[0]?.clientX ?? null;
  };

  /**
   * 터치 이동 거리가 기준을 넘으면 좌우 방향에 맞는 슬라이드로 이동함
   *
   * @author HanWon.Jang
  ...
  const handleTouchEnd = (event: TouchEvent<HTMLDivElement>): void => {
    ...
    // 다음 터치가 이전 좌표를 재사용하지 않도록 시작 지점을 비움
    touchStartXRef.current = null;

    // 시작점이나 종료점이 없으면 넘김 방향을 계산하지 않음
    if (touchStartX === null || touchEndX === undefined) {
      // 좌표가 불완전한 터치 이벤트 처리를 종료함
      return;
    }

    const distance = touchStartX - touchEndX;

    // 왼쪽으로 충분히 밀었으면 다음 소개 화면을 엶
    if (distance > SWIPE_THRESHOLD_PX) {
      // 오른쪽에 있는 다음 슬라이드로 이동함
      handleNext();
      // 한 번의 터치가 두 방향으로 처리되지 않도록 종료함
      return;
    }

    // 오른쪽으로 충분히 밀었으면 이전 소개 화면을 엶
    if (distance < -SWIPE_THRESHOLD_PX) {
      // 왼쪽에 있는 이전 슬라이드로 이동함
      handlePrevious();
    }
  ...
  /**
   * 취향 목록의 스크롤 위치에 따라 상단 잘림 안내를 표시함
   *
   * @author HanWon.Jang
  ...
  const handleInterestScroll = (event: UIEvent<HTMLElement>): void => {
    ...
    // 목록을 아래로 내린 동안에만 상단 경계도 페이드 처리함
    setIsInterestScrolled(scrollElement.scrollTop > 0);
    // 마지막 항목이 모두 보이면 아래쪽 페이드가 목록 끝을 가리지 않도록 해제함
  ...
  /**
   * 관리 이미지 로딩이 끝나면 다음 관리 이미지를 순차적으로 미리 불러옴
   *
   * @author SeungHyeon.Kang
  ...
  const handleManagedImageSettled = (event: SyntheticEvent<HTMLImageElement>): void => {
    ...
    // 이미지에 연결된 관리 페이지 번호가 아니면 다음 선로딩 대상을 계산하지 않음
    if (!Number.isInteger(pageIndex) || pageIndex < 0) {
      // 유효하지 않은 이미지 이벤트 처리를 종료함
      return;
    }

    // 현재 이미지 다음 한 장까지 로딩 대상으로 열어 대용량 이미지의 동시 요청을 막음
    setManagedImageLoadCount((currentCount) => Math.max(currentCount, pageIndex + 2));
  };

  /**
   * 최초 로그인 사용자가 누른 독서 관심분야의 선택 상태를 전환함
   *
   * @author SeungHyeon.Kang
  ...
  const handleInterestToggle = (event: MouseEvent<HTMLButtonElement>): void => {
    ...
    // 화면에 존재하는 관심분야 키만 선택 상태에 반영함
    if (!interestKey) {
      // 유효한 관심분야가 없는 클릭 처리를 종료함
      return;
    }

    // 기존 Set을 직접 변경하지 않도록 새 선택 집합을 생성함
    setSelectedInterestKeys((currentKeys) => {
      const nextKeys = new Set(currentKeys);
      // 이미 선택한 관심분야는 다시 눌렀을 때 해제함
      if (nextKeys.has(interestKey)) {
        // 선택 집합에서 현재 관심분야를 제거함
        nextKeys.delete(interestKey);
      } else {
        // 새 관심분야를 개수 제한 없이 선택 집합에 추가함
  ...
  /**
   * 대분류에 속한 모든 소분류 관심분야의 선택 상태를 한 번에 전환함
   *
   * @author SeungHyeon.Kang
  ...
  const handleStart = async (): Promise<void> => {
    ...
    // 이전 단계 이후 닉네임 상태가 변경되었으면 최종 저장을 중단함
    if (!normalizedUserNick) {
      // 닉네임 설정 단계로 돌아가 입력값을 수정할 수 있도록 저장 요청을 종료함
      setActiveSlide(NICKNAME_SLIDE_INDEX);
      return;
    }

    // 관심분야를 최소 선택 개수보다 적게 골랐다면 저장 전에 선택 기준을 안내함
    if (selectedInterestKeys.size < MIN_INTEREST_SELECTION_COUNT) {
      ...
      // 사용자가 관심분야를 추가 선택할 수 있도록 현재 단계에서 저장 요청을 종료함
      return;
    }

    // 저장 중 버튼을 다시 눌러 동일 요청이 중복되지 않도록 차단함
    if (isSaving) {
      // 진행 중인 저장 요청이 끝날 때까지 추가 제출을 중단함
      return;
    }

    // 시작하기 버튼과 닉네임 입력을 저장 완료까지 비활성화함
    setIsSaving(true);

    // 닉네임 저장과 인증 상태 갱신 실패를 하나의 사용자 오류 흐름으로 처리함
    try {
      ...
      // 유효한 공통코드 조합으로 사용자의 관심분야를 전체 교체함
      await updateUserInterestsApi({ interestList });

      // 닉네임과 최초 로그인 완료 상태를 서버에 함께 저장함
      await updateOnboardingApi({ userNick: normalizedUserNick });
      // 보호 라우트가 완료된 온보딩 상태를 사용하도록 인증 정보를 즉시 다시 조회함
      await queryClient.refetchQueries({ queryKey: ["auth"], type: "active" });
      // 웰컴 화면을 방문 이력에 남기지 않고 서비스 홈으로 이동함
      navigate("/home", { replace: true });
    }

    // 중복 닉네임이나 비속어를 포함한 서버 검증 메시지를 사용자에게 안내함
    catch (error) {
      void sweetError(
    ...
    // 성공과 실패 모두에서 사용자가 다시 입력할 수 있도록 저장 상태를 해제함
    finally {
      // 온보딩 저장 요청이 끝났음을 버튼 상태에 반영함
      setIsSaving(false);
    }
  };

  /**
   * 웰컴 하단 기본 버튼으로 다음 단계 이동 또는 최종 온보딩 완료를 실행함
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleFooterPrimaryAction = (): void => {
    ...
    // 마지막 관심분야 단계에서는 다음 이동 대신 웰컴 완료 저장을 실행함
    if (activeSlide === LAST_SLIDE_INDEX) {
      // 비동기 저장 결과는 저장 상태와 공통 오류 안내에서 처리함
      void handleStart();
      // 최종 저장과 다음 슬라이드 이동이 함께 실행되지 않도록 종료함
      return;
    }

    // 마지막 단계 전에는 현재 화면에 맞는 다음 슬라이드로 이동함
    handleNext();
  };

  // 가입 시 발급된 닉네임을 조회하는 동안 공통 로딩 화면을 반환함
  if (isLoading || isWelcomePageLoading) {
    return <Loading title={/* "로그인 중" */ message("frontend.common.loginLoading")} />;
  }

  // 서비스 소개와 닉네임 및 관심분야 설정을 단계별로 넘겨보는 웰컴 페이지를 반환함
  return (
      ...
      >
        <div className={styles.track}>
          ...
          {managedPages.map((managedPage, pageIndex) => {
            ...
            // 관리 순서에 고정 환영 인사 위치를 더한 소개 화면을 반환함
            return (
              <section
            ...
            >
              <div className={styles.interestGroups}>
                {interestGroups.map((group) => {
                  ...
                  // 대분류 제목과 전체 선택 상태가 반영된 소분류 관심분야 묶음을 반환함
                  return (
                    ...
                    <section className={styles.interestGroup} key={group.categoryName}>
                      ...
                      <div className={styles.interestList}>
                        {group.interests.map((interest) => {
                          ...
                          // 현재 대분류에 속한 소분류 관심분야 선택 버튼을 반환함
                          return (
                            <button
