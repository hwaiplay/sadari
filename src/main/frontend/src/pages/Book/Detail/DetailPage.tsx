/**
 * 독후감 상세 조회와 직접 편집 및 도서 정보 전환 화면을 구성
 *
 * @author HanWon.Jang
 */
...
/**
 * 읽기 상태의 기록 본문에서 사용자가 클릭한 문자와 커서 줄 위치를 계산
 *
 * @author HanWon.Jang
...
function getRecordCaretTarget(
  ...
  // 표준 API에서 기록 텍스트 위치를 찾지 못하면 WebKit 호환 API를 사용
  if (caretOffset === contentLength) {
    // WebKit 계열 브라우저에서 클릭한 문자 범위를 조회
    const fallbackRange = document.caretRangeFromPoint?.(
      event.clientX,
      event.clientY,
    );

    // 호환 API가 기록 텍스트 위치를 반환하면 클릭한 문자 오프셋을 사용
    if (fallbackRange
...
function DetailPage() {
  ...
  // 댓글 알림 링크가 요청한 댓글 목록 자동 열기 여부를 조회
  const [searchParams] = useSearchParams();
  // 명시적으로 댓글 목록을 요청한 알림 링크인지 판정
  const shouldOpenReplies = searchParams.get("showReplies") === "Y";
  // 알림이 지정한 댓글 번호를 양수 정수인 경우에만 댓글 포커스로 사용
  ...
  // 기록 편집으로 전환한 뒤 복원할 클릭 커서 위치를 보관
  ...
  useEffect(() => {
    // 일반 상세 진입에서는 사용자가 선택한 현재 댓글 목록 상태를 유지
    if (!shouldOpenReplies) {
      return;
    }

    // 댓글 또는 댓글 좋아요 알림이 지정한 독후감의 댓글 목록을 표시
    setIsReplySheetOpen(true);
  }, [shouldOpenReplies]);

  // 상세 직접 편집과 등록 화면이 같은 공통코드 캐시를 사용하도록 상태와 색상 코드를 함께 조회
  ...
  // 동일 ISBN 독후감 선택창에서 전달한 편집 진입 상태를 상세 조회 완료 후 반영
  useEffect(() => {
    const pageState = location.state as DetailPageState | null;

    // 동일 ISBN 선택창에서 수정한 경우에만 상세 조회 직후 편집 명령을 표시
    if (!bookData || pageState?.startEditing !== true) {
      // 일반 상세 진입에서는 현재 읽기 상태를 유지
      return;
    }

    // 기존 독후감 수정을 바로 시작할 수 있도록 상세 화면을 편집 상태로 전환
    setIsEditing(true);
  ...
  useEffect(() => {
    ...
    // 상세 화면을 벗어난 뒤 예약된 콘텐츠 전환이 실행되지 않도록 타이머를 정리
    return () => {

      // 상세 하단 전환 타이머가 있을 때만 브라우저 예약 작업을 취소
  ...
  useLayoutEffect(() => {
    ...
    if (recordCaretTarget) {
      ...
      // 클릭한 커서 줄을 향해 화면이 부드럽게 올라가도록 스크롤
      window.scrollTo({
        top: scrollTop,
        behavior: "smooth",
      });
  ...
  /**
   * 독후감 상세와 도서 정보 하단 콘텐츠를 페이드아웃 후 교체
   *
   * @author HanWon.Jang
  ...
  function switchDetailContent(nextShowBookInfo: boolean) {
    ...
    // 페이드아웃이 끝난 뒤 목표 콘텐츠를 표시하고 페이드인을 시작
  ...
  const showBookInfoView = () => {
    ...
    // 같은 상세 페이지 안에서 도서 정보가 페이드 전환되도록 목표 화면을 설정
    switchDetailContent(true);
  };

  /**
   * 세로 행 요약 항목을 누른 최초 시점부터 상세 화면을 편집 상태로 전환
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  function handleEditStart() {
    ...
    // 편집 시도 전에는 숨겨진 삭제와 취소 및 저장 명령을 표시
    setIsEditing(true);
  }

  /**
   * 상세 화면을 편집 상태로 전환하고 수정 가능한 요소 선택 방법을 안내
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  function handleEditGuide() {
    ...
    // 수정 가능한 요약과 기록 요소를 클릭할 수 있도록 상세 편집 상태를 활성화
    handleEditStart();
    // 수정 대상을 위에서부터 확인할 수 있도록 문서의 스크롤 위치를 최상단으로 이동
    window.scrollTo({
      top: 0,
      left: 0,
      behavior: "auto",
  ...
  /**
   * 선택한 독서 상태를 반영하고 읽는 중에는 공개 여부와 평점을 초기화
   *
   * @author HanWon.Jang
  ...
  function applyStatusSelection(nextStatus: ReadingStatusType): void {
    ...
    // 선택한 상태를 상세 요약과 수정 요청에 반영
    setStatus(nextStatus);

    // 읽는 중으로 되돌리면 완료 또는 중단 상태에서 입력한 공개 여부와 평점을 제거
    if (nextStatus === REPORT_STATUS_READ) {
      // 선택 불가한 평점을 미선택 내부값으로 복원
      setGrade(0);
      // 다른 사용자에게 노출되지 않도록 공개 여부를 비공개로 복원
      setPubcYsno("N");
    }
  }

  /**
   * 독서 상태 변경에 따른 평점 및 공개 여부 초기화와 종료일 보정을 확인
   *
   * @author HanWon.Jang
   * @param nextStatus 사용자가 선택한 다음 독서 상태 코드
   * @return 반환값이 없음
  ...
  async function handleStatusChange(nextStatus: ReadingStatusType) {
    ...
    if (needsReadingResetConfirm) {
      ...
      // 평점 삭제와 공개 범위 변경에 동의하는 경우에만 읽는 중 상태를 적용
      ...
      // 사용자가 초기화를 취소하면 기존 독서 상태와 평점 및 공개 여부를 유지
      if (!confirmed.isConfirmed) {
        return;
      }

      // 확인된 읽는 중 상태를 반영하면서 평점과 공개 여부를 정책 기본값으로 초기화
    ...
    // 종료일 확인이 필요하지 않은 상태는 상세 화면 요약과 저장 요청에 즉시 반영
    if (!needsEndDateConfirm) {
      // 상태별 평점 및 공개 정책을 함께 적용해 편집값을 설정
      applyStatusSelection(nextStatus);
      return;
    }
    ...
    // 사용자가 날짜 보정과 상태 변경을 취소하면 기존 독서 상태를 유지
    if (!confirmed.isConfirmed) {
      return;
    }

    // 확인된 완료 또는 중단 상태를 상세 화면 요약과 저장 요청에 반영
    applyStatusSelection(nextStatus);
    // 확인된 독서 상태에 맞춰 독서 종료일을 오늘로 설정
    setEndDate(formatDateValue(new Date()));
  }

  /**
   * 달력에서 확정한 독서 시작일과 종료일을 상세 편집 상태에 반영
   *
   * @author HanWon.Jang
  ...
  function handleRangeChange(nextStartDate: string, nextEndDate: string) {
    ...
    // 선택한 기간을 세로 행 요약과 저장 요청에 함께 사용할 수 있도록 설정
    setStartDate(nextStartDate);
    setEndDate(nextEndDate);
  }

  /**
   * 기록 본문을 클릭하면 테두리 없는 직접 입력 상태로 전환
   *
   * @author HanWon.Jang
   * @param event 기록 본문 클릭 이벤트
   * @return 반환값이 없음
  ...
  function handleRecordEditStart(event: MouseEvent<HTMLButtonElement>) {
    ...
    // 읽기 상태에서 클릭한 문자와 커서 줄 위치를 편집 전환 후 복원할 대상으로 설정
    recordCaretTargetRef.current = getRecordCaretTarget(event, content.length);
    // 기록 입력과 상세 편집 명령을 함께 활성화
    setIsRecordEditing(true);
    setIsEditing(true);
  }

  /**
   * 기록 입력값을 저장 허용 바이트 안으로 보정해 상세 편집 상태에 반영
   *
   * @author HanWon.Jang
  ...
  function handleContentChange(event: ChangeEvent<HTMLTextAreaElement>) {
    ...
    // UTF-8 저장 허용량을 넘는 뒷부분을 제거한 기록값을 계산
    const nextContent = truncateUtf8Bytes(event.currentTarget.value);

    // 보정한 기록 본문과 저장 기준 바이트 길이를 함께 갱신
  ...
  function handleEditCancel() {
    ...
    // 취소 시 서버 조회값을 다시 반영해 아직 저장하지 않은 변경을 모두 제거
    if (bookData) {
      setStatus(bookData.reptStat ?? "");
      setInitialStatus(bookData.reptStat ?? "");
      const isReadingReport = bookData.reptStat === REPORT_STATUS_READ;
  ...
  async function handleEditSave() {
    ...
    // 등록 화면과 같은 필수값 및 공통코드 검증을 상세 직접 편집에도 적용
    ...
    // 검증에 실패하면 저장 요청 없이 누락되거나 잘못된 항목을 안내
    ...
    // 상세 화면이 보유한 상태별 허용값으로 별도 수정 페이지 없이 독후감 수정 요청을 전송
    updateReport(
      {
        reptNumb: idNum,
        data: {
  ...
  const handleDelete = async () => {
    ...
    // 삭제 확인을 취소하면 상세 편집 상태를 그대로 유지
    if (!confirmed.isConfirmed) {
      return;
    }

    // 확인된 독후감 번호로 삭제 요청을 전송
    deleteReport(idNum);
  };

  /**
   * get Like Count Label 정보를 조회
   *
   * @author HanWon.Jang
  ...
  // 읽는 중인 독후감은 저장된 종료일이 목표일이므로 목표 독서기간으로 구분
  ...
  const showReportDetailView = () => {
    ...
    // 같은 상세 페이지 안에서 독후감 정보가 페이드 전환되도록 목표 화면을 설정
    switchDetailContent(false);
  };

  /**
   * 현재 도서와 같은 ISBN으로 작성된 공개 독후감 목록으로 이동
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const goPublicReportsPage = () => {
    ...
    // 현재 도서의 ISBN을 URL 쿼리에서 사용할 수 있는 문자열로 변환
    const encodedBookIsbn = encodeURIComponent(bookData.bookIsbn);

    // 현재 조회된 도서 정보를 전달해 공개 독후감 목록의 헤더를 즉시 구성
    navigate(
  ...
  const handleLikeToggle = () => {
    ...
    // 상세 화면에 이미 조회된 대상 정보를 사용해 별도 독후감 조회 없이 좋아요를 변경
    likeMutation.mutate({
      tagtType: "REPORT",
      tagtNumb: idNum,
  ...
  const handleReportAlimChange = (
    ...
    // 현재 상세 독후감과 사용자가 선택한 유형별 설정을 서버에 반영
    reportAlimMutation.mutate({reptNumb: idNum, alimType, useYsno});
  };

  // 같은 상세 API에서 받은 도서 정보를 사용해 추가 조회 없이 도서 정보 화면을 구성
  if (showBookInfo) {
    return (
              ...
              <div className={styles.bookInfoRows}>
                ...
                <div className={styles.bookInfoRow}>
                  ...
                  {bookData.bookAthr?.trim() ? (
                    <>
                      {/* 저자명이 있으면 해당 저자명 검색 화면으로 이동하는 링크를 표시 */}
                      <Link
                ...
                <div className={styles.bookInfoRow}>
                  ...
                  {bookData.bookPubl?.trim() ? (
                    <>
                      {/* 출판사명이 있으면 해당 출판사 검색 화면으로 이동하는 링크를 표시 */}
                      <Link
                        className={styles.bookInfoSearchLink}
                        to="/book/search"
                        state={{
