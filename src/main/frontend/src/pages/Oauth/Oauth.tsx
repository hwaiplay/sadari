...
const Oauth = () => {
  ...
  useEffect(() => {
    ...
    // 서버가 로그인 실패를 확정한 경우에는 인증 재조회가 끝나기 전에도 오류를 안내
    ...
    if (!isOauthFailure && isAuthenticated) {
      ...
      // 비활성화 계정 복귀 로그인은 정책을 다시 확인한 뒤 정상 서비스 화면으로 이동
      if (isAccountReactivated) {
        // 개발 모드의 Effect 재실행과 인증 상태 갱신에도 복귀 안내를 한 번만 표시
        if (reactivationNoticeShownRef.current) {
          // 이미 열린 복귀 안내가 닫힐 때까지 현재 OAuth 화면을 유지
          return;
        }

        // 복귀 안내를 열기 전에 표시 상태를 기록해 중복 팝업을 방지
        ...
        void sweetSuccess(
          ...
          // 복귀 정책을 확인한 사용자를 선택한 정상 서비스 화면으로 이동
          navigate(destination, { replace: true });
        });
        // 복귀 안내가 닫히기 전에 OAuth 화면을 벗어나지 않도록 처리를 종료
        return;
      }

      // 인증 직후 선택한 사용자 전용 화면으로 이동
      navigate(destination, { replace: true });
      return;
    }
    ...
    // 인증 조회 완료로 Effect가 다시 실행되어도 실패 알림은 한 번만 표시
    if (failureNoticeShownRef.current) {
      return;
    }

    // 실패 안내를 열기 전에 표시 상태를 기록해 중복 팝업을 방지
    failureNoticeShownRef.current = true;

    // 탈퇴한 과거 회원 번호에 유효 제재가 남아 있으면 재가입 제한 사유를 안내
    ...
    // 판별한 인증 실패 사유를 안내한 뒤 로그인 화면으로 이동
    void sweetError(errorTitle, errorMessage).then(() => {

      // 실패한 OAuth 화면을 기록에 남기지 않고 로그인 화면으로 교체
