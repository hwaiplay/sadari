...
/**
 * 영구 삭제 대기 회원에게 삭제 예정일과 취소 기능만 제공
 *
 * @author HanWon.Jang
...
const WithdrawalPendingPage = () => {
  ...
  // 화면 진입 시 영구 삭제 예정일을 조회
  useEffect(() => {
    // 비동기 상태 조회를 화면 생명주기 안에서 실행
    void (async () => {
      // API 실패를 사용자 안내로 전환하기 위한 처리 블록
      try {
        // 로그인 회원의 영구 삭제 대기 정보를 조회
        const result = await getWithdrawalStatusApi();
        // 화면에 표시할 삭제 대기 정보를 설정
        setStatus(result.data ?? null);
      }

      // 상태 조회 실패 내용을 사용자에게 안내
      catch (error) {
        // "탈퇴 상태를 확인할 수 없어요."
        await sweetError(
  ...
  const handleCancel = async (): Promise<void> => {
    ...
    try {
      ...
      // 관리자 이용 정지가 남아 있으면 정지 안내로, 아니면 정상 서비스 홈으로 이동
  ...
  // 영구 삭제 대기 상태를 조회하는 동안 간단한 로딩 화면을 반환
  if (isLoading) {
    // 삭제 예정일 조회 중 안내 화면을 반환
    // "탈퇴 상태를 확인하고 있어요."
