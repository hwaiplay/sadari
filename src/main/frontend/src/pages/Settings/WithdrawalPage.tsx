...
/**
 * 계정 비활성화와 영구 탈퇴 정책을 비교해 선택하고 Kakao 재인증을 시작
 *
 * @author HanWon.Jang
...
function WithdrawalPage({ hardOnly = false }: { hardOnly?: boolean }) {
  ...
  const handlePolicyHelpOpen = useCallback((): void => {
    ...
    // 이전 닫기 상태를 제거해 도움말 팝업을 열림 애니메이션으로 시작
    policyHelpClosingRef.current = false;
    // 화면에 남아 있는 닫기 애니메이션 상태를 제거
    setIsPolicyHelpClosing(false);
    // 계정 처리 정책 도움말을 화면에 표시
    setIsPolicyHelpOpen(true);
  }, []);

  /**
   * 닫기 애니메이션이 끝난 계정 처리 정책 도움말을 화면에서 제거
   *
   * @author HanWon.Jang
   * @return 반환값 없음
   */
  const completePolicyHelpClose = useCallback((): void => {
    ...
    // 닫기 애니메이션이 끝난 도움말 팝업을 화면에서 제거
    setIsPolicyHelpOpen(false);
    // 다음 열기 동작에 닫기 스타일이 남지 않도록 상태를 초기화
    setIsPolicyHelpClosing(false);
    // 다음 열기 동작에서 닫기 요청을 다시 받을 수 있도록 진행 상태를 초기화
    policyHelpClosingRef.current = false;
    // 완료된 닫기 타이머 참조를 제거
  ...
  const handlePolicyHelpClose = useCallback((): void => {
    ...
    if (policyHelpClosingRef.current) {
      // 진행 중인 닫기 애니메이션을 유지하고 처리를 종료
      return;
    }

    // 상태 갱신 전 연속 입력도 차단할 수 있도록 닫기 진행 여부를 즉시 기록
    policyHelpClosingRef.current = true;
    // 도움말 팝업과 배경에 닫기 애니메이션을 적용
    setIsPolicyHelpClosing(true);
    // 닫기 애니메이션이 끝난 뒤 팝업을 화면에서 제거
  ...
  /**
   * 계정 처리 정책 도움말이 열린 동안 배경 스크롤과 Escape 키 동작을 관리
   *
   * @author HanWon.Jang
  ...
  const syncPolicyHelpModal = useCallback((): (() => void) | undefined => {
    ...
    lockBodyScroll();
    // 키보드 사용자가 닫기 버튼부터 조작할 수 있도록 포커스를 이동
    policyCloseButtonRef.current?.focus();
    // 키보드로 도움말을 닫을 수 있도록 Escape 키 이벤트를 등록
    window.addEventListener("keydown", handlePolicyHelpKeyDown);

    /**
     * 도움말이 닫히거나 페이지가 해제될 때 브라우저 상태를 복구
     *
     * @author HanWon.Jang
     * @return 반환값 없음
     */
    const cleanupPolicyHelpModal = (): void => {
      ...
      // 페이지 이동 중 남아 있는 닫기 타이머가 상태를 변경하지 않도록 해제
      ...
      // 도움말이 닫힌 뒤 기존 화면을 다시 스크롤할 수 있게 복구
      unlockBodyScroll();
      // 중복 키보드 처리를 막기 위해 Escape 키 이벤트를 해제
      window.removeEventListener("keydown", handlePolicyHelpKeyDown);
    };

    // 팝업 종료 시 실행할 브라우저 상태 정리 함수를 반환
    return cleanupPolicyHelpModal;
  }, [handlePolicyHelpClose, isPolicyHelpOpen]);

  // 계정 처리 정책 도움말의 열림 상태에 맞춰 스크롤과 키보드 동작을 동기화
  useEffect(syncPolicyHelpModal, [syncPolicyHelpModal]);

  /**
   * 비활성화 및 탈퇴 사유 입력을 UTF-8 최대 저장 바이트 안에서 화면 상태에 반영
   *
   * @author HanWon.Jang
  ...
  const handleWithdrawalReason = (event: React.ChangeEvent<HTMLTextAreaElement>): void => {
    ...
    // 다중 바이트 문자를 포함한 입력을 500바이트 안으로 제한
    const limitedReason = truncateWithdrawalReason(event.target.value);
    // 검증된 비활성화 및 탈퇴 사유를 입력 영역에 반영
    setRsonCntn(limitedReason);
  };

  /**
   * 선택한 계정 처리 정책을 확인하고 Kakao 재인증 화면으로 이동
   *
   * @author HanWon.Jang
   * @return 반환값 없음
   */
  const handleWithdrawal = async (): Promise<void> => {
    ...
    // 비활성화는 재로그인 복구 범위와 자동 복원되지 않는 데이터를 다시 안내
    if (wthdType === "SOFT") {
      // "계정을 비활성화하시겠어요?"
    ...
    // 영구 탈퇴는 유예기간과 유예기간 안의 취소 가능 여부를 다시 안내
    else {
      // "영구 탈퇴를 신청하시겠어요?"
    ...
    // 계정 처리 유형별 영향을 확인한 사용자만 Kakao 재인증을 진행
    ...
    // API 실패를 사용자 안내로 전환하기 위한 비동기 처리 블록
    try {
      // 입력한 계정 처리 유형과 사유로 Kakao 재인증 URL을 요청
    ...
    // 재인증 화면으로 이동하지 못한 경우 버튼을 다시 활성화
    finally {
      // 계정 처리 요청 진행 상태를 해제
  ...
  /**
   * 관리자 설정 또는 기본 계정 처리 정책 문구를 목록 항목으로 표시
   *
   * @author HanWon.Jang
  ...
  const renderPolicyItem = (policyItem: string): React.ReactNode => {
    // 개별 정책 문구를 안정적인 문자열 key와 함께 목록 항목으로 반환
    return <li key={policyItem}>{policyItem}</li>;
  };

  // 계정 처리 정책과 사유를 한 화면에서 선택하는 설정 화면을 반환
  return (
