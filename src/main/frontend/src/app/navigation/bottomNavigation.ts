...
// 화면에 표시되는 하단 내비게이션의 좌우 순서를 단일 기준으로 관리한다
...
/**
 * 현재 경로가 속한 하단 내비게이션 탭의 표시 순서를 조회한다
 *
 * @author HanWon.Jang
...
export const getBottomNavIndex = (pathname: string): number | null => {
  ...
  // 마이페이지 루트와 하위 화면은 다섯 번째 탭으로 판정한다
  if (pathname === "/mypage" || pathname.startsWith("/mypage/")) {
    // 마이페이지 탭의 표시 순서를 반환한다
    return BOTTOM_NAV_ORDER.indexOf(BOTTOM_NAV_PATH.myPage);
  }

  // 하단 탭에 속하지 않는 화면은 기존 라우터 전환을 사용하도록 null을 반환한다
  return null;
};

/**
 * 하단 내비게이션 탭 사이의 이동 순서를 위치 상태로 생성한다
 *
 * @author HanWon.Jang
...
export const getBottomNavState = (fromPath: string, toPath: string): BottomNavState | null => {
  ...
  // 출발 화면이 속한 하단 탭 순서를 조회한다
  const fromIndex = getBottomNavIndex(fromPath);
  // 도착 화면이 속한 하단 탭 순서를 조회한다
  const toIndex = getBottomNavIndex(toPath);

  // 탭 외부 화면 이동과 같은 탭 재선택은 기존 페이지 전환을 유지한다
  if (fromIndex === null || toIndex === null || fromIndex === toIndex) {
    // 하단 탭 전용 방향을 적용하지 않도록 null을 반환한다
    return null;
  }

  // 하단 탭의 출발 및 도착 순서를 포함한 일회성 위치 상태를 반환한다
  return {
    bottomNavTransition: {
      fromIndex,
      toIndex,
...
/**
 * 현재 위치 상태를 검증하여 하단 탭 전용 화면 진입 방향을 결정한다
 *
 * @author HanWon.Jang
...
export const getBottomNavDirection = (state: unknown, pathname: string, isHistoryPop: boolean): BottomNavDirection | null => {
  ...
  // 브라우저 앞뒤 이동에서는 이력에 저장된 과거 탭 전환 상태를 재사용하지 않는다
  if (isHistoryPop) {
    // 기존 POP 전환 방향을 유지하도록 null을 반환한다
    return null;
  }

  // 객체가 아닌 외부 위치 상태는 하단 탭 이동 정보로 사용하지 않는다
  if (typeof state !== "object" || state === null
      || !("bottomNavTransition" in state)) {
    // 유효한 하단 탭 이동 상태가 없어 null을 반환한다
    return null;
  }

  const transition = state.bottomNavTransition;

  // 출발 및 도착 순서가 숫자인 위치 상태만 방향 계산에 사용한다
  if (typeof transition !== "object" || transition === null
      || !("fromIndex" in transition) || !("toIndex" in transition)
      || typeof transition.fromIndex !== "number"
      || typeof transition.toIndex !== "number") {
    // 손상되거나 임의로 조작된 위치 상태를 적용하지 않도록 null을 반환한다
    return null;
  }
