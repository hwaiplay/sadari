...
// 화면에 표시되는 하단 내비게이션의 좌우 순서를 단일 기준으로 관리
...
/**
 * 현재 경로가 속한 하단 내비게이션 탭의 표시 순서를 조회
 *
 * @author HanWon.Jang
...
export const getBottomNavIndex = (pathname: string): number | null => {
  ...
  // 마이페이지 루트와 하위 화면은 다섯 번째 탭으로 판정
  if (pathname === "/mypage" || pathname.startsWith("/mypage/")) {
    // 마이페이지 탭의 표시 순서를 반환
    return BOTTOM_NAV_ORDER.indexOf(BOTTOM_NAV_PATH.myPage);
  }

  // 하단 탭에 속하지 않는 화면은 기존 라우터 전환을 사용하도록 null을 반환
  return null;
};

/**
 * 하단 내비게이션 탭 사이의 이동 순서를 위치 상태로 생성
 *
 * @author HanWon.Jang
...
export const getBottomNavState = (fromPath: string, toPath: string): BottomNavState | null => {
  ...
  // 출발 화면이 속한 하단 탭 순서를 조회
  const fromIndex = getBottomNavIndex(fromPath);
  // 도착 화면이 속한 하단 탭 순서를 조회
  const toIndex = getBottomNavIndex(toPath);

  // 탭 외부 화면 이동과 같은 탭 재선택은 기존 페이지 전환을 유지
  if (fromIndex === null || toIndex === null || fromIndex === toIndex) {
    // 하단 탭 전용 방향을 적용하지 않도록 null을 반환
    return null;
  }

  // 하단 탭의 출발 및 도착 순서를 포함한 일회성 위치 상태를 반환
  return {
    bottomNavTransition: {
      fromIndex,
      toIndex,
...
/**
 * 현재 위치 상태를 검증하여 하단 탭 전용 화면 진입 방향을 결정
 *
 * @author HanWon.Jang
