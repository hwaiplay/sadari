...
/**
 * 현재 History State를 보존하면서 이동 차단 표식을 추가할 객체로 변환
 *
 * @author SeungHyeon.Kang
 * @return 현재 History State의 복사본
 */
function getHistoryState(): Record<string, unknown> {
  // React Router의 현재 History State가 객체이면 기존 위치 정보를 유지
  if (typeof window.history.state === "object" && window.history.state !== null) {
    // 기존 History State를 복사하여 이동 차단 표식의 기반값으로 반환
    return { ...window.history.state } as Record<string, unknown>;
  }

  // 저장된 History State가 없으면 이동 차단 표식만 담을 빈 객체를 반환
  return {};
}

/**
 * History State에서 Sadari 이동 차단 표식을 타입 안전하게 조회
 *
 * @author SeungHyeon.Kang
...
function getNavigationGuardMarker(state: unknown): NavigationGuardMarker | null {
  ...
  // 식별자와 History 항목 구분값이 모두 유효할 때만 이동 차단 표식으로 인정
  if (typeof markerId !== "string"
      || (markerEntry !== "base" && markerEntry !== "sentinel")) {
    // 유효하지 않은 이동 차단 표식을 무시하도록 null을 반환
    return null;
  }

  // 검증된 이동 차단 식별자와 History 항목 구분값을 반환
  return {
    id: markerId,
    entry: markerEntry,
  };
...
/**
 * 새로운 이동 차단 History 쌍을 구분할 식별자를 생성
 *
 * @author SeungHyeon.Kang
...
function createNavigationGuardId(): string {
  ...
  // 페이지 재시작 이후에도 기존 History 표식과 충돌하지 않도록 시간과 순번을 조합
  return `${Date.now()}-${nextNavigationGuardId}`;
}

/**
 * 현재 History 항목을 이동 차단 기준 항목으로 표시
 *
 * @author SeungHyeon.Kang
...
function pushNavGuardSentinel(guardId: string): void {
  ...
  // 저장 중 뒤로가기가 실제 이전 화면에 도달하기 전에 같은 URL의 차단 항목을 만나도록 추가
  window.history.pushState(guardState, "", window.location.href);
  // pushState는 popstate를 발생시키지 않으므로 현재 차단 항목을 직접 관찰값으로 기록
  lastObservedNavigationGuardMarker = getNavigationGuardMarker(guardState);
}

/**
 * 현재 History State에서 이동 차단 표식만 제거
 *
 * @author SeungHyeon.Kang
...
const removeNavGuardMarker = (state: Record<string, unknown>): Record<string, unknown> => {
  ...
  // 저장 가드만 제거해 홈 종료 가드와 React Router 인덱스는 원래 값으로 유지
  delete cleanedState[NAVIGATION_GUARD_STATE_KEY];
  // 임시 저장 가드가 제거된 History State를 반환
  return cleanedState;
};

/**
 * 홈 루트 정리가 저장 가드의 기준 항목에 도착해도 앱 외부로 추가 이동하지 않게 표시
 *
 * @author SeungHyeon.Kang
 * @return 반환값이 없음
 */
...
function handleBeforeUnload(event: BeforeUnloadEvent): void {
  // 서버 처리 결과가 확정되기 전에 문서가 종료되지 않도록 기본 이탈 동작을 취소
  event.preventDefault();
  // 브라우저별 기본 이탈 확인 절차가 실행되도록 반환값을 설정
  event.returnValue = "";
}

/**
 * History 이동 방향과 저장 진행 상태에 따라 실제 화면 이탈이나 내부 중복 항목을 처리
 *
 * @author SeungHyeon.Kang
...
function handlePopState(event: PopStateEvent): void {
  ...
  // 이후 History 이동 방향을 비교할 수 있도록 이동 대상의 표식을 관찰값으로 기록
  lastObservedNavigationGuardMarker = currentMarker;

  // 저장 완료 정리로 기준 항목에 도착하면 현재 항목의 임시 표식만 제거하고 작업을 완료
  if (pendingNavigationGuardCleanup !== null) {
    ...
    // 예상한 기준 항목이면 Router 인덱스와 PWA 종료 가드를 유지한 채 저장 표식만 제거
    ...
    // 정리 POP이 일반 사용자 뒤로가기로 이어지지 않도록 관찰 상태를 초기화
    lastObservedNavigationGuardMarker = null;
    pendingCleanup.resolve();
    // 같은 POP을 완료된 가드의 추가 뒤로가기로 처리하지 않고 종료
    return;
  }
  ...
  if (isHomeHistoryResetPending && !isNavigationGuardActive) {
    ...
    // 홈 Provider가 도착한 앱 루트 항목을 홈으로 교체하도록 현재 POP 처리를 종료
    return;
  }

  // 저장이 다시 시작된 경우에는 홈 정리 표시를 폐기하고 활성 저장 가드를 우선
  isHomeHistoryResetPending = false;

  // 상태 변경 작업 중 차단 쌍의 기준 항목으로 돌아오면 같은 URL의 차단 항목을 즉시 복원
  if (isNavigationGuardActive && activeNavigationGuardId !== null
      && currentMarker?.id === activeNavigationGuardId && currentMarker.entry === "base") {
    // PWA 스와이프와 하드웨어 뒤로가기가 실제 이전 화면으로 이어지지 않도록 차단 항목을 복원
    pushNavGuardSentinel(activeNavigationGuardId);
    // 저장 중 뒤로가기 처리를 현재 화면에서 종료
    return;
  }

  // 상태 변경 작업 중 예상하지 못한 History 위치로 이동하면 가능한 경우 기존 차단 항목으로 복귀
  if (isNavigationGuardActive) {
    // 사용자가 저장 중인 화면에서 벗어나지 않도록 브라우저의 다음 History 항목으로 복귀
    window.history.forward();
    // 저장 중 예외 History 이동 처리를 종료
    return;
  }
  ...
  if (previousMarker?.entry === "sentinel" && currentMarker?.entry === "base"
    ...
    // 저장 완료 시에는 이동하지 않고 이후 사용자의 뒤로가기 시점에만 실제 이전 화면으로 이동
    window.history.back();
  }
}

/**
 * 저장 완료 뒤에도 남은 내부 History 쌍을 처리할 애플리케이션 수명 이벤트를 한 번 등록
 *
 * @author SeungHyeon.Kang
 * @return 반환값이 없음
 */
function ensurePopStateListener(): void {
  ...
  if (isPopStateListenerRegistered) {
    // 기존 이벤트가 이후 History 이동도 처리하도록 등록 절차를 종료
    return;
  }

  // 저장 완료 후 사용자가 실제 뒤로갈 때 내부 기준 항목을 건너뛸 수 있도록 계속 감시
  window.addEventListener("popstate", handlePopState);
  isPopStateListenerRegistered = true;
}

/**
 * 상태 변경 작업 동안 현재 화면을 유지할 History 가드를 활성화
 *
 * @author SeungHyeon.Kang
 * @return 반환값이 없음
 */
function activateNavigationGuard(): void {
  // 이미 활성화된 가드는 중첩 요청이 완료될 때까지 그대로 유지
  if (isNavigationGuardActive) {
    // 중복 History 항목을 만들지 않고 활성화 처리를 종료
    return;
  }

  // 저장 완료 후 남은 History 항목도 처리할 공통 감시 이벤트를 준비
  ensurePopStateListener();

  const currentMarker = getNavigationGuardMarker(window.history.state);

  // 현재 항목이 이전 작업에서 만든 차단 항목이면 새 중복 항목 없이 재사용
  if (currentMarker?.entry === "sentinel") {
    activeNavigationGuardId = currentMarker.id;
    // 현재 차단 항목부터 이동 방향을 추적하도록 관찰값을 갱신
    lastObservedNavigationGuardMarker = currentMarker;
  }

  // 현재 항목을 재사용할 수 없으면 기준 항목과 차단 항목을 새로 구성
  else {
    const guardId = createNavigationGuardId();
    activeNavigationGuardId = guardId;
    // 현재 React Router 위치를 이동 차단 History 쌍의 기준 항목으로 표시
    replaceNavGuardBase(guardId);
    // 사용자의 뒤로가기를 받을 같은 URL의 차단 항목을 추가
    pushNavGuardSentinel(guardId);
  }

  isNavigationGuardActive = true;
  // 새로고침과 창 닫기는 브라우저가 제공하는 기본 이탈 확인 절차로 보호
  window.addEventListener("beforeunload", handleBeforeUnload);
}

/**
 * 상태 변경 완료 후 동일 URL의 임시 History 항목을 제거하고 이동 차단 상태를 해제
 *
 * @author SeungHyeon.Kang
 * @return 반환값이 없음
 */
async function deactivateNavigationGuard(): Promise<void> {
  ...
  if (!isNavigationGuardActive) {
    // 저장 완료 후 중복 해제 요청을 완료
    return;
  }

  const completedGuardId = activeNavigationGuardId;
  const currentMarker = getNavigationGuardMarker(window.history.state);
  // 저장 결과가 확정되었으므로 정리 POP이 활성 차단으로 다시 복원되지 않게 먼저 비활성화
  isNavigationGuardActive = false;
  activeNavigationGuardId = null;
  // 저장 결과가 확정되었으므로 새로고침과 창 닫기 보호 이벤트를 해제
  ...
  if (completedGuardId === null || currentMarker?.id !== completedGuardId
    ...
    // 정리할 동일 URL 쌍이 없으므로 현재 화면을 유지
    return;
  }

  // 앞으로가기에 남을 임시 항목이 이후 가드로 오인되지 않도록 현재 표식을 먼저 제거
  ...
  await new Promise<void>((resolve) => {
    ...
    // 저장 가드가 만든 한 항목만 제거해 React Router의 원래 인덱스로 복귀
    window.history.back();
  });
}

/**
 * 상태 변경 작업의 처리 중 모달과 화면 이동 차단을 시작
 *
 * @author SeungHyeon.Kang
...
export function beginBlockingOperation(options: BlockingOperationOptions = {}): number {
  ...
  // 동시에 실행되는 상태 변경 요청을 개별적으로 완료 처리할 수 있도록 식별값을 등록
  activeOperationIds.add(operationId);

  // 최초 작업에서만 모달과 History 가드를 생성하여 중첩 요청의 화면 중복을 막음
  if (isFirstOperation) {
    // 작업 완료 신호로만 닫히는 처리 중 모달의 제어 객체를 생성
    modalAbortController = new AbortController();
    // 상태 변경 응답이 확정될 때까지 현재 화면의 이동을 차단
  ...
  // 호출부가 작업 완료 후 정확한 요청만 해제하도록 식별값을 반환
  return operationId;
}

/**
 * 상태 변경 작업을 완료하고 마지막 요청이면 처리 중 모달과 이동 차단을 해제
 *
 * @author SeungHyeon.Kang
...
export async function endBlockingOperation(
  ...
  if (!activeOperationIds.delete(operationId)) {
    // 중복 해제 요청을 추가 화면 변경 없이 종료
    return;
  }

  // 다른 상태 변경 작업이 남아 있으면 공통 모달과 이동 가드를 유지
  if (activeOperationIds.size > 0) {
    // 마지막 요청이 완료될 때까지 현재 처리 중 화면을 유지하고 종료
    return;
  }

  const completedModalPromise = modalResultPromise;
  // 성공 또는 실패 알림이 열리기 전에 동일 URL의 임시 이력과 논리 가드를 함께 해제
  await deactivateNavigationGuard();
  // 성공 정보를 지정한 작업은 현재 DOM을 유지한 채 로딩 상태를 성공 상태로 전환
  ...
  // 성공 전환을 요청한 호출부는 사용자가 같은 모달을 확인한 뒤 후속 처리를 실행하도록 처리
