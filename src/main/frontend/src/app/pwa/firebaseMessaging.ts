...
/**
 * 사용자의 버튼 클릭 흐름 안에서 브라우저 알림 권한을 요청
 *
 * Notification.requestPermission()은 사용자의 직접 동작과 분리되면 브라우저가
 * 권한 팝업을 차단할 수 있으므로 Firebase 설정 조회보다 먼저 호출해야 하는 조건
 *
 * @author HanWon.Jang
 * @return 허용된 알림 권한
 */
export async function requestPushPermission() {
  ...
  // 이미 허용된 브라우저에서는 팝업을 다시 띄우지 않고 다음 토큰 발급 단계로 진행
  if (Notification.permission === "granted") {
    return Notification.permission;
  }

  // 사용자가 한 번 차단한 권한은 코드로 다시 요청할 수 없으므로 사이트 설정 변경을 안내
...
async function waitServiceWorkerReady() {
  ...
  // 브라우저의 window.setTimeout 반환값은 number
  // 프로젝트에 @types/node도 포함되어 있어 ReturnType을 쓰면 NodeJS.Timeout으로 잘못 추론될 수 있으므로 명시
...
/**
 * 서버 설정 DTO를 Firebase Web SDK 초기화 옵션으로 변환
...
/**
 * Firebase app은 한 번만 초기화해야 하므로 이미 초기화된 app이 있으면 재사용
 *
 * @author HanWon.Jang
 * @param config 서버에서 받은 Firebase Web 설정
 * @return Firebase app
...
/**
 * 브라우저 알림 권한을 요청하고 FCM registration token을 발급
 * 권한 거부, 미지원 브라우저, service worker 미준비 상태는 호출부에서 사용자에게 안내할 수 있도록 Error로 반환
 *
 * @author HanWon.Jang
 * @param config 서버에서 받은 Firebase Web 설정
...
/**
 * 앱이 열린 상태에서 도착한 FCM 메시지를 구독
 *
 * @param config 서버에서 받은 Firebase Web 설정
 * @param listener 포그라운드 메시지 수신 콜백
 * @return 구독 해제 함수. 미지원 환경에서는 아무 작업도 하지 않는 함수를 반환
 */
