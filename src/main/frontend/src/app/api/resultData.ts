...
export const ACCESS_REJECTED_CODE = 2020;
...
/**
 * assert Result Data Success 기능을 처리
 *
 * @author HanWon.Jang
 * @param result result 입력값
 * @return 처리 결과
...
export const getApiErrorMessage = (error: unknown, fallbackMessage: string): string => {
  // 공통 응답 검증에서 확인한 업무 실패는 서버가 제공한 메시지를 우선 사용
  if (error instanceof ResultDataError) {
    // 비어 있는 서버 메시지는 화면별 공통 오류 문구로 보정
    return error.message || fallbackMessage;
  }

  // Axios 응답은 서버가 명시한 공통 실패 코드와 메시지를 기준으로 안내
  if (isAxiosError<ResultData>(error)) {
    const resultCode = Number(error.response?.data?.code);

    // 서버가 JDBC 연결 실패로 확정한 경우에만 데이터베이스 전용 문구를 사용
    ...
    // 일반 5xx와 타임아웃 및 네트워크 오류에는 서버 메시지나 화면별 대체 문구를 사용
    return error.response?.data?.message ?? fallbackMessage;
  }

  // API 응답으로 분류할 수 없는 오류에는 화면별 공통 오류 문구를 사용
