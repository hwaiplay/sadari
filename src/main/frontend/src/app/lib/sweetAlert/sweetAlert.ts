...
/**
 * 처리 중 모달 종료 사유에서 같은 모달에 표시할 성공 정보를 조회
 *
...
/**
 * 처리 중 모달의 DOM을 유지한 채 성공 상태로 전환하도록 완료 신호를 전달
 *
 * @author SeungHyeon.Kang
...
export function completeSweetBlockingOperation(
  ...
  // 일반 종료와 구분되는 내부 표식을 사유에 담아 같은 모달의 성공 전환을 요청
  controller.abort({
...
/**
 * SweetAlert 모달에 필요한 스타일 태그를 한 번만 주입
 *
 * @author HanWon.Jang
 * @return
 */
...
/**
 * SweetAlert 모달을 닫고 body 스크롤 상태를 복구
 *
 * @author HanWon.Jang
...
/**
 * 처리 중 모달을 열기 전에 예정된 성공 알림의 실제 높이를 측정해 같은 크기를 확보
 *
 * @author SeungHyeon.Kang
...
function reserveSweetBlockingCompletionHeight(
  ...
  // 성공 상태의 아이콘과 제목으로 바꿔 글자 줄바꿈까지 포함한 높이를 계산
  ...
  // 성공 본문이 있으면 처리 중 본문 위치에서 실제 성공 문구 높이를 측정
  if (completion.text) {
  ...
  // 첫 화면이 그려지기 전에 성공 알림의 실제 반응형 높이를 브라우저에서 계산
  ...
  // 로딩 내용이 더 길지 않은 일반 흐름에서는 처음부터 성공 알림과 같은 높이를 유지
  modal.style.minHeight = `${completionHeight}px`;
}

/**
 * 제목, 본문, 아이콘, 확인/취소 버튼 옵션으로 커스텀 알림 모달을 표시
 *
 * @author HanWon.Jang
...
export const sweetAlert = (options: SweetAlertOptions) => {
  ...
  return new Promise<SweetAlertResult>((resolve) => {
    ...
    // 호출 전에 작업이 끝난 안내 모달은 화면에 추가하지 않고 닫힘 결과로 완료
    if (options.closeSignal?.aborted) {
      // 이미 종료된 작업의 모달 Promise를 닫힘 상태로 완료
      resolve({
        isConfirmed: false,
        isDenied: false,
      ...
      // 종료된 작업의 DOM 생성을 차단하도록 모달 처리를 종료
      return;
    }
    ...
    const close = (result: SweetAlertResult) => {
      ...
      // 외부 작업 종료 신호 구독이 남아 다른 알림까지 닫지 않도록 현재 모달의 구독을 제거
      if (closeSignalHandler && options.closeSignal) {
        // 현재 모달에 연결한 작업 종료 이벤트만 해제
        options.closeSignal.removeEventListener("abort", closeSignalHandler);
      }

      // 닫힌 알림이 이후 뒤로가기까지 구독하지 않도록 현재 알림의 이동 이벤트를 해제
      if (popStateHandler) {
        // 현재 알림에 연결한 뒤로가기 이벤트만 해제
    ...
    popStateHandler = (): void => {
      // 뒤로가기 후 이전 화면 위에 알림이 남지 않도록 닫힘 상태로 완료
      close({
        isConfirmed: false,
        isDenied: false,
        isSecondaryAction: false,
    ...
    if (!options.closeSignal) {
      // 현재 일반 알림이 열린 동안 브라우저 뒤로가기 이벤트를 구독
    ...
    /**
     * 우측 상단 닫기 버튼을 기존 취소 선택과 같은 결과로 처리
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const handleCloseButtonClick = (): void => {
      // 사용자의 닫기 선택으로 현재 알림을 취소 완료
      close({
        isConfirmed: false,
        isDenied: false,
    ...
    // 하단 취소 버튼 대신 우측 상단 닫기 버튼을 요청한 모달에만 제공
    ...
    if (options.content) {
      ...
      // 여러 안내 문구를 각각 독립된 문단으로 제공해 항목별 스타일과 접근성을 유지
    ...
    // 취소 또는 보조 동작이 필요한 모달에만 취소 버튼을 제공
    ...
    // 현재 동작과 구분되는 두 번째 확정 선택이 필요할 때 거부 버튼을 제공
    ...
    /**
     * 하단 확인 버튼 선택으로 현재 알림 모달을 완료
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const handleConfirmButtonClick = (): void => {
      // 사용자의 확인 선택으로 현재 알림을 완료
      close({
        isConfirmed: true,
        isDenied: false,
        isSecondaryAction: false,
    ...
    /**
     * 처리 중 모달의 기존 DOM을 성공 아이콘과 완료 문구 및 확인 버튼으로 교체
     *
     * @author SeungHyeon.Kang
     * @param completion 성공 제목과 선택 본문
     * @return 반환값이 없음
    ...
    const transitionToSuccess = (
      ...
      iconType = "success";
      // 보조기기가 처리 완료 상태와 바뀐 제목을 다시 안내할 수 있도록 상태 속성을 갱신
      modal.setAttribute("aria-busy", "false");
      modal.setAttribute("aria-live", "polite");
      // 같은 아이콘 엘리먼트의 클래스를 변경해 로딩 링을 성공 표시로 전환
      icon.className = `sadari-swal-icon ${ICON_CLASS[iconType]}`;
      iconLabel.textContent = ICON_LABEL[iconType];
      ...
      // 완료 본문이 있으면 기존 처리 중 본문을 같은 위치에서 교체
      if (completion.text) {
        ...
        // 처리 중 모달에 본문이 없었던 경우 제목 다음 위치에 완료 본문을 추가
        if (!processingText) {
          title.insertAdjacentElement("afterend", completionText);
        }
      } else {
        // 완료 본문을 지정하지 않았으면 처리 중 안내 문구를 제거
        processingText?.remove();
      }

      // 처리 중에는 없던 확인 버튼을 성공 상태에서 하나만 제공
      actions.replaceChildren();
      confirmButton = createConfirmButton();
      actions.appendChild(confirmButton);

      // 버튼이 없어서 붙이지 않았던 작업 영역을 같은 모달 하단에 추가
      if (!actions.isConnected) {
        modal.appendChild(actions);
      }

      // 크기는 처음부터 확보되어 있으므로 내용 전환 직후 확인 버튼으로 조작 위치를 이동
    ...
    /**
     * 비동기 작업 종료 사유에 따라 처리 중 모달을 닫거나 같은 모달에서 성공 상태로 전환
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    closeSignalHandler = () => {
      ...
      if (completion) {
        ...
        // 성공 모달은 사용자가 확인할 때까지 유지하도록 일반 닫기 처리를 종료
        return;
      }
    ...
    // 작업 종료 신호가 제공되면 현재 모달만 닫거나 완료 상태로 전환하도록 구독
    if (options.closeSignal) {
      // 작업이 끝나는 최초 신호만 처리하도록 일회성 이벤트를 등록
    ...
    if (actions.childElementCount > 0) {
      // 사용자가 선택할 수 있는 알림 동작을 모달 하단에 추가
      modal.appendChild(actions);
    }
    ...
    // 완료 문구를 미리 아는 처리 중 알림은 첫 렌더링부터 성공 알림 높이를 확보
    ...
    // 사용 가능한 기본 동작이 있으면 해당 버튼에 포커스를 두고 버튼이 없으면 모달 본문을 선택
    if (confirmButton) {
      // 일반 알림의 기본 확인 동작에 포커스를 설정
      confirmButton.focus();
    } else {
      // 버튼 없는 처리 중 모달은 모달 본문에 포커스를 두어 배경 화면 조작을 차단
      modal.focus();
    }
  });
};
...
/**
 * 상태 변경 작업이 끝날 때까지 닫을 수 없는 처리 중 모달을 표시
 *
 * @author SeungHyeon.Kang
...
export function sweetBlockingOperation(options: SweetBlockingOperationOptions) {
  ...
  // 사용자가 서버 처리 도중 화면을 이탈하지 않도록 모든 닫기 버튼을 제거한 모달을 반환
  return sweetAlert({
    title: options.title,
    text: options.text,
    icon: "loading",
...
export function sweetConfirm(options: SweetAlertOptions) {
  ...
  // 호출 화면이 취소 버튼 노출 여부를 지정하면 해당 설정을 유지
  return sweetAlert({
    icon: "question",
    // "확인"
...
/**
 * 수정 가능한 예시 요소와 클릭 강조 애니메이션을 포함한 안내 모달을 표시
 *
 * @author HanWon.Jang
...
export function sweetEditGuide(
  ...
  // 번역된 문구를 HTML 문자열로 해석하지 않도록 안내 콘텐츠를 DOM 엘리먼트로 구성
  const content = document.createElement("div");
  ...
  // 클릭 가능한 요소의 시각적 예시를 포함한 안내 모달을 반환
  return sweetAlert({
    title,
    icon: "info",
  });
