...
const ProtectedRoute = ({ children }: { children: ReactNode }) => {
  ...
  // 영구 삭제 대기 회원은 일반 서비스 화면 대신 취소 전용 화면으로 이동
  if (isDeletePending) {
    if (location.pathname !== "/withdrawal/pending") {
      // 영구 삭제 예정일과 취소 버튼만 제공하는 화면으로 이동
      return <Navigate to="/withdrawal/pending" replace />;
    }

    // 영구 삭제 대기 상태는 온보딩 여부보다 우선해 전용 화면만 반환
    return children;
  }
  ...
  if (isSuspended) {
    ...
    // 관리자 정지 상태는 온보딩 여부보다 우선해 제한된 전용 화면만 반환
    return children;
  }

  // 온보딩을 완료하지 않은 신규 회원은 일반 서비스 화면보다 웰컴 화면을 먼저 확인
  if (isOnboardingRequired && location.pathname !== "/welcome") {
    // 닉네임을 확정할 수 있는 최초 로그인 화면으로 이동
    return <Navigate to="/welcome" replace />;
  }

  // 온보딩을 이미 완료한 사용자가 웰컴 경로를 직접 열면 홈으로 이동
  if (!isOnboardingRequired && location.pathname === "/welcome") {
    // 완료된 웰컴 화면이 다시 노출되지 않도록 홈 화면으로 이동
    return <Navigate to="/home" replace />;
  }

  // 활성 사용자의 신고 결과 전용 팝업과 보호 화면을 함께 반환
  return (
    <>
