...
/**
 * 인기 검색어 한 건씩 세로로 교체하고 선택한 검색어를 즉시 조회한다
 *
...
export function PopularKeywordSlider({
  ...
  const setKeywordSlideTimer = useCallback((): (() => void) | undefined => {
    ...
    /**
     * 인기 검색어가 바뀌거나 컴포넌트가 해제될 때 기존 교체 예약을 취소한다
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없다
     */
    function clearKeywordSlideTimer(): void {
      // 해제된 화면에서 검색어 상태를 변경하지 않도록 예약을 제거한다
      window.clearTimeout(timerId);
    }

    // 다음 Effect 실행 전에 현재 검색어 교체 예약을 정리할 함수를 반환한다
    return clearKeywordSlideTimer;
  }, [handleKeywordAdvance, isPaused, keywordList.length]);

  // 활성 검색어와 사용자 조작 상태에 맞는 다음 교체 예약을 관리한다
  useEffect(setKeywordSlideTimer, [setKeywordSlideTimer]);

  /**
   * 새 인기 검색어 목록을 받으면 유효한 첫 번째 순위부터 다시 표시한다
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없다
   */
  const resetActiveKeyword = useCallback((): void => {
    // 이전 목록 순번이 새 목록 범위를 벗어나지 않도록 첫 항목으로 초기화한다
    setActiveIndex(0);
  }, [keywordList]);

  // 서버에서 인기 검색어 목록이 바뀌면 첫 번째 검색어로 표시 순서를 초기화한다
  useEffect(resetActiveKeyword, [resetActiveKeyword]);

  /**
   * 마우스와 키보드로 현재 검색어를 조작하는 동안 자동 교체를 멈춘다
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없다
   */
  function handleKeywordPause(): void {
    // 사용자가 읽고 선택할 현재 검색어를 고정한다
    setIsPaused(true);
  }

  /**
   * 현재 검색어 조작이 끝나면 다음 항목 자동 교체를 재개한다
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없다
   */
  function handleKeywordResume(): void {
    // 전체 표시 간격을 다시 제공한 뒤 다음 검색어가 나오도록 재개 상태를 설정한다
    setIsPaused(false);
  }

  /**
   * 현재 표시 중인 인기 검색어를 검색 입력과 첫 페이지 조회에 전달한다
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없다
   */
  function handleKeywordSelect(): void {
    // 검색 중이거나 표시할 검색어가 없으면 중복 검색을 실행하지 않는다
    if (isDisabled || activeKeyword === undefined) {
      // 현재 검색 처리 또는 빈 인기 검색어 상태를 유지한다
      return;
    }

    // 클릭하거나 터치한 인기 검색어로 즉시 첫 페이지 검색을 실행한다
    void onSelect(activeKeyword.keyword);
  }

  // 인기 검색어가 없으면 입력창과 인기 도서 선택 사이의 기존 여백만 유지한다
  if (activeKeyword === undefined) {
    // 빈 인기 검색어 영역을 렌더링하지 않는다
    return null;
  }
  ...
  // 현재 순위의 인기 검색어 한 건과 즉시 검색 동작을 제공하는 세로 슬라이더를 반환한다
  return (
    <section
