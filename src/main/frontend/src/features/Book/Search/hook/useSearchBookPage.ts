...
function saveSearchCache(cache: SearchBookCache): void {
  ...
  // 상세 화면에서 돌아올 때 검색 결과를 복원할 수 있도록 저장
  sessionStorage.setItem(SEARCH_STORAGE_KEY, JSON.stringify(cache));
}

/**
 * 책 검색 화면의 조회, 캐시, 더보기와 선택 이동 상태를 관리
 *
 * @author HanWon.Jang
...
export function useSearchBookPage() {
  ...
  /**
   * 검색 화면의 부가 영역에 표시할 안전한 인기 검색어를 독립적으로 조회
   *
   * @author SeungHyeon.Kang
  ...
  const loadPopularKeywords = useCallback(async (): Promise<void> => {
    ...
    // 인기 검색어 장애는 도서 검색과 인기 도서 목록을 막지 않도록 별도 실패 경로로 처리
    try {
      // 최근 고유 회원 검색 수 기준의 안전한 인기 검색어를 조회
      const popularKeywords = await getPopularKeywords();
      // 조회된 인기 검색어를 세로 슬라이더 표시 순서로 설정
      setPopularKeywordList(popularKeywords);
    }

    // 인기 검색어 부가 조회 실패는 사용자 오류 알림 없이 영역만 숨김
    catch {
      // 도서 검색과 인기 도서 목록을 유지하고 인기 검색어만 빈 상태로 전환
      setPopularKeywordList([]);
    }
  }, []);

  /**
   * 검색 화면 진입 시 인기 검색어 부가 조회를 시작
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  ...
  // 화면 진입 시 한 번 인기 검색어 부가 데이터를 조회
  useEffect(startPopularKeywordLoad, [startPopularKeywordLoad]);

  /**
   * 검색 화면 최초 진입과 기간 변경에 표시할 인기 도서를 조회
   *
   * @author SeungHyeon.Kang
   * @param period 주간과 월간 및 연간 중 조회할 인기 도서 기간
  ...
  const loadPopularBooks = useCallback(async (
    ...
    if (popularLoadingPeriodRef.current === period) {
      // 이미 실행 중인 같은 기간의 인기 도서 조회를 유지하고 중복 API 호출을 종료
      return;
    }

    // 완료 전까지 동일 기간의 요청은 막고 다른 기간 선택은 최신 요청으로 허용하도록 진행 상태를 기록
    popularLoadingPeriodRef.current = period;
    // 인기 도서 응답보다 늦게 시작한 직접 검색이 우선하도록 요청 순번을 발급
    const requestId = ++resultRequestIdRef.current;

    try {
      // 페이지 최초 인기 도서 조회 상태를 화면에 반영
      setIsInitialLoading(true);
      // 기간 선택 목록과 인기 도서 카드가 표시되는 초기 화면 모드를 유지
      setIsPopularMode(true);
      // 선택 기간의 독후감 작성자 수 기준 상위 도서를 조회
      ...
      if (requestId !== resultRequestIdRef.current) {
        // 최신 직접 검색이 화면을 갱신하도록 이전 인기 응답 처리를 종료
        return;
      }

      // 인기 도서 최대 10권을 검색 결과와 같은 목록 영역에 설정
      setBookResult(popularBookList);
      // 인기 도서는 최대 10권 전체를 한 번에 노출
      setVisibleCount(popularBookList.length);
      // 인기 도서에는 다음 카카오 검색 위치가 없음을 설정
      setNextStart(null);
      // 인기 도서 목록은 추가 페이지가 없는 종료 상태로 설정
      setIsEnd(true);
      // 기간 선택과 순위 및 작성자 수가 표시되도록 초기 화면 모드를 설정
      setIsPopularMode(true);
    } catch (error) {
      // 인기 도서 조회 뒤 직접 검색이 시작됐으면 이전 요청의 오류를 사용자에게 표시하지 않음
      if (requestId !== resultRequestIdRef.current) {
        // 최신 직접 검색 흐름을 유지하고 오래된 인기 조회 오류 처리를 종료
        return;
      }
      ...
      // 조회 실패 뒤 검색 결과 없음 상태를 화면에 표시
      setBookResult([]);
    } finally {
      // 현재 요청이 마지막으로 시작한 기간 조회이면 다음 선택을 받을 수 있도록 진행 상태를 해제
      if (popularLoadingPeriodRef.current === period) {
        popularLoadingPeriodRef.current = null;
      }
      // 최신 요청인 경우에만 페이지 최초 조회 상태를 해제
      if (requestId === resultRequestIdRef.current) {
        // 성공과 실패에 관계없이 최신 인기 도서 최초 조회 상태를 해제
  ...
  /**
   * 주간과 월간 및 연간 인기 도서 선택을 화면과 조회 결과에 반영
   *
   * @author SeungHyeon.Kang
  ...
  function handlePopularPeriodChange(period: PopularBookPeriodType): void {
    ...
    // 선택한 기간을 표시하고 해당 기간의 인기 도서 목록을 새로 조회
    setPopularPeriod(period);
    void loadPopularBooks(period);
  }

  /**
   * 검색어로 첫 페이지를 조회하고 화면과 세션 캐시를 갱신
   *
   * @author HanWon.Jang
   * @param keyword 검색할 책 제목 또는 저자명
  ...
  const executeBookSearch = useCallback(async (keyword: string): Promise<void> => {
    ...
    // 같은 검색어의 자동 검색과 수동 제출이 겹치면 기존 결과를 유지
    if (lastSearchedKeywordRef.current === keyword) {
      // 이미 조회했거나 조회 중인 검색어의 중복 API 호출을 종료
      return;
    }

    // 검색어가 없으면 필수 입력 안내를 표시
    if (keyword === "") {
      // 검색어 입력이 필요하다는 공통 경고를 표시
      await sweetWarning(
    ...
    // 직접 검색이 진행되면 이전 목록 응답이 결과를 덮어쓰지 못하도록 요청 순번을 갱신
    const requestId = ++resultRequestIdRef.current;
    // 응답 대기 중인 검색어도 중복 요청 대상에서 제외
    lastSearchedKeywordRef.current = keyword;

    try {
      // 첫 페이지 조회 중임을 화면에 반영
      setIsSearching(true);
      // 사용자가 직접 검색하면 인기 도서 최초 로딩 화면을 종료
      setIsInitialLoading(false);
      // 새 조회가 끝나기 전에는 이전 추가 노출 상태를 초기화
      setIsEnd(true);

      // 카카오 API에서 검색 결과의 첫 50권 페이지를 조회
      const responseData = await fetchBooks(keyword, 1);

      // 더 늦게 시작한 검색이 있으면 오래된 검색 응답으로 목록을 덮어쓰지 않음
      if (requestId !== resultRequestIdRef.current) {
        // 최신 검색 요청이 화면을 갱신하도록 이전 응답 처리를 종료
        return;
      }
      ...
      // 조회된 최대 50권 중 첫 10권과 다음 카카오 페이지 상태를 화면에 반영
      ...
      if (requestId !== resultRequestIdRef.current) {
        // 최신 검색 흐름을 유지하고 오래된 요청의 오류 처리를 종료
        return;
      }

      // 실패한 검색어는 검색 버튼이나 다음 입력에서 다시 시도할 수 있도록 해제
      lastSearchedKeywordRef.current = "";
      console.error("도서 검색 중 오류 발생: ", error);
      // 검색 실패 원인을 공통 오류 알림으로 표시
      await sweetError(
        message("frontend.alert.searchFailedTitle"),
        getApiErrorMessage(error, message("frontend.book.search.failed")),
      );
    } finally {
      // 최신 요청인 경우에만 첫 페이지 조회 상태를 해제
      if (requestId === resultRequestIdRef.current) {
        // 성공과 실패에 관계없이 최신 검색의 진행 상태를 해제
  ...
  const clearAutoSearch = useCallback((): void => {
    ...
    // 예약된 자동 검색이 없으면 해제 작업을 생략
    if (autoSearchTimerRef.current === null) {
      // 해제할 타이머가 없는 처리를 종료
      return;
    }

    // 이전 입력에서 예약한 자동 검색 실행을 취소
  ...
  const handleKeywordChange = (event: ChangeEvent<HTMLInputElement>): void => {
    // 입력 중인 검색어를 검색창과 자동 검색 조건에 반영
    setSearchKeyword(event.target.value);
  };

  /**
   * 두 글자 이상 입력이 멈추면 자동 검색을 예약
   *
   * @author HanWon.Jang
  ...
  const startAutoSearch = useCallback((): (() => void) => {
    ...
    // 현재 입력을 다음 변경 시 빈 검색어 전환 여부 판단에 사용
    previousKeywordRef.current = keyword;
    // 입력이 바뀔 때 이전 검색 예약부터 취소
    clearAutoSearch();

    // 검색어를 모두 지우면 기본 인기 도서 화면으로 돌아감
    if (keyword === "") {
      // 빈 초기 화면에서는 이미 실행 중인 인기 도서 조회를 유지
      if (previousKeyword === "") {
        // 검색어 변화가 없는 초기 처리를 종료
        return clearAutoSearch;
      }

      // 진행 중인 직접 검색 응답이 인기 도서 화면을 덮어쓰지 못하게 무효화
      resultRequestIdRef.current += 1;
      // 같은 검색어를 다시 입력하면 새로 조회할 수 있도록 중복 기준을 초기화
      lastSearchedKeywordRef.current = "";
      // 무효화한 직접 검색의 진행 표시를 즉시 해제
      setIsSearching(false);
      // 비운 검색어에 이전 직접 검색 결과가 복원되지 않도록 캐시를 제거
      sessionStorage.removeItem(SEARCH_STORAGE_KEY);
      // 현재 선택된 기간의 인기 도서와 인기 검색어 화면을 복원
      void loadPopularBooks(popularPeriod);
      void loadPopularKeywords();
      // 다음 입력 변경 시 예약된 타이머를 정리할 함수를 반환
      return clearAutoSearch;
    }

    // 두 글자 미만이면 자동 검색 없이 검색 버튼과 Enter만 허용
    if (
      keyword.length < AUTO_SEARCH_MIN_LENGTH
      || lastSearchedKeywordRef.current === keyword
    ) {
      // 자동 검색 조건을 충족하지 않은 처리를 종료
      return clearAutoSearch;
    }

    /**
     * 입력 대기 시간이 지난 검색어로 첫 페이지 자동 검색을 실행
     *
     * @author HanWon.Jang
     * @return 반환값이 없음
     */
    const executeAutoSearch = (): void => {
      // 실행된 타이머 참조를 비우고 현재 검색어 조회를 시작
      autoSearchTimerRef.current = null;
      void executeBookSearch(keyword);
    };

    // 연속 입력마다 대기 시간을 다시 시작해 마지막 문자열만 조회
    autoSearchTimerRef.current = setTimeout(
      executeAutoSearch,
      AUTO_SEARCH_DELAY_MS,
    );

    // 입력 변경이나 화면 해제 시 남은 자동 검색을 취소
    return clearAutoSearch;
  }, [
    clearAutoSearch,
    executeBookSearch,
  ...
  // 검색어와 입력기 조합 상태가 바뀔 때 자동 검색 예약을 갱신
  useEffect(startAutoSearch, [startAutoSearch]);

  /**
   * 클릭하거나 터치한 인기 검색어를 입력창에 반영하고 즉시 첫 페이지를 조회
   *
   * @author SeungHyeon.Kang
  ...
  async function selectPopularKeyword(keyword: string): Promise<void> {
    // 사용자가 선택한 검색어를 입력창에도 동일하게 표시
    setSearchKeyword(keyword);
    // 별도 제출 동작 없이 선택한 검색어로 도서 첫 페이지를 즉시 조회
    await executeBookSearch(keyword);
  }

  /**
   * 클릭하거나 키보드로 선택한 작가명을 입력창에 반영하고 즉시 조회
   *
   * @author SeungHyeon.Kang
  ...
  async function handleAuthorSelect(author: string): Promise<void> {
    ...
    // 선택한 작가명을 검색 입력창에 동일하게 표시
    setSearchKeyword(keyword);
    // 별도 검색 버튼 조작 없이 선택한 작가명으로 도서 첫 페이지를 즉시 조회
    await executeBookSearch(keyword);
  }
  ...
  useEffect(() => {
    ...
    if (!shouldRestoreSearch) {
      sessionStorage.removeItem(SEARCH_STORAGE_KEY);
      // 신규 진입의 빈 화면 대신 기본 월간 인기 도서를 조회
      void loadPopularBooks("monthly");
      // 신규 진입의 인기 도서 조회를 시작하고 캐시 복원 처리를 종료
      return;
    }

    const cached = sessionStorage.getItem(SEARCH_STORAGE_KEY);

    // 복원할 검색 상태가 없으면 현재 빈 화면을 유지
    if (!cached) {
      // 저장된 직접 검색이 없으면 기본 월간 인기 도서를 다시 조회
      void loadPopularBooks("monthly");
      // 인기 도서 조회를 시작하고 빈 캐시 복원 처리를 종료
      return;
    }
    ...
    try {
      ...
      // 복원한 목록은 사용자가 직접 조회한 검색 결과로 표시
      setIsPopularMode(false);
      // 세션 검색 결과 복원이 끝났으므로 페이지 최초 로딩 상태를 해제
      setIsInitialLoading(false);
    } catch {
      // 손상된 검색 캐시는 다음 진입에 재사용되지 않도록 제거
      sessionStorage.removeItem(SEARCH_STORAGE_KEY);
      // 손상된 직접 검색 캐시 대신 기본 월간 인기 도서를 조회
      void loadPopularBooks("monthly");
    }
  }, [executeBookSearch, loadPopularBooks, location.key, location.state, navigationType]);

  /**
   * 검색 폼 제출을 막고 현재 입력된 검색어를 조회
   *
   * @author HanWon.Jang
  ...
  const handleSearchClick = async (
    ...
    // 브라우저의 기본 폼 제출 동작을 중지
    event?.preventDefault();
    // 검색 버튼과 Enter가 실행되면 대기 중인 같은 입력의 자동 검색을 취소
    clearAutoSearch();
    // 앞뒤 공백을 제거한 검색어로 첫 페이지를 조회
    await executeBookSearch(searchKeyword.trim());
  };

  /**
   * 현재 검색 결과 다음 페이지를 조회해 기존 목록 뒤에 추가
   *
   * @author HanWon.Jang
  ...
  const handleLoadMore = async (): Promise<void> => {
    ...
    // 입력 중인 다음 검색어가 아니라 현재 결과를 만든 검색어로 다음 페이지를 조회
    const keyword = lastSearchedKeywordRef.current;

    // 검색어가 없거나 첫 페이지 및 추가 노출 중이면 중복 요청을 차단
    if (!keyword || isSearching || isLoadingMore) {
      // 실행할 수 없는 추가 조회를 종료
      return;
    }

    // 추가 조회 뒤 새 첫 페이지 검색이 시작되면 오래된 응답을 버릴 요청 순번을 보관
    const requestId = resultRequestIdRef.current;
    ...
    try {
      ...
      // 이미 받아둔 50권 안에 미노출 도서가 있으면 외부 호출 없이 다음 10권만 표시
      if (visibleCount < (bookResult?.length ?? 0)) {
        const nextVisibleCount = Math.min(
          visibleCount + SEARCH_PAGE_SIZE,
          bookResult?.length ?? 0,
        );
        // 공용 50권 결과에서 다음 10권까지 화면 노출 범위를 확장
        setVisibleCount(nextVisibleCount);
        // 상세 화면 복귀 시 같은 노출 위치를 유지하도록 검색 상태를 저장
        saveSearchCache({
          searchKeyword: keyword,
        ...
        // 카카오 API를 호출하지 않은 추가 노출 처리를 종료
        return;
      }

      // 마지막 페이지이거나 다음 위치가 없으면 불필요한 빈 카카오 호출을 차단
      if (isEnd || nextStart === null) {
        // 추가 검색할 카카오 페이지가 없는 처리를 종료
        return;
      }

      // 받아둔 50권을 모두 소진한 경우에만 다음 카카오 검색 페이지를 호출
      const responseData = await fetchBooks(keyword, nextStart);

      // 추가 조회 중 새 첫 페이지 검색이 시작됐으면 이전 결과를 합치지 않음
      if (requestId !== resultRequestIdRef.current) {
        // 최신 첫 페이지 검색이 목록을 갱신하도록 오래된 추가 응답 처리를 종료
        return;
      }
      ...
      // 기존 목록 뒤에 새 50권 페이지를 추가하고 화면에는 다음 10권까지만 노출
      setBookResult(mergedResult);
      setVisibleCount(nextVisibleCount);
      setNextStart(responseData.nextStart ?? null);
      setIsEnd(responseData.end);
      // 합쳐진 결과와 실제 카카오 페이지 상태를 세션 캐시에 저장
      saveSearchCache({
        searchKeyword: keyword,
      ...
      if (requestId !== resultRequestIdRef.current) {
        // 최신 검색 흐름을 유지하고 오래된 오류 처리를 종료
        return;
      }

      console.error("도서 검색 결과 추가 조회 중 오류 발생: ", error);
      // 추가 조회 실패 원인을 공통 오류 알림으로 표시
      await sweetError(
        message("frontend.alert.searchFailedTitle"),
        getApiErrorMessage(error, message("frontend.book.search.failed")),
      );
    } finally {
      // 성공과 실패에 관계없이 추가 조회 상태를 해제
  ...
  /**
   * 선택한 책을 개인 독후감 또는 모임 독서 등록 흐름으로 전달
   *
   * @author HanWon.Jang
   * @param book 선택한 책 정보
  ...
  const handleSelectBook = async (book: BookSearchResultType): Promise<void> => {
    ...
    if (isTimerBookSearch) {
      // 선택한 도서를 타이머 전용 목표 독서기간 모달에 전달
      setTimerPeriodBook(book);
      // 기존 독후감 확인과 일반 등록 화면 이동을 실행하지 않음
      return;
    }

    // 다른 책의 개인 독후감 확인이 진행 중이면 중복 선택을 차단
    if (selectingBookIsbn !== null) {
      // 진행 중인 책 선택 요청을 유지하고 새 요청을 종료
      return;
    }

    // 모임 독서용 검색이면 선택한 책을 예정된 독서 등록 URL로 전달
    if (isClubBookSearch) {
      ...
      // 다음 도서 추천에서 진입한 검색은 선택 도서를 투표 화면으로 바로 전달
      ...
      // 모임 독서 수정용 도서 선택은 검색 목록 이력을 내 모임 이력으로 교체
      if (Number.isSafeInteger(editRondNumb) && editRondNumb > 0) {
        // 수정 완료 후 뒤로가기에서 내 모임 화면을 표시
        navigate("/reading-clubs/mine", { replace: true });
      }
    ...
    // 개인 책 선택의 중복 요청을 막기 위해 현재 ISBN을 저장
    setSelectingBookIsbn(book.isbn);

    try {
      // 기존 독후감 확인 결과에 따라 수정 또는 새 작성 화면으로 이동
      await moveToReportEntry(book, navigate);
    } finally {
      // 선택 흐름이 끝나면 다른 책을 선택할 수 있도록 진행 상태를 해제
  ...
  function handleMoreInfo(book: BookSearchResultType): void {
    ...
    // 모임 검색에서는 상세 화면에서도 동일한 모임 선택 흐름을 유지
    if (isClubBookSearch) {
      // 올바르지 않은 모임 번호이면 모임 목록으로 이동
      if (!hasValidClubNumb) {
        // 모임 목록으로 돌아가 다시 진입하도록 처리
        navigate("/reading-clubs/mine", { replace: true });
        // 잘못된 모임 번호의 상세 이동을 종료
        return;
      }

      // 선택한 책 정보를 모임 책 상세 화면으로 전달
  ...
  function returnToTimer(selectedReport?: number): void {
    ...
    // PWA History State 교체 후에도 모달 재실행 정보가 남도록 타이머 복귀 경로를 생성
    const timerReturnPath = getTimerReturnPath(selectedReport);
    // 검색 화면 이력을 타이머로 교체하고 현재 읽는 도서 모달 및 신규 도서 선택 정보를 전달
  ...
  function closeTimerPeriod(): void {
    ...
    // 독후감 저장 중에는 화면을 이탈해 처리 결과를 잃지 않도록 처리
    if (isTimerReportSaving) {
      // 진행 중인 저장 요청을 유지하고 닫기 처리를 종료
      return;
    }
  ...
  /**
   * 선택한 도서와 목표 독서기간으로 읽는 중 독후감을 등록
   *
   * @author SeungHyeon.Kang
  ...
  async function saveTimerReport(
    ...
    if (!timerPeriodBook || isTimerReportSaving) {
      // 등록할 수 없는 현재 상태의 처리를 종료
      return;
    }

    // 타이머에서 생성하는 독후감은 기존 읽는 중 정책의 비공개와 미평점 값을 사용
    ...
    try {
      // 모달 안에 공통 소형 회전 링을 표시하도록 저장 상태를 시작
      setIsTimerReportSaving(true);

      /**
       * 읽는 중 독후감을 등록하고 타이머 공용 요약 캐시를 최신 상태로 준비
       *
       * @author SeungHyeon.Kang
       * @return 새로 등록한 독후감 번호 Promise
       * @throws 독후감 등록 또는 타이머 요약 캐시 갱신에 실패하면 발생
       */
      const persistTimerReport = async (): Promise<number> => {
        // 기존 독후감 등록 API로 읽는 중 독후감과 도서 정보를 함께 저장
        const reportResponse = await setReportApi(reportData);
        const timerSummaryOptions = getTimerSummaryOptions();
        // 등록 전 캐시가 타이머 복귀 화면에 남지 않도록 공용 요약을 만료 처리
        await queryClient.invalidateQueries({
          queryKey: timerSummaryOptions.queryKey,
          refetchType: "none",
        });
        try {
          // 타이머 복귀 전에 최신 현재 읽는 도서 목록을 강제로 조회해 공용 캐시에 저장
          await queryClient.fetchQuery({
            ...timerSummaryOptions,
            staleTime: 0,
          });
        } catch {
          // 사전 조회 실패는 등록 성공을 취소하지 않고 타이머 화면의 진입 조회에서 다시 시도하도록 만료 상태를 유지
          await queryClient.invalidateQueries({
            queryKey: timerSummaryOptions.queryKey,
            refetchType: "none",
          });
        }
        // 타이머 복귀 화면에서 선택 상태로 사용할 신규 독후감 번호를 반환
        return reportResponse.data;
      };

      // 등록과 캐시 준비가 끝나면 처리 중 알림을 같은 저장 성공 알림으로 전환
  ...
  // 책 검색 화면이 렌더링과 이벤트 연결에 사용할 값을 반환
  return {
