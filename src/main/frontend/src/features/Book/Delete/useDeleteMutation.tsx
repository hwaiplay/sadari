/**
 * src/main/frontend/src/features/Book/Delete/useDeleteMutation.tsx 파일의 프론트엔드 화면, API, 훅 또는 유틸 로직을 담당
 *
 * @author HanWon.Jang
 */
...
/**
 * 삭제한 독후감을 타이머 요약의 도서 연결 정보에서 제거
 *
...
const removeDeletedTimerBook = (summary: ReadingTimerSummary, reptNumb: number): ReadingTimerSummary => {
  ...
  // 삭제 결과가 즉시 반영된 현재 도서 목록과 실행 세션을 반환
  return {
    ...summary,
    activeTimer,
    currentReadingList,
...
/**
 * use Delete Mutation 상태와 처리 함수를 제공
 *
 * @author HanWon.Jang
...
export const useDeleteMutation = () => {
  ...
  // 삭제 완료 후 이전 상세 이력을 남기지 않도록 홈 루트 이동 함수를 조회
  const moveHome = useHomeNavigation();

  /**
   * 독후감 삭제 성공 즉시 관련 캐시를 정리
   *
   * @author SeungHyeon.Kang
  ...
  const deleteReport = async (
    ...
    const deleteReportAndRefresh = async (): ReturnType<typeof delReportApi> => {
      ...
      // 성공 응답 직후 캐시를 정리하고 삭제된 상세 화면을 제거
      handleDeleteSuccess(response, reptNumb);
      // Mutation 결과로 사용할 삭제 응답을 반환
      return response;
    };

    // 삭제 후처리가 끝나면 처리 중 알림을 삭제 성공 알림으로 전환
  ...
  const handleDeleteError = (error: unknown): void => {
    ...
    // 서버 메시지가 없으면 공통 재시도 문구로 삭제 실패 내용을 구성
  ...
  // 독후감 삭제 요청과 성공 및 실패 화면 처리를 결합한 Mutation을 반환
  return useMutation({
    mutationFn: deleteReport,
    onSuccess: () => {
      // 처리 중 이동 가드가 해제된 뒤 삭제한 독후감 상세 화면을 홈 루트로 교체
      moveHome();
    },
