/**
 * 독후감 수정 요청과 상세 조회 캐시 갱신 및 성공 이동을 처리
 *
 * @author HanWon.Jang
 */
...
/**
 * use Update Mutation 상태와 처리 함수를 제공
 *
 * @author HanWon.Jang
...
export const useUpdateMutation = () => {
  ...
  const updateReport = async (
    ...
    const updateReportAndInvalidate = async (): ReturnType<typeof uptReportApi> => {
      ...
      // 상세 화면에서 직접 수정한 경우 기존 상세 조회 캐시를 갱신
      await queryClient.invalidateQueries({
        queryKey: ["detail", response.data],
      });
      // 화면 이동에 사용할 수정 응답을 반환
      return response;
    };

    // 수정과 캐시 갱신 완료 후 처리 중 알림을 저장 성공 알림으로 전환
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      // 수정 저장 후 상세 화면은 새 히스토리로 쌓지 않고 현재 수정 화면을 교체
