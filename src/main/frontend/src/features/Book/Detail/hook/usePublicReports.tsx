/**
 * src/main/frontend/src/features/Book/Detail/hook/usePublicReports.tsx 파일의 프론트엔드 화면, API, 훅 또는 유틸 로직을 담당함
 *
 * @author HanWon.Jang
 */
import {
...
/**
 * React Query 항목이 모임 회차 독후감 무한 목록인지 확인함
 *
 * @author HanWon.Jang
...
const isClubReportQuery = (query: Query): boolean => {
  ...
  // 모임 회차 보고서 키 구조와 일치하는 캐시만 좋아요 병합 대상으로 허용함
...
const createOptimisticLike = (
  ...
  // 현재 상태를 반전하고 좋아요 수가 음수가 되지 않도록 보정함
  return {
...
const mergePublicReportPages = (
  ...
  // 모든 조회 페이지에서 동일 독후감의 좋아요 상태만 변경함
  return {
    ...current,
...
const mergeClubReportPages = (
  ...
  // 모든 회차 페이지에서 동일 독후감의 좋아요 상태만 변경함
  return {
    ...current,
    pages: current.pages.map((page) => ({
      ...page,
...
/**
 * 공개 독후감 목록과 모임 목록 및 상세 캐시에 좋아요 상태를 함께 반영함
 *
 * @author HanWon.Jang
...
/**
 * use Public Reports By Isbn 상태와 처리 함수를 제공함
 *
 * @author HanWon.Jang
 * @param isbn isbn 입력값
...
export const usePublicReportsByIsbn = (
  ...
  // ISBN과 정렬 및 상태별 공개 독후감 페이지를 하나의 무한 조회 캐시로 관리함
  return useInfiniteQuery({
    queryKey: ["publicReports", "isbn", isbn, sortType, reptStat],
    /**
     * 현재 조건의 공개 독후감 한 페이지를 조회함
     *
...
/**
 * use Public Report Like Mutation 상태와 처리 함수를 제공함
 *
 * @author HanWon.Jang
 * @return 화면에서 사용할 상태와 처리 함수
 */
export const usePublicReportLike = () => {
  ...
  return useMutation({
    ...
    /**
     * 서버 응답을 기다리지 않고 현재 좋아요 상태를 화면 캐시에 반영함
     *
     * @author HanWon.Jang
     * @param request 좋아요 대상과 현재 화면 상태
    ...
    onMutate: async (
      ...
      // 진행 중인 동일 범위 조회가 즉시 반영 상태를 덮어쓰지 않도록 취소함
      await Promise.all([
      ...
      // 클릭 즉시 좋아요 상태와 수를 모든 관련 화면 캐시에 반영함
      mergeLikeCaches(
        queryClient,
        request.tagtNumb,
      ...
      // 핵심 좋아요 요청 실패 시 사용할 이전 캐시 상태를 반환함
      return context;
    },
    /**
     * 서버가 확정한 좋아요 상태로 낙관적 값을 보정함
     *
     * @author HanWon.Jang
    ...
    onSuccess: (result, request): void => {
      // 서버가 반환한 확정 상태가 있으면 관련 캐시의 낙관적 값을 보정함
      if (result.data) {
        mergeLikeCaches(queryClient, request.tagtNumb, result.data);
      }
    },
    /**
     * 핵심 좋아요 요청 실패 시 이전 화면 상태로 원복하고 오류를 안내함
     *
     * @author HanWon.Jang
    ...
    /**
     * 좋아요 처리 종료 후 서버 상태를 백그라운드에서 재확인함
     *
     * @author HanWon.Jang
     * @return 반환값이 없음
     */
    onSettled: (): void => {
      // 현재 화면을 막지 않고 관련 조회 캐시를 서버 최종 상태와 동기화함
