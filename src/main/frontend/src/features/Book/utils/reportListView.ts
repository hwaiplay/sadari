/**
 * 독후감 목록 API 데이터를 공통 카드 표시 모델로 변환
 *
 * @author HanWon.Jang
 */
...
/** 독후감 목록 카드가 접힌 상태로 표시할 본문 길이 기준 */
export const REPORT_CONTENT_PREVIEW_LENGTH = 180;

/**
 * 독후감의 독서 상태 코드를 비교 가능한 대문자로 정규화
 *
 * @author HanWon.Jang
...
export function getReportStatus(report: PublicReportType): string {
  // 목록 필터와 상태 이름 조회에 사용할 정규화된 코드를 반환
...
/**
 * 독후감의 좋아요와 댓글 개수를 화면 표시 문자열로 변환
 *
 * @author HanWon.Jang
...
function getCountLabel(countValue?: number): string {
  ...
  // 네 자리 이상의 개수는 카드 너비를 넘지 않도록 최대 표시 문구를 사용
  if (count > 999) {
    // 카드 지표 영역의 최대 개수 문구를 반환
    return "999+";
  }

  // 세 자리 이하의 개수를 숫자 문자열로 반환
  return String(count);
}

/**
 * 독서 상태 코드에 대응하는 카드 색상 구분값을 결정
 *
 * @author HanWon.Jang
...
function getStatusTone(reportStatus: string): ReportStatusTone {
  // 완독 상태는 완료 전용 색상 구분값을 사용
  if (reportStatus === "DONE") {
    // 완독 상태 색상 구분값을 반환
    return "done";
  }

  // 독서 중단 상태는 중단 전용 색상 구분값을 사용
  if (reportStatus === "STOP") {
    // 독서 중단 상태 색상 구분값을 반환
    return "stopped";
  }

  // 나머지 상태는 독서 중 색상 구분값을 반환
  return "reading";
}

/**
 * 독후감 목록을 카드가 계산 없이 렌더링할 수 있는 표시 모델로 변환
 *
 * @author HanWon.Jang
...
export function createReportListItems(
  ...
  // 각 독후감에 별점과 상태 및 개수 표시값을 결합해 반환
  return reports.map((report) => {
    ...
    // 공통 독후감 카드가 사용할 계산 결과를 원본 데이터와 함께 반환
    return {
      ...report,
      rating,
      reportStatus,
