/**
 * 독후감 목록 API 데이터를 공통 카드 표시 모델로 변환한다.
 *
 * @author HanWon.Jang
 */
...
export function getReportStatus(report: PublicReportType): string {
  // 목록 필터와 상태 이름 조회에 사용할 정규화된 코드를 반환한다
...
/**
 * 독후감의 좋아요와 댓글 개수를 화면 표시 문자열로 변환한다.
 *
 * @author HanWon.Jang
...
/**
 * 독서 상태 코드에 대응하는 카드 색상 구분값을 결정한다.
 *
 * @author HanWon.Jang
...
/**
 * 독후감 목록을 카드가 계산 없이 렌더링할 수 있는 표시 모델로 변환한다.
 *
 * @author HanWon.Jang
...
export function createReportListItems(
  ...
  // 각 독후감에 별점과 상태 및 개수 표시값을 결합해 반환한다
  return reports.map((report) => {
    ...
    // 공통 독후감 카드가 사용할 계산 결과를 원본 데이터와 함께 반환한다
    return {
      ...report,
      rating,
      reportStatus,
