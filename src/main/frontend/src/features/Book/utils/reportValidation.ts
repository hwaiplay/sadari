...
/**
 * DB 저장 기준 byte 계산을 위해 HTML 특수문자를 entity로 변환
 *
 * @author HanWon.Jang
...
/**
 * 독후감 내용이 DB에 저장될 때의 UTF-8 byte 길이를 계산
 * 화면 입력값은 저장 전에 sanitize와 HTML escape를 거치므로 같은 기준으로 길이를 계산
 *
 * @author HanWon.Jang
...
/**
 * 사용자 입력값에서 script, HTML tag, inline event, javascript scheme을 제거
 *
 * @author HanWon.Jang
 * @param value 정리할 입력값
...
/**
 * 도서 검색 API가 여러 저자를 ^ 구분자로 내려주는 경우 저장 요청에 그대로 들어가지 않도록 정리
 *
 * @author HanWon.Jang
...
/**
 * 독후감 등록과 수정 폼의 필수값, 공통코드, 내용 byte 제한을 검증
 *
 * @author HanWon.Jang
...
export function validateReportForm(values: ReportFormValues) {
  ...
  // 완료 상태에서는 화면 라벨이 목표 날짜로 바뀌므로 필수값 안내 문구도 같은 명칭을 사용
...
/**
 * 독후감 등록 시 선택된 도서 정보가 저장 가능한 형태인지 검증
 *
 * @author HanWon.Jang
