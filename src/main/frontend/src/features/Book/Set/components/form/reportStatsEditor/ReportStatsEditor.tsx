/**
 * 독후감 상태에 따라 세로 행 요약과 단계형 입력 모달을 구성
 *
 * @author HanWon.Jang
 */
...
/**
 * YYYY-MM-DD 값을 독서기간 계산에 사용할 UTC 기준 일련번호로 변환
 *
 * @author HanWon.Jang
...
function getDateSerial(value: string) {
  ...
  // 브라우저 시간대에 따른 날짜 차이를 제거하도록 연월일을 분리
  const [year, month, day] = value.split("-").map(Number);

  // 날짜 형식이 올바르지 않으면 요약값 대신 미입력 상태를 유지
  if (!year || !month || !day) {
    // 잘못된 날짜의 계산 결과를 반환
    return null;
  }

  // 시간대 차이가 없는 UTC 기준 날짜 일련번호를 반환
  return Date.UTC(year, month - 1, day);
}

/**
 * 입력된 독서기간을 시작일과 종료일을 포함한 일수 요약으로 변환
 *
 * @author HanWon.Jang
...
function getPeriodSummary(startDate: string, endDate: string) {
  ...
  // 시작일과 종료일이 모두 입력된 경우에만 기간 정보 행에 값을 표시
  if (!startDate || !endDate) {
    // 아직 입력되지 않은 기간 표시값을 반환
    return "-";
  }

  // 독서 시작일을 날짜 차이 계산 기준으로 변환
  const startSerial = getDateSerial(startDate);
  // 읽는 중에는 목표 종료일을 사용하고 완료 또는 중단에는 실제 종료일을 사용
...
function getStatusValueClassName(status: ReadingStatusType) {
  ...
  // 완료 상태는 상세 화면과 같은 연녹색으로 구분
  if (status === REPORT_STATUS_DONE) {
    // 완료 상태 글자색 클래스를 반환
    return styles.statusDone;
  }

  // 중단 상태는 상세 화면과 같은 연한 빨간색으로 구분
  if (status === REPORT_STATUS_STOP) {
    // 중단 상태 글자색 클래스를 반환
    return styles.statusStop;
  }

  // 읽는 중과 미입력 상태는 기본 글자색 클래스를 반환
  return styles.statusRead;
}

/**
 * 독후감 입력값을 세로 행으로 요약하고 선택한 항목을 단계형 모달에서 수정
 *
 * @author HanWon.Jang
...
const ReportStatsEditor = ({
  ...
  // 현재 상태 코드에 대응하는 서버 공통코드명을 독서 상태 정보 행에 사용
  for (const statusCode of statusCodes) {
    // 현재 선택값과 일치한 코드명을 찾으면 이후 불필요한 순회를 중단
  ...
  // 모달이 열린 동안 배경 스크롤과 네비게이션 위치를 함께 고정
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      // Escape 외의 키 입력은 현재 입력 컴포넌트에 그대로 전달
      if (event.key !== "Escape") {
        return;
      }
    ...
    // 모달을 키보드로 닫을 수 있도록 문서 이벤트를 등록
    document.addEventListener("keydown", handleKeyDown);

    // 모달이 닫히거나 단계가 바뀌면 중복 키보드 이벤트를 제거
    return () => {
  ...
  function handleSummaryClick(event: MouseEvent<HTMLButtonElement>) {
    ...
    // 읽는 중에는 공개 여부와 평점 단계를 열지 않아 숨겨진 입력을 조작할 수 없도록 처리
    if (!availableSteps.includes(nextStep)) {
      return;
    }

    // 요약 영역에서 직접 연 단계는 오른쪽에서 진입하는 기본 전환 방향을 사용
    onEditStart?.();
  ...
  function handleClose() {
    ...
    // 세로 행 요약이 최신 입력값을 보여주도록 모달 상태만 제거
    setActiveStep(null);
  }
  ...
  function handlePrevious() {
    ...
    // 이전 단계 콘텐츠가 왼쪽에서 들어오도록 전환 방향을 먼저 설정
    setStepTransitionDirection("backward");

    // 현재 상태에서 실제로 노출되는 단계 순서를 기준으로 이전 입력으로 이동
    setActiveStep((currentStep) => {

      const currentStepIndex = availableSteps.indexOf(currentStep ?? firstStep);
      // 첫 단계보다 앞으로 이동하지 않도록 이전 단계 위치를 보정해 반환
  ...
  function handleNext() {
    ...
    // 다음 단계 콘텐츠가 오른쪽에서 들어오도록 전환 방향을 먼저 설정
    setStepTransitionDirection("forward");

    // 읽는 중에는 상태 다음에 공개 여부와 평점을 건너뛰고 목표 독서 기간으로 이동
    setActiveStep((currentStep) => {

      const currentStepIndex = availableSteps.indexOf(currentStep ?? firstStep);
      // 마지막 단계보다 뒤로 이동하지 않도록 다음 단계 위치를 보정해 반환
      return availableSteps[Math.min(availableSteps.length - 1, currentStepIndex + 1)] ?? lastStep;
    });
  }

  /**
   * 모달에서 선택한 독서 상태를 상위 폼 상태에 반영
   *
   * @author HanWon.Jang
  ...
  function handleStatusInputChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    // 수정 화면의 종료일 확인 분기까지 실행되도록 상위 상태 변경 콜백을 호출
  ...
  function handlePublicOff() {
    ...
    // 비공개 선택값을 상위 폼 상태에 설정
    onPublicChange("N");
  }

  /**
   * 독서 상태 공통코드를 선택 가능한 모달 항목으로 변환
   *
   * @author HanWon.Jang
  ...
  // 현재 독서 상태에서 허용된 항목을 세로 행으로 구성한 요약과 단계형 모달을 반환
  return (
    <>
            ...
            >
              ...
              >
                ...
                <footer className={styles.modalFooter}>
                  {/* 첫 단계에서는 이전 이동 대신 현재 입력을 닫는 취소 버튼을 표시 */}
                  ...
                  {/* 마지막 독서기간 단계에서는 이동 화살표 대신 입력 완료 버튼을 표시 */}
                  {activeStep === lastStep ? (
                    <button
