...
type CalendarDatePickerProps = {
  name: string;
  label?: string;
  // 외부 폼 state에서 날짜 값을 직접 제어해야 할 때 사용하는 선택 날짜
  value?: string;
  defaultValue?: string;
  placeholder?: string;
  // 날짜 선택이 확정되면 부모 폼의 state와 hidden input 값을 함께 동기화
  onChange?: (value: string) => void;
  // 시작일/종료일 역전처럼 선택 즉시 막아야 하는 검증을 부모 폼에서 실행
...
/**
 * 달력 팝오버를 통해 날짜를 선택하고 hidden input으로 값을 전달
 *
 * @author HanWon.Jang
...
const CalendarDatePicker = ({
  ...
  useEffect(() => {
    ...
    // 인라인 달력은 외부 클릭으로 닫지 않고 현재 입력 영역에 계속 표시
    if (inline || !isOpen) {
      return;
    }
  ...
  useEffect(() => {
    // 외부 state로 제어하는 날짜가 바뀌면 달력이 해당 월을 바라보도록 동기화
    if (value === undefined) {
      return;
    }
  ...
  /**
   * 선택한 일을 yyyy-MM-dd 값으로 변환해 hidden input 값으로 반영
   *
   * @author HanWon.Jang
   * @param day 선택한 일
   * @return
  ...
  const selectDay = (day: number) => {
    ...
    if (isRangePicker) {
      // 진행 중인 모임 독서의 시작일은 유지하고 종료일만 변경
        ...
        <div className={inline ? styles.inlineCalendar : styles.popover}>
          ...
          >
            {days.map((day, index) => {
              ...
              // 미래 선택이 허용되지 않은 화면에서는 오늘 이후 날짜를 비활성화
              ...
              // 종료일을 고르기 전에는 시작일을 반원 범위가 아닌 단독 원형 날짜로 표시
          ...
          {/* 인라인 달력 아래에 현재 선택한 시작일과 종료일을 표시 */}
