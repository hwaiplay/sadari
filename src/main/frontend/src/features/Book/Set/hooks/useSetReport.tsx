...
export const useSetReport = () => {
  ...
  const setReport = (
    ...
    const requestSetReport = (): ReturnType<typeof setReportApi> => {
      // 검증을 마친 독후감 입력값을 등록 API에 전달
      return setReportApi(params);
    };

    // 등록 완료 시 처리 중 알림을 닫지 않고 저장 성공 알림으로 전환
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      // 등록 화면 아래에 남아 있는 도서 검색 이력 대신 홈을 뒤로가기 목적지로 지정
      navigate("/home", { replace: true });
      // 홈 위에 등록된 독후감 상세 화면을 새 이력으로 추가
