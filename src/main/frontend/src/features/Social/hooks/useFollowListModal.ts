...
/**
 * 본인과 공개 프로필의 팔로우 목록 모달 조회 및 페이지 상태를 공통으로 관리함
 *
...
export function useFollowListModal(options: FollowListModalOptions) {
  ...
  const openFollowList = async (type: FollowListType): Promise<void> => {
    // 새 목록을 열 때 이전 사용자와 페이지 상태를 초기화함
  ...
  /**
   * 열린 팔로우 목록의 다음 서버 페이지를 이어서 조회함
   *
   * @author SeungHyeon.Kang
   * @return 다음 페이지 조회 완료 Promise
   */
  const loadMoreFollow = async (): Promise<void> => {
    // 열린 목록이 없거나 다음 페이지 조회 중 또는 마지막 페이지이면 중복 요청하지 않음
    if (!followListType || !hasNextFollowUser || isNextFollowLoading) {
      return;
    }

    // 목록 끝에 추가 조회 로딩 상태를 표시함
    setIsNextFollowLoading(true);

    // 추가 페이지 실패 시 기존 목록을 유지하고 오류만 안내함
    try {
      // 마지막 성공 페이지 다음 번호를 조회함
      const pageData = await getFollowPage(followListType, followPage + 1);

      // 공통 응답에 페이지 데이터가 없으면 기존 목록을 유지하고 실패 처리함
      if (!pageData) {
        throw new Error("FOLLOW_PAGE_EMPTY");
      }

      // 기존 사용자 목록 뒤에 다음 페이지를 연결함
      setFollowUsers((currentUsers) => [...currentUsers, ...pageData.list]);
      // 마지막 성공 페이지 번호를 갱신함
      setFollowPage(pageData.page);
      // 서버가 판정한 다음 페이지 여부를 갱신함
      setHasNextFollowUser(pageData.hasNext);
    }

    // 다음 페이지 조회 실패를 현재 모달 안에서 재시도할 수 있게 안내함
    catch (error) {
      // "조회에 실패했습니다."
      await sweetError(
        message("frontend.alert.loadFailedTitle"),
  ...
  /**
   * 팔로우 목록 모달과 조회된 페이지 상태를 초기화함
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const closeFollowList = (): void => {
    // 모달 식별값과 사용자 목록을 함께 비움
    setFollowListType(null);
    setFollowUsers([]);
    setFollowPage(0);
    setHasNextFollowUser(false);
  ...
  // 두 프로필 화면이 공유할 팔로우 목록 상태와 조작 함수를 반환함
  return {
    followListType,
    followUsers,
    isFollowListLoading,
