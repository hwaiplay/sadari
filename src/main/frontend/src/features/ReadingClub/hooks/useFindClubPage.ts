 ...
 * author         : Hanwon.Jang
 * date           : 2026-08-14
 * description    : 모임 찾기 화면의 조회와 관심분야 수정 상태를 관리
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
/**
 * 관심분야 코드를 사용자 관심분야 수정 요청 항목으로 변환
 *
 * @author Hanwon.Jang
...
const getInterestParam = (intrCode: string): { intrCode: string } => {
  // API 요청에 필요한 관심분야 코드 객체를 반환
  return { intrCode };
};

/**
 * 모임 찾기 화면의 조회 조건과 관심분야 수정 동작을 제공
 *
 * @author Hanwon.Jang
...
export const useFindClubPage = () => {
  ...
  /**
   * 관심분야와 추천 모임을 조회하여 화면 초기 상태를 구성
   *
   * @author Hanwon.Jang
   * @return
   * @throws 관심분야 또는 모임 조회 요청이 실패하면 공통 오류 알림을 표시
   */
  const getInitialData = useCallback(async (): Promise<void> => {
    // 첫 화면 데이터가 준비될 때까지 로딩 상태를 표시
    setIsLoading(true);

    try {
      // 관심 카테고리 원본과 사용자의 현재 선택을 함께 조회
      const [nextCatalog, nextSelected] = await Promise.all([
        getUserInterestCatalogApi(),
        getUserInterestListApi(),
      ]);

      // 관심 카테고리 팝업에서 사용할 전체 목록을 설정
      setCatalog(nextCatalog);
      // 내 관심분야 칩에 표시할 현재 선택을 설정
      setSelectedInterests(nextSelected);

      // 관심분야가 있는 사용자만 추천 모임 API를 호출
      if (nextSelected.length > 0) {
        // 관심분야 일치도를 적용한 공개 모임을 조회
        const nextClubs = await getFindClubListApi("");
        // 추천 모임 목록을 최신 응답으로 교체
        setClubs(nextClubs);
      }
    } catch (error: unknown) {
      // 초기 조회 실패 원인을 공통 오류 알림으로 전달
      showLoadError(error);
    } finally {
      // 성공 여부와 관계없이 초기 로딩 표시를 종료
      setIsLoading(false);
    }
  }, [showLoadError]);

  /**
   * 화면 진입 시 관심분야와 추천 모임 초기 조회를 시작
   *
   * @author Hanwon.Jang
   * @return 반환값이 없음
   */
  ...
  const handleSearch = (event: FormEvent<HTMLFormElement>): void => {
    // 브라우저의 기본 폼 이동을 차단하고 SPA 검색을 유지
    event.preventDefault();
    // 검색 결과가 준비될 때까지 로딩 상태를 표시
    setIsLoading(true);
    ...
    const getSearchResult = async (): Promise<void> => {
      try {
        // 현재 입력한 검색어로 공개 모임을 조회
        const nextClubs = await getFindClubListApi(keyword);
        // 추천 모임 영역을 검색 결과로 교체
        setClubs(nextClubs);
      } catch (error: unknown) {
        // 검색 실패 원인을 공통 오류 알림으로 전달
        showLoadError(error);
      } finally {
        // 검색 요청이 끝나면 로딩 표시를 종료
        setIsLoading(false);
      }
    };
  ...
  const handleOpenInterests = (): void => {
    // 현재 선택값을 수정할 수 있도록 관심 카테고리 팝업을 표시
  ...
  const handleSaveInterests = (codes: string[]): void => {
    ...
    const uptInterests = async (): Promise<void> => {
      try {
        ...
        // 저장된 코드에 해당하는 관심분야 표시 정보를 구성
        const nextSelected = catalog.filter(hasSelectedCode);
        // 내 관심분야 칩을 저장된 선택으로 갱신
        setSelectedInterests(nextSelected);
        // 관심분야 변경 후 전체 추천을 보여주도록 검색어를 초기화
        setKeyword("");
        // 변경된 관심분야 일치도를 적용한 추천 모임을 다시 조회
        const nextClubs = await getFindClubListApi("");
        // 추천 모임 목록을 최신 응답으로 교체
        setClubs(nextClubs);
        ...
        // 저장 또는 재조회 실패 원인을 공통 오류 알림으로 전달
        showUptInterestsFailedError(error);
      }
    };

    // 관심분야 선택 완료와 비동기 저장을 연결
    void uptInterests();
  };

  // 모임 찾기 화면에서 사용하는 상태와 동작만 반환
  return {
    catalog,
    clubs,
