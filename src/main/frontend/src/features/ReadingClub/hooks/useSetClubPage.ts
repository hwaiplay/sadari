...
export function useSetClubPage(mode: SetClubPageMode = "create") {
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && !Number.isFinite(clubNumb)) {
      ...
      // 잘못된 경로 처리를 마치고 로딩 상태를 해제
      setIsLoading(false);
      return;
    }

    // 수정 화면은 관심분야와 기존 모임 상세를 함께 조회
    ...
    void pageRequest
      .then(([nextCatalog, detail]) => {
        ...
        // 예정 또는 진행 중인 독서 회차가 있으면 기존 공개 범위를 유지
  ...
  const selectVisibility = (clubVisb: "PUBLIC" | "PRIVATE"): void => {
    ...
    if (isVisibilityLocked) {
      // 서버 정책과 동일하게 기존 공개 범위를 유지
      return;
    }

    // 비공개 모임은 초대 가입으로 자동 고정
    setForm((current) => ({
      ...current,
      clubVisb,
  ...
  /**
   * 즉시 가입 방식을 선택하고 기존 승인 질문을 제거
   *
   * @author Hanwon.Jang
   * @return 반환값이 없음
   */
  const selectOpenJoin = (): void => {
    // 즉시 가입에는 승인 질문이 필요하지 않으므로 빈 목록을 설정
    setForm((current) => ({ ...current, joinType: "OPEN", questionList: [] }));
  };

  /**
   * 승인 가입 방식을 선택하고 첫 질문 입력란을 준비
   *
   * @author Hanwon.Jang
   * @return 반환값이 없음
   */
  const selectApprovalJoin = (): void => {
    // 기존 질문이 없으면 첫 승인 질문 입력란을 추가
    setForm((current) => ({
      ...current,
      joinType: "APPROVAL",
  ...
  const submitForm = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (!form.clubName.trim() || !form.clubCntn.trim() || form.categoryList.length === 0
      ...
      // 필수 입력이 누락되거나 수정 대상이 잘못된 저장 요청을 중단
      return;
    }
    ...
    try {
      ...
      // 완료된 생성 또는 수정 폼이 뒤로가기로 다시 열리지 않도록 상세 화면으로 교체
  ...
  // 모임 생성 화면 렌더링에 필요한 상태와 이벤트 처리 함수를 반환
  return {
    addQuestion,
    catalog,
    closeCategoryModal,
