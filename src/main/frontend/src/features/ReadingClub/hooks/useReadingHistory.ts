...
/**
 * 현재 활성 모임원에게 가입 이전을 포함한 종료 회차 페이지를 조회
 *
 * @author HanWon.Jang
...
export const useReadingHistory = (clubNumb: number, enabled: boolean) => {
  // 모임별 종료 회차를 페이지 순서대로 누적하는 조회 상태를 반환
  return useInfiniteQuery({
    queryKey: ["readingClub", clubNumb, "readingHistory"],
    /**
     * 현재 모임의 이전 독서 기록 한 페이지를 조회
     *
     * @author HanWon.Jang
    ...
    queryFn: async ({ pageParam }) => {
      // 가입 시점과 관계없이 접근 가능한 종료 회차 페이지를 반환
      return await getClubReadingHistoryApi(clubNumb, pageParam);
    },
    initialPageParam: 1,
    /**
     * 마지막 응답에서 다음 이전 독서 기록 페이지 번호를 계산
     *
     * @author HanWon.Jang
