...
const isOwnerClub = (club: ReadingClub): boolean => {
  // 가입 신청 조회 권한이 있는 모임장 여부를 반환
  return club.membRole === "OWNER";
};

/**
 * 승인 대기 신청이 있는 모임 요약인지 판정
 *
 * @author HanWon.Jang
...
const hasPendingSummary = (
  ...
  // 빈 조회 결과를 제외할 수 있도록 판정 결과를 반환
  return item !== null;
};

/**
 * 모임장인 모임 한 곳의 승인 대기 가입 신청 건수를 조회
 *
 * @author HanWon.Jang
 * @param club 가입 신청을 확인할 모임
 * @return 승인 대기 신청이 있는 모임 요약 또는 빈 값
 * @throws 가입 신청 조회에 실패하면 발생
 */
const getPendingApplications = async (
  club: ReadingClub,
): Promise<PendingClubApplications | null> => {
  // 모임장 권한이 확인된 모임의 기존 가입 신청 목록을 조회
  const applications = await getClubApplicationListApi(club.clubNumb);
  // 승인 대기 신청이 없으면 상단 알림 대상에서 제외
  if (applications.length === 0) {
    // 알림 대상이 아님을 나타내는 빈 값을 반환
    return null;
  }

  // 모임별 승인 대기 건수와 관리 화면 이동에 필요한 식별값을 반환
  return {
...
/**
 * 모임의 모든 관심분야명을 카드 상단에 표시할 문구로 변환
 *
 * @author HanWon.Jang
 * @param club 표시할 모임
...
export const getGoalProgressText = (club: ReadingClub): string => {
  // 진행 중인 독서 목표가 없으면 다음 도서를 정하는 상태로 안내
  if (!club.currentRondNumb) {
    // "도서 선정 중"
  ...
  // 모임장을 포함한 참여 인원이 항상 한 명 이상 표시되도록 보정
  const goalMemberCnt = Math.max(1, club.currentGoalMembCnt ?? 0);
  // 비정상 집계값이 참여 인원 범위를 벗어나지 않도록 달성 인원을 보정
  const goalAchvCnt = Math.min(goalMemberCnt, Math.max(0, club.currentGoalAchvCnt ?? 0));
  // "{달성 인원}/{참여 인원}명 목표 달성"
...
/**
 * 내 모임 화면의 조회 상태와 초대 처리 및 상세 이동을 관리
 *
 * @author HanWon.Jang, SeungHyeon.Kang
 * @return 내 모임 화면 상태와 이벤트 처리 함수
 */
export const useMyClubPage = () => {
  // 모임 상세 이동에 사용할 라우터 함수를 조회
  ...
  /**
   * 내 모임과 받은 초대를 동시에 새로 조회
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   * @throws 모임 또는 초대 목록 조회에 실패하면 발생
   */
  ...
  /**
   * 최초 모임 목록 조회 완료 후 로딩 화면을 해제
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleLoadComplete = useCallback((): void => {
    // 목록 화면을 표시할 수 있도록 로딩 상태를 해제
    setIsLoading(false);
  }, []);

  /**
   * 페이지 최초 진입 시 모임과 초대 목록 조회를 시작
   *
   * @author HanWon.Jang
  ...
  /**
   * 받은 초대 상세 목록의 표시 상태를 전환
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleNoticeToggle = (): void => {
    // 현재 상태의 반대로 모임 알림 상세 목록을 전환
  ...
  const handleClubMove = (clubNumb: number): void => {
    // 선택한 모임 번호를 상세 경로에 포함
    navigate(`/reading-clubs/${clubNumb}`);
  };

  /**
   * 키보드 Enter 입력으로 선택한 모임 상세 화면에 이동
   *
   * @author HanWon.Jang
  ...
  const handleClubKeyDown = (event: KeyboardEvent<HTMLElement>): void => {
    ...
    // Enter 입력에서만 카드의 상세 이동을 실행
    if (event.key === "Enter" && Number.isFinite(clubNumb)) {
      // 키보드로 선택한 모임 상세 화면으로 이동
      handleClubMove(clubNumb);
    }
  };

  /**
   * 포인터로 선택한 모임 카드의 상세 화면에 이동
   *
   * @author HanWon.Jang
  ...
  const handleClubClick = (event: MouseEvent<HTMLElement>): void => {
    const clubNumb = Number(event.currentTarget.dataset.clubNumb);
    // 유효한 모임 번호가 있는 카드만 상세 화면으로 이동
    if (Number.isFinite(clubNumb)) {
      // 포인터로 선택한 모임 상세 화면으로 이동
      handleClubMove(clubNumb);
    }
  };

  /**
   * 초대 수락 실패를 사용자 공통 알림으로 처리
   *
   * @author HanWon.Jang
  ...
  const handleAcceptInvitation = (event: MouseEvent<HTMLButtonElement>): void => {
    const clubNumb = Number(event.currentTarget.dataset.clubNumb);
    // 유효한 모임 번호가 있는 초대만 수락
    if (Number.isFinite(clubNumb)) {
      // 선택한 초대를 수락하고 실패 시 사용자에게 안내
  ...
  const handleDeclineInvitation = (event: MouseEvent<HTMLButtonElement>): void => {
    const clubNumb = Number(event.currentTarget.dataset.clubNumb);
    // 유효한 모임 번호가 있는 초대만 거절
    if (Number.isFinite(clubNumb)) {
      // 선택한 초대를 거절하고 실패 시 사용자에게 안내
      void declineInvitation(clubNumb).catch(handleDeclineError);
    }
  };

  // 화면 렌더링에 필요한 상태와 이벤트 처리 함수를 반환
  return {
    clubs,
    invitations,
    pendingApplications,
