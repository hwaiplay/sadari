...
/**
 * 모임 다음 도서 추천과 투표 화면의 상태와 기능을 관리함
 *
 * @author HanWon.Jang
...
export const useClubBookVotePage = () => {
  ...
  /**
   * 모임의 다음 도서 추천 후보와 현재 투표 상태를 조회함
   *
   * @author HanWon.Jang
  ...
  useEffect(() => {
    ...
    const executeRecommendation = async (): Promise<void> => {
      try {
        // 추천 등록이 끝날 때까지 화면 이동을 차단함
        await runBlockingOperation(saveRecommendation);
        // 같은 도서가 다시 등록되지 않도록 라우트 전달 상태를 제거함
  ...
  /**
   * 추천 후보로 등록할 도서를 선택하는 검색 화면으로 이동함
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleRecommendation = (): void => {
    ...
    // 후보 등록 가능 시점이며 아직 내 후보가 없을 때만 검색 화면으로 이동함
    if (!votePage?.canRecommend || votePage.hasRecommended) {
      // 후보 등록이 제한된 상태에서는 화면 이동을 종료함
      return;
    }

    // 검색 완료 후 현재 투표 화면으로 돌아오도록 경로를 전달함
  ...
  /**
   * 사용자가 선택한 추천 후보 번호를 투표 선택 상태에 반영함
   *
   * @author HanWon.Jang
  ...
  const handleCandidateSelect = (recmNumb: number): void => {
    // 선택한 추천 후보 번호를 현재 투표 상태에 반영함
    setSelectedRecommendation(recmNumb);
  };

  /**
   * 자신의 도서 추천을 확인 후 취소하고 후보 목록을 갱신함
   *
   * @author HanWon.Jang
  ...
  const handleDelete = async (recmNumb: number): Promise<void> => {
    ...
    // 사용자가 확인하지 않으면 추천과 연결 투표를 그대로 유지함
    if (!confirmResult.isConfirmed) {
      // 취소 선택 뒤 삭제 API 호출 없이 처리를 종료함
      return;
    }

    try {
      /**
       * 선택한 추천 후보를 삭제하고 최신 후보 목록을 조회함
       *
       * @author HanWon.Jang
       * @return 추천 삭제와 후보 조회가 끝나면 완료되는 Promise
       */
      const deleteRecommendation = async (): Promise<void> => {
        // 선택한 추천 후보와 연결된 투표를 삭제함
        await deleteClubBookRecommApi(clubNumb, recmNumb);
        // 삭제 결과를 반영하도록 추천 후보를 다시 조회함
        await loadCandidates();
      };
      // 추천 취소가 끝날 때까지 화면 이동을 차단함
  ...
  const handleVote = async (): Promise<void> => {
    ...
    try {
      ...
      const updateVote = async (): Promise<void> => {
        // 선택한 추천 후보에 현재 사용자의 투표를 등록함
        await updateClubBookVoteApi(clubNumb, selectedCandidate.recmNumb);
        // 투표 결과를 반영하도록 추천 후보를 다시 조회함
        await loadCandidates();
      };

      // 투표가 끝날 때까지 화면 이동을 차단하고 성공 상태로 전환함
  ...
  // 다음 도서 추천과 투표 화면에서 사용할 상태와 이벤트 처리 함수를 반환함
  return {
    candidates,
    selectedCandidate,
