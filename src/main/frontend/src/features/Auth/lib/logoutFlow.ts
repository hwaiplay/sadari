...
/**
 * 로그아웃 Alert에서 현재 기기와 전체 기기 중 하나를 선택
 *
...
/**
 * 현재 브라우저에서 이미 허용된 FCM token을 사용자 팝업 없이 조회
 *
 * @author SeungHyeon.Kang
...
async function getCurrentPushToken(): Promise<string | undefined> {
  ...
  try {
    ...
    // 현재 기기 로그아웃에서 푸시 토큰이 실제로 필요할 때만 Firebase SDK를 지연 로드
    const { requestFirebaseToken } = await import("@/app/pwa/firebaseMessaging");
    return await requestFirebaseToken(response.data);
  } catch {
    // 푸시 token 조회 실패가 인증 세션 로그아웃을 막지 않도록 처리
    return undefined;
  }
}

/**
 * 선택한 범위의 서버 세션을 종료하고 같은 브라우저의 모든 탭을 로그인 화면 상태로 동기화
 *
 * @author SeungHyeon.Kang
...
export async function runLogout(scope: LogoutScope): Promise<void> {
  ...
  try {
    ...
    // 다음 로그인 계정에 이전 인증과 사용자 데이터가 노출되지 않도록 세션 캐시를 제거
