...
export default function AuthSyncProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    /**
     * 브라우저 캐시에서 앱 문서가 복원되면 현재 쿠키 기준으로 인증 상태를 다시 확인한다.
     *
     * @author HanWon.Jang
    ...
    const handlePageShow = (event: PageTransitionEvent): void => {
      ...
      // 일반 문서 진입은 인증 Query의 최초 조회가 처리하므로 캐시 복원만 다시 확인한다
      if (!event.persisted) {
        return;
      }

      // 로그인 전후의 오래된 인증 화면이 복원되지 않도록 활성 인증 Query를 즉시 갱신한다
      void queryClient.refetchQueries({ queryKey: queryKeys.auth, type: "active" });
    };

    /**
     * 같은 브라우저의 로그아웃 이벤트를 현재 탭의 인증 상태와 화면에 반영한다.
     *
     * @author HanWon.Jang
    ...
    const handleAuthEvent = (authEvent: AuthEvent): void => {
      ...
      // 로그아웃 외의 인증 이벤트가 추가되더라도 현재 정리 정책을 적용하지 않는다
      if (authEvent.type !== "LOGOUT") {
        return;
      }

      // 현재 탭의 메모리 인증 상태를 제거한다
      ...
      // 로그인 화면이 아닌 경우에만 현재 문서를 로그인 경로로 교체한다
      if (window.location.pathname !== "/login") {
        // 로그아웃 전 보호 화면이 뒤로가기에 남지 않도록 현재 이력을 로그인 화면으로 교체한다
        window.location.replace("/login");
      }
    };

    // 뒤로가기와 앞으로가기로 캐시 문서가 복원되는 시점을 구독한다
    window.addEventListener("pageshow", handlePageShow);
    // 다른 탭과 현재 탭에서 발생한 로그아웃을 같은 인증 상태로 반영한다
    const unsubscribeAuthEvents = subscribeAuthEvents(handleAuthEvent);

    /**
     * 인증 동기화 Provider가 해제될 때 브라우저와 탭 간 이벤트 구독을 정리한다.
     *
     * @author HanWon.Jang
     * @return 반환값이 없다
     */
    const cleanupAuthSync = (): void => {
      ...
      // 브라우저 캐시 복원 인증 재조회를 해제한다
      window.removeEventListener("pageshow", handlePageShow);
      // 탭 간 로그아웃 동기화 구독을 해제한다
      unsubscribeAuthEvents();
    };

    // 인증 동기화 Provider가 해제되면 브라우저와 탭 간 이벤트 구독을 함께 정리한다
