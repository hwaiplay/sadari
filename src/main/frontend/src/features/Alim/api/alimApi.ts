...
/**
 * 로그인 사용자의 알림 목록을 조회
 * 읽음 상태는 변경하지 않고 삭제되지 않은 알림을 읽음 여부와 함께 화면으로 전달
 *
 * @author HanWon.Jang
...
/**
 * 햄버거 메뉴 알림 버튼에 표시할 미읽음 알림 수만 조회
 * 목록 전체를 조회하지 않고 배지 숫자만 갱신할 때 사용
 *
 * @author HanWon.Jang
...
/**
 * 사용자별 알림번호와 클릭 시점의 접근 상태로 계산된 내부 이동 주소를 조회
 *
...
export const getAlimTargetApi = async (alimNumb: number) => {
  ...
  // 공통 응답 검증이 끝난 알림 이동 주소를 반환
  return assertResultDataSuccess(res.data);
};

/**
 * 알림센터 항목 또는 푸시 알림을 클릭한 경우 사용자별 알림 한 건을 읽음 처리
 *
 * @author HanWon.Jang
...
/**
 * 화면에 아직 불러오지 않은 알림까지 모두 삭제 상태로 변경
 * 알림 페이지의 모두 지우기 버튼에서만 사용
 *
 * @author HanWon.Jang
