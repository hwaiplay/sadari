...
/**
 * 본인 댓글 삭제를 확인한 뒤 논리 삭제 API를 호출하고 관련 조회 캐시를 갱신
 *
 * @author HanWon.Jang
...
export const useDelReply = ({ tagtType, tagtNumb, onDeleted }: UseDelReplyProps) => {
  ...
  const handleDeleteReply = async (replNumb: number): Promise<void> => {
    ...
    // 댓글 삭제 여부를 사용자가 선택할 수 있는 확인 모달을 표시
    ...
    // API 실패도 사용자 안내 후 현재 화면에서 복구할 수 있도록 예외 경로를 분리
    try {
      /**
       * 댓글 삭제와 관련 화면 캐시 갱신을 하나의 차단 작업으로 실행
       *
       * @author SeungHyeon.Kang
       * @return 댓글 삭제 및 캐시 갱신 완료 Promise
       * @throws 댓글 삭제 또는 관련 캐시 갱신에 실패하면 발생
       */
      const deleteReplyAndRefresh = async (): Promise<void> => {
        // 로그인 사용자의 작성자 및 계정 상태를 검증하는 댓글 삭제 API를 호출
        await delReplyMutation.mutateAsync({ tagtType, tagtNumb, replNumb });
        // 삭제된 댓글을 참조하는 화면 입력 상태가 있으면 호출부에서 정리하도록 알림
        onDeleted?.(replNumb);
        // 삭제 상태와 공개 독후감 댓글 수를 최신 서버 값으로 갱신
        await Promise.all([
      ...
      // 삭제와 캐시 갱신 완료 후 처리 중 알림을 같은 삭제 성공 알림으로 전환
      ...
      // 삭제 실패 원인과 재시도 안내를 공통 오류 형식으로 표시
      // "댓글 삭제에 실패했습니다."
      await sweetError(
  ...
  // 댓글 메뉴가 사용할 삭제 이벤트와 중복 요청 차단 상태를 반환
  return {
