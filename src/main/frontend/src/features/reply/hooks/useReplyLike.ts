...
/**
 * 댓글 좋아요 등록과 취소 결과를 현재 댓글 목록 캐시에 반영
 *
 * @author HanWon.Jang
...
export const useReplyLike = (target: ReplyTarget) => {
  // 댓글 목록 캐시를 서버의 좋아요 변경 결과와 동기화할 Query Client를 조회
  ...
  /**
   * 현재 좋아요 여부에 맞는 등록 또는 취소 API를 호출
   *
   * @author HanWon.Jang
  ...
  const replyLikeMutation = useMutation({
    ...
    onMutate: async (request: ReplyLikeRequest): Promise<ReplyLikeContext> => {
      // 진행 중인 댓글 조회가 즉시 반영 상태를 덮어쓰지 않도록 취소
      ...
      // 핵심 요청 실패 시 사용할 이전 캐시를 반환
      return { snapshot };
    },
    /**
     * 서버가 확정한 댓글 좋아요 상태로 낙관적 값을 보정
     *
     * @author HanWon.Jang
    ...
    /**
     * 핵심 댓글 좋아요 요청 실패 시 화면을 원복하고 오류를 안내
     *
     * @author HanWon.Jang
     * @param error 댓글 좋아요 변경 중 발생한 오류
    ...
    onError: (
      ...
      // 요청 전 캐시가 있으면 핵심 요청 실패 상태를 화면에 원복
      if (context?.snapshot) {
    ...
    /**
     * 댓글 좋아요 처리 종료 후 서버 최종 상태를 백그라운드에서 재확인
     *
     * @author HanWon.Jang
     * @return 반환값이 없음
     */
    onSettled: (): void => {
      // 화면을 막지 않고 현재 댓글 목록만 서버 상태와 동기화
      void queryClient.invalidateQueries({ queryKey: replyListQueryKey });
    },
  });

  /**
   * 현재 좋아요 여부에 따라 댓글 좋아요 등록 또는 취소 API를 호출
   *
   * @author HanWon.Jang
  ...
  const handleToggleReplyLike = (reply: ReplyLikeRequest): void => {
    // 진행 중인 요청이 있으면 연속 클릭으로 반대 요청이 중복되지 않도록 차단
    if (replyLikeMutation.isPending) {
      // 현재 좋아요 요청이 완료될 때까지 추가 처리를 종료
      return;
    }
