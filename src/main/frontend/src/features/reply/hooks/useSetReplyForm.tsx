 ...
 * author         : Hanwon.Jang
 * date           : 2026-07-28
 * description    : 댓글 등록 폼 동작을 처리하는 훅
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
import { setReplyApi } from "@/features/reply/api/replyApi";
...
export function useSetReplyForm({
  ...
  /**
   * 공백을 제거한 댓글 내용을 댓글 등록 API로 전송한다
   *
   * @author HanWon.Jang
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    // 공백 댓글과 진행 중인 요청의 중복 제출은 서버에 전달하지 않는다
    if (comment.length === 0 || setReplyMutation.isPending) {
  ...
  const isSubmitDisabled =
    commentInput.trim().length === 0 || setReplyMutation.isPending;
