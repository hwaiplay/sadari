...
/**
 * 댓글 등록 일시가 같을 때 댓글 번호까지 비교하여 답글의 시간순 정렬을 확정함
 *
 * @author HanWon.Jang
...
const sortChildReplies = (
  ...
  // 서버 응답 배열을 변경하지 않고 답글 표시 순서만 정렬한 새 배열을 반환함
  return [...childReplies].sort(compareChildReplies);
};

/**
 * 댓글 작성자와 로그인 사용자의 일치 여부에 따라 이동할 프로필 경로를 생성함
 *
 * @author HanWon.Jang
...
export const getReplyProfilePath = (
  ...
  // 로그인 사용자가 작성한 댓글은 본인 프로필 편집 화면으로 연결함
  if (myReplyYn === "Y") {
    // 마이페이지 프로필 경로를 반환함
    return "/mypage/profile";
  }

  // 다른 사용자의 사용자 번호를 포함한 소셜 프로필 경로를 반환함
  return `/social/profile/${userNumb}`;
};

/**
 * 서버 댓글 목록을 부모별 댓글 묶음과 닉네임별 프로필 경로로 변환함
 *
 * @author HanWon.Jang
...
const createReplyCollection = (
  ...
  // 서버 목록을 부모 댓글과 부모 번호별 자식 댓글로 분리해 초기 화면에는 부모 댓글만 표시함
  for (const reply of replies) {
    // 댓글 내용의 닉네임 언급을 사용자 프로필에 연결할 수 있도록 작성자 정보를 보관함
    if (reply.userNick && reply.userNumb > 0) {
      // 본인 여부에 따라 생성한 프로필 경로를 중복되지 않는 닉네임과 연결함
    ...
    // 부모 여부가 명시된 댓글만 최상위 목록에 추가함
    if (reply.parentYn === "Y") {
      // 부모 댓글 렌더링 순서는 서버가 반환한 정렬 순서를 유지함
      parentReplies.push(reply);
      // 자식 댓글 분류를 이어서 처리함
      continue;
    }

    // 부모 여부가 N이 아닌 데이터는 자식 댓글로 분류하지 않음
    if (reply.parentYn !== "N") {
      // 정의되지 않은 부모 여부 값이 댓글 계층에 섞이지 않도록 다음 항목으로 이동함
      continue;
    }

    // 부모 번호가 없는 비부모 데이터는 연결 대상을 확정할 수 없어 자식 목록에서 제외함
    if (reply.uperNumb === null || reply.uperNumb === undefined) {
      // 잘못된 계층 데이터가 다른 댓글 아래에 표시되지 않도록 다음 항목으로 이동함
      continue;
    }

    const childReplies = childRepliesByParent.get(reply.uperNumb) ?? [];
    // 동일한 부모 번호를 가진 답글을 서버 정렬 순서대로 누적함
    childReplies.push(reply);
    // 부모 댓글 번호별 자식 댓글 목록을 최신 배열로 설정함
    childRepliesByParent.set(reply.uperNumb, childReplies);
  }

  const replyThreads: ReplyThread[] = [];

  // 부모 댓글의 서버 정렬 순서를 유지하면서 연결된 답글을 표시 순서로 정리함
  for (const parentReply of parentReplies) {
    // 부모 댓글과 시간순 답글 목록을 하나의 화면 단위로 추가함
    replyThreads.push({
      parentReply,
  ...
  // 댓글 화면이 반복 가공 없이 사용할 댓글 묶음과 프로필 경로를 반환함
  return {
    replyThreads,
    profilePathByNick,
  };
...
/**
 * 댓글 바텀시트의 조회, 등록, 수정, 삭제, 좋아요 및 화면 상호작용 상태를 통합 관리함
 *
 * @author HanWon.Jang
...
export const useReplySheetController = ({
  ...
  // 댓글 바텀시트의 닫기와 드래그 상호작용 속성을 조회함
  const sheetInteraction = useReplySheet({ onClose });
  // 댓글 등록 API와 연결된 입력 상태와 제출 이벤트를 조회함
  const replyForm = useSetReplyForm({
    ...target,
  });

  /**
   * 삭제된 댓글이 수정 중인 댓글과 같으면 하단 입력 폼을 일반 등록 상태로 되돌림
   *
   * @author HanWon.Jang
  ...
  const handleReplyDeleted = (deletedReplyNumb: number): void => {
    // 현재 수정 중인 댓글이 삭제된 경우에만 수정 입력 상태를 초기화함
    if (replyForm.editingReplyNumb === deletedReplyNumb) {
      // 삭제가 확정된 댓글의 수정 모드를 일반 댓글 등록 상태로 되돌림
      replyForm.handleCancelEditReply();
    }
  };

  // 본인 댓글 삭제 확인과 API 요청 상태를 조회함
  const deleteReply = useDelReply({
    ...target,
    onDeleted: handleReplyDeleted,
  });
  // 댓글 좋아요 등록과 취소 API 및 목록 캐시 갱신 상태를 조회함
  ...
  const [openActionReplyNumb, setOpenActionReplyNumb] = useState<
    number | null
  >(null);
  // 선택한 독후감의 댓글과 답글 목록을 서버 캐시에서 조회함
  const replyListQuery = useReplyList(target, focusReplNumb);
  // 서버 페이지가 바뀔 때만 부모 댓글과 연결 답글을 정렬 순서대로 연결함
  ...
  /**
   * 댓글 입력값 렌더링이 끝난 뒤 입력창의 포커스와 커서를 문자열 끝으로 이동함
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const focusCommentInput = (): void => {
    ...
    // 바텀시트가 닫히지 않고 입력창이 남아 있을 때만 포커스를 이동함
    if (input) {
      // 사용자가 댓글 내용을 바로 이어서 변경할 수 있도록 입력창에 포커스함
      input.focus();
      // 현재 입력값 뒤에서 작성을 계속할 수 있도록 커서를 문자열 끝으로 이동함
      input.setSelectionRange(input.value.length, input.value.length);
    }
  };

  /**
   * 선택한 부모 댓글의 자식 댓글 목록을 펼치거나 접음
   *
   * @author HanWon.Jang
  ...
  const handleToggleChildReplies = (replNumb: number): void => {
    /**
     * 다른 댓글의 펼침 상태를 유지하면서 선택한 댓글 상태만 반전함
     *
     * @author HanWon.Jang
    ...
    const getNextExpandedReplyMap = (
      ...
      // 선택한 부모 댓글의 펼침 여부만 반전한 새 상태를 반환함
      return {
        ...currentMap,
    ...
    // 다른 부모 댓글의 펼침 상태를 유지하면서 선택한 댓글 상태만 반전함
    setExpandedReplyMap(getNextExpandedReplyMap);
  };

  /**
   * 선택한 댓글 작성자를 언급하고 해당 댓글의 최상위 부모 번호로 답글 입력을 시작함
   *
   * @author HanWon.Jang
  ...
  const handleEditReply = (reply: ReplyDtoType): void => {
    // 선택 메뉴가 수정 입력창 위에 남지 않도록 열린 메뉴를 닫음
    setOpenActionReplyNumb(null);
    // 선택한 댓글 번호와 원문을 하단 댓글 입력 상태에 반영함
    replyForm.handleStartEditReply(reply.replNumb, reply.replCntn, reply.editVersion);
    // 입력값 렌더링이 끝난 프레임에 수정 입력 위치를 설정함
    window.requestAnimationFrame(focusCommentInput);
  };

  /**
   * 선택한 본인 댓글의 메뉴를 닫고 삭제 확인 및 API 처리를 시작함
   *
   * @author HanWon.Jang
  ...
  const handleDeleteReply = (replNumb: number): void => {
    // 삭제 확인 모달을 표시하기 전에 댓글 액션 메뉴를 닫음
    setOpenActionReplyNumb(null);
    // 사용자 확인 후 실제 댓글 삭제 API가 실행되도록 비동기 처리를 시작함
    void deleteReply.handleDeleteReply(replNumb);
  };

  /**
   * 선택한 댓글의 수정과 삭제 또는 신고와 차단 액션 메뉴를 펼치거나 닫음
   *
   * @author HanWon.Jang
  ...
  const handleToggleActionMenu = (replNumb: number): void => {
    /**
     * 현재 열린 댓글과 선택한 댓글을 비교하여 다음 메뉴 상태를 결정함
     *
     * @author HanWon.Jang
    ...
    const getNextActionReplyNumb = (
      ...
      // 같은 댓글은 메뉴를 닫고 다른 댓글은 해당 메뉴를 열도록 다음 번호를 반환함
      return currentReplNumb === replNumb ? null : replNumb;
    };

    // 하나의 댓글 액션 메뉴만 열리도록 현재 메뉴 상태를 변경함
    setOpenActionReplyNumb(getNextActionReplyNumb);
  };

  /**
   * 댓글 액션 메뉴에서 포커스가 완전히 벗어나면 메뉴를 닫음
   *
   * @author HanWon.Jang
  ...
  const handleActionMenuBlur = (event: FocusEvent<HTMLDivElement>): void => {
    // 메뉴 내부의 다른 버튼으로 포커스가 이동하는 동안에는 열린 상태를 유지함
    if (event.currentTarget.contains(event.relatedTarget)) {
      // 메뉴 내부 포커스 이동은 닫기 처리 없이 종료함
      return;
    }

    // 메뉴 밖으로 포커스가 이동하면 현재 열린 댓글 액션 메뉴를 닫음
    setOpenActionReplyNumb(null);
  };

  /**
   * Escape 키로 열린 댓글 액션 메뉴를 닫음
   *
   * @author HanWon.Jang
  ...
  const handleActionMenuKeyDown = (
    ...
    // Escape 키가 아닌 입력은 메뉴 내부 버튼의 기본 동작을 유지함
    if (event.key !== "Escape") {
      // 별도 키보드 처리 없이 종료함
      return;
    }

    // 브라우저의 추가 Escape 동작을 막고 현재 댓글 액션 메뉴를 닫음
    event.preventDefault();
    // 열린 댓글 액션 메뉴 번호를 초기화함
    setOpenActionReplyNumb(null);
  };

  /**
   * API가 연결되기 전 신고 또는 차단 메뉴 선택 시 열린 메뉴만 닫음
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleCloseActionMenu = (): void => {
    // 선택한 액션의 후속 API가 추가될 때까지 메뉴 표시 상태만 초기화함
    setOpenActionReplyNumb(null);
  };

  // 댓글 바텀시트 화면에서 사용할 기능 상태와 이벤트를 반환함
  return {
    ...sheetInteraction,
    ...replyForm,
    ...deleteReply,
