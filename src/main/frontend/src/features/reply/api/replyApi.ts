 ...
 * author         : Hanwon.Jang
 * date           : 2026-07-28
 * description    : 댓글과 답글 API 요청 및 응답 계약을 정의
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
export const setReplyApi = async (
  ...
  // 로그인 사용자 정보는 인증 쿠키에서 확인하고 댓글 요청 데이터만 서버에 전달
  const response = await api.post<SetReplyResponse>("/reply", data);

  // 서버가 반환한 공통 응답 코드가 성공인 경우에만 등록 결과를 반환
  return assertResultDataSuccess(response.data);
};

/**
 * 로그인 사용자가 작성한 댓글 또는 답글의 내용을 수정
 *
 * @author HanWon.Jang
...
export const uptReplyApi = async (
  ...
  // 복합 식별값을 경로에 포함하고 검증할 댓글 내용만 서버에 전달
  ...
  // 서버가 반환한 공통 응답 코드가 성공인 경우에만 수정 결과를 반환
  return assertResultDataSuccess(response.data);
};

/**
 * 로그인 사용자가 작성한 댓글 또는 답글을 삭제 상태로 전환
 *
 * @author HanWon.Jang
...
export const delReplyApi = async (
  ...
  // 복합 식별값을 경로에 포함하여 작성자와 계정 상태를 서버에서 검증
  const response = await api.delete<DelReplyResponse>(
    `/reply/${data.tagtType}/${data.tagtNumb}/${data.replNumb}`,
  );

  // 서버가 반환한 공통 응답 코드가 성공인 경우에만 삭제 결과를 반환
  return assertResultDataSuccess(response.data);
};

/**
 * 로그인 사용자의 미삭제 댓글 좋아요를 등록
 *
 * @author HanWon.Jang
...
export const setReplyLikeApi = async (
  ...
  // 댓글 복합 식별값을 경로에 포함하여 좋아요 등록 API를 호출
  ...
  // 서버가 반환한 공통 응답 코드가 성공인 경우에만 변경 상태를 반환
  return assertResultDataSuccess(response.data);
};

/**
 * 로그인 사용자의 미삭제 댓글 좋아요를 취소
 *
 * @author HanWon.Jang
 * @param data 좋아요 대상 댓글의 독후감 번호와 댓글 번호
 * @return 변경 후 댓글 좋아요 상태와 좋아요 수
...
export const delReplyLikeApi = async (
  ...
  // 댓글 복합 식별값을 경로에 포함하여 좋아요 취소 API를 호출
