...
/**
 * Query Key에 포함된 현재 경로로 사용자 메뉴 정보를 조회
 *
...
async function fetchUserMenu(
  ...
  // TM_URMENU에 등록된 현재 경로의 메뉴명과 사용자 메뉴 트리를 함께 조회
  const response = await getUserMenuApi(menuUrlx);

  // 응답 데이터가 비어 있어도 메뉴 미등록 상태로 처리할 수 있는 구조를 반환
  return {
...
/**
 * 현재 경로의 사용자 메뉴 정보를 공통 Query 캐시로 조회
 *
 * @author SeungHyeon.Kang
...
export const useUserMenuQuery = (menuUrlx: string, enabled = true) => {
  ...
  // 같은 경로를 조회하는 헤더와 로딩 화면이 서버 상태와 진행 상태를 공유하도록 반환
  return useQuery({
    queryKey,
