...
export const getPopupContentApi = async (
  ...
  // 팝업 복합 식별값을 바인딩하여 사용자 안내 콘텐츠를 조회
  ...
  // 공통 응답 코드를 검증하여 실패 응답을 React Query 오류 상태로 전달
  const result = assertResultDataSuccess(response.data);

  // 콘텐츠가 없으면 화면이 현재 기본 문구를 유지하도록 조회 실패로 처리
  if (!result.data) {
    // 팝업 콘텐츠 누락을 조회 실패로 전달할 오류 객체를 생성
    throw new Error("팝업 콘텐츠가 없습니다.");
  }

  // 검증된 사용자 안내 팝업 콘텐츠를 반환
  return result.data;
};
