...
/**
 * 사용자 안내 팝업 콘텐츠를 React Query 캐시에 저장하여 화면 간 재사용
 *
 * @author HanWon.Jang
...
export const usePopupContent = (popupContentKey: PopupContentKey) => {
  /**
   * 현재 화면이 요청한 복합 식별값으로 사용자 안내 팝업 콘텐츠를 조회
   *
   * @author HanWon.Jang
  ...
  const getPopupContentQuery = async (): Promise<PopupContentData> => {
    // 현재 화면의 팝업 복합 식별값으로 사용자 안내 콘텐츠를 조회
    const popupContent = await getPopupContentApi(popupContentKey);

    // 조회된 사용자 안내 팝업 콘텐츠를 React Query에 전달
    return popupContent;
  };

  // 같은 팝업 콘텐츠 요청이 여러 화면에서 중복 실행되지 않도록 공통 캐시 키를 사용
  return useQuery({
    queryKey: [
      "popupContent",
