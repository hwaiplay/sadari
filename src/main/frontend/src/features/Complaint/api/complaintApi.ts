...
/**
 * 인증 사용자의 콘텐츠 신고를 대상 원문 스냅샷과 함께 접수
 *
...
export const setComplaintApi = async (complaint: ComplaintCreate): Promise<number> => {
  ...
  // 서버가 대상 원문을 직접 확인해 저장하도록 대상 식별값과 사유만 전달
...
export const getPendingResultApi = async (): Promise<ComplaintPendingResult> => {
  ...
  if (!result.data) {
    // API 계약 누락을 호출 화면의 조회 실패 경로로 전달
    throw new Error("신고 조치 결과를 확인할 수 없습니다.");
  }

  // 팝업 표시와 확인 요청에 사용할 미확인 결과 요약을 반환
  return result.data;
};

/**
 * 팝업 조회 시점의 마지막 번호까지 신고 조치 결과를 확인 처리
 *
 * @author HanWon.Jang
...
export const uptResultConfirmApi = async (resultNumb: number): Promise<void> => {
  // 조회 이후 생성된 결과를 제외하도록 마지막 결과 번호를 경로에 전달
