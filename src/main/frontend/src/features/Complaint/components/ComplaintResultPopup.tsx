...
/** 수신자 유형에 맞춰 신고 조치 결과 한 건의 상세 문구를 생성함 */
const createResultText = (item: ComplaintResultItem): string => {
  // 피신고자에게는 신고번호와 접수일 및 정확한 누적 건수를 표시하지 않음
  ...
  // 신고자에게는 자신이 접수한 건을 구분할 신고번호와 사유 및 처리 결과를 표시함
  return [
...
/**
 * 활성 사용자가 아직 확인하지 않은 신고 조치 결과를 전용 팝업으로 안내함
 *
 * @author HanWon.Jang
 * @return 별도 화면 요소 없이 전역 알림만 표시함
 */
const ComplaintResultPopup = () => {
  // 같은 조회 결과가 렌더링 과정에서 중복 팝업으로 열리지 않도록 마지막 표시 번호를 보관함
  const shownResultNumbRef = useRef<number | null>(null);
  // 앱 재진입과 창 포커스 시 현재 활성 사용자의 미확인 결과를 다시 조회함
  ...
  // 새 미확인 결과가 조회되면 사용자가 확인할 때까지 닫히지 않는 안내를 표시함
  useEffect(() => {
    const pendingResult = pendingResultQuery.data;
    const lastRsltNumb = pendingResult?.lastRsltNumb;

    // 결과가 없거나 같은 조회 경계를 이미 표시했다면 팝업을 다시 열지 않음
    if (!pendingResult || pendingResult.rsltCntt < 1
        || !lastRsltNumb
        || shownResultNumbRef.current === lastRsltNumb) {
      // 현재 렌더링에서는 신고 조치 결과 안내를 생략함
      return;
    }

    // 현재 조회 경계를 먼저 기록해 Effect 재실행에 따른 중복 팝업을 차단함
    shownResultNumbRef.current = lastRsltNumb;

    /**
     * 사용자 확인 뒤 조회 시점까지의 신고 조치 결과만 확인 처리함
     *
     * @author HanWon.Jang
     * @return 반환값이 없음
     */
    const showComplaintResult = async (): Promise<void> => {
      ...
      // 서버가 수신자 정책에 맞춰 제공한 상세 결과를 팝업 문단으로 변환함
      const texts = pendingResult.resultList.map(createResultText);
      // 확인 버튼으로 읽은 경우에만 서버 확인 일시를 저장하도록 전용 안내를 표시함
      ...
      // 확인 버튼 이외의 사유로 알림이 종료되면 미확인 상태를 유지함
      if (!alertResult.isConfirmed) {
        // 다음 앱 접근에서 다시 표시할 수 있도록 현재 처리를 종료함
        return;
      }

      // 확인 요청 실패 시 결과를 남겨 다음 접근에서 다시 안내함
      try {
        await uptResultConfirmApi(lastRsltNumb);
        // 확인 처리 중 새로 생성된 결과가 있으면 새 팝업으로 이어지도록 최신 상태를 조회함
        await pendingResultQuery.refetch();
      }

      catch (error) {
        // 실패한 조회 경계를 다음 포커스 재조회에서 다시 표시할 수 있도록 해제함
