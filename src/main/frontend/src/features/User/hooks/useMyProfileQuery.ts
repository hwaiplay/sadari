...
/**
 * 헤더와 내비게이션 및 프로필 화면이 공유할 내 프로필 조회 옵션을 생성한다
 *
 * @author SeungHyeon.Kang
 * @return 내 프로필 React Query 옵션
 */
export function getMyProfileOptions() {
  // 같은 사용자 프로필 요청이 하나의 Query Key와 캐시를 사용하도록 옵션을 반환한다
  return queryOptions({
    queryKey: queryKeys.myProfile,
    /**
     * 로그인 사용자의 최신 프로필을 조회한다
     *
     * @author SeungHyeon.Kang
     * @return 로그인 사용자 프로필
...
/**
 * 로그인 사용자 프로필의 공유 서버 상태를 제공한다
 *
 * @author SeungHyeon.Kang
 * @return 내 프로필 조회 데이터와 요청 상태
 */
export function useMyProfileQuery() {
  // 공통 프로필 Query Key로 중복 요청을 합친 조회 상태를 반환한다
