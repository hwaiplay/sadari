...
/**
 * 서버 프로필 이미지 경로를 브라우저에서 사용할 수 있는 값으로 보정함
 *
 * @author HanWon.Jang
...
export function normalizeProfileImageSource(source?: string | null) {
  ...
  // 문자열 경로의 불필요한 공백을 제거해 공백만 있는 값을 이미지 요청에 사용하지 않게 함
  const normalizedSource = source?.trim() ?? "";
  // 대소문자 차이와 관계없이 비어 있는 문자열 값과 알려진 경로 형식을 판정함
  const lowerSource = normalizedSource.toLowerCase();

  // DB 또는 API에서 빈 값을 문자열로 반환한 경우에도 기본 프로필 이미지를 사용함
  if (!normalizedSource || lowerSource === "null" || lowerSource === "undefined") {
    // 기본 프로필 이미지 경로를 반환함
    return DEFAULT_PROFILE_IMAGE;
  }

  // HTTPS 화면에서 차단될 수 있는 Kakao 프로필 CDN 주소는 보안 프로토콜로 변환함
  if (lowerSource.startsWith("http://k.kakaocdn.net/")) {
    // Kakao 프로필 이미지의 경로는 유지하고 프로토콜만 HTTPS로 변경함
    return normalizedSource.replace("http://", "https://");
  }

  // 업로드 상대 경로에 루트 슬래시가 빠졌으면 현재 화면 URL에 종속되지 않게 보정함
  if (lowerSource.startsWith("uploads/")) {
    // 서버 업로드 정적 리소스의 루트 상대 경로를 반환함
...
/**
 * 프로필 이미지가 없거나 로드되지 않을 때 기본 프로필 이미지를 표시함
 *
 * @author HanWon.Jang
...
function ProfileImage({ src, ...imageProps }: ProfileImageProps) {
  ...
  // 화면에 전달할 프로필 이미지 경로를 빈 값 정책에 맞게 보정함
  const imageSource = normalizeProfileImageSource(src);

  /**
   * 원본 이미지 로드 실패 시 기본 프로필 이미지로 한 번만 교체함
   *
   * @author HanWon.Jang
  ...
  const handleImageError = (event: SyntheticEvent<HTMLImageElement>) => {
    ...
    // 기본 이미지 자체의 실패에서는 같은 경로를 반복 대입하지 않음
    if (failedImage.getAttribute("src") === DEFAULT_PROFILE_IMAGE) {
      // 추가 대체 처리 없이 종료함
      return;
    }

    // 만료되거나 존재하지 않는 원본 경로를 기본 프로필 이미지로 교체함
    failedImage.src = DEFAULT_PROFILE_IMAGE;
  };

  // 경로 보정과 로드 실패 대체가 적용된 프로필 이미지를 반환함
