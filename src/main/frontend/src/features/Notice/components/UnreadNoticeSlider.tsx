...
function getMarqueeDuration(notice: UnreadNotice): number {
  ...
  // 고정 카테고리를 제외한 제목 길이를 기준으로 읽기 쉬운 이동 속도를 유지
  const contentLength = notice.notiTitl.length;

  // 짧은 제목도 너무 빠르게 지나가지 않도록 최소 표시 시간을 보장
  return Math.max(
...
export function UnreadNoticeSlider() {
  ...
  /**
   * 현재 미읽음 공지 다음 항목을 순환 순서로 활성화
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleNoticeAdvance = useCallback((): void => {
    // 다음 공지는 처음 위치에서 가로 이동을 시작하도록 완료 상태를 초기화
    setIsMarqueeDone(false);
    // 최신 목록 길이를 기준으로 다음 공지 순번을 화면 상태에 반영
    setActiveIndex(getNextNoticeIndex);
  }, [getNextNoticeIndex]);

  /**
   * 제목 끝부분을 확인할 시간을 제공한 뒤 다음 공지 전환 예약을 생성
   *
   * @author SeungHyeon.Kang
  ...
  const setNoticeHoldTimer = useCallback((): (() => void) | undefined => {
    ...
    if (!isMarqueeDone || isPaused || noticeList.length <= 1) {
      // 전환 타이머를 만들지 않은 Effect 정리 상태를 반환
      return undefined;
    }

    // 제목 끝부분이 보이는 현재 위치를 3초 유지한 뒤 다음 공지로 전환
    const timerId = window.setTimeout(handleNoticeAdvance, NOTICE_END_HOLD_MS);

    /**
     * 공지 상태가 바뀌거나 컴포넌트가 해제되면 기존 전환 예약을 취소
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    function clearNoticeHoldTimer(): void {
      // 이전 공지의 대기 시간이 다음 공지 전환에 영향을 주지 않도록 예약을 제거
      window.clearTimeout(timerId);
    }

    // 다음 Effect 실행 전에 현재 공지 전환 예약을 정리할 함수를 반환
    return clearNoticeHoldTimer;
  }, [handleNoticeAdvance, isMarqueeDone, isPaused, noticeList.length]);

  // 제목 끝부분 노출과 사용자 조작 상태에 맞는 다음 공지 전환 예약을 관리
  useEffect(setNoticeHoldTimer, [setNoticeHoldTimer]);

  /**
   * 한 건 공지의 이어 붙인 제목이 왼쪽 경계에 도달하면 3초 뒤 가로 순환을 재개
   *
   * @author SeungHyeon.Kang
   * @return 예약된 타이머를 해제할 정리 함수 또는 미예약 상태
   */
  const setSingleNoticeHoldTimer = useCallback((): (() => void) | undefined => {
    // 한 건 공지의 반복 경계에서만 다음 순환 재개를 예약
    if (!isSingleNoticeHolding || noticeList.length !== 1) {
      // 순환 재개 타이머를 만들지 않은 Effect 정리 상태를 반환
      return undefined;
    }

    /**
     * 왼쪽 경계에서 제공한 확인 시간이 끝나면 한 건 공지의 가로 순환을 재개
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    function releaseSingleNoticeHold(): void {
      // 다음 회차가 왼쪽 경계에서 출발할 수 있도록 반복 대기 상태를 해제
      setIsSingleNoticeHolding(false);
    }

    // 이어 붙인 제목이 왼쪽 경계에 걸린 상태를 3초 유지한 뒤 다음 가로 순환을 재개
    const timerId = window.setTimeout(releaseSingleNoticeHold, NOTICE_LEFT_EDGE_HOLD_MS);

    /**
     * 공지 상태가 바뀌거나 컴포넌트가 해제되면 기존 순환 재개 예약을 취소
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    function clearSingleNoticeHoldTimer(): void {
      // 이전 공지의 대기 시간이 새 공지의 가로 순환에 영향을 주지 않도록 예약을 제거
      window.clearTimeout(timerId);
    }

    // 다음 Effect 실행 전에 한 건 공지의 순환 재개 예약을 정리할 함수를 반환
    return clearSingleNoticeHoldTimer;
  }, [isSingleNoticeHolding, noticeList.length]);

  // 한 건 공지의 이어 붙인 제목이 왼쪽 경계에 도달한 시점부터 3초 대기 시간을 관리
  useEffect(setSingleNoticeHoldTimer, [setSingleNoticeHoldTimer]);

  /**
   * 새 미읽음 공지 목록을 받으면 첫 번째 공지부터 다시 표시
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const resetActiveNotice = useCallback((): void => {
    // 새 목록의 첫 공지가 가로 이동을 완료하지 않은 상태로 표시되도록 초기화
    setIsMarqueeDone(false);
    // 새 목록은 첫 공지의 최초 표시 대기부터 시작하도록 반복 경계 대기 상태를 초기화
    setIsSingleNoticeHolding(false);
    // 상세 조회로 목록이 줄어든 경우에도 유효한 첫 항목부터 표시
    setActiveIndex(0);
  }, [noticeList]);

  // 서버의 미읽음 공지 목록이 바뀌면 첫 번째 제목으로 표시 순서를 초기화
  ...
  function handleNoticePause(): void {
    // 사용자가 읽고 선택할 현재 공지 제목을 고정
    setIsPaused(true);
  }

  /**
   * 현재 공지 조작이 끝나면 다음 제목 자동 교체를 재개
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  function handleNoticeResume(): void {
    // 전체 표시 간격을 다시 제공한 뒤 다음 공지가 나오도록 재개 상태를 설정
    setIsPaused(false);
  }

  /**
   * 여러 공지 중 현재 항목의 가로 슬라이드가 끝나면 다음 공지를 활성화
   *
   * @author SeungHyeon.Kang
  ...
  function handleMarqueeEnd(event: AnimationEvent<HTMLSpanElement>): void {
    ...
    // 하위 요소의 이벤트이거나 공지가 하나뿐이면 현재 공지의 가로 반복을 유지
    if (event.target !== event.currentTarget || noticeList.length <= 1) {
      // 다음 공지로 전환하지 않고 현재 가로 슬라이드를 계속
      return;
    }

    // 제목 끝부분이 보이는 현재 위치에서 3초 대기를 시작하도록 완료 상태를 기록
  ...
  function handleSingleMarqueeIteration(event: AnimationEvent<HTMLSpanElement>): void {
    ...
    if (event.target !== event.currentTarget || noticeList.length !== 1) {
      // 현재 애니메이션 상태를 유지하고 반복 경계 처리를 종료
      return;
    }

    // 이어 붙인 제목이 왼쪽 끝에 걸린 현재 위치에서 3초 대기를 시작
    setIsSingleNoticeHolding(true);
  }

  // 조회 중이거나 실패했거나 미읽음 공지가 없으면 홈의 기존 영역을 유지
  ...
  // 현재 미읽음 공지 제목 한 건과 상세 이동을 제공하는 가로 슬라이드를 반환
  return (
    <section
