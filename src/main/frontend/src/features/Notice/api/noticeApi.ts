...
/**
 * 홈 화면에 표시할 로그인 사용자의 미읽음 공지 카테고리와 제목 목록을 조회한다
 *
 * @author SeungHyeon.Kang
...
export const getUnreadNoticeListApi = async (): Promise<UnreadNotice[]> => {
  ...
  // 응답 데이터가 없으면 홈 미읽음 공지 안내를 숨길 수 있도록 빈 목록을 반환한다
  return result.data ?? [];
};
...
/**
 * 주키에 해당하는 현재 배포 공지사항의 읽음 이력을 저장한다.
 *
 * @author SeungHyeon.Kang
