...
function formatPlayerTime(totalSeconds: number): string {
  ...
  // 홈 플레이어에 표시할 시분초 문자열을 반환
  return `${hours}:${minutes}:${seconds}`;
}

/**
 * 현재 세션의 목표시간 여부에 맞는 홈 플레이어 표시 초를 계산
 *
 * @author SeungHyeon.Kang
...
function getPlayerTimeSeconds(timer: ReadingTimer, elapsedSeconds: number): number {
  ...
  // 목표시간이 설정된 세션은 타이머 화면과 동일하게 남은 시간을 표시
  if (typeof timer.targSecs === "number") {
    // 목표시간을 넘긴 경우에도 음수 시간이 표시되지 않게 제한해 반환
    return Math.max(0, timer.targSecs - elapsedSeconds);
  }

  // 일반 세션은 현재까지 누적한 독서 시간을 반환
  return elapsedSeconds;
}

/**
 * 홈의 독후감 등록 버튼 왼쪽에 활성 독서 타이머 제어 플레이어를 표시
 *
 * @author SeungHyeon.Kang
...
export function HomeTimerPlayer() {
  ...
  useEffect(() => {
    // 서버에 활성 세션이 있을 때만 닫힘 애니메이션용 이전 상태를 최신값으로 교체
    if (!activeTimer) {
      // 활성 세션이 없는 동안에는 완료 애니메이션의 이전 세션을 유지
      return;
    }

    // 캐시 수신 시각부터 실제로 흐른 시간을 포함해 홈 플레이어에 반영
    setElapsedSeconds(getLiveTimerSecs(timerSummaryQuery.data));
    // 새 활성 세션에는 이전 완료 애니메이션 상태를 적용하지 않음
    setClosingTimer(undefined);
    // 활성 세션 플레이어를 정상 위치에 표시
    setIsExiting(false);
  ...
  useEffect(() => {
    ...
    const tickPlayer = (): void => {
      ...
      // interval 지연과 백그라운드 정지를 건너뛰도록 동기화 시각부터 실제 경과 초를 다시 계산
      setElapsedSeconds(getLiveTimerSecs(timerSummaryQuery.data));
    };

    // 실행 중인 홈 플레이어를 1초 간격으로 갱신
    const intervalId = window.setInterval(tickPlayer, 1000);

    /**
     * 홈 플레이어 상태가 바뀌거나 컴포넌트가 해제되면 초 증가 작업을 정리
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const clearPlayerInterval = (): void => {
      ...
      // 더 이상 사용하지 않는 반복 작업을 브라우저에서 제거
      window.clearInterval(intervalId);
    };

    // 홈 플레이어 상태 변경 시 실행할 반복 작업 정리 함수를 반환
    return clearPlayerInterval;
  }, [activeTimer?.tmrxStat, timerSummaryQuery.data]);

  useEffect(() => {
    /**
     * 홈 화면을 벗어날 때 예약된 플레이어 종료 애니메이션 작업을 정리
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const clearPlayerExit = (): void => {
      ...
      // 종료 예약이 있는 경우에만 브라우저 타이머를 제거
      if (exitTimerRef.current !== undefined) {
        // 언마운트 뒤 닫힘 상태가 변경되지 않도록 예약을 취소
        window.clearTimeout(exitTimerRef.current);
      }
    };

    // 홈 타이머 플레이어가 해제될 때 실행할 종료 예약 정리 함수를 반환
    return clearPlayerExit;
  }, []);

  /**
   * 서버의 상태 변경 결과를 홈 플레이어와 공통 타이머 캐시에 반영
   *
   * @author SeungHyeon.Kang
  ...
  const applySummary = useCallback((nextSummary: ReadingTimerSummary): void => {
    ...
    // 상태 변경 응답에 최초 브라우저 수신 시각을 기록
    const syncedSummary = syncTimerSummary(nextSummary);
    // 타이머 화면과 내비게이션이 즉시 같은 결과를 사용하도록 공통 캐시를 갱신
  ...
  /**
   * 완료 저장이 끝난 홈 타이머 플레이어를 화면 아래로 내린 뒤 제거
   *
   * @author SeungHyeon.Kang
  ...
  const closeCompletedPlayer = useCallback((completedTimer: ReadingTimer): void => {
    ...
    // 서버 응답에서 활성 세션이 사라져도 애니메이션 동안 표시할 세션을 보관
    setClosingTimer(completedTimer);
    // 플레이어가 아래로 내려가는 종료 전환을 시작
    setIsExiting(true);

    /**
     * 종료 전환이 끝난 플레이어의 이전 세션 정보를 제거
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const finishPlayerExit = (): void => {
      ...
      // 종료 애니메이션이 끝난 플레이어를 렌더링에서 제거
      setClosingTimer(undefined);
      // 다음 세션이 정상 위치에서 표시되도록 종료 상태를 초기화
      setIsExiting(false);
      // 소비한 브라우저 타이머 식별값을 초기화
      exitTimerRef.current = undefined;
    };

    // CSS 전환 시간 뒤 이전 타이머 플레이어를 완전히 제거
    exitTimerRef.current = window.setTimeout(finishPlayerExit, PLAYER_EXIT_DURATION_MS);
  }, []);

  /**
   * 현재 홈 독서 타이머를 요청한 상태로 변경
   *
   * @author SeungHyeon.Kang
  ...
  const changeTimerStatus = useCallback(async (
    ...
    if (!activeTimer || isChanging) {
      // 처리할 수 없는 현재 상태에서 타이머 제어를 종료
      return;
    }

    // 연속 제어 입력을 막도록 홈 플레이어를 처리 중 상태로 변경
    setIsChanging(true);
    try {
      // 재생과 일시정지는 플레이어 안에서 즉시 처리하므로 화면 전체 처리 중 알림을 생략
      const skipBlockingOperation = forceSkipBlocking || targetStatus !== "COMPLETED";
      // 사용자 소유 타이머의 일시정지, 재개 또는 완료 저장을 서버에 요청
      const response = await uptReadingTimerApi(activeTimer.tmrxNumb, targetStatus, skipBlockingOperation);

      // 서버가 최신 타이머 요약을 반환한 경우에만 공통 화면 상태를 갱신
      if (response.data) {
        // 완료 상태는 캐시에서 활성 세션을 제거하기 전에 애니메이션용 세션을 보관
        if (targetStatus === "COMPLETED") {
          // 저장이 끝난 플레이어의 아래 방향 종료 전환을 시작
          closeCompletedPlayer(activeTimer);
        }

        // 검증된 서버 상태를 홈, 타이머 및 내비게이션에 함께 반영
        applySummary(response.data);

        // 완료 저장은 도서별 누적 독서시간의 기존 페이지를 다음 조회 전에 초기화
        if (targetStatus === "COMPLETED") {
          // 최신 완료 세션을 첫 페이지부터 다시 조회하도록 도서별 누적 캐시를 제거
  ...
  useEffect(() => {
    ...
    // 서버 응답을 기다리는 동안 같은 세션이 다시 요청되지 않도록 번호를 보관
  ...
  const pauseTimer = (): void => {
    ...
    // 현재 세션의 측정 구간을 서버에 확정하고 일시정지
    void changeTimerStatus("PAUSED");
  };

  /**
   * 일시정지한 홈 독서 타이머를 다시 실행
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const resumeTimer = (): void => {
    ...
    // 서버 기준 새 측정 구간을 시작하도록 현재 세션을 재개
    void changeTimerStatus("RUNNING");
  };

  /**
   * 홈 독서 타이머를 완료하고 확정 독서시간을 저장
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const completeTimer = (): void => {
    ...
    // 마지막 측정 구간을 저장하고 세션을 다시 재개할 수 없는 완료 상태로 변경
  ...
  // 최상위 문서에서 독후감 등록 버튼 왼쪽에 고정되는 홈 타이머 플레이어를 반환
  return createPortal(
    /* 홈 활성 독서 타이머 플레이어 영역 */
    <section
