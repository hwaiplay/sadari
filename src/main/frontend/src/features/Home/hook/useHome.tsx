 ...
 * author         : Hanwon.Jang
 * date           : 2026-08-14
 * description    : 홈 화면의 독후감 조회와 검색 및 정렬 상태를 관리
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
function getMonthGroup(
  ...
  // 선택한 날짜 정렬 기준과 동일한 독서 날짜를 그룹 계산에 사용
  const targetDate =
    sortType === "START_DATE_DESC" ? book.reptStdt : book.reptEndt;
  // 날짜 문자열에서 연도와 월만 안전하게 추출
  const match = targetDate?.match(/^(\d{4})-(\d{2})/);

  // 독서 날짜가 없거나 형식이 잘못되면 별도 그룹으로 구분
  if (!match) {
    // 날짜가 없는 독후감의 그룹 정보를 반환
    return {
      key: "unknown",
      // "날짜 없음"
...
function getGradeGroup(book: HomeBookType): HomeGroupLabel {
  ...
  // 숫자로 변환할 수 있는 평점만 화면에서 지원하는 0점부터 5점 범위로 제한
  ...
  // 평점이 없을 때도 평점 기준 그룹임을 식별할 수 있도록 빈 별을 표시
  if (grade === 0) {
    // 빈 별 아이콘을 평점 그룹 라벨에 추가
    gradeIcons.push(
      <img
        key="empty-star"
...
function getBookRows(bookList: HomeBookType[], size: number): HomeBookType[][] {
  // 독후감 목록을 화면 그리드의 행 단위로 변환
  return Array.from(
    { length: Math.ceil(bookList.length / size) },
    /**
     * 행 순서에 해당하는 독후감 목록을 추출
     *
     * @author Hanwon.Jang
...
function getBookGroups(
  ...
  // 각 그룹을 화면 그리드의 행 구조로 변환해 반환
  return groups.map(
    /**
     * 독후감 그룹을 화면 렌더링 모델로 변환
     *
     * @author Hanwon.Jang
...
/**
 * 홈 화면의 독후감 조회 상태와 검색 및 정렬 동작을 제공
 *
 * @author Hanwon.Jang
...
export function useHome() {
  ...
  const handleSearchSubmit = (event: FormEvent<HTMLFormElement>): void => {
    // 브라우저 폼 제출로 전체 화면이 새로고침되지 않도록 처리
    event.preventDefault();
    // 검색어 앞뒤 공백을 제거한 값을 목록 조회 조건으로 적용
