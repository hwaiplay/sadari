 ...
 * author         : Hanwon.Jang
 * date           : 2026-08-14
 * description    : 홈 화면의 독후감 조회와 검색 및 정렬 상태를 관리함
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
function getMonthGroup(
  ...
  // 선택한 날짜 정렬 기준과 동일한 독서 날짜를 그룹 계산에 사용함
  const targetDate =
    sortType === "START_DATE_DESC" ? book.reptStdt : book.reptEndt;
  // 날짜 문자열에서 연도와 월만 안전하게 추출함
  const match = targetDate?.match(/^(\d{4})-(\d{2})/);

  // 독서 날짜가 없거나 형식이 잘못되면 별도 그룹으로 구분함
  if (!match) {
    // 날짜가 없는 독후감의 그룹 정보를 반환함
    return {
      key: "unknown",
      // "날짜 없음"
...
function getGradeGroup(book: HomeBookType): HomeGroupLabel {
  ...
  // 숫자로 변환할 수 있는 평점만 화면에서 지원하는 0점부터 5점 범위로 제한함
  ...
  // 평점이 없을 때도 평점 기준 그룹임을 식별할 수 있도록 빈 별을 표시함
  if (grade === 0) {
    // 빈 별 아이콘을 평점 그룹 라벨에 추가함
    gradeIcons.push(
      <img
        key="empty-star"
...
function getBookRows(bookList: HomeBookType[], size: number): HomeBookType[][] {
  // 독후감 목록을 화면 그리드의 행 단위로 변환함
  return Array.from(
    { length: Math.ceil(bookList.length / size) },
    /**
     * 행 순서에 해당하는 독후감 목록을 추출함
     *
     * @author Hanwon.Jang
...
function getBookGroups(
  ...
  // 서버 정렬 순서를 유지하면서 연속된 독후감을 같은 그룹으로 묶음
  for (const book of bookList) {
    // 평점 정렬은 별점으로 묶고 날짜 정렬은 해당 연월로 묶음
    const groupLabel =
      sortType === "GRADE_DESC"
        ? getGradeGroup(book)
  ...
  // 각 그룹을 화면 그리드의 행 구조로 변환해 반환함
  return groups.map(
    /**
     * 독후감 그룹을 화면 렌더링 모델로 변환함
     *
     * @author Hanwon.Jang
...
/**
 * 홈 화면의 독후감 조회 상태와 검색 및 정렬 동작을 제공함
 *
 * @author Hanwon.Jang
...
export function useHome() {
  ...
  const resetHomeSearch = (): void => {
    ...
    // 홈 검색 초기화 요청이 없으면 현재 검색 조건을 유지함
  ...
  const handleSearchSubmit = (event: FormEvent<HTMLFormElement>): void => {
    // 브라우저 폼 제출로 전체 화면이 새로고침되지 않게 함
    event.preventDefault();
    // 검색어 앞뒤 공백을 제거한 값을 목록 조회 조건으로 적용함
    setAppliedSearchKeyword(searchKeyword.trim());
  };

  /**
   * 홈 독후감 목록의 정렬 기준을 변경함
   *
   * @author Hanwon.Jang
