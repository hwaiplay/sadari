...
/**
 * 도서별 누적 독서 시간을 최근 기록순 서버 페이지로 조회
 *
...
export function useBookTimeQuery() {
  ...
  // 모든 누적시간 페이지가 동일한 타이머 전용 캐시를 사용하도록 조회 상태를 반환
  return useInfiniteQuery({
    queryKey: queryKeys.readingTimerBookTimes,
    /**
     * 현재 페이지의 도서별 누적 독서 시간 최대 20건을 조회
     *
     * @author SeungHyeon.Kang
    ...
    queryFn: async ({ pageParam }) => {
      // 서버가 검증하는 현재 페이지 번호를 전용 API에 전달
      return await getBookTimePageApi(pageParam);
    },
    initialPageParam: 1,
    /**
     * 마지막 누적시간 응답에서 이어서 조회할 페이지 번호를 계산
     *
     * @author SeungHyeon.Kang
