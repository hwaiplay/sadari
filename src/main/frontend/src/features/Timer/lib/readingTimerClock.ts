...
/**
 * 서버 타이머 요약에 브라우저 수신 기준 시각을 한 번만 기록
 *
...
export function syncTimerSummary(summary: ReadingTimerSummary): ReadingTimerSummary {
  // 캐시에서 다시 사용한 요약은 최초 수신 시각을 유지해 누락된 경과시간을 보존
  if (summary.clientSyncMillis !== undefined) {
    // 이미 동기화된 타이머 요약을 그대로 반환
    return summary;
  }

  // 서버 응답을 받은 브라우저 기준 시각을 현재 타이머 계산 기준으로 조회
  const clientSyncMillis = Date.now();
  // 서버 누적시간과 브라우저 경과시간을 함께 계산할 새 요약을 반환
  return { ...summary, clientSyncMillis };
}

/**
 * 서버 누적시간과 실제 브라우저 경과시간으로 실행 중인 타이머 초를 계산
 *
 * @author SeungHyeon.Kang
