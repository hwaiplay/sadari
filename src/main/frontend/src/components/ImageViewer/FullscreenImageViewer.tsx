...
export function ImageViewerProvider({
  ...
  // 전체 화면 이미지가 열려 있는 동안 배경 문서의 스크롤을 잠금
  ...
  useLayoutEffect(
    ...
    function focusViewerCloseButton(): void {
      ...
      // 전체 화면이 열린 직후 유일한 명령인 닫기 버튼에 키보드 초점을 둠
  ...
  useEffect(
    /**
     * 이미지 뷰어가 닫힌 뒤 사용자가 열었던 버튼으로 초점을 되돌림
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
...
export function useFullscreenImageViewer(): FullscreenImageViewerContextValue {
  ...
  // 최상위 공급자 밖에서 호출된 잘못된 구성은 개발 단계에서 즉시 알림
  if (!context) {
    // 필수 공급자가 누락된 상태로 화면을 계속 렌더링하지 않음
...
export function FullscreenImageButton({
  ...
  useEffect(
    /**
     * 알림 경로가 지정한 현재 사진을 최초 렌더링에서 전체 화면으로 엶
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
  ...
  /**
   * 현재 버튼에 표시된 이미지를 전체 화면 원본 보기로 엶
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
