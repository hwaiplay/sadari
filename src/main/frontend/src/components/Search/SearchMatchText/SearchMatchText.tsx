...
/**
 * 검색 결과 텍스트에서 현재 검색어와 일치하는 모든 부분을 브랜드 색상으로 강조한다
 *
 * @author HanWon.Jang
...
const SearchMatchText = ({ text, keyword }: SearchMatchTextProps): ReactNode => {
  // 화면에 표시 중인 검색어의 양끝 공백을 제거한다
  const normalizedKeyword = keyword.trim();

  // 원문이나 검색어가 없으면 강조 요소를 만들지 않는다
  if (!text || !normalizedKeyword) {
    // 서버 원문을 변경하지 않고 반환한다
    return text;
  }

  // 영문 검색도 대소문자와 관계없이 강조할 수 있도록 비교값을 소문자로 변환한다
  const normalizedText = text.toLocaleLowerCase();
  // 표시 원문은 유지하고 위치 판정에만 소문자 검색어를 사용한다
  const comparableKeyword = normalizedKeyword.toLocaleLowerCase();
  // 원문과 강조 요소를 표시 순서대로 담을 목록을 생성한다
  const highlightedParts: ReactNode[] = [];
  // 첫 번째 일치 부분 전까지의 원문 시작 위치를 관리한다
  let currentIndex = 0;
  // 현재 검색어가 처음 등장하는 원문 위치를 조회한다
  let matchIndex = normalizedText.indexOf(comparableKeyword);

  // 원문에 반복되는 검색어도 빠짐없이 같은 색상으로 강조한다
  while (matchIndex >= 0) {
    // 일치 부분 앞의 원문을 기존 색상으로 유지한다
