...
/**
 * 기존 앱 이력을 남기지 않고 홈 루트로 이동하는 링크를 표시한다
 *
...
const HomeLink = ({
  ...
  /**
   * 일반 클릭은 홈 루트 이동으로 처리하고 새 탭 클릭은 링크 기본 동작을 유지한다
   *
   * @author SeungHyeon.Kang
  ...
  const handleClick = (event: MouseEvent<HTMLAnchorElement>): void => {
    ...
    // 새 탭 또는 새 창을 요청한 클릭은 브라우저의 링크 동작을 유지한다
    if (event.button !== 0 || event.metaKey
        || event.ctrlKey || event.shiftKey
        || event.altKey) {
      // 수정 키가 포함된 클릭을 SPA 루트 이동으로 가로채지 않고 종료한다
      return;
    }
