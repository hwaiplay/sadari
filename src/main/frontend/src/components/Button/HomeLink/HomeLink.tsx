...
/**
 * 기존 앱 이력을 남기지 않고 홈 루트로 이동하는 링크를 표시
 *
...
const HomeLink = ({
  ...
  // 홈 링크가 모든 중간 이력을 제거하도록 공통 루트 이동 함수를 조회
  const moveHome = useHomeNavigation();

  /**
   * 일반 클릭은 홈 루트 이동으로 처리하고 새 탭 클릭은 링크 기본 동작을 유지
   *
   * @author SeungHyeon.Kang
  ...
  const handleClick = (event: MouseEvent<HTMLAnchorElement>): void => {
    ...
    // 새 탭 또는 새 창을 요청한 클릭은 브라우저의 링크 동작을 유지
    if (event.button !== 0 || event.metaKey
        || event.ctrlKey || event.shiftKey
        || event.altKey) {
      // 수정 키가 포함된 클릭을 SPA 루트 이동으로 가로채지 않고 종료
      return;
    }

    // 일반 링크가 홈 이력을 새로 추가하지 않도록 기본 이동을 차단
    event.preventDefault();
    // 현재 앱 이력을 정리한 뒤 홈 검색 초기화 정책과 함께 루트로 이동
    moveHome({
      resetHomeSearch,
  ...
  // 새 탭 접근성을 유지하면서 일반 클릭만 홈 루트 복귀로 처리하는 링크를 반환
  return (
