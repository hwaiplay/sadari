...
/**
 * 배경사진의 실제 로드 상태를 추적하여 완료 전까지 영역 중앙에 공통 소형 회전 링을 표시
 *
 * @author HanWon.Jang
...
const BackgroundImage = ({
  ...
  /**
   * 현재 표시 대상 배경사진의 로드 완료 상태를 반영
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleImageLoad = (): void => {
    // 이전 경로의 최종 실패 상태가 현재 표시 이미지에 영향을 주지 않게 초기화
    setUnavailableSource(null);
    // 현재 원본 또는 대체 배경사진의 로드 완료 경로를 저장
    setLoadedSource(displayedSource);
  };

  /**
   * 원본 배경사진 요청 실패 시 지정된 대체 이미지를 한 번만 요청
   *
   * @author HanWon.Jang
   * @return 반환값이 없음
   */
  const handleImageError = (): void => {
    // 아직 대체 이미지를 요청하지 않았다면 원본 실패 상태를 기록해 경로를 교체
    if (canUseFallback && !isFallbackActive) {
      // 실패한 원본 경로를 저장하여 다음 렌더링에서 대체 이미지를 사용
      setFailedSource(source);
      // 대체 이미지 로드 완료 전까지 회전 링을 유지하도록 종료
      return;
    }

    // 대체 이미지가 없거나 대체 요청도 실패하면 회전 링이 계속 남지 않게 종료 상태로 처리
    setUnavailableSource(displayedSource);
    // 실패한 이미지 요소는 숨기되 현재 경로의 로드 상태 추적을 종료
    setLoadedSource(displayedSource);
  };

  // 실제 이미지와 로드 중 공통 소형 회전 링을 같은 배경 영역에 반환
  return (
    <>
      <img
        key={displayedSource}
