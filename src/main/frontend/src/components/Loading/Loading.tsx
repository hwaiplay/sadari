...
/**
 * 별도 문구 또는 현재 메뉴명으로 최종 로딩 제목을 결정한다
 *
...
function resolveLoadingTitle(title?: string, menuName?: string) {
  ...
  // 로그인이나 등록처럼 호출 화면이 작업 문구를 지정하면 메뉴명보다 우선한다
  if (title !== undefined) {
    // 호출 화면이 지정한 작업 문구를 그대로 반환한다
    return title;
  }

  // 현재 경로에 메뉴명이 등록되어 있으면 메뉴 대상을 포함한 조회 문구를 사용한다
  if (menuName) {
    // "{메뉴명} 조회 중"
    const menuLoadingTitle = message("frontend.common.loadingMenu", [menuName]);

    // TM_URMENU의 메뉴명을 포함한 조회 문구를 반환한다
  ...
  // 등록된 메뉴명이 없거나 메뉴 조회가 실패하면 공통 목록 문구를 반환한다
...
const Loading = ({
  ...
  // 현재 브라우저 경로에 대응하는 사용자 메뉴를 조회하기 위해 위치 정보를 가져온다
  const location = useLocation();
  const shouldUseMenuTitle = !isCompact && title === undefined;
  // 헤더와 동일한 Query Key로 현재 경로의 메뉴 조회 결과를 공유한다
  const { data: userMenuData } = useUserMenuQuery(
    location.pathname,
    shouldUseMenuTitle,
  );
  // 공백만 저장된 메뉴명은 미등록 상태로 처리하기 위해 표시값을 정리한다
  const menuName = userMenuData?.currentMenu?.menuName?.trim();
  // 호출 화면의 작업 문구와 현재 메뉴명 우선순위에 따라 최종 제목을 결정한다
  const loadingTitle = resolveLoadingTitle(title, menuName);

  // 모달과 페이지 및 내부 영역이 각각 필요한 높이 스타일을 사용하도록 구분한다
  ...
  return (
    ...
    >
      {/* 모달에서는 공통 회전 링을 감싼 영역만 축소하여 같은 애니메이션을 유지한다 */}
      <div className={isCompact ? styles.compactSpinner : undefined}>
        <div className={styles.spinner} aria-hidden="true" />
      </div>
      {!isCompact && (
        /* 현재 경로에 등록된 메뉴명으로 조회 상태를 안내하고 메뉴가 없으면 목록 문구를 사용한다 */
