...
const ReportMenuOptions = ({
  ...
  // 표시할 신고 선택지가 없으면 재귀 렌더링을 종료
  if (!option) {
    // 남은 신고 선택지가 없는 빈 영역을 반환
    return null;
  }

  /**
   * 현재 메뉴 항목의 신고 대상을 상위 메뉴에 전달
   *
   * @author SeungHyeon.Kang
   * @return 반환값이 없음
   */
  const handleOptionClick = (): void => {
    // 사용자가 선택한 신고 대상을 신고 화면 이동 처리에 전달
    onSelect(option.target);
  };

  // 현재 신고 항목 뒤에 남은 신고 항목을 이어서 표시
  return (
    <>
      <button
...
/**
 * 다른 사용자를 신고하거나 차단할 수 있는 공통 더보기 메뉴를 표시
 *
 * @author HanWon.Jang
...
const UserActionMenu = ({
  ...
  const handleBlockClick = async (): Promise<void> => {
    ...
    const blockUser = async (): Promise<void> => {
      // 공통 메뉴가 가진 신고 대상의 사용자 번호로 실제 차단 관계를 등록
      await setUserBlockApi(reportTarget.userNumb);
      // 차단 완료 뒤 현재 화면이 제공한 캐시와 이동 후처리를 실행
      await onBlockConfirm?.();
    };
    ...
    try {
      // 사용자가 확인하면 처리 중 화면과 이동 가드를 유지한 채 차단
      const isConfirmed = await confirmUserBlock(userNick, async () => {
        // 차단 처리 완료까지 같은 모달을 유지하고 성공 상태로 전환
      ...
      // 차단 완료 후 화면별 콜백이 없으면 안전한 이전 화면 또는 현재 목록 새로 조회로 이동
      if (isConfirmed && !onBlockConfirm) {
        ...
        if (window.location.pathname.startsWith("/social/profile/")
          ...
          // 차단 성공 확인 후 사용자가 보던 안전한 이전 화면으로 이동
          navigate(-1);
        }

        // 목록과 댓글 화면은 서버 원본의 첫 페이지를 다시 조회
        else {
          // 현재 경로를 다시 열어 차단 사용자의 항목과 집계를 서버 기준으로 제거
          navigate(0);
        }
      }
    }
      ...
      // 화면 이동이 없으면 키보드 사용자가 기존 더보기 버튼에서 조작을 이어가도록 초점을 복원
