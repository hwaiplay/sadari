...
export const useHeaderTitle = (title?: string | null): void => {
  ...
  const syncHeaderTitle = (): (() => void) => {
    ...
    // 전달된 화면 제목을 공통 헤더 상태에 반영
    setHeaderTitle(normalizedTitle);

    /**
     * 현재 화면이 닫히면 등록한 헤더 제목을 제거
     *
     * @author SeungHyeon.Kang
     * @return 반환값이 없음
     */
    const clearHeaderTitle = (): void => {
      ...
      // 다음 화면에 이전 동적 제목이 남지 않도록 현재 제목을 해제
      setHeaderTitle(null);
    };

    // 화면 해제 시 사용할 제목 정리 함수를 반환
    return clearHeaderTitle;
  };

  // 제목 값이 바뀌면 공통 헤더에 최신 문자열을 연결
