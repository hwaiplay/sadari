...
/**
 * 브라우저가 로컬 개발 서버에 보낸 Origin을 내부 백엔드 프록시 요청에서 제거
 *
 * @author HanWon.Jang
...
function removeForwardedOrigin(proxyRequest: ClientRequest): void {
  ...
  // 브라우저와 Vite 사이에서는 동일 출처 요청이므로 내부 전달 단계에서 외부 CORS 요청으로 오인되지 않도록 처리
  proxyRequest.removeHeader("origin");
}

/**
 * 로컬 개발 프록시에 동일 출처 요청 헤더 보정 처리를 등록
 *
 * @author HanWon.Jang
...
function configureDevelopmentProxy(proxy: HttpProxy.ProxyServer): void {
  ...
  // POST와 PUT 저장 요청도 조회 요청과 같은 내부 프록시 흐름으로 처리되도록 Origin 보정을 등록
  proxy.on("proxyReq", removeForwardedOrigin);
}

/**
 * API와 업로드 경로에 공통으로 적용할 로컬 개발 프록시 옵션을 생성
 *
 * @author HanWon.Jang
...
function createDevProxyOptions(target: string): ProxyOptions {
  ...
  // 운영 CORS 허용 범위를 넓히지 않고 로컬 개발 프록시에서만 요청 출처를 보정
  return {
    target,
    changeOrigin: true,
    // Spring이 localhost와 Tailnet 요청을 구분하도록 브라우저가 사용한 원래 Host를 전달
    xfwd: true,
...
/**
 * Spring yml의 환경변수 placeholder를 현재 프로세스 환경값으로 해석
 *
 * @author HanWon.Jang
...
const resolveSpringValue = (configuredValue: string) => {
  ...
  // 환경변수 placeholder가 아니면 yml에 작성된 고정값을 그대로 사용
  if (!placeholder) {
    // yml에 작성된 고정 프록시 주소를 반환
  ...
  // 운영 프로필로 개발 서버를 실행하면서 필수 환경변수가 없으면 잘못된 proxy로 조용히 실행하지 않고 즉시 중단
  if (!resolvedValue) {
    throw new Error(
  ...
  // 현재 프로세스 환경에 맞게 해석한 프록시 주소를 반환
  return resolvedValue;
};

/**
 * 현재 Spring profile의 application yml에서 Vite 개발 서버 연결 설정을 읽음
 * 외부 Tailscale 호스트와 내부 API 전달 주소를 분리하여 프록시 순환과 임의 Host 요청을 차단
 *
 * @author HanWon.Jang
...
const getDevServerConfig = (): DevelopmentServerConfig => {
  ...
  // 환경변수를 파일명에 사용하므로 경로 문자가 포함된 profile 값은 yml 디렉터리 밖을 읽지 못하게 차단
  ...
  // 내부 프록시와 이전 호환용 백엔드 주소가 모두 없으면 잘못된 개발 서버 실행을 차단
  ...
  // 외부 접속 주소가 없으면 Vite Host 허용 범위를 안전하게 확정할 수 없어 실행을 차단
  ...
  // 외부 Tailscale 호스트와 후행 슬래시를 제거한 내부 프록시 대상을 반환
  return {
...
export default defineConfig(({ command }) => {
  ...
  // 개발 서버에서는 고정 포트와 내부 API 프록시를 적용하고 운영 빌드에서는 정적 자원 설정만 반환
  return {
    plugins: [react(), vanillaExtractPlugin()],
    build: {
      rollupOptions: {
        output: {
          // 화면 공통 프레임워크는 업무 코드와 분리하여 변경 시 브라우저 캐시를 재사용
          manualChunks: {
