 ...
 * date           : 2026-07-30
 * description    : 최초 로그인 닉네임 저장과 온보딩 완료 처리를 검증
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
class UserServiceImplTest {
    ...
    /**
     * 관심분야 검증 실패 응답이 실제 공통 메시지를 조회할 수 있도록 테스트 메시지 소스를 초기화
     *
     * @author SeungHyeon.Kang
     */
    @BeforeEach
    void setUpMessageSource() {
        // 공통 응답 메시지를 실제 프로퍼티에서 읽을 메시지 소스를 생성
        ResourceBundleMessageSource messageSource = new ResourceBundleMessageSource();
        // 서버 공통 메시지 프로퍼티를 테스트 조회 기준으로 설정
        messageSource.setBasename("messages");
        // 한글 메시지 원문이 손상되지 않도록 프로퍼티 파일 인코딩을 설정
        messageSource.setDefaultEncoding("UTF-8");
        // ResultData 실패 응답이 초기화된 공통 메시지 소스를 사용하도록 등록
    ...
    @AfterEach
    void resetLocale() {
        // 테스트 스레드의 Locale 컨텍스트를 기본 상태로 복원
    ...
    void uptLangRejectsInvalid() {
        ...
        // 검증 실패값이 데이터베이스에 전달되지 않았는지 확인
        verify(userMapper, never()).uptUserLanguageSetting(request);
    }

    /**
     * 유효한 닉네임 저장 시 사용자 행과 Redis 닉네임을 갱신하고 완료 프로필을 반환하는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void uptOnboardCompletes() {
        ...
        // 유효한 닉네임에는 비속어가 없도록 탐지 결과를 구성
        when(badWordDetectionService.findBadWord("차분한독서가")).thenReturn(Optional.empty());
        // 다른 회원과 닉네임이 중복되지 않도록 조회 결과를 구성
        when(userMapper.getUserNickDuplicateCnt(request)).thenReturn(0);
        // 닉네임과 온보딩 완료 UPDATE가 한 행에 반영되도록 결과를 구성
        when(userMapper.uptUserOnboarding(request)).thenReturn(1);
        // 완료 응답에서 최신 프로필을 반환하도록 사용자 조회 결과를 구성
        when(userMapper.getUserByNumb(31L)).thenReturn(savedUser);

        // 유효한 닉네임으로 최초 로그인 온보딩 완료를 요청
        ResultData result = userService.uptOnboarding(31L, request);

        // 온보딩 완료 API가 공통 성공 코드로 응답하는지 확인
        assertEquals(200, result.getCode());
        // 완료 응답에 저장한 닉네임이 포함되는지 확인
        assertEquals("차분한독서가", ((Map<?, ?>) result.getData()).get("userNick"));
        // 완료 응답에 온보딩 완료 상태가 포함되는지 확인
        assertEquals("Y", ((Map<?, ?>) result.getData()).get("onbdYsno"));
        // DB 커밋 후처리 경로에서 로그인 세션의 닉네임을 같은 값으로 갱신하는지 확인
        verify(tokenRedisService).uptUserNick(31L, "차분한독서가");
    }

    /**
     * 공백이 포함된 닉네임은 온보딩 완료와 저장 처리 전에 거절하는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void uptOnboardRejectsSpace() {
        // 공백 포함 닉네임 검증에 사용할 요청 DTO를 생성
        UserDto request = new UserDto();
        // 저장할 수 없는 공백 포함 닉네임을 요청 DTO에 설정
        request.setUserNick("차분한 독서가");

        // 공백 포함 닉네임으로 최초 로그인 온보딩 완료를 요청
        ResultData result = userService.uptOnboarding(31L, request);

        // 잘못된 닉네임 형식에 대응하는 공통 요청 오류인지 확인
        assertEquals(2009, result.getCode());
        // 형식 검증을 통과하지 못한 닉네임은 중복 조회에 사용하지 않는지 확인
        verify(userMapper, never()).getUserNickDuplicateCnt(request);
        // 형식 검증을 통과하지 못한 닉네임을 저장하지 않는지 확인
        verify(userMapper, never()).uptUserOnboarding(request);
    }

    /**
     * 다른 회원이 사용 중인 닉네임은 온보딩 완료 상태를 변경하지 않고 거절하는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void uptOnboardRejectsDup() {
        // 닉네임 중복 검증에 사용할 온보딩 요청을 생성
        UserDto request = new UserDto();
        // 다른 회원이 사용 중인 닉네임을 요청 DTO에 설정
        request.setUserNick("차분한독서가");

        // 중복 검증까지 진행되도록 닉네임 비속어 검사 결과를 구성
        when(badWordDetectionService.findBadWord("차분한독서가")).thenReturn(Optional.empty());
        // 다른 회원이 같은 닉네임을 사용하는 조회 결과를 구성
        when(userMapper.getUserNickDuplicateCnt(request)).thenReturn(1);

        // 중복된 닉네임으로 최초 로그인 온보딩 완료를 요청
        ResultData result = userService.uptOnboarding(31L, request);

        // 공통 닉네임 중복 오류를 반환하는지 확인
        assertEquals(ResultEnum.USER_NICK_DUPLICATED.getCode(), result.getCode());
        // 중복 닉네임으로 온보딩 완료 상태를 저장하지 않는지 확인
        verify(userMapper, never()).uptUserOnboarding(request);
    }

    /**
     * 활성 공통코드로 선택한 관심분야가 로그인 사용자의 목록으로 전체 교체되는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    ...
    void uptRejectsInactiveCode() {
        ...
        UserDto.UserInterestDto requestedInterest = new UserDto.UserInterestDto();
        // 활성 허용 목록에 없는 관심분야 세부코드를 설정
        requestedInterest.setIntrCode("INACTIVE_CODE");

        UserDto.UserInterestReqDto request = new UserDto.UserInterestReqDto();
        // 유효하지 않은 관심분야를 교체 요청 목록에 설정
        request.setInterestList(List.of(requestedInterest));

        // 서버가 비교할 CATE_CODE의 활성 하위 코드 목록을 구성
        when(userMapper.getUserInterestCatalog()).thenReturn(List.of(catalogInterest));

        // 비활성 관심분야 코드가 포함된 전체 교체를 요청
        ResultData result = userService.uptUserInterests(31L, request);

        // 유효하지 않은 공통코드 요청 오류를 반환하는지 확인
        assertEquals(2009, result.getCode());
        // 검증 실패 전에 기존 관심분야를 삭제하지 않는지 확인
        verify(userMapper, never()).delUserInterests(31L);
        // 검증하지 않은 관심분야를 저장하지 않는지 확인
        verify(userMapper, never()).setUserInterest(31L, requestedInterest);
    }

    /**
     * 프로필과 배경 이미지를 교체하면 사용자 참조 변경 뒤 기존 파일 정리를 요청하는지 검증
     *
     * @author SeungHyeon.Kang
