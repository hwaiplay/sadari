 ...
 * author         : HanWon.Jang
 * date           : 2026-08-26
 * description    : 좋아요 저장과 분리된 알림 이벤트 발행 및 비동기 처리 경계를 검증한다
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
class LikeAlimEventFlowTest {
    ...
    /**
     * 각 테스트에서 사용할 이벤트 발행기와 리스너 및 작업자를 생성한다.
     *
     * @author HanWon.Jang
     */
    @BeforeEach
    void setUp() {
        // 각 테스트가 독립적인 Mock 의존성을 사용하도록 대상 객체를 새로 생성한다
    ...
    /**
     * 유효한 좋아요 알림 이벤트가 Spring 이벤트 발행기로 전달되는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    void publishesEvent() {
        ...
        // 좋아요 알림 이벤트를 현재 트랜잭션에 등록한다
        publisher.setLikeAlim(event);

        // 동일 이벤트가 한 번만 Spring 이벤트 발행기로 전달되었는지 확인한다
        verify(eventPublisher).publishEvent(event);
    }

    /**
     * 이벤트 발행 실패가 좋아요 호출 경로로 전파되지 않는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    void ignoresPublishFailure() {
        ...
        // 알림 이벤트 발행 실패가 호출자에게 전파되지 않는지 확인한다
        assertDoesNotThrow(() -> publisher.setLikeAlim(event));
    }

    /**
     * 커밋 이후 리스너가 좋아요 알림을 비동기 작업자에게 위임하는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    void delegatesToWorker() {
        ...
        // 커밋 완료 이벤트를 비동기 알림 작업자에게 전달한다
        listener.handleLikeAlim(event);

        // 동일 이벤트가 작업자에게 한 번 전달되었는지 확인한다
        verify(likeAlimWorker).sendLikeAlim(event);
    }

    /**
     * 비동기 작업 접수 실패가 커밋된 좋아요 결과로 전파되지 않는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    void ignoresDispatchFailure() {
        ...
        // 작업 접수 실패가 커밋 이후 호출 경계 밖으로 전파되지 않는지 확인한다
        assertDoesNotThrow(() -> listener.handleLikeAlim(event));
    }

    /**
     * 이벤트에 포함된 닉네임으로 좋아요 알림 저장을 호출하는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    /**
     * 이벤트에 닉네임이 없으면 Redis 사용자 정보로 알림을 완성하는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    /**
     * 알림 저장 실패가 비동기 작업 경계 밖으로 전파되지 않는지 검증한다.
     *
     * @author HanWon.Jang
     */
    @Test
    ...
    void ignoresAlimFailure() {
        ...
        // 알림 저장 실패가 비동기 실행기의 예외 처리기로 전파되지 않는지 확인한다
        assertDoesNotThrow(() -> worker.sendLikeAlim(event));
    }

    /**
     * 테스트에서 공통으로 사용할 좋아요 알림 이벤트를 생성한다.
     *
     * @author HanWon.Jang
