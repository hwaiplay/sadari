 ...
 * date           : 2026-08-13
 * description    : 제한 계정 상태의 허용 경로와 인증 Token 용도를 검증
 * ===========================================================
 * DATE              AUTHOR             NOTE
 * -----------------------------------------------------------
...
class JwtFilterTest {
    ...
    void localLoginSkipsJwtFilter() {
        // 기존 인증 Cookie를 검사하지 않을 로컬 간편 로그인 요청을 생성
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/oauth/local-login");
        // 제한 상태 또는 만료된 기존 Cookie가 계정 전환을 막지 않도록 필터 제외 여부를 확인
    ...
    void refreshRejectedAsAccess() throws Exception {
        // Refresh Token 문자열이 Access Token 쿠키에 잘못 전달된 요청을 생성
