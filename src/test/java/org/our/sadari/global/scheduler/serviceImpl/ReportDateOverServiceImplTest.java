...
class ReportDateOverServiceImplTest {
    ...
    /**
     * 운영 yml의 기본값과 같은 100건 제한값으로 테스트 대상 서비스를 구성함
     *
     * @author SeungHyeon.Kang
     */
    @BeforeEach
    void setUp() {
        ...
        // 목표 독서기간 만료 스케줄러 단위 테스트 대상을 담을 객체를 생성함
    ...
    /**
     * Mapper 조회에 max-size 100을 전달하고 ReportDto의 책 제목을 #{bookTitl} 치환 Map으로 발송하는지 검증함
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void sendOverdueUsesConfig() {
        // createTarget 호출로 후속 처리에 필요한 객체를 생성함
        ...
        when(alimService.sendAlim(
              ...
              , eq((Long) null)
              , any()
        // 정상 처리 결과를 공통 성공 응답으로 생성함
        )).thenReturn(ResultData.success());

        // sendReportDateOverAlim 업무 로직을 schedulerService에 위임함
    ...
    /**
     * 한 대상의 발송 예외가 발생해도 다음 대상 알림이 계속 처리되는지 검증함
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void sendOverdueContinues() {
        ...
        // 스케줄러가 대상 없음 상황을 예외 없이 처리하는지 검증함
    ...
    /**
     * 조회 대상이 없을 때 실행 로그를 등록하거나 수정하지 않는지 검증함
     *
     * @author SeungHyeon.Kang
     */
    @Test
    ...
    private ReportDto createTarget(Long reptNumb, Long userNumb, String bookTitl) {
        ...
        // ReptNumb 업무 값을 target DTO에 설정함
        target.setReptNumb(reptNumb);
        // UserNumb 업무 값을 target DTO에 설정함
        target.setUserNumb(userNumb);
        // BookTitl 업무 값을 target DTO에 설정함
        target.setBookTitl(bookTitl);
        // 테스트에 사용할 목표기간 초과 독후감 DTO를 생성한 결과를 반환함
        return target;
    }
}
