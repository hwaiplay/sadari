...
class AlimDeleteServiceImplTest {
    ...
    /**
     * 각 테스트에서 독립적인 서비스 구현체를 생성
     *
     * @author SeungHyeon.Kang
     */
    @BeforeEach
    void setUp() {
        // 알림 삭제 스케줄러 단위 테스트 대상을 담을 객체를 생성
        alimDeleteService = new AlimDeleteServiceImpl(alimDeleteMapper, schedulerLogSupport, 30);
    }

    /**
     * 실제 삭제 건수가 대상 및 성공 건수에 동일하게 기록되는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void delAlimCountsSuccess() {
        // 성공 실행 로그에 사용할 실행 번호를 설정
    ...
    /**
     * 삭제 대상이 없을 때 불필요한 스케줄러 로그를 저장하지 않는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void delAlimSkipsEmptyLog() {
        ...
        // 처리 건수가 없는 실행은 마스터 로그 수정도 요청하지 않았는지 검증
        verify(schedulerLogSupport, never()).uptSchedulerLogSafely(any(SchedulerLogDto.SchedulerRunDto.class));
    }

    /**
     * DELETE 예외가 실패 상세와 마스터 실패 상태에 모두 반영되는지 검증
     *
     * @author SeungHyeon.Kang
     */
    @Test
    void delAlimRethrowsFailure() {
        // 실패 상세 로그에 연결할 실행 번호를 설정
        when(schedulerLogSupport.setSchedulerLogSafely(any())).thenReturn(1L);
        // 스케줄러 실패 상황을 재현할 예외를 담을 객체를 생성
