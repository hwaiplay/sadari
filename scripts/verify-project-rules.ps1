...
# 스크립트 위치를 기준으로 저장소 루트를 고정하여 다른 디렉터리의 파일을 검사하지 않도록 처리
$scriptDirectory = Split-Path -Parent $MyInvocation.MyCommand.Path
$repositoryRoot = [System.IO.Path]::GetFullPath((Join-Path $scriptDirectory ".."))
# Git 설정 값에서는 Windows 역슬래시가 이스케이프로 해석되지 않도록 슬래시 경로를 사용
$gitSafeRepositoryRoot = $repositoryRoot.Replace('\', '/')

# 여러 검증 실패를 한 번에 확인할 수 있도록 결과를 누적
...
function Add-RuleFailure {
    ...
    # 전체 검증이 끝난 뒤 한 번에 보고할 실패 내용을 저장
...
function Invoke-RepositoryGit {
    ...
    # Git 명령이 실패하면 불완전한 검사 결과가 성공으로 처리되지 않게 중단
    if ($LASTEXITCODE -ne 0) {
        # 실패한 Git 인자와 출력만 포함하여 원인을 확인할 수 있게 예외를 생성
        throw "Git 명령에 실패했습니다. arguments=$($Arguments -join ' '), output=$($output -join [Environment]::NewLine)"
    }

    # 호출자가 변경 파일과 검사 결과를 사용할 수 있도록 표준 출력을 반환
    return @($output)
}

Write-Host "[규칙 검사] 저장소: $repositoryRoot"

# 규칙 로딩 계약에 필요한 파일 목록을 정의
$requiredRuleFiles = @(
    "AGENTS.md"
...
# 규칙 문서 자체가 변경되면 해당 영역 규칙을 활성 목록에 표시
$ruleDocumentByPath = @{
...
# 변경 파일 확장자와 역할에 따라 추가로 읽어야 하는 영역 규칙을 계산
...
# Git 공백 오류는 실패 출력까지 수집할 수 있도록 별도로 실행
$previousErrorActionPreference = $ErrorActionPreference
# Native stderr가 PowerShell 예외로 승격되어 검사 결과가 사라지지 않게 일시적으로 계속 진행
$ErrorActionPreference = "Continue"
# 현재 저장소의 공백 오류와 충돌 표식을 검사
$diffCheckOutput = & git -c "safe.directory=$gitSafeRepositoryRoot" -c "core.safecrlf=false" -C $repositoryRoot diff --check HEAD -- 2>&1
# Git 종료 코드를 이후 복원과 관계없이 보존
$diffCheckExitCode = $LASTEXITCODE
# 나머지 검증은 다시 엄격한 오류 처리 정책을 사용
$ErrorActionPreference = $previousErrorActionPreference

# Git이 공백 오류를 찾으면 구체적인 위치와 함께 실패로 기록
if ($diffCheckExitCode -ne 0) {
    # Git의 구체적인 위치 정보를 실패 목록에 추가
...
# 변경된 SQL과 MyBatis 파일에서 SELECT 전체 컬럼 조회를 검사
foreach ($relativePath in $changedFiles | Where-Object { $_ -match '(?i)\.(sql|xml)$' }) {
    # 삭제되거나 저장소 밖인 경로를 검사하지 않도록 실제 파일 경계를 확인
    $absolutePath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot $relativePath))

    # 변경 파일이 저장소 루트 안의 일반 파일일 때만 내용을 검사
    if ($absolutePath.StartsWith($repositoryRoot, [System.StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $absolutePath -PathType Leaf)) {
        ...
        # 필요한 컬럼을 명시하지 않은 조회는 계약과 성능을 불명확하게 하므로 실패로 기록
        if ($sqlContent -match '(?is)\bSELECT\s+\*') {
            # 위반 파일을 저장소 상대 경로로 보고
...
# 변경된 Java 스케줄러가 환경별 cron을 코드에 고정하지 않았는지 검사
foreach ($relativePath in $changedFiles | Where-Object { $_ -match '(?i)\.java$' }) {
    # 저장소 루트 아래의 실제 Java 파일만 검사
    $absolutePath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot $relativePath))

    # 존재하는 변경 Java 파일에만 스케줄러 검사를 적용
    if ($absolutePath.StartsWith($repositoryRoot, [System.StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $absolutePath -PathType Leaf)) {
        # 이번 변경에서 추가된 어노테이션의 cron 값이 Placeholder인지 확인
        $javaContent = Get-AddedSourceContent -RelativePath $relativePath

        # 문자열로 고정한 cron은 프로필별 설정 동기화를 막으므로 실패로 기록
...
# 변경된 Java 파일에서 프로젝트 제한보다 긴 메서드명을 검사
foreach ($relativePath in $changedFiles | Where-Object { $_ -match '(?i)\.java$' }) {
    # 존재하는 저장소 내부 Java 파일만 명명 검사를 수행
    $absolutePath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot $relativePath))

    # 삭제된 파일과 저장소 밖 경로는 검사하지 않음
    if ($absolutePath.StartsWith($repositoryRoot, [System.StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $absolutePath -PathType Leaf)) {
        # 이번 변경에서 추가된 선언만 기존 코드 부채와 분리하여 검사
        $javaContent = Get-AddedSourceContent -RelativePath $relativePath
        # 추가된 Java 선언부에서 반환 타입 다음의 메서드명을 추출
...
# 변경된 TypeScript와 JavaScript 파일에서 이름 있는 함수의 길이를 검사
foreach ($relativePath in $changedFiles | Where-Object { $_ -match '(?i)\.(ts|tsx|js|jsx)$' }) {
    # 존재하는 저장소 내부 Script 파일만 명명 검사를 수행
    $absolutePath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot $relativePath))

    # 삭제된 파일과 저장소 밖 경로는 검사하지 않음
    if ($absolutePath.StartsWith($repositoryRoot, [System.StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $absolutePath -PathType Leaf)) {
        # 이번 변경에서 추가된 함수 선언만 기존 코드 부채와 분리하여 검사
        ...
        # 추출한 함수명을 프로젝트 25자 제한과 대조
        foreach ($functionName in $functionNames) {
            # 외부 라이브러리 계약 여부는 자동 판정할 수 없으므로 긴 이름을 검토 실패로 표시
            if ($functionName.Length -gt 25) {
                # 위반 파일과 함수명 및 길이를 실패 목록에 추가
...
# 변경된 MyBatis XML의 쿼리 ID가 Java Mapper 계약에 존재하는지 검사
...
# 백엔드와 프론트엔드의 기본 언어 및 영어 메시지 키 집합을 검사
$messageFilePairs = @(
...
# 각 메시지 파일 쌍의 중복 키와 언어별 누락 키를 검사
foreach ($messageFilePair in $messageFilePairs) {
    # 기본 언어와 영어 파일의 절대 경로를 계산
    ...
    if ((Test-Path -LiteralPath $defaultMessagePath -PathType Leaf) -and (Test-Path -LiteralPath $englishMessagePath -PathType Leaf)) {
        ...
        # 기본 언어 중복 키는 마지막 값으로 조용히 덮어써질 수 있으므로 실패로 기록
        ...
        # 영어 중복 키는 마지막 값으로 조용히 덮어써질 수 있으므로 실패로 기록
        ...
        # 어느 한 언어에만 있는 키는 사용자 문구 계약 불일치로 기록
        foreach ($messageKeyDifference in $messageKeyDifferences) {
            # 차이가 난 키와 존재하는 언어 방향을 실패 목록에 추가
...
if (@($changedApplicationYmlFiles).Count -gt 0) {
    ...
    if (-not (Test-Path -LiteralPath $deploymentDocumentPath -PathType Leaf)) {
        ...
        # 변경된 각 YML의 환경변수 Placeholder를 배포 문서와 대조
        foreach ($relativePath in $changedApplicationYmlFiles) {
            # 존재하는 저장소 내부 YML 파일만 검사
            $absolutePath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot $relativePath))

            # 삭제된 파일과 저장소 밖 경로는 검사하지 않음
            if ($absolutePath.StartsWith($repositoryRoot, [System.StringComparison]::OrdinalIgnoreCase) -and (Test-Path -LiteralPath $absolutePath -PathType Leaf)) {
                # YML에서 대문자 환경변수 Placeholder 이름을 추출
                ...
                # 각 환경변수가 운영 문서에 이름 그대로 기록되어 있는지 확인
                foreach ($environmentName in $environmentNames) {
                    # 운영 문서에 없는 환경변수는 배포 누락 위험으로 기록
                    if ($deploymentDocumentContent -notmatch ("\b" + [regex]::Escape($environmentName) + "\b")) {
                        # 위반 YML과 환경변수 이름을 실패 목록에 추가
...
if ($Full) {
    ...
    try {
        ...
        # 출력 파일을 만들지 않고 TypeScript 계약을 검사
        & npx.cmd tsc --noEmit

        # TypeScript 검사가 실패하면 전체 검증 실패로 기록
        if ($LASTEXITCODE -ne 0) {
            # TypeScript 실패를 최종 결과에 포함
            Add-RuleFailure "npx tsc --noEmit에 실패했습니다."
        }

        Write-Host "[전체 검사] Frontend build를 실행합니다."
        # 실제 번들 생성 경로까지 확인하여 화면 변경의 빌드 가능성을 검증
        & npm.cmd run build

        # 프론트엔드 빌드가 실패하면 전체 검증 실패로 기록
        if ($LASTEXITCODE -ne 0) {
            # 빌드 실패를 최종 결과에 포함
        ...
        # 이후 명령이 프론트엔드 디렉터리에서 실행되지 않도록 원래 위치를 복원
        Pop-Location
    }

}

# 하나 이상의 실패가 있으면 모든 내용을 출력하고 실패 종료 코드를 반환
if ($failures.Count -gt 0) {
    Write-Host "[규칙 검사] 실패 $($failures.Count)건" -ForegroundColor Red

    # 사용자가 한 번에 수정할 수 있도록 누적 실패를 모두 표시
    foreach ($failure in $failures) {
        Write-Host "- $failure" -ForegroundColor Red
    }

    # 자동화에서 실패를 감지할 수 있도록 비정상 종료
    exit 1
}
