...
Sadari는 독서 기록, 목표, 소셜 활동과 독서 모임을 하나의 흐름으로 연결한 React PWA 기반 독서 플랫폼입니다. 사용자 경험뿐 아니라 인증 수명주기, 데이터 정합성, 동시성, 파일 보안과 운영 자동화까지 실제 서비스에서 발생할 수 있는 문제를 중심으로 설계했습니다.
...
사용자 서비스와 관리자 서비스는 동기 REST 호출 대신 공통 MySQL 업무 테이블과 파일 저장소를 사용합니다. 사용자 요청 경로와 관리자 장애를 분리하는 대신 발생하는 스키마 결합은 다음 기준으로 통제합니다.
...
마이페이지의 기간별 독서량·목표·달성 횟수와 도서 목록을 구성하기 위해 최대 19회의 SQL이 실행되고 있었습니다. 완료 독후감을 조건부 집계로 한 번 계산하고, 현재 읽는 책과 완료 목록을 한 번 조회한 뒤 애플리케이션에서 기간별로 분류하도록 변경했습니다.
...
개발 DB 결과는 준비 10회 후 100회, 격리 MySQL 결과는 준비 5회 후 30회 반복한 단일 연결 JDBC 측정값입니다. Spring MVC·MyBatis·직렬화와 동시 요청을 포함한 API 응답 시간은 아니므로 운영 성능으로 일반화하지 않았습니다.

격리 MySQL 8.4.10 환경에서는 더미 독후감 수를 늘리며 기존 다중 쿼리와 통합 쿼리를 번갈아 실행했습니다. 최근 5년의 목표 329건과 `DONE 90% / READ 10%` 분포를 사용했고, 모든 결과 행을 소비한 뒤 두 방식의 결과가 같은지도 함께 검증했습니다.
...
개발 DB에서는 네트워크 왕복 횟수 감소가 가장 큰 개선 요인이었고, 데이터가 커질수록 통합 쿼리의 CTE와 집계 비용이 상대적으로 커졌습니다. 따라서 단순히 “쿼리 한 번이 빠르다”라고 결론 내리지 않고, 호출 수 감소와 대량 집계 비용 사이의 변화까지 기록했습니다.
...
Kakao OAuth 로그인 후 기기별 `sid`를 포함한 Access Token과 Refresh Token을 HttpOnly Cookie로 발급합니다. Redis의 `auth:session:{sid}` Hash와 회원별 `sid` Set으로 각 기기의 Refresh Token을 독립적으로 관리합니다.

- `JwtFilter`가 JWT 서명·만료뿐 아니라 Redis의 활성 `sid`와 계정 상태를 함께 검증합니다.
- 한 탭의 중복 Refresh 요청은 하나의 Promise를 공유하고, 여러 탭·서비스워커의 동시 재발급은 Redis Lua 회전과 유예시간으로 수렴시킵니다.
...
- 계정 상태 캐시가 없으면 DB를 조회해 복원하되, 정지·탈퇴 상태는 Redis 장애나 캐시 누락 때문에 허용되지 않도록 실패 방향을 구분했습니다.
- 브라우저 탭 간에는 `BroadcastChannel`과 storage 이벤트로 로그아웃을 전파하여 한 탭에 만료된 인증 화면이 남지 않도록 했습니다.
...
이를 통해 푸시는 발송됐지만 알림이 없는 상태, 롤백된 DB가 삭제된 파일을 참조하는 상태와 도서만 남는 부분 저장을 줄였습니다.
...
관리자가 사용자를 정지·해제하면 DB 원본과 이력을 같은 트랜잭션에서 변경하고 `USER_STATUS_CHANGED` 이벤트를 `TB_EVTBOX`에 저장합니다. 사용자 백엔드는 이벤트의 복제 상태값이 아니라 처리 시점의 최신 DB 상태를 읽어 Redis를 갱신합니다.

한 이벤트가 실패해도 다음 사용자를 계속 처리하고, Redis 반영에 성공한 이벤트만 삭제합니다. 빠르게 연속된 정지·해제에서도 오래된 이벤트가 최신 상태를 덮어쓰지 않으며 실패 이벤트는 다음 스케줄러 실행에서 재시도됩니다.
...
독서 모임 가입·초대·승인은 화면에 표시된 여석을 신뢰하지 않고 모임 행을 잠근 뒤 최신 좌석을 다시 계산합니다. 만료되지 않은 초대는 예약 좌석으로 포함하고, 초대 수락은 같은 멤버 행을 `ACTIVE`로 전환합니다.

가입 신청 승인과 초대 수락처럼 서로 다른 경로가 같은 정원을 소비하므로, 각 API에 개별 카운트 검사를 두는 대신 정원 판정 시점을 잠금 이후로 통일했습니다. 중복 가입과 모임장 권한도 서버에서 다시 검증하여 오래된 화면 상태나 동시 요청으로 정책을 우회할 수 없게 했습니다.
...
모바일 PWA에서 고해상도 프로필 이미지를 편집할 때는 원본을 브라우저 상태나 영구 저장소에 먼저 넣지 않습니다. 서버가 임시 업로드 토큰을 발급해 비공개 공간에 최대 30분 보관하고, 브라우저에는 축소된 미리보기만 반환합니다. 최종 저장·로그아웃·비활성화 과정에서는 임시 파일을 정리하여 중단된 작성 흐름이 장기 잔존하지 않도록 했습니다.

- [콘텐츠·파일 보안 정책](docs/policies/content-file-policy.md)

### 7. 외부 API 쿼터를 Redis 방어 계층으로 보호

카카오 도서 검색은 반환 권수가 아니라 요청 횟수로 일일 쿼터가 차감됩니다. 화면의 10권 단위 탐색 경험은 유지하면서 서버가 한 번에 최대 50권을 받아두고, 프론트엔드가 이를 10권씩 나눠 표시하도록 호출 경계를 분리했습니다. 따라서 사용자가 50권을 확인할 때 필요한 카카오 호출은 최대 5회에서 1회로 줄어듭니다.

검색 전 빈 화면에는 `TM_REPORT`의 도서별 고유 작성자 수를 집계한 주간·월간·연간 인기 도서 10권을 선택해 노출합니다. 회원 상태, 독서 상태와 공개 여부를 순위 조건으로 사용하지 않으며, 도서 정보와 같은 평균 평점을 함께 제공합니다. 직접 검색 결과가 반환되면 같은 카드 UI에서 기간 선택과 인기 순위 및 평균 평점을 제거하고 출판사와 도서 소개를 표시합니다.
...
- 동일 검색어와 페이지는 검색어 원문 대신 SHA-256 해시 키로 10분간 공유하여 캐시 유효기간 안의 후속 반복 검색은 외부 호출 없이 처리합니다.
- 캐시 미스에서만 앱 전체 실제 호출 예산을 차감하고, 일일 30,000건 중 기본 3,000건을 비상 여유로 남깁니다.
- Redis 장애 시 제한을 건너뛰지 않고 검색을 차단하는 방향으로 실패시켜 외부 쿼터가 무방비로 소모되지 않게 했습니다.
...
비속어 필터는 입력마다 전체 사전을 순회하지 않고 Aho-Corasick 자동자를 사용해 한 번의 문자열 순회로 후보를 찾습니다. 활성 사전은 10분 동안 캐시하고, 만료 시 이중 확인 잠금으로 한 요청만 자동자를 다시 구성합니다. 공백·기호·반복 문자를 정규화하되 허용 단어가 더 긴 표현에 포함된 경우는 차단하지 않도록 예외를 함께 판정합니다.

표지 색상 탐색은 RGB 좌표의 단순 거리 대신 사람이 느끼는 색 차이에 가까운 CIELAB 색차를 사용합니다. 외부 도서 이미지에서 추출한 색을 서비스의 제한된 공통 색상 집합에 매핑하여, 서로 다른 표지에서도 일관된 탐색 결과를 제공하도록 했습니다.

- [성능과 알고리즘 설계](docs/portfolio/performance-algorithms.md)

### 프론트엔드 실패를 한 번의 복구 흐름으로 수렴

API 응답은 HTTP 상태와 함께 업무 결과 코드를 사용하고, Axios 계층에서 60초 타임아웃·CSRF 갱신·Refresh Token 재발급을 공통 처리합니다. 인증 또는 CSRF 재시도는 각각 한 번으로 제한하여 무한 재요청을 막고, 같은 쓰기 요청에는 operation ID를 유지해 사용자가 중복 결과를 만들지 않도록 했습니다.

로그아웃·계정 상태 변경처럼 화면 전환이 중요한 작업은 차단 모달과 history guard를 사용합니다. PWA Service Worker와 FCM 초기화도 시간 제한을 두어 푸시 구성 실패가 로그인이나 핵심 화면 진입까지 막지 않도록 분리했습니다.
...
Kakao 연결 해제에는 Redis에 10분 만료 `state`를 저장해 재인증 요청을 검증합니다. 외부 연결 해제에 실패하면 내부 계정 상태부터 변경하지 않고 `UNLINK_PENDING`으로 남겨, 외부 계정은 연결된 채 사용자만 접근할 수 없는 불일치를 피했습니다.
...
- 현재 CI는 로컬 MySQL·Redis와 Git에서 제외된 프로필에 의존하는 테스트 때문에 패키징 단계에서 `-x test`를 사용합니다. 따라서 CI 성공은 전체 테스트 통과가 아니라 빌드·패키징 성공을 의미합니다.

## 현재 한계와 개선 방향

| 현재 한계 | 개선 방향 |
| --- | --- |
| 테스트가 CI 배포 게이트에 포함되지 않음 | CI 전용 프로필과 Testcontainers를 도입하여 PR마다 테스트 실행 |
| 스케줄러가 다중 인스턴스 소유권을 보장하지 않음 | 분산 락과 실행 소유권·중복 실행 방지 정책 추가 |
| 대량 푸시가 동기 후처리 구조에 가까움 | 비동기 큐, 재시도 횟수와 실패 메시지 보관 도입 |
| 공통 DB로 서비스 간 스키마 결합이 존재함 | 변경 빈도와 트래픽 증가 시 도메인 API·이벤트 계약으로 단계적 분리 |
| 사용자 신고 화면과 저장 API가 연결되지 않음 | 신고 등록 API와 `TH_CMPLNT` 저장·관리 처리 흐름 완성 |
