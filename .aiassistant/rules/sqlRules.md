...
- 테이블 별칭은 한 글자로 제한하지 않고 테이블의 역할을 알아볼 수 있는 영문 대문자로 작성합니다.
...
- 실제 SQL 시작 키워드 뒤에는 Mapper ID를 SQL 주석으로 작성합니다.
...
- 닫는 괄호는 인라인 뷰를 시작한 `FROM` 또는 `JOIN`의 데이터 시작 기준에 맞추고, 같은 줄에 한 글자 대문자 별칭을 작성합니다.
