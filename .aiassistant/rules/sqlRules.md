...
- 시트에는 실제 DDL을 기준으로 테이블명, 테이블 설명, 컬럼명, 컬럼 설명, 데이터 타입과 크기, PK 여부, 기본값, NULL 허용 여부, CREATE SQL 및 COMMENT SQL을 반영합니다.
- 메타데이터, 테이블 정보, 컬럼 헤더, PK 행, 제약조건 및 인덱스 영역에 적용된 기존 색상과 구분 형식을 유지합니다.
- 시트에 비밀번호, 토큰, 개인키 및 운영 접속 정보와 같은 비밀값을 기록하지 않습니다.
- 작업 완료 전에 `01-create.sql`과 시트의 테이블·컬럼 수 및 마지막 테이블 반영 여부를 대조합니다. 외부 접근 또는 인증 문제로 시트를 갱신하지 못하면 누락한 채 완료하지 않고 미반영 사유를 사용자에게 보고합니다.
