...
| `JAVA-VALID-001` | MUST | 3 | 입력, Null, 예외 및 비속어 검증 |
...
| `JAVA-LOG-001` | MUST | 3.6 | 요청·외부 연동·백그라운드 실패 로그와 민감정보 제외 검증 |
...
- `@Scheduled`의 cron 표현식과 시간대는 환경별 설정으로 관리하고 YML 또는 환경변수의 값을 Placeholder로 바인딩합니다.
- 스케줄러 메서드에서는 `@Scheduled(cron = "${scheduler.example.cron}", zone = "${scheduler.example.zone}")`처럼 실제 설정 키를 확인할 수 있게 작성합니다.
...
- 브라우저가 자동으로 전송하는 Cookie로 인증하는 API는 CSRF 보호를 비활성화하지 않습니다.
- `POST`, `PUT`, `PATCH`, `DELETE` 요청은 서버가 발급한 CSRF Token을 브라우저가 자동으로 추가하지 않는 요청 Header로 검증합니다.
- CSRF Token 조회 API는 상태를 변경하지 않는 `GET`으로 제공하고, 허용된 프론트엔드 출처만 응답을 읽을 수 있도록 기존 CORS 정책을 유지합니다.
- 인증 Cookie와 CSRF Cookie의 `Secure`, `SameSite`, `Path`는 환경별 Cookie 설정과 일치시킵니다.

### 2.4 내부 도메인 작업의 API 노출 제한

- 알림 발송처럼 수신자와 처리 내용을 호출자가 선택할 수 있는 내부 도메인 작업은 화면의 직접 호출 요구가 없으면 Controller Endpoint로 노출하지 않습니다.
- 내부 도메인 작업은 Service 계약을 유지하여 다른 Service와 Scheduler에서만 호출하고, 외부 요청 재사용을 목적으로 별도 Endpoint를 만들지 않습니다.
...
### 3.3 사용자 작성 문자열의 비속어 검증

- 사용자가 직접 작성하여 저장하거나 다른 사용자 또는 관리자에게 전달하는 모든 문자열은 Service의 등록 및 수정 처리에서 `BadWordDetectionService`로 비속어를 검증합니다.
- 비속어 검증 대상에는 닉네임, 한줄소개, 독후감, 댓글, 고객문의 제목과 본문 및 향후 추가되는 사용자 작성 입력을 포함합니다.
- 프론트엔드 검증만으로 저장을 허용하지 않으며 백엔드 검증을 최종 기준으로 적용합니다.
- 비속어가 탐지되면 `ResultEnum.COMMON_BAD_WORD_INCLUDED` 공통 실패 응답으로 저장과 후속 처리를 중단합니다.
- 향후 채팅 기능은 실시간 대화 정책과 처리 방식이 별도로 확정될 때까지 이 공통 비속어 검증 의무의 적용 대상에서 제외합니다.
...
- 본인 화면과 다른 사용자 화면이 같은 Service와 Mapper를 재사용하면 공개 범위 파라미터를 Service 계약에 명시합니다.
- 본인 조회는 공개 범위 제한이 없는 값을 전달하고, 다른 사용자 조회는 Controller 또는 Service에서 공개 상수를 전달합니다.
- 클라이언트가 보낸 공개 범위 값을 그대로 신뢰하지 않고 인증 사용자와 조회 대상의 관계에 따라 서버에서 결정합니다.

### 3.6 운영 로그

- `JAVA-LOG-001`은 API, 외부 연동, 비동기 처리 및 스케줄러 로그에 적용합니다.
- API 로그는 공통 필터와 응답 Advice에서 요청 식별자, 서버 매핑 패턴, HTTP 상태, 업무 결과 코드 및 소요 시간을 기록합니다. Controller마다 같은 완료 로그를 중복 작성하지 않습니다.
- 요청·응답 본문, 쿼리 문자열, Cookie, 인증 Header, 토큰 및 OAuth 식별자를 기록하지 않습니다. 경로 변수는 원시 URL 대신 서버 매핑 패턴으로 대체합니다.
- 예외는 `LogSafe.getFailure`로 원인 유형과 제한된 코드 위치만 기록하며 원시 Throwable, `getMessage()` 및 외부 오류 본문을 직접 로그에 전달하지 않습니다.
- 외부 HTTP 공통 클라이언트는 공급자 호스트, 상태 및 소요 시간을 기록합니다. 별도 SDK·비동기 처리·스케줄러는 공통 요청 로그 밖의 실패와 정상 생략을 구분할 수 있게 기록합니다.
- 정상 완료는 INFO, 예상 거절·부분 실패·지연은 WARN, 서버 장애는 ERROR를 사용합니다. 고빈도 무작업 폴링과 상세 시작 로그는 로그 증폭 방지를 위해 DEBUG를 허용합니다.
- 로그를 위해 응답 본문을 버퍼링하거나 API 결과·트랜잭션·예외 전파 정책을 변경하지 않습니다. 실행 식별자 MDC는 종료 시 기존 값 복원 또는 제거를 보장합니다.
- 검증은 성공, HTTP 200 업무 실패, 보안 필터 거절, 미처리 예외, 외부 연동 실패 및 민감한 테스트 문자열 비출력 테스트로 수행합니다. 기존 프레임워크·외부 라이브러리 자체 로그는 이 규칙의 직접 수정 대상에서 제외하되 운영 로그 레벨을 별도 검토합니다.
...
```java
log.error("알림 발송에 실패했습니다. failure={}", LogSafe.getFailure(e));
...
- 브라우저가 `<img>` 또는 다운로드 URL로 직접 요청하는 이미지, PDF 등 바이너리 리소스 조회 Endpoint는 예외로 두며 `ResponseEntity<byte[]>` 또는 스트리밍 응답을 사용할 수 있습니다.
- 바이너리 리소스 조회 Endpoint는 객체 부재와 잘못된 경로를 HTTP 상태 코드로 구분하고, 원시 예외 내용이나 내부 저장소 경로를 응답에 포함하지 않습니다.
- 브라우저 최상위 탐색으로 인증 Cookie를 발급한 뒤 설정에서 허용한 프런트엔드 주소로 이동하는 OAuth 및 개발용 로그인 Endpoint는 예외로 두며 리다이렉트 응답을 사용할 수 있습니다.
- 인증 리다이렉트 대상은 요청 파라미터를 그대로 사용하지 않고 환경별 허용 주소 중에서 서버가 선택합니다.
...
- `author`에는 항상 `HanWon.Jang`을 작성합니다.
...
- 클래스를 수정할 때 이전 날짜의 변경 이력을 삭제하거나 덮어쓰지 않고 새 날짜의 이력을 마지막 행에 추가합니다.
- 변경 이력은 날짜별로 정확히 한 행만 작성합니다. 같은 날짜에 여러 번 수정한 경우 새 행을 추가하지 않고 해당 날짜의 기존 행에 내용을 합칩니다.
- 같은 날짜의 수정자가 여러 명이면 `AUTHOR`에 중복을 제거한 이름을 쉼표로 구분하여 한 행에 작성합니다.
- 변경 이력의 `NOTE`는 세부 변경 사항을 나열하지 않고 관련 기능을 묶어 30자 이내의 짧은 명사형 문구로 요약합니다.
- 변경 이력의 중복 행 병합, 정렬 또는 문구 축약만 수행한 경우에는 해당 클래스에 새 날짜의 이력을 추가하지 않습니다.
 ...
 * 2026-08-14        SeungHyeon.Kang,Hanwon.Jang    인증·조회 정책 정리
 */
...
- `if` 조건절 전체가 들여쓰기를 포함하여 120자 이내이고 비교 조건이 3개 이하이면 논리 연산자가 두 번 사용되더라도 한 줄로 작성합니다.
- 짧은 조건절을 한 줄로 작성할 수 있으면 조건 개수만을 이유로 불필요하게 줄바꿈하지 않습니다.
- 첫 번째 조건은 `if (`와 같은 줄에 작성하고, 다음 줄부터 현재 `if` 문의 들여쓰기보다 8칸 더 들여씁니다.
- 120자를 초과하거나 비교 조건이 4개 이상인 여러 줄 조건절은 한 줄에 조건을 최대 두 개까지 작성하며, `||` 또는 `&&` 논리 연산자를 한 번 사용해 두 조건을 계산한 뒤 줄바꿈합니다.
